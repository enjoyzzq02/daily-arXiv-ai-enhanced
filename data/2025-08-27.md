...
Method: 开发了两种免训练检测范式：1）基于物理声纳模拟生成合成数据训练神经网络；2）基于目标物体几何先验的模型模板匹配系统。

Result: 神经网络在模拟数据上达到98% mAP，但在真实声纳数据上降至40%；模板匹配方法无需训练，在真实数据上保持83% mAP，对声学噪声和环境变化具有鲁棒性。

Conclusion: 研究挑战了水下领域数据密集型深度学习的传统观念，证明了基于几何先验的模板匹配方法在数据稀缺环境中的有效性，为自主水下航行器导航等应用开辟了新可能性。
...
<details>
  <summary>Details</summary>
Motivation: 现有脑肿瘤MRI检测方法存在泛化能力有限、计算效率低、缺乏可解释性和透明度等问题，限制了临床可信度。

Method: 使用MobileNetV2和DenseNet201的双流融合模型，采用特征级融合策略，在增强的6020个MRI扫描数据集上进行训练，包含胶质瘤、脑膜瘤、垂体瘤和正常样本。

Result: 在严格的5折交叉验证下，训练准确率99.75%，测试准确率98.35%，F1分数0.9835（95% CI: 0.9743-0.9920），比VGG19准确率提升3.67%，训练时间减少39.3%。

Conclusion: MobileDenseAttn是一个高效、高性能、可解释的模型，GradCAM热图能清晰显示肿瘤区域，具有成为临床实用工具的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 识别视觉语言模型在多模态基础方面的系统性缺陷，特别是当实体参考从文本变为视觉时模型表现的变化，以改进模型的可信度和可靠性。

Method: 通过控制研究比较VLMs在文本参考和视觉参考下的表现，分析模型内部状态模式，并开发无需重新训练的探针来识别模型失败情况。

Result: 当VLMs被迫依赖图像表示时，回忆事实知识的能力减半；内部状态探针达到92%以上的准确率来标记不可靠响应；在视觉问答任务中，探针使覆盖率提高7.87%，错误风险降低0.9%。

Conclusion: 视觉语言模型在多模态基础方面存在可检测的系统性缺陷，通过内部状态分析可以识别模型失败情况，这为改进语言基础提供了重要方向和建议。
...
<details>
  <summary>Details</summary>
Motivation: 解决稀疏图像输入导致的严重辐射模糊和特征不匹配问题，提高神经隐式表示的重建质量。

Method: 在神经隐式表示中添加基于补丁的语义逻辑，与符号距离场和辐射场联合优化，并引入基于几何基元掩模的新型正则化方法来缓解形状模糊。

Result: 在DTU数据集上，相比SparseNeuS和VolRecon分别减少了44%和20%的平均倒角距离；作为NeuS和Neuralangelo等密集重建基线的插件时，平均误差分别降低了69%和68%。

Conclusion: 该方法有效解决了稀疏图像重建中的辐射模糊问题，显著提高了3D重建的精度和保真度，可作为插件提升现有密集重建方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 非法垃圾填埋场对全球构成严重威胁，但人工识别困难且缺乏高质量公开数据集。AerialWaste数据集提供了意大利伦巴第地区的10434张航拍图像，适合进行可扩展研究

Method: 使用Mobilenetv2、Googlenet、Densenet、MobileVit等轻量级深度学习模型进行训练，发现复杂模型容易过拟合，因此选择简单模型并采用集成融合技术

Result: 通过集成方法在二分类任务上取得了92.33%准确率、92.67%精确率、92.33%灵敏度、92.41% F1分数和92.71%特异性的优异性能

Conclusion: 轻量级模型结合集成技术能有效识别非法垃圾填埋场，避免过拟合问题，为环境监测提供了高效可靠的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法往往忽视模态特定的结构依赖和语义错位问题，限制了模型的质量、可解释性和鲁棒性。

Method: 提出结构-语义统一器(SSU)框架：1)动态构建模态特定图(文本使用语言句法，声学和视觉使用轻量级文本引导注意力机制)；2)引入基于全局文本语义的语义锚点作为跨模态对齐中心；3)开发多视图对比学习目标来促进判别性、语义一致性和结构连贯性。

Result: 在CMU-MOSI和CMU-MOSEI基准数据集上持续实现最先进性能，同时显著降低了计算开销。定性分析验证了模型的可解释性和通过语义基础交互捕捉细微情感模式的能力。

Conclusion: SSU框架通过系统整合结构信息和语义对齐，有效解决了多模态情感分析中的关键挑战，为多模态表示学习提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人脸3D重建方法要么速度慢（需要逐人优化），要么质量差（前馈方法重建质量不佳），无法满足消费级和交互式系统的实时应用需求。

Method: 使用编码器-解码器神经网络设计，首先构建3DGS人脸模板模型，然后将输入图像编码为身份特定且姿态不变的潜在嵌入，解码后预测模板高斯模型的结构和外观参数残差。

Result: 在重建质量上显著优于现有前馈方法（如GAGAvatar），速度比逐人优化方法（如FlashAvatar）快1000倍，同时支持实时身份插值和属性编辑。

Conclusion: FastAvatar通过结合优秀的重建质量和极快的速度，扩展了3D高斯泼溅技术在消费级和交互式系统中光真实感头像应用的范围。
...
<details>
  <summary>Details</summary>
Motivation: 在紧急人道主义场景中使用生物识别技术可以提高操作效率，但会给处于脆弱环境的数据主体带来隐私风险，需要开发适合此类场景的生物特征模板保护方案

Method: 通过制定功能、操作和安全隐私需求，比较分析生物特征模板保护方案，选择PolyProtect方法应用于神经网络人脸嵌入，并使用EdgeFace特征提取器在真实人道主义数据集上进行评估

Result: 实验结果表明PolyProtect在验证和识别准确性、不可逆性和不可链接性方面表现良好，并且首次在指纹生物识别中进行了评估，显示出良好的跨模态适用性

Conclusion: PolyProtect是一种有效、模块化且计算轻量的生物特征模板保护方案，适用于人道主义紧急场景，未来计划开源代码以促进进一步研究
...
Method: 提出在基础模型中集成轻量级、上下文自适应的图推理模块，实例化为动态关系图（其拓扑和边语义从输入和任务上下文中推断），通过稀疏语义节点推理实现高效计算。

Result: 在人类操作动作识别和脑肿瘤分割等跨领域任务中，增强图推理模块的基础模型相比纯基础模型基线，在细粒度语义保真度、分布外鲁棒性、可解释性和计算效率方面均有提升，同时具备良好的内存和硬件效率。

Conclusion: 提出了针对基础模型图混合体的研究议程，重点包括学习动态图构建、多层次关系推理、跨模态融合以及直接探测结构化视觉任务中关系能力的评估协议。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动车牌识别(ALPR)中低质量车牌识别困难的问题，通过选择性图像预处理来优化模型性能和计算效率。

Method: 创建包含多种车辆类型、光照条件和图像质量的数据集，采用细粒度标注策略（包括遮挡、4种可读性类别和字符标签），并使用ViT、ResNet和YOLO三种图像识别网络进行基准测试。

Result: 所有三种基线模型的总体F1分数均低于80%，表明该任务具有挑战性，需要进一步研究。

Conclusion: LPLC数据集为车牌可读性分类研究提供了重要资源，当前方法的性能限制凸显了该领域需要更多的研究努力。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在医疗任务中诊断准确性有限、模型过大导致推理成本高的问题，为临床部署提供轻量级可信赖的AI系统

Method: 采用专家-通才框架：1)轻量级领域训练图像分类器提供快速准确诊断预测；2)压缩的对话VLM生成自然语言解释；3)知识图谱检索模块确保回答基于事实皮肤病学知识

Result: 在皮肤病数据集上诊断准确性比最强基线提高18%，平均VRAM需求降低至少20%，延迟降低至少5%
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中面部图像常受噪声、模糊和压缩伪影等退化影响，降低图像质量并阻碍后续任务处理。

Method: 组织ICCV 2025研讨会挑战赛，参赛者开发高效轻量模型，使用相关指标在野外面部图像数据集上进行全面评估。

Result: 吸引了127名参与者，提交了1519份最终方案，推动了实用面部图像质量评估方法的发展。

Conclusion: 该挑战赛成功促进了轻量级面部图像质量评估模型的开发，为处理真实退化图像提供了实用解决方案。
...
Conclusion: 通过结合时空动态建模和语义上下文理解，该零样本异常检测框架在监控场景中表现出强大的泛化能力，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统需要准确的3D场景流估计来安全导航动态环境，但大规模人工标注数据集成本高昂且不可扩展，自监督方法在长距离和恶劣天气场景下性能不足

Method: 提出跨模态标签迁移方法，通过4D雷达多普勒测量实时计算运动伪标签，使用动态感知关联和模糊解析传播将标签迁移到LiDAR域

Result: 在MAN TruckScenes数据集上显著超越现有自监督方法，仅使用10%的标注数据即可达到全监督方法90%以上的性能

Conclusion: DoGFlow提供了一种可扩展的自监督解决方案，能够有效解决3D场景流估计中的标注瓶颈问题，在自动驾驶感知系统中具有重要应用价值
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的3D生成方法难以从卫星图像的俯视图准确恢复建筑结构，而3D细节化方法又过度依赖详细体素输入，无法从简单几何先验（如长方体）生成满意结果。

Method: 提出建模从插值噪声粗先验到详细几何的转换方法，实现灵活的几何控制且不增加计算成本。同时开发了包含5万多个独特风格化3D建筑资产的大规模数据集Skylines-50K。

Result: 广泛评估表明该方法有效且具有强大的泛化能力。

Conclusion: SatSkylines方法成功解决了卫星图像3D建筑生成的几何精度和细节化问题，为大规模详细建筑模型生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在复杂3D游戏环境中，玩家依赖视觉可供性来发现地图过渡点。高效识别这些点对客户端自动地图绘制很重要，并为评估地图提示呈现提供了客观基础。

Method: 采用两阶段深度学习管道：首先使用Faster R-CNN检测潜在STP，然后通过轻量级MSTP选择器融合局部和全局视觉特征对它们进行排序。使用参数高效的适配器，并引入可选的检索增强融合步骤。

Result: 实验发现关键权衡：全网络微调在有足够数据时产生优越的STP检测性能，而仅适配器迁移在低数据场景和MSTP选择任务中显著更鲁棒和有效。

Conclusion: 通过定义这一新颖问题、提供基线管道和数据集，以及提供关于高效模型适应的初步见解，旨在为未来AI驱动的导航辅助和数据驱动的关卡设计工具做出贡献。
...
TL;DR: Wan-S2V是一个音频驱动的角色动画模型，专门针对电影级制作需求设计，在表达性和保真度方面显著优于现有方法，支持长视频生成和精确唇形同步编辑。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的音频驱动角色动画方法主要针对语音和歌唱场景，但在复杂的影视制作中（需要细腻的角色互动、真实的身体运动和动态镜头）表现不足，需要实现电影级的角色动画。

Method: 基于Wan构建的音频驱动模型Wan-S2V，通过改进模型架构和训练策略来增强在电影场景中的表现力和保真度。

Result: 在与Hunyuan-Avatar和Omnihuman等先进模型的对比实验中，Wan-S2V始终显著优于现有解决方案，并在长视频生成和精确唇形同步编辑应用中展现出良好的 versatility。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在固定融合方案导致模型过度依赖预定义特征组合，以及MoE融合中专家网络处理分离特征限制信息交互的问题

Method: 提出DeReF框架，包含模态解耦、随机特征重组和动态MoE融合模块，并在解耦模块中融入区域交叉注意力网络提升特征表示质量

Result: 在内部肝癌数据集和三个TCGA公共数据集上的广泛实验证实了所提方法的有效性

Conclusion: DeReF框架通过创新的特征重组策略解决了多模态融合中的关键挑战，提升了癌症生存分析的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法在处理对象副作用（阴影、反射等）时表现不佳，主要原因是缺乏配对的监督数据来训练模型消除这些副作用。

Method: 使用3D渲染引擎生成合成数据，构建大规模配对数据集；基于扩散变换器构建视频修复模型；引入额外监督来显式预测受副作用影响的区域；通过差分掩码定位所有对象相关区域。

Result: ROSE在视频对象移除方面表现出优越性能，相比现有方法能更好地处理各种副作用，并且在真实世界视频场景中具有良好的泛化能力。

Conclusion: 该框架通过系统性研究对象对环境的影响，构建了全面的基准测试ROSE-Bench，为解决视频对象移除中的副作用问题提供了有效解决方案，在合成和真实数据上都取得了显著效果。
...
Method: 1) 数据层面：通过两阶段流程构建HMD-270K数据集（运动-细节融合和细粒度检查）；2) 优化层面：基于组相对策略优化提出字幕集等价奖励机制，通过单元到集合匹配和双向验证增强完整性和准确性。

Result: OwlCap模型在细节导向的VDC基准上提升4.2 Acc，在运动导向的DREAM-1K基准上提升4.6 F1，显著优于基线模型。

Conclusion: 提出的运动-细节平衡方法有效解决了视频字幕生成的不平衡问题，HMD-270K数据集和OwlCap模型将为视频字幕研究社区的发展提供有力支持。
...
Result: 在三个主流检测框架（Faster R-CNN、DETR和Sparse R-CNN）上进行实验，所有框架都展现出显著的性能提升。

Conclusion: 该方法有效解决了甲骨文拓片图像检测中的挑战，通过聚类表示学习显著提升了检测网络的性能，为数字考古提供了有效的技术解决方案。
...
Method: 提出傅里叶注意力SNR先验变换器(FAST)在频域分解特征为振幅和相位谱，结合频率自适应变换器(FAT)瓶颈，采用门控注意力机制融合高低频分支，构建统一的U型架构SFormer

Result: 在UIEB、EUVP和LSUI数据集的4800对图像上训练，SFormer相比现有方法PSNR提升3.1 dB，SSIM提升0.08，成功恢复水下场景的颜色、纹理和对比度

Conclusion: 频域SNR先验比空间域方法更有效，提出的SFormer模型在水下图像增强任务中取得了显著性能提升
...
Method: 采用分层设计：低层使用卷积网络处理单帧图像保留细节，高层使用Mamba架构捕捉时空关系；提出时空交叉扫描(STCS)模块整合长程上下文信息

Result: 网络设计平衡了单帧和多帧处理，避免了单帧处理的局部误差累积和多帧处理的细节忽略问题

Conclusion: 该方法通过分层时空建模和跨帧扫描机制，能够有效减轻超声图像噪声等因素对EF计算的偏差影响
...
Method: 提出特征空间平面搜索器(FPS)框架，冻结预训练特征编码器，利用特征空间的几何模式（类内聚类和类间分离）优化决策边界，支持离线特征提取和全数据集单次计算优化。
...
Method: 集成定制InceptionV3提取空间特征，LSTM建模时序依赖，采用基于集成遗传算法的ADFSA特征选择策略，动态平衡准确性、冗余性和复杂度。

Result: 在UCF-YouTube数据集上实现99.65%识别准确率，特征数量减少至7个，推理时间显著提升，支持树莓派等边缘设备实时部署。

Conclusion: 该框架提供了轻量级、可扩展的人体活动识别解决方案，适用于公共安全、辅助技术和自主监控系统等资源受限环境。
...
Result: 在DaVinci机器人手术视频和基准数据集上的实验显示，ColorGS达到PSNR 39.85（比之前3DGS方法高1.5）和SSIM 97.25%的优异性能，同时保持实时渲染效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决神经网络在不平衡数据集上训练时，模型预测偏向多数类而导致少数类识别性能显著下降的问题。

Method: 扩展洪水正则化到类别级别，根据类别频率为每个类别分配特定的洪水阈值，防止多数类过拟合的同时允许少数类充分学习。

Result: 在图像不平衡分类任务中，相比传统洪水正则化方法，提升了少数类的分类性能并获得了更好的整体泛化效果。

Conclusion: 类别级洪水正则化是处理不平衡数据集的有效方法，能够平衡不同类别的学习过程，提高模型的整体泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统课程学习在平坦最小化区域中难以有效评估样本难度，因为损失值和梯度范数都变得均匀小，限制了课程学习与SAM方法的结合效果

Method: 提出对抗难度度量(ADM)，通过计算原始样本与对抗样本之间的归一化损失差距来量化对抗脆弱性，保持难度评估的有效性，并将其集成到基于课程学习的SAM训练中

Result: 在图像分类、细粒度识别和领域泛化任务上验证，方法同时保留了课程学习和SAM的优势，超越了现有的课程学习和平坦感知训练策略

Conclusion: ADM提供了一种在平坦最小化环境下有效评估样本难度的方法，成功实现了课程学习与SAM优化的有机结合，显著提升了模型的泛化性能
...
<details>
  <summary>Details</summary>
Motivation: 水下目标检测面临图像质量退化、类别分布不平衡和独特视觉特征等挑战，某些物种检测性能不佳但原因不明，需要探究数据量之外的影响因素并系统改进水下物种检测。

Method: 使用DUO数据集，将目标检测任务分解为定位和分类两个阶段，利用YOLO11和TIDE分析扇贝类别的定位问题，通过数据平衡实验研究分类性能差距。

Result: 定位分析发现前景-背景区分是最困难的阶段且与数据量无关；分类实验显示即使数据平衡后精度差距仍然存在，表明存在内在特征挑战；建议精度优先时使用不平衡分布，召回优先时使用平衡分布。

Conclusion: 改进水下目标检测性能应关注算法创新，特别是在定位模块方面；研究公开了代码和数据集供后续研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法主要从点云、网格或文本输入生成实体模型，但与传统从2D工程图开始的工业工作流程存在根本性差异，从2D矢量图自动生成参数化CAD模型这一关键工程步骤仍未被充分探索。
...
Conclusion: 该研究填补了从2D工程图自动生成参数化CAD模型的空白，提出的序列到序列学习框架能够有效保持几何精度和设计意图，为工业设计工作流程提供了重要技术支持。
...
Method: 使用音频-视觉特征融合的路由器，基于标记级声学损坏分数自适应重新加权音频和视觉特征，通过门控跨注意力机制在解码器层中强化视觉线索

Result: 在LRS3数据集上相比AV-HuBERT实现了16.51-42.67%的词错误率相对降低

Conclusion: 路由器机制和门控机制都有助于提高模型在真实世界声学噪声下的鲁棒性，该方法能够有效在音频质量恶化时转向视觉模态
...
<details>
  <summary>Details</summary>
Motivation: 现有HOI基准要求与标注类别精确匹配，不适合生成式VLM的多解释特性，可能导致有效预测被错误惩罚，需要新的评估协议。

Method: 将HOI检测重构为多答案多选题任务，每个问题只包含真实正选项和精心筛选的负选项，避免模糊情况下的有效预测被标记为错误。

Result: 创建了首个同时适用于VLM和HOI方法的评估协议，能够直接比较两种范式在HOI理解方面的表现。

Conclusion: 新基准解决了传统精确匹配评估对生成式VLM的不公平问题，为HOI检测领域提供了更合理的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 随着智能环境和低功耗计算设备的普及，实时远程生理监测面临可扩展性、互操作性和性能方面的挑战，需要为资源受限平台开发高效的实时监测系统。

Method: 基于Face2PPG流水线，采用多线程架构管理视频捕获、实时处理、网络通信和GUI更新，结合函数式响应式编程(FRP)和Actor模型的混合编程模型实现事件驱动处理和任务并行化。

Result: 系统在实时约束下表现出鲁棒性，能够以30fps连续可靠运行，同时最小化计算开销，提供连续视频流和按需生命体征检索的RESTful API。
...
Method: 通过多视角图像重建道路表面（使用高斯泼溅技术）和预训练2D分割网络的语义信息生成伪标签，引入掩码感知分配算法和损失函数处理部分掩码的伪标签

Result: 首次实现了无需任何真值地图的在线地图模型训练，伪标签还可用于半监督预训练以利用大规模无标注众包数据

Conclusion: 该方法为在线地图学习提供了一种成本效益高且可扩展的解决方案，代码已开源
...
<details>
  <summary>Details</summary>
Motivation: 废物分类对可持续回收至关重要，但该领域的AI研究因数据集有限和依赖传统目标检测器而落后于商业系统，需要建立更好的基准和开发更有效的检测方法。

Method: 首先在ZeroWaste数据集上评估开放词汇目标检测模型，然后微调基于transformer的检测器，最后提出使用空间和共识感知加权的集成预测融合策略进行软伪标签生成。

Result: LLM优化的提示显著提升零样本准确率，微调后达到51.6 mAP的新基准，半监督训练在未标注数据上超越了全监督训练的性能。

Conclusion: 本研究通过建立严格基准、引入鲁棒的集成伪标签管道、生成高质量标注以及系统评估开放词汇模型，为废物检测研究社区做出了重要贡献。
...
<details>
  <summary>Details</summary>
Motivation: 不同字体样式传达不同的视觉印象，字体形状与印象标签之间存在密切关系，但标准词嵌入方法无法有效捕捉这种关系

Method: 构建印象标签共现图，节点代表印象标签，边编码共现关系，然后应用谱嵌入技术获得每个标签的印象向量

Result: 在定性和定量评估中，该方法在印象引导的字体生成任务上表现优于BERT和CLIP等标准词嵌入方法

Conclusion: 所提出的基于形状-印象关系的嵌入方法能更好地表示字体印象，特别适用于印象驱动的字体生成和检索任务
...
<details>
  <summary>Details</summary>
Motivation: 国家森林清单(NFI)需要大量人力进行现场调查，而遥感技术结合机器学习可以提供更频繁、更大规模的更新。当前方法主要依赖随机森林分类器和手工设计的物候特征，深度特征提供了补充策略。

Method: 使用Google Earth Engine从Sentinel-1、Sentinel-2、ERA5和SRTM卫星数据中提取时间序列数据，对公开可用的遥感时间序列基础模型进行微调，用于树种分类任务。

Result: 微调预训练模型在所有数据集上都大幅超越荷兰NFI分类的最先进方法，准确率提升高达10%，表明传统手工定义的谐波特征过于简单。

Conclusion: 利用开放卫星数据和预训练模型，深度AI特征在数据有限的应用如NFI分类中具有巨大潜力，能有效补充现有森林清单流程，显著提高分类准确性。
...
<details>
  <summary>Details</summary>
Motivation: 解决组织病理学图像中正常有丝分裂图形(NMFs)与非典型有丝分裂图形(AMFs)的二元分类问题，应对严重类别不平衡、高度形态变异性和跨不同肿瘤类型、物种及扫描仪的领域异质性等关键挑战。

Method: 采用ConvNeXt V2基础架构，配合60%中心裁剪预处理和混合精度训练，通过5折交叉验证集成策略构建模型。

Result: 该方法在多样化的MIDOG 2025数据集上实现了稳健的性能表现。

Conclusion: 证明了现代卷积架构在有丝分裂图形亚型分类中的有效性，同时通过精心选择的架构和训练优化保持了计算效率。
...
Method: 提出先验引导的视频级回归方法，引入可扩展的时间间隔选择策略，综合考虑微表情的时间演化、持续时间和类别分布特征。采用协同优化框架，让检测和识别任务共享除分类头外的参数，充分利用互补信息。

Result: 在多个基准数据集上取得最先进性能，在CAS(ME)^3上STRS为0.0562，在SAMMLV上STRS为0.2000。
...
<details>
  <summary>Details</summary>
Motivation: 当前显微外科吻合评估方法依赖主观判断，存在偏见影响评估的可靠性和效率，需要客观量化评估体系

Method: 利用三个医院数据集，采用图像处理技术和几何建模方法，开发错误检测和评分机制

Result: 几何度量指标能够有效复制专家评分者对错误的评分结果

Conclusion: 该定量框架提高了显微外科技能评估的效率和可靠性，有助于推进培训协议的发展
...
Method: 采用快速适应策略，包括急动定位模块和针对性适应策略，专注于高急动片段进行适应，以最少适应步骤最大化稳定性。

Result: 在多样化真实世界数据集上的广泛实验表明，该方法能持续改进各种全帧合成模型的性能，在定性和定量指标上均优于现有方法。

Conclusion: 所提出的方法使现代稳定器能够超越长期的最先进方法，同时保持现代方法的全帧特性，并为用户提供类似经典方法的控制机制。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学诊断中跨群体公平性问题，特别是在成像设备和临床实践变化导致的分布偏移下，现有方法独立处理视觉和文本模态导致残留的跨模态错位和公平性差距

Method: 采用并行双分支架构分离敏感属性和目标属性，构建近似正交的文本锚点指导跨注意力机制，使用超网络生成实例感知的视觉提示，并应用基于原型的正则化

Result: 在8个医学影像数据集和4种模态上的实验表明，DualFairVL在分布内和分布外设置下均实现了最先进的公平性和准确性，仅使用360万可训练参数

Conclusion: 该方法有效解决了医学影像诊断中的公平性问题，通过联合去偏和对齐跨模态表示，在保持高精度的同时显著提升了模型公平性
...
TL;DR: 提出双查询增强网络(DQEN)，通过对象感知编码器特征增强对象查询，利用CLIP模型提取语义特征增强交互查询初始化，在HOI检测任务上取得竞争性性能
...
Conclusion: DQEN通过分别增强对象查询和交互查询，有效提升了HOI检测的准确性，证明了针对不同查询类型进行专门增强的有效性
...
<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶AI决策难以解释，特别是在复杂城市场景中，深度神经网络的非线性决策边界使得理解AI决策逻辑具有挑战性
...
Conclusion: 该方法不仅提高了可解释性，还与减少违规行为相关，产生了更安全、高性能的驾驶模型
...
TL;DR: EVENTA Grand Challenge是首个大规模事件级多模态理解基准，通过整合上下文、时间和语义信息来捕捉图像背后的完整事件信息，包含检索和字幕生成两个赛道，45支团队参与评估。


<details>
  <summary>Details</summary>
Motivation: 传统图像描述和检索任务主要关注人物、物体和场景的表面识别，往往忽略了定义真实世界事件的上下文和语义维度。EVENTA旨在填补这一空白。

Method: 基于OpenEvents V1数据集，设置两个赛道：事件增强图像检索与字幕生成、基于事件的图像检索。通过公开和私有测试阶段进行评估，确保公平性和可复现性。

Result: 来自6个国家的45支团队参与挑战，前三名团队受邀在ACM Multimedia 2025上展示解决方案。
...
TL;DR: 基于Transformer的轨迹分类框架，用于区分群体和单个行人，建立空间利用和行为模式分析指标体系


<details>
  <summary>Details</summary>
Motivation: 分析群体和单个行人在空间利用和行为模式上的差异，为人群动力学研究和空间设计验证提供基础

Method: 将行人轨迹分割为固定时间窗口，使用Transformer模型进行成对分类识别群体，建立空间指标（凸包面积、最小包围圆半径、热力图密度）和行为指标（速度变化、角度偏差、避让半径、轨迹直线度）

Result: 建立了完整的分类流水线和数据集结构，支持60、100、200帧不同序列长度的可扩展分析

Conclusion: 为后续定量分析群体与单个行人的空间行为差异奠定了基础，未来将完成指标量化分析及其在行人仿真和空间设计验证中的应用
...
<details>
  <summary>Details</summary>
Motivation: 监测大空间尺度的珊瑚礁是一个开放挑战，对于评估生态系统健康和指导保护工作至关重要。虽然无人机航拍图像提供了广阔的空间覆盖，但其有限的分辨率难以可靠地区分精细尺度的类别（如珊瑚形态类型）。同时，在大空间范围内获取像素级标注成本高且劳动密集，限制了基于深度学习的航拍图像分割方法的可扩展性。

Method: 多尺度弱监督语义分割框架，结合基于分类的监督、空间插值和自蒸馏技术，将水下图像的精细生态信息迁移到航拍数据。

Result: 该方法能够实现珊瑚形态类型的大面积分割，并展示了集成新类别的灵活性。
...
TL;DR: 本文提出了一种基于生成式AI和矢量数据引导的地图生成方法，能够根据文本提示生成不同风格的精确地图，并通过用户研究验证了其有效性和实用性。


<details>
  <summary>Details</summary>
Motivation: 传统GIS制图需要专业知识且耗时，生成式AI为自动化制图提供了新机遇，但现有模型在空间构图和语义布局控制方面存在不足。

Method: 整合矢量数据来引导地图生成，通过文本提示控制地图风格，并开发了网页应用程序来提高可用性和可访问性。

Result: 用户研究表明，开发的应用在生成地图保真度和可用性方面表现良好，生成式AI模型能够帮助非专业用户和专业人士更高效地创建地图。

Conclusion: 该方法展示了生成式AI在辅助制图方面的潜力，同时指出了进一步的技术改进方向，并强调了制图师在AI辅助制图新范式中的重要作用。
...
Conclusion: 该架构在保持高性能的同时实现了更高的计算效率，具有优秀的训练稳定性，为固定上下文长度任务提供了有效的解决方案
...
TL;DR: USO提出了一个统一的风格-主体优化定制模型，通过构建大规模三元组数据集、解耦学习方案和风格奖励学习范式，在风格相似性和主体保真度两方面都达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有文献通常将风格驱动和主体驱动生成视为两个独立任务，前者注重风格相似性，后者强调主体一致性，造成明显的对立。作者认为这两个目标可以在单一框架下统一，因为它们都涉及内容和风格的解耦与重组。

Method: 1) 构建包含内容图像、风格图像及其对应风格化内容图像的大规模三元组数据集；2) 引入解耦学习方案，通过风格对齐训练和内容-风格解耦训练两个互补目标同时对齐风格特征并解耦内容与风格；3) 采用风格奖励学习范式(SRL)进一步提升模型性能；4) 发布首个联合评估风格相似性和主体保真度的基准USO-Bench。

Result: 大量实验表明，USO在开源模型中实现了最先进的性能，在主体一致性和风格相似性两个维度上都表现出色。

Conclusion: USO成功地将风格驱动和主体驱动生成统一在一个框架下，通过内容和风格的解耦与重组，同时实现了优秀的风格相似性和主体保真度，为风格化图像生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: NeRF方法在视觉定位中广泛应用，但其编码的精细场景细节存在隐私泄露风险，特别是在云服务部署时敏感信息可能被恢复。

Method: 提出隐私保护评估协议，并开发ppNeRF变体，使用自监督分割监督替代RGB图像训练，确保标签足够粗糙以隐藏可识别细节，同时保持3D区分性。

Result: 发现基于光度损失的NeRF在几何表示中存储细粒度细节，易受隐私攻击；ppNeSF在分割空间中实现准确视觉定位，达到最先进结果。

Conclusion: ppNeSF方法有效解决了NeRF隐私泄露问题，在保护隐私的同时保持了视觉定位的准确性，为云基定位服务提供了安全解决方案。
...
TL;DR: 本文系统评估了在文档视觉问答中引入检索增强生成(RAG)的效果，通过文本检索和视觉检索两种方式，在多个多页数据集上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的文档VQA系统要么拼接所有页面内容（内存消耗大），要么依赖超大规模视觉语言模型。RAG提供了一种更高效的替代方案，通过先检索相关片段再生成答案来降低内存需求。

Method: 采用两种检索变体：基于OCR文本的检索和纯视觉检索（无需OCR）。在MP-DocVQA、DUDE和InfographicVQA等多页数据集上进行系统评估，并分析检索、重排序和布局引导分块策略的效果。

Result: 文本检索变体相比"拼接所有页面"基线提升了最多+22.5 ANLS，视觉检索变体无需文本提取也实现了+5.0 ANLS的提升。实验确认检索和重排序组件是性能提升的主要驱动力。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在现实应用中的广泛使用，理解其人口统计偏见变得至关重要，需要系统评估模型在不同人口属性上的偏见表现

Method: 提出GRAS基准测试框架，涵盖性别、种族、年龄和肤色四个维度，设计GRAS偏见评分指标，对5个最先进的VLMs进行偏见评估，并采用多种问题表述方式进行视觉问答测试

Result: 测试结果显示所有VLMs都存在严重的偏见问题，表现最好的模型GRAS偏见评分仅为2分（满分100分），同时发现评估VQA偏见时需要考虑多种问题表述方式

Conclusion: VLMs存在显著的人口统计偏见问题，GRAS基准和评分指标为系统评估和缓解这类偏见提供了有效工具，多问题表述的评估方法对准确衡量偏见至关重要
...
TL;DR: 提出RoofSeg网络，一种基于Transformer的端到端方法，用于从LiDAR点云中分割屋顶平面，解决了现有深度学习方法的三个主要问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的屋顶平面分割方法存在三个未解决的问题：非真正端到端、边缘特征判别性低、平面几何特征考虑不足。

Method: 开发了RoofSeg网络，采用Transformer编码器-解码器框架，使用可学习的平面查询预测平面实例掩码；设计了边缘感知掩码模块(EAMM)增强边缘判别性；提出了自适应权重策略和平面几何损失函数。

Result: 该方法实现了真正的端到端屋顶平面分割，提高了边缘区域的准确性，并通过几何约束优化了网络训练。

Conclusion: RoofSeg网络有效解决了当前深度学习方法在屋顶平面分割中的三个关键问题，为3D建筑模型重建提供了更优的解决方案。
...
TL;DR: 提出MicroDetect-Net模型，结合荧光显微镜、尼罗红染色和深度学习技术，用于检测血液样本中的微塑料，准确率达到92%


<details>
  <summary>Details</summary>
Motivation: 随着全球每年塑料产量超过3.68亿吨，微塑料污染已广泛存在于空气、水、土壤和生物体中，对人类健康造成肝脏感染、肠道损伤和肠道菌群失衡等危害，需要开发有效的检测方法

Method: 使用尼罗红染料染色和荧光显微镜技术，结合卷积神经网络进行图像分割，构建MDN模型对276张染色血液图像进行微塑料定位和计数

Result: 模型达到92%的准确率，交并比87.4%，F1分数92.1%，精确率90.6%，召回率93.7%，表现出强大的检测性能

Conclusion: MDN模型在微塑料检测方面效果显著，虽然使用蛤蜊血液存在局限性，但为应用于更一致的人类样本提供了可行途径
...
TL;DR: ProPy模型通过提示金字塔结构和祖先-后代交互机制，将CLIP模型适配到部分相关视频检索任务，在三个公开数据集上实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有PRVR方法主要处理单模态特征，而强大的预训练视觉-语言模型如CLIP在该领域尚未充分探索，需要专门设计架构来适应部分相关视频检索任务

Method: 提出ProPy模型，包含两个关键创新：1）提示金字塔结构组织多粒度事件提示；2）祖先-后代交互机制实现事件间的动态语义交互

Result: 在三个公开数据集上达到最先进性能，显著超越先前模型

Conclusion: 通过系统性的CLIP架构适配，ProPy成功解决了部分相关视频检索任务，证明了预训练视觉-语言模型在该领域的潜力
...
TL;DR: 利用大规模几何血管模型数据集进行自监督预训练，提升冠状动脉壁剪切应力评估的准确性，即使在有限临床数据下也能改善血流动力学生物标志物的分割性能。


<details>
  <summary>Details</summary>
Motivation: 医疗大数据在心血管健康等领域具有巨大潜力，但获取足够数据训练机器学习模型评估血流动力学生物标志物（如壁剪切应力）极具挑战。冠状动脉领域尚未充分研究如何利用学习表示来改善血流动力学评估。

Method: 使用8449个3D血管几何模型的大规模数据集进行自监督预训练，通过计算热核签名（基于拉普拉斯特征向量）创建自监督目标，捕获形状本质特征。然后将学习到的几何表示应用于小规模临床试验（49名患者）的冠状动脉壁剪切应力评估。

Result: 研究表明，从大规模几何数据集中学习到的表示能够显著提升冠状动脉壁剪切应力的区域分割性能（分为低、中、高时间平均壁剪切应力区域），即使在有限训练数据条件下也能取得良好效果。

Conclusion: 通过自监督预训练和基础模型利用大规模几何血管数据集，可以有效解决医疗数据稀缺问题，显著改善冠状动脉血流动力学生物标志物的评估准确性，为临床应用提供了有前景的解决方案。
...
TL;DR: SuperSimpleNet是一个高效且适应性强的表面缺陷检测模型，能够在无监督、弱监督、混合监督和全监督四种场景下进行高效训练，是首个能够充分利用所有可用数据标注的模型，在多个基准数据集上实现了最先进的性能，同时推理时间低于10毫秒。


<details>
  <summary>Details</summary>
Motivation: 现有的表面缺陷检测方法往往局限于特定的监督场景，难以适应真实制造过程中遇到的多样化数据标注（如无监督、弱监督、混合监督和全监督设置），无法满足工业对高性能、高效率和适应性的需求。

Method: 基于SimpleNet构建，引入了新颖的合成异常生成过程、增强的分类头和改进的学习过程，使模型能够在所有四种监督场景下进行高效训练。

Result: 在四个具有挑战性的基准数据集上均达到了最先进的性能标准，推理时间低于10毫秒，表现出卓越的速度和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 单像素成像通过结构化光模式顺序照明来重建物体，但需要优化任务特定的二进制照明模式以提高重建性能

Method: 使用直通估计器解决二进制模式优化的不可微问题，在双层公式中利用总深度变分正则化器

Result: 在CytoImageNet显微镜数据集上验证，学习到的模式相比基线方法实现更优的重建性能，特别是在高度欠采样情况下
...
TL;DR: 提出一种轻量级方法，通过旋转不平衡质量产生周期性振动来维持事件相机的持续事件生成，结合运动补偿管道消除注入的运动，为下游感知任务提供干净的运动校正事件。


<details>
  <summary>Details</summary>
Motivation: 事件相机在静态或低运动场景下无法生成事件，限制了其在计算机视觉任务中的应用。现有运动诱导方法需要复杂硬件或额外光学组件。

Method: 使用简单的旋转不平衡质量产生周期性振动来诱导运动，结合运动补偿管道去除注入的运动，获得干净的运动校正事件。

Result: 硬件原型和真实世界数据集验证表明，该方法能可靠恢复运动参数，在图像重建和边缘检测方面优于无运动诱导的事件传感。

Conclusion: 提出的轻量级振动诱导方法有效解决了事件相机在静态场景中的局限性，无需复杂硬件即可实现持续事件生成和高质量感知性能。
...
TL;DR: 本文提出了一种基于轻量级UNet++架构和拓扑感知损失函数的少样本文本行分割方法，仅需3个标注页面即可在历史文档上实现最先进的性能
...
Method: 使用轻量级UNet++架构配合连接性感知损失函数（最初用于神经元形态学），显式惩罚结构错误如行碎片化和意外行合并，通过在每份手稿仅3个标注页面上提取的小块进行训练

Result: 在U-DIADS-TL数据集上显著超越当前最先进方法，识别准确率提高200%，行交并比提高75%，在DIVA-HisDB基准检测任务中达到或超过竞赛获胜者的F-Measure分数

Conclusion: 小规模简单架构配合拓扑感知损失函数比复杂替代方案更准确且数据高效，证明了该方法在历史文档文本行分割中的有效性和数据效率
...
TL;DR: 本文提出两种简单有效的方法来增强单目3D视觉定位任务中文本嵌入的3D感知能力，解决了预训练语言模型对数值单位不敏感的问题，在Mono3DRefer数据集上取得了显著性能提升
...
Method: 提出3D文本增强(3DTE)预处理方法增强不同单位映射关系的理解，以及文本引导几何增强(TGE)模块将文本特征投影到几何一致空间来增强3D-文本信息

Result: 在Mono3DRefer数据集上相比之前方法有显著改进，在"Far"场景中获得11.94%的准确率提升，达到新的最先进水平

Conclusion: 通过增强文本嵌入的3D感知能力和几何特征，有效解决了单位敏感性导致的3D理解问题，显著提升了单目3D视觉定位的性能
...
TL;DR: 提出Weierstrass椭圆函数位置编码(WEF-PE)，通过复数域表示直接处理二维坐标，利用椭圆函数的双周期特性与视觉数据的平移不变性模式对齐，在多个基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统Vision Transformer使用一维位置嵌入会破坏图像的二维空间结构，缺乏几何约束，无法建立欧几里得空间距离与序列索引距离之间的单调对应关系，限制了模型利用空间邻近先验的能力。

Method: 利用椭圆函数的非线性几何性质自然编码空间距离关系，通过代数加法公式直接从绝对编码推导任意补丁对之间的相对位置信息，实现二维坐标的直接处理。

Result: 在ViT-Tiny架构上CIFAR-100从头训练达到63.78%准确率，ViT-Base微调达到93.28%，在VTAB-1k基准任务上持续改进，理论分析证实距离衰减特性，注意力可视化显示增强的几何归纳偏置。

Conclusion: WEF-PE提供了一种数学原理严谨的位置编码方法，有效解决了传统方法在二维空间表示上的局限性，为视觉Transformer提供了更好的空间结构建模能力。
...
TL;DR: SoccerNet 2025挑战赛是第五年度足球视频理解基准测试，包含四个计算机视觉任务：团队球动作检测、单目深度估计、多视角犯规识别和游戏状态重建，旨在推动足球视频分析研究发展。
...
Method: 设立四个视觉任务：1)团队球动作检测；2)单目深度估计；3)多视角犯规识别；4)游戏状态重建。提供标注数据集、评估协议和基线方法供参与者使用。

Result: 报告呈现了各挑战任务的结果，突出了表现最佳的解决方案，并提供了社区进展的深入分析。

Conclusion: SoccerNet挑战赛持续推动计算机视觉、人工智能和体育交叉领域的可重复开放式研究，为研究者提供了标准化的评估平台和开发工具包。
...
Method: 使用自回归模型生成顶点，双向变换器捕获顶点关系构建邻接矩阵来定义面，并引入保真度增强器和后处理框架

Result: 将所需标记数量减少到现有最紧凑标记器的约23%，生成速度比最先进方法快8倍以上，同时产生更高质量的网格

Conclusion: 该框架通过分离顶点和面生成有效解决了网格表示中的冗余问题，在速度和质量方面都显著优于现有方法
...
TL;DR: 提出All-in-One Slider方法，通过将文本嵌入空间分解为稀疏的语义属性方向，实现单一模块控制多种属性，支持零样本属性和多属性组合操作


<details>
  <summary>Details</summary>
Motivation: 现有T2I扩散模型在渐进式属性操控方面存在挑战，特别是对于人脸等细节丰富的内容。传统方法需要为每个属性训练独立滑块，导致参数冗余和扩展性受限

Method: 训练轻量级模块将文本嵌入空间分解为稀疏的语义属性方向，形成通用滑块，支持细粒度连续控制、零样本属性操作和多属性组合
...
<details>
  <summary>Details</summary>
Motivation: 现有神经重建方法局限于静态环境且场景控制有限，而图像/视频扩散模型生成的数据缺乏几何基础和因果性，需要在这两者之间建立桥梁

Method: 结合代理几何和环境表示的生成，利用学习到的2D图像先验进行分数蒸馏，支持基于提示的几何控制和地图布局条件生成

Result: 能够生成具有精确几何、高保真纹理和结构的大规模3D驾驶场景，实现因果性新视角合成和显式3D几何估计

Conclusion: 该方法成功填补了神经重建和扩散生成之间的空白，提供了既具有几何准确性又具有高度可控性的大规模3D场景生成解决方案
...
TL;DR: 提出了OmniHuman-1.5框架，通过多模态大语言模型生成结构化文本指导，结合专门的多模态DiT架构，实现语义一致且富有表现力的角色动画生成。
...
Method: 1) 利用多模态大语言模型合成结构化文本表示，提供高级语义指导；2) 引入带有伪最后一帧设计的专门多模态DiT架构，有效融合多模态输入并缓解模态间冲突。

Result: 在唇形同步准确性、视频质量、动作自然度和文本提示语义一致性等综合指标上达到领先性能，并展现出对多人场景和非人类主体的出色扩展性。

Conclusion: 该框架成功实现了物理合理且语义连贯的角色动画生成，能够准确理解音频、图像和文本的联合语义，产生与角色、场景和语言内容深度一致的动作。
...
Method: 提出了Visual Feature Tracking (VFTrack)框架，集成了手工制作或深度特征检测器和匹配器，在粒子追踪框架内实现碳纳米管微柱生长的运动学分析。通过13,540条手动标注轨迹的系统性测试，确定了ALIKED检测器与LightGlue匹配器的最佳组合。
...
TL;DR: AUSM是一个基于状态空间模型的自回归通用分割模型，统一了提示和无提示视频分割任务，支持任意长度视频流处理，并在多个基准测试中优于现有方法，训练速度提升2.5倍


<details>
  <summary>Details</summary>
Motivation: 现有的视频分割方法在处理无提示分割（自动检测和跟踪所有对象）时存在碎片化问题，需要针对特定任务的模型和流水线，缺乏统一的解决方案

Method: 将流式视频分割重新构建为序列掩码预测问题，提出基于状态空间模型的AUSM架构，维护固定大小的空间状态，支持任意长度视频流，所有组件设计为跨帧并行训练

Result: 在DAVIS17、YouTube-VOS、MOSE、YouTube-VIS、OVIS等标准基准测试中优于先前的通用流式视频分割方法，在16帧序列上实现高达2.5倍的训练加速

Conclusion: AUSM成功统一了提示和无提示视频分割任务，通过自回归掩码预测和状态空间模型实现了高效的长视频处理，为视频分割提供了统一的端到端解决方案
...
TL;DR: Style4D-Bench是首个专门为4D风格化设计的基准套件，包含评估协议、强基线方法和高质量4D场景数据集，旨在标准化评估并推动这一新兴领域的发展。


<details>
  <summary>Details</summary>
Motivation: 4D风格化是一个新兴领域，缺乏标准化的评估基准和方法，需要建立统一的评估标准和高质量数据集来促进该领域的研究进展。

Method: 提出了Style4D框架，基于4D高斯泼溅技术，包含三个关键组件：基础4DGS场景表示、风格高斯表示（使用轻量级MLP进行时空感知外观控制）和整体几何保持风格转换模块（通过对比一致性学习和结构内容保持增强时空一致性）。

Result: 在Style4D-Bench上的广泛实验表明，Style4D在4D风格化方面实现了最先进的性能，能够生成具有稳定时间动态和一致多视图渲染的细粒度风格细节。

Conclusion: Style4D-Bench有望成为动态3D场景风格化渲染研究中宝贵的基准资源，推动该领域的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉和语言模型取得了进展，但通过语言控制来调整3D资产姿态的任务仍然具有挑战性。需要一种能够保持原始网格身份的同时实现自由形式文本提示控制的方法。

Method: 将问题分解为两个步骤：1）修改图像生成器（引入自注意力重连机制RSActrl）创建基于输入图像和文本指令的目标图像；2）通过多视角姿态优化将网格与目标图像对齐，使用关键点而非可微分渲染来建立对应关系。

Result: 该方法在多样化的3D对象和自由形式文本提示上表现有效，成功操纵姿态同时保持网格原始身份。定量评估和用户研究表明，该方法在85%的情况下优于现有方法。

Conclusion: Articulate3D提供了一种有效的训练自由方法，通过语言控制实现3D资产的姿态调整，解决了现有方法在保持结构一致性和姿态控制方面的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 游戏行业和机器人交互需要精确的3D局部编辑，现有方法在保持未编辑区域和整体连贯性方面存在挑战。

Method: 首先预测3D模型的反转轨迹获取潜在表示和键值令牌，然后在去噪编辑阶段用反转的潜在表示替换保留区域的特征，确保一致性。
...
TL;DR: 本文提出了一个基于复数意义空间中语义吸引子的理论框架，用于构建语义型通用人工智能，通过张量变换和旋转语义结构来建模语言的多义性，而非传统的统计预测方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的语言模型依赖统计性的下一词预测，无法真正理解语义的深层含义。作者希望开发一种能够主动塑造语言意义而非被动预测的新型认知架构。

Method: 使用复数运算和循环操作（涉及虚数单位i）构建旋转语义结构，通过梯度流、张量变形和迭代矩阵动力学来描述语义吸引子（Microvitum）的运作机制。

Result: 提出了一个能够建模反讽、同音异义和歧义等复杂语言现象的数学框架，该框架在数学上具有启发性，在哲学上也具有重要意义。

Conclusion: 真正的意义不是通过模拟产生的，而是通过向语义连贯性的递归收敛而涌现的，这需要一种全新的认知架构设计理念。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM在多智能体协作中的信任形成机制和社交动态影响，解决现有研究主要关注从众偏见而忽略其他关键社交因素的局限性。

Method: 开发KAIROS基准测试模拟问答竞赛环境，控制专家-新手角色、噪声群体和对抗性同伴等条件，使用提示工程、监督微调和GRPO强化学习进行缓解策略评估。

Result: GRPO结合多智能体上下文、基于结果的奖励和无约束推理实现最佳整体性能，但相比基础模型降低了社交影响力鲁棒性。

Conclusion: 多智能体强化学习能有效提升LLM协作性能，但需要在性能提升和社交鲁棒性之间寻求平衡，为构建更可靠的集体智能系统提供重要见解。
...
TL;DR: LangCrUX数据集首次大规模收集了12种非拉丁语系网站的12万个网站数据，揭示了多语言网页可访问性普遍存在的问题：语言提示与可见内容不一致，严重影响屏幕阅读器效果。


<details>
  <summary>Details</summary>
Motivation: 随着多语言内容在网页中的增长，辅助技术对非拉丁语系支持不足，导致视觉障碍用户面临严重可访问性挑战，但缺乏大规模多语言网页数据集限制了相关研究。

Method: 构建LangCrUX数据集（12种语言的12万个流行网站），进行系统性多语言可访问性分析，并开发Kizuki语言感知自动化测试扩展工具。

Result: 发现网页可访问性提示普遍忽视语言多样性，与可见内容语言不一致，降低了屏幕阅读器的有效性，限制了网页可访问性。

Conclusion: 多语言网页可访问性存在系统性缺陷，需要语言感知的自动化测试工具来改善非拉丁语系内容的辅助技术支持。
...
TL;DR: 提出backprompting方法生成生产级标注数据，结合稀疏人工聚类技术，用于健康建议护栏开发，在参数少400倍的情况下性能超越GPT-4o达3.73%


<details>
  <summary>Details</summary>
Motivation: 企业环境中大型语言模型(LLMs)使用带来风险，护栏技术需要过滤输入输出文本，但获取真实LLM输出的生产质量标注数据困难

Method: backprompting方法生成生产级标注数据，结合稀疏人工聚类技术标注生成数据，构建平行语料库并注入现有数据集

Result: 在健康建议识别这一最困难的任务中，检测器性能优于其他解决方案，比GPT-4o提升3.73%，参数量少400倍

Conclusion: backprompting方法能有效生成生产级标注数据，为LLM护栏开发提供高质量训练数据，在资源有限情况下实现优异性能
...
TL;DR: 提出Integral Transformer，通过积分采样logit分布来降噪注意力，在保持特殊令牌作用的同时减少注意力噪声，在多个语言基准测试中表现优于传统注意力变体。
...
Method: 提出Integral Transformer自注意力机制，通过对logit分布进行积分采样来降噪注意力，平衡注意力分布并减少秩塌陷。

Result: 在知识推理语言基准测试中优于vanilla、Cog和Differential注意力变体，分析显示下层使用传统自注意力可提升性能。

Conclusion: Integral Transformer能有效降噪同时保留关键特殊令牌的贡献，在上下层注意力机制中实现良好平衡。
...
TL;DR: 提出Latent Self-Consistency (LSC)方法，使用可学习的token嵌入选择语义最一致的回答，在短式和长式推理基准测试中均优于现有方法，且计算开销极小。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在复杂或长式问题上概率解码产生不一致输出的问题，现有方法在短式和长式回答格式之间存在准确率权衡。

Method: 通过可学习的token嵌入选择语义最一致的回答，生成轻量级摘要token，推理时间增加不到1%，无需修改模型架构。

Result: 在6个短式和5个长式推理基准测试中，LSC在所有基准上平均优于SC、USC和WUCS方法，同时保持可忽略的计算开销。
...
TL;DR: 本文挑战了OOD评估能反映真实世界部署失败的假设，发现不同QA数据集对模型鲁棒性的评估质量差异巨大，有些甚至不如简单的分布内评估。
...
Result: 发现不同OOD数据集对模型抗捷径鲁棒性的评估质量差异很大，部分数据集表现甚至不如简单的分布内评估。虚假捷径在ID和OOD数据集间共享，但训练和评估质量存在脱节。
...
TL;DR: 分析不同训练方法对大型语言模型在重排序任务中语义理解能力的影响，并研究模型是否能生成更好的文本推理来提高透明度和应对有限训练数据的挑战


<details>
  <summary>Details</summary>
Motivation: 随着LLM语义理解能力提升，虽然更符合人类价值观但透明度下降，且在新系统中缺乏足够排名数据时准确重排序面临挑战，需要深入理解模型内部工作机制

Method: 使用环境与地球科学领域相对较小的排名数据集进行内容重排序，分析不同训练方法对语义理解的影响，并检查可解释信息是否能用于推理重排序决策

Result: 研究发现某些训练方法比其他方法具有更好的可解释性，表明并非所有训练方法都能学习到准确的语义理解，有些只是获得了抽象知识来优化评估

Conclusion: 需要仔细选择训练方法以确保LLM真正学习语义理解而非仅仅优化评估指标，这对LLM在重排序任务中的可靠性提出了重要问题
...
TL;DR: 本研究通过IPIS数据集微调多语言和波兰语专用LLMs，旨在解决波兰语中男性偏见问题，使性别包容性成为模型的内在特征。


<details>
  <summary>Details</summary>
Motivation: 波兰语语法性别系统存在历史性男性偏见，导致LLMs生成性别不平衡的输出，需要系统性解决方案来减轻这种偏见。

Method: 使用IPIS数据集（包含人工制作的性别包容性校对和翻译指令）微调LLMs，设计包含明确性别包容性指南的系统提示。

Result: 对Llama-8B、Mistral-7B、Mistral-Nemo等多语言模型以及Bielik、PLLuM等波兰语专用模型进行了IPIS微调。

Conclusion: 该方法为减轻波兰语生成中的性别偏见提供了系统性解决方案，使性别包容性成为语言模型的内在特征。
...
TL;DR: 本文提出了一种基于多重检验假设测试的方法来检测大语言模型中的幻觉问题，将幻觉检测问题建模为假设检验问题，并与机器学习中的分布外检测问题进行类比。
...
Method: 将幻觉检测问题形式化为假设检验问题，提出基于多重检验的方法，并与机器学习中的分布外检测问题进行类比。

Result: 通过大量实验验证了所提出方法相比现有最先进方法具有更好的鲁棒性。

Conclusion: 提出的多重检验方法能够有效检测大语言模型中的幻觉，为解决模型幻觉问题提供了新的思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器翻译自动评估指标通常只考虑源句子和单个翻译，而人类评估时会参考多个备选翻译，这种评估设置差异可能影响自动指标的性能。

Method: COMET-polycand使用同一源句的多个备选翻译进行比较分析；COMET-polyic借鉴检索式上下文学习，输入相似源文本的翻译及其人工标注质量分数来指导评估。

Result: COMET-polycand加入单个额外翻译即可提升段级指标性能（Kendall's tau-b相关性从0.079提升至0.118），更多翻译带来进一步增益；COMET-polyic通过检索示例获得类似改进（0.079到0.116）。

Conclusion: 引入多个翻译或相似示例信息能显著提升机器翻译自动评估指标的性能，模型已公开发布。
...
TL;DR: 提出了一种自评估视觉隐喻生成框架，通过源-目标-意义(S-T-M)分解提示和轻量级强化学习来提高隐喻对齐效果，在分解、CLIP和意义对齐指标上超越强基线模型。


<details>
  <summary>Details</summary>
Motivation: 视觉隐喻生成需要同时理解语言含义并保持视觉连贯性，现有方法在隐喻对齐方面存在挑战，需要更好的评估和改进机制。

Method: 提出训练自由和训练增强两条路径：训练自由方法通过S-T-M分解提示进行图像合成；训练方法使用自评估奖励模式改进对齐，无需大规模重新训练。

Result: 训练自由方法在测试集上超越GPT-4o和Imagen等强基线模型，在分解、CLIP和意义对齐指标上表现优异；用户研究显示GPT-4o整体更受欢迎，但训练自由方法在开源方法中领先并在抽象隐喻上优于Imagen。

Conclusion: 结构化提示和轻量级强化学习在有限计算资源下能有效实现隐喻对齐，与人类偏好的差距主要来自美学质量和采样敏感性，S-T-M提示对长文本和抽象隐喻特别有效。
...
TL;DR: 本文认为大型语言模型主要建模的是训练语料库而非人类认知能力，基于Transformer架构的计算限制与人类语言处理的非线性特征差异


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型的本质——它们是在模拟人类认知能力还是仅仅在建模训练语料库，旨在为后一种观点提供非贬义性辩护

Method: 通过分析Transformer架构的计算特性（线性处理格式）与人类语言处理所需的超线性计算格式之间的差异，结合Liu等人关于快捷自动机的理论

Result: 论证了LLMs与人类语言处理在计算架构上的根本差异，支持了LLMs主要建模语料库而非人类认知能力的观点

Conclusion: 语言不仅是表达内在状态的工具，更是一种"话语机器"，人类和LLMs通过不同方式学会了使用这种技术，这并非贬义性的结论
...
TL;DR: NOOV是一个新的神经机器翻译系统，专门用于从英语到西班牙语的电子健康记录翻译，通过整合双语词典和短语查找表来解决未知词问题，在缺乏平行语料的情况下仍能实现准确流畅的翻译


<details>
  <summary>Details</summary>
Motivation: 电子健康记录从英语翻译到西班牙语具有重要临床意义，但面临缺乏平行对齐语料库和大量未知词汇的挑战

Method: 提出NOOV系统，整合从平行语料库自动学习的双语词典和从大型生物医学知识资源提取的短语查找表，缓解NMT中的未知词问题和词汇重复挑战

Result: 评估显示NOOV能够生成更好的电子健康记录翻译，在准确性和流畅性方面都有改进

Conclusion: NOOV系统有效解决了临床文本翻译中的关键挑战，为跨语言医疗信息交流提供了可行的解决方案
...
TL;DR: 本文通过实证研究建立了任务分层缩放定律，揭示了后训练量化(PTQ)对LLM知识能力的影响：知识记忆比知识利用对有效比特宽度、校准集大小和模型规模的变化更敏感


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署面临规模挑战，后训练量化是实用压缩方案，但缺乏对PTQ如何精确影响不同LLM知识能力的全面理解，现有量化模型缩放定律常忽略PTQ特定参数和任务敏感性

Method: 进行广泛实证研究，将LLM知识解构为记忆和利用能力，开发包含模型大小、有效比特宽度、校准集大小和分组大小的统一量化框架

Result: 核心发现表明知识记忆对有效比特宽度、校准集大小和模型大小的变化比更稳健的知识利用表现出明显更大的敏感性
...
TL;DR: TBYS框架通过在推理步骤间主动生成insight来指导LLMs的复杂推理过程，解决了训练数据中人类内在思维缺失的问题，在数学推理任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: LLMs在复杂推理任务（如数学）上表现不佳，主要原因是训练数据中缺乏人类的内在思维过程（意图和方法论），这些关键信息对于连接推理步骤至关重要。

Method: 提出了Thinking Before You Speak (TBYS)框架，在连续推理步骤之间插入主动生成的insight来审查状态并启动下一步推理。设计了自动收集和过滤上下文示例的流程，减少人工标注和微调开销。

Result: 在具有挑战性的数学数据集上的实验验证了TBYS框架的有效性。

Conclusion: 通过主动生成insight来模拟人类仔细思考的过程，可以有效提升LLMs在复杂推理任务上的表现，该方法相比静态提示策略更加灵活有效。
...
TL;DR: CoDec方法通过动态整合外部知识生成概率和内部参数生成概率，打破忠实性和表达性之间的权衡，在保持表达性的同时显著提升大语言模型输出的忠实性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在整合外部知识时难以同时保持忠实性和表达性，导致输出要么缺乏知识支持而失去忠实性，要么过于冗长而不自然。

Method: 提出协作解码(CoDe)方法，基于分布散度和模型置信度动态整合有无外部知识的输出概率，并引入知识感知重排序机制防止过度依赖先验参数知识。

Result: 实验表明CoDe框架在各种大语言模型和评估指标上都能在不牺牲表达性的情况下显著提升忠实性，验证了其有效性和泛化能力。

Conclusion: CoDe是一个即插即用的框架，能够有效解决大语言模型在知识整合中忠实性与表达性的权衡问题，具有很好的实用价值。
...
TL;DR: 提出Emotion Omni模型架构，通过有限数据和无需大规模训练的方式，实现语音LLM对用户语音情感的理解和共情语音响应生成


<details>
  <summary>Details</summary>
Motivation: 现有语音大语言模型仅简单转换回复内容为语音，未能充分理解用户查询中丰富的情感和副语言线索，且大多数具有共情能力的语音LLM需要海量数据和计算资源进行训练

Method: 提出新颖的Emotion Omni模型架构，并基于开源TTS框架开发数据生成管道，构建了20万条情感对话数据集

Result: 成功开发了能够理解用户语音情感内容并生成共情语音响应的模型，支持构建具有共情能力的语音助手

Conclusion: Emotion Omni为解决在有限数据条件下开发具有情感理解能力的语音LLM提供了有效方案，改善了人机交互中的用户体验
...
TL;DR: 提出基于难度平衡采样的多模态思维链提示框架，通过模型感知难度和样本内在复杂度来优化示例选择，显著提升多模态推理性能


<details>
  <summary>Details</summary>
Motivation: 传统多模态思维链提示使用随机或人工选择的示例，无法考虑模型特定知识分布和任务内在复杂度，导致性能次优且不稳定

Method: 将提示选择重构为提示课程设计问题，整合模型感知难度（通过主动学习中的预测分歧量化）和样本内在复杂度，开发难度平衡采样策略

Result: 在五个挑战性基准测试和多个流行多模态大语言模型上的实验表明，该方法带来显著且一致的改进，大幅减少随机采样导致的性能差异

Conclusion: 该方法为增强多模态推理提供了一种原则性和鲁棒性的方法，通过难度平衡的提示示例选择优化模型性能
...
TL;DR: 医学视觉语言模型在面对医学问题的语义等价重述时存在答案不一致的脆弱性问题，作者构建了RoMed数据集并提出了CCL方法来提升模型鲁棒性


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型在医学视觉问答中存在答案不一致的问题，当面对语义相同但表述不同的医学问题时，模型给出的答案会出现显著波动，这严重影响了医疗诊断的可靠性

Method: 构建了包含14.4万个问题的RoMed数据集，涵盖词汇级、句子级和语义级扰动；提出了CCL方法，包括知识锚定一致性学习和偏差感知对比学习两个关键组件

Result: 在RoMed数据集上评估SOTA模型发现性能显著下降（如召回率下降40%）；CCL方法在三个流行VQA基准测试中达到SOTA性能，并将答案一致性提高了50%

Conclusion: 医学视觉语言模型存在严重的鲁棒性问题，通过CCL方法可以有效提升模型的一致性和鲁棒性，为高风险的医疗应用提供更可靠的诊断支持
...
Method: 提出Attention2Probability方法，将语音与术语之间的交叉注意力权重转换为存在概率，并采用课程学习来增强检索准确性。同时创建并发布了包含术语的语音数据集。

Result: 实验结果显示，Attention2Probability显著优于VectorDB方法，中文最大召回率达92.57%，英文达86.83%，每查询延迟仅8.71ms。术语干预使SLM的术语准确率提升6-17%。

Conclusion: 该方法有效提升了语音大模型处理领域术语的能力，但当前SLM对术语的利用仍存在局限性，需要进一步研究改进。
...
TL;DR: 本文提出了自适应原创性过滤(AOF)提示框架，通过余弦相似度过滤冗余生成，在多语言谜语生成任务中显著提升了GPT-4o的词汇多样性和文化适应性。


<details>
  <summary>Details</summary>
Motivation: 解决多语言谜语生成中LLMs倾向于重复记忆谜语或进行浅层改写的问题，需要在文化流畅性和创造性抽象之间取得平衡。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在客服中的广泛应用带来了用户冒充风险，现有检测方法在在线对话场景中可靠性不足，且缺乏对非专家用户的解释性。

Method: 提出EMMM框架，采用先解释后检测的方法，平衡延迟、准确性和非专家导向的可解释性。

Result: 实验显示EMMM为非专家用户提供可理解的解释，70%的人类评估者偏好其输出，在保持1秒内低延迟的同时达到与最先进模型相当的准确率。

Conclusion: EMMM框架成功解决了客服场景中机器生成文本检测的可解释性问题，为可信AI部署提供了实用解决方案。
...
Conclusion: DIVER框架成功解决了广告标题生成中质量与多样性的平衡问题，通过联合优化实现了实际业务指标的显著提升，证明了该方法的有效性和实用性。
...
TL;DR: 本文提出了M3HG模型和MECAD数据集，用于多模态对话中的情感原因三元组提取，解决了现有数据集稀缺和模型忽略情感因果上下文的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态情感原因三元组提取(MECTEC)领域缺乏多样化数据集，现有方法未能显式建模情感和因果上下文，且忽略了不同层次语义信息的融合，导致性能下降。

Method: 提出M3HG模型，通过多模态异构图显式捕捉情感和因果上下文，并在话语间和话语内两个层次有效融合上下文信息。同时构建了首个多模态多场景MECAD数据集。

Result: 大量实验证明M3HG模型相比现有最先进方法具有更好的效果，代码和数据集已开源。

Conclusion: M3HG模型通过显式建模情感因果上下文和多层次信息融合，有效提升了多模态对话中情感原因三元组提取的性能，MECAD数据集为该领域研究提供了重要资源。
...
Conclusion: 时序顺序推理对于解决叙事问答任务至关重要，ChronoRAG有效解决了现有RAG方法在叙事文本处理中的局限性
...
TL;DR: ThinkDial是首个开源端到端框架，通过离散操作模式实现类似GPT-oss的可控推理，提供高、中、低三种推理模式，在显著减少计算token的同时保持性能阈值。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具有强大的推理能力，但计算成本控制仍是实际部署的重大挑战。开源社区缺乏类似GPT-oss的离散操作模式控制能力。

Method: 采用端到端训练范式：预算模式监督微调将可控推理能力嵌入学习过程，以及两阶段预算感知强化学习与自适应奖励塑造。

Result: 实现了目标压缩-性能权衡，高模式（全能力）、中模式（50% token减少，性能下降<10%）、低模式（75% token减少，性能下降<15%），并在分布外任务上表现出强泛化能力。

Conclusion: ThinkDial成功实现了开源可控推理框架，为实际部署提供了有效的计算成本控制解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统编码器-解码器模型在语法纠错任务中已有一定成功，但大语言模型在该领域的应用仍未被充分探索。当前研究主要依赖监督微调来训练LLMs直接生成修正句子，这限制了模型强大的推理能力

Method: 提出基于规则强化学习（Rule-Based RL）的新框架，通过强化学习来引导大语言模型进行语法纠错

Result: 在中文数据集上的实验表明，基于规则的RL框架达到了最先进的性能，召回率显著提升

Conclusion: 结果清晰展示了使用强化学习引导大语言模型的优势，为语法纠错领域的未来发展提供了更可控和可靠的范式
...
<details>
  <summary>Details</summary>
Motivation: 随着语音和自然语言处理技术的进步，对话分析领域需要更高效的自动化方法来处理大规模对话数据，特别是在客户支持和销售等领域。传统意图检测方法使用固定意图集，而主题检测提供了更灵活的用户面向对话摘要。

Method: 提出了可控对话主题检测问题，将其定义为对话话语的联合聚类和主题标注，通过用户偏好数据控制主题簇的粒度。使用公开数据集和自动/人工评估指标进行评测。

Result: 在DSTC 12竞赛中设立了该赛道，提供了相关数据集和评估方法，收集了参赛团队的解决方案并进行了分析。所有材料在GitHub上公开可用。

Conclusion: 主题检测是对话分析中的重要任务，通过可控聚类方法可以有效识别对话核心主题，为大规模对话分析提供自动化解决方案，具有实际应用价值。
...
TL;DR: LaTeXTrans是一个多智能体协作系统，专门用于翻译LaTeX格式文档，在保持格式完整性和结构保真度方面优于主流机器翻译系统


<details>
  <summary>Details</summary>
Motivation: 尽管现代机器翻译系统在通用文本上取得了显著进展，但翻译结构化LaTeX格式文档仍然是一个重大挑战，这些文档通常交织自然语言和特定领域语法（如数学公式、表格、图形和交叉引用），需要准确保持语义完整性和可编译性

Method: LaTeXTrans采用六个专门化智能体协作：1)解析器通过占位符替换和语法过滤将LaTeX分解为翻译友好单元；2)翻译器、验证器、总结器和术语提取器协作确保上下文感知、自我纠正和术语一致的翻译；3)生成器将翻译内容重构为结构良好的LaTeX文档
...
Method: 整合抽象意义表示(AMR)的复杂语义关系和新闻传播动态，使用LLM生成的负锚点图对比损失(LGCL)增强特征可分性，采用多视图图掩码自编码器学习社交上下文图的传播特征

Result: 大量实验表明，该自监督框架在有限标注数据集下相比其他最先进方法实现了更优性能，并提高了泛化能力

Conclusion: 该研究提出的结合语义和传播特征的自监督框架能有效区分真假新闻，在资源受限情况下仍能取得良好效果
...
TL;DR: 提出程序辅助合成框架，系统生成高质量数学语料库，通过双边验证机制保证数据多样性和正确性，显著提升大语言模型的数学推理能力
...
TL;DR: ConfTuner是一种通过微调方法来校准大语言模型置信度的技术，使用tokenized Brier score损失函数，无需真实置信度标签即可改善模型校准性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在科学、法律、医疗等高风险领域部署时存在过度自信问题，生成错误答案却表达高置信度，影响可靠性和信任度。现有方法效果有限且泛化性差。

Method: 提出ConfTuner微调方法，基于tokenized Brier score损失函数（理论证明为proper scoring rule），无需真实置信度分数或代理置信度估计，通过最小化损失来校准模型置信度表达。

Result: 在多种推理任务上显著改善校准性能，可泛化到GPT-4o等黑盒模型，更好的校准置信度带来了自我纠正和模型级联等下游任务的性能提升。

Conclusion: ConfTuner推进了可信赖大语言模型系统的开发，通过简单高效的微调方法有效解决了LLM过度自信问题，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的快速发展，自动选择优化提示的需求日益增长，需要更精确和全面的提示搜索方法

Method: 基于进化算法的自反进化方法，在交叉和精英变异前使用短期和长期反射操作来增强修改质量，并在每个epoch基于当前种群更新积累的知识

Result: 在33个分类和文本生成数据集上测试显示，相比最先进方法平均显著改进28%（如在BBH上相比EvoPrompt）

Conclusion: ReflectivePrompt成为进化算法基自动提示中最有效的解决方案之一
...
TL;DR: 提出了一种结合大型语言模型的内容分析方法(LACA)，用于处理计算教育研究中的大量文本数据，帮助研究者进行更大规模的研究


<details>
  <summary>Details</summary>
Motivation: 计算教育研究(CER)领域的研究者通常缺乏资源进行大规模、可泛化的研究，需要能够处理大量定性数据而不增加研究者负担的方法

Method: LLM辅助内容分析(LACA)方法，结合传统内容分析和大型语言模型技术，使用计算教育数据集进行可复现的严谨分析

Result: 该方法能够使研究者进行原本无法完成的大规模研究，产生更可泛化的研究结果

Conclusion: LACA方法在计算教育研究领域具有潜力，结合类似方法的发展可以推动CER学科实践和研究质量的进步
...
TL;DR: 本研究使用自然语言处理技术自动分析欧洲六个国家议会演讲，发现所有议会都存在情感极化现象，即对反对派表现出更多负面情绪，且互惠机制是情感极化的促成因素。


<details>
  <summary>Details</summary>
Motivation: 情感极化（对对立群体表现出更多负面情绪和敌意）已成为全球政治话语的显著特征，需要系统性地研究欧洲议会中是否存在这种现象。

Method: 收集六个欧洲国家议会的演讲语料库，运用自然语言处理技术估计议员的情绪倾向，通过比较对反对派和己方成员的负面情绪水平来识别情感极化模式。

Result: 在所有六个欧洲议会中都发现了持续的情感极化现象；活动程度与负面情绪相关，但活跃度不同的议员之间在情感极化程度上没有差异；互惠机制是议会成员间情感极化的促成因素。

Conclusion: 欧洲议会普遍存在情感极化，这种极化现象具有跨国的普遍性，且通过互惠机制得以维持，这对理解现代民主政治的健康状况具有重要意义。
...
TL;DR: 提出了一种新颖的多智能体框架来生成用于RAG评估的合成QA数据集，重点关注语义多样性和隐私保护，通过三个智能体分别处理多样性、隐私检测和QA对合成。
...
Method: 采用多智能体框架：1)多样性智能体使用聚类技术最大化主题覆盖和语义变异性；2)隐私智能体检测和屏蔽多领域的敏感信息；3)QA策划智能体合成适合作为RAG评估基准的私密且多样的QA对。

Result: 实验表明，我们的评估集在多样性方面优于基线方法，并在领域特定数据集上实现了强大的隐私屏蔽效果。
...
TL;DR: 提出AI母语框架，让神经网络模型发展原生符号语言，同时支持直觉推理、组合符号链和内在可解释性，通过符号捕获语义模式、链式追踪决策路径，实现透明灵活的推理。


<details>
  <summary>Details</summary>
Motivation: 解决传统后解释方法的问题，将推理直接嵌入模型表示中，实现内在可解释性，同时保持神经模型的灵活性。

Method: 引入互补训练目标增强符号纯度和决策稀疏性，采用顺序专业化策略先建立广泛符号能力再细化直觉判断，使用门控归纳机制指导选择性关注。

Result: 在AI任务实验中展示出竞争性准确度，同时提供可验证的推理轨迹，证明AI母语可作为统一机制支持可解释性、直觉和符号推理。

Conclusion: AI母语框架成功实现了神经模型中可解释性、直觉推理和符号推理的统一，为构建透明且强大的AI系统提供了新途径。
...
TL;DR: DistillPrompt是一种基于大语言模型的新型自动提示方法，通过多阶段集成任务特定信息到提示中，在文本分类和生成任务上相比现有方法取得了显著改进


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型研究的快速发展，提示工程日益重要，自动选择优化提示的需求不断增加

Method: 采用蒸馏、压缩和聚合操作的多阶段集成方法，将训练数据中的任务特定信息整合到提示中，更全面地探索提示空间

Result: 在不同数据集的文本分类和生成任务上测试，相比现有方法（如Grips）在关键指标上平均提升20.12%
...
Conclusion: 该工作推动了AI系统对电影理解的发展，揭示了当前VQA模型在处理具有挑战性的电影内容问题时的能力和局限性，为未来研究提供了有价值的见解和资源。
...
TL;DR: HiPlan是一个分层规划框架，通过自适应全局-局部指导来提升LLM智能体的决策能力，在复杂长时域规划任务中显著优于现有基线方法


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在复杂长时域规划场景中缺乏宏观指导和持续监督，容易迷失方向并对环境变化反应不足

Method: 分层规划框架：离线阶段构建里程碑库，执行阶段动态适配轨迹片段生成逐步提示，将当前观察与里程碑目标对齐

Result: 在两个挑战性基准测试中，HiPlan大幅超越强基线方法，消融研究验证了分层组件的互补效益

Conclusion: HiPlan通过结构化经验重用和动态适配机制，有效解决了LLM智能体在复杂规划任务中的迷失和偏差问题
...
TL;DR: 本研究首次分析医患对话中的医生意图轨迹，开发了基于SOAP框架的细粒度意图分类体系，建立了大规模标注数据集，并评估了现有模型在医疗意图分类任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 医患对话中医生通过有针对性的提问来收集信息并制定治疗方案，但现有研究缺乏对医生意图轨迹的系统分析，需要建立标准化的意图分类体系和评估基准。

Method: 使用ACI-bench数据集，与医学专家合作开发基于SOAP框架的细粒度意图分类法，通过众包平台招募医学专家对5000多个医患对话轮次进行大规模标注。

Result: 模型能够高精度理解医疗对话的一般结构，但在识别SOAP类别转换方面表现不佳；首次报告了医疗对话结构的常见轨迹；意图过滤显著提升了医疗对话摘要性能。

Conclusion: 本研究提供了首个医疗意图分类的大规模标注数据集和基准，揭示了模型在意图识别方面的局限性，为设计鉴别诊断系统提供了宝贵见解，相关代码和数据已公开。
...
TL;DR: 本文分析了大型语言模型(LLMs)如何通过上下文学习(ICL)和参数高效微调(PEFT)来支持极低资源语言，发现零样本ICL加语言对齐对极低资源语言效果显著，而少样本ICL或PEFT更适合相对资源较多的语言。


<details>
  <summary>Details</summary>
Motivation: 极低资源语言，特别是使用稀有文字的语言，由于缺乏训练数据等原因，在大型语言模型中得不到充分支持。本文旨在全面分析LLMs是否能够通过上下文学习来掌握这些语言。

Method: 系统评估了20种代表性不足的语言，在三种最先进的多语言LLMs上测试了零样本/少样本ICL（带或不带辅助对齐信号）以及PEFT方法的性能。

Result: 研究发现当语言及其文字都极低代表性时，PEFT存在局限性；零样本ICL加语言对齐对极低资源语言效果显著；少样本ICL或PEFT对相对资源较多的语言更有利。

Conclusion: 为LLM从业者提供了基于结果的指导原则：避免在未见文字的语言上微调多语言模型，零样本ICL加对齐是处理极低资源语言的有效方法。
...
TL;DR: 本文提出了一个基于检索增强生成(RAG)的艺术品溯源研究框架，针对盖蒂溯源索引数据库，通过语义检索和上下文摘要实现自然语言和多语言搜索，解决了艺术品溯源研究中碎片化多语言档案数据检索效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 艺术品溯源研究对于验证真伪、支持归还和法律主张、理解艺术品文化历史背景至关重要，但当前搜索门户需要精确元数据，限制了探索性搜索，且碎片化的多语言档案数据阻碍了高效检索。

Method: 采用检索增强生成(RAG)框架，结合语义检索和上下文摘要技术，实现对盖蒂溯源索引-德国拍卖记录的10,000条样本数据进行自然语言和多语言搜索，减少对元数据结构的依赖。

Result: 该方法能够有效检索和总结拍卖记录，为导航艺术市场档案提供了可扩展的解决方案，为历史学家和文化遗产专业人士进行历史敏感性研究提供了实用工具。

Conclusion: RAG框架为艺术品溯源研究提供了一种创新的技术路径，通过语义检索和生成式摘要显著提升了多语言档案数据的检索效率和用户体验，具有重要的实践应用价值。
...
TL;DR: 提出结合词汇和语义方法的透明混合框架，用于定量话语分析，通过Python工具链实现可重现和可解释的分析流程


<details>
  <summary>Details</summary>
Motivation: 解决现有黑盒软件（如MAXQDA、NVivo）在方法论透明度和研究目标对齐方面的不足，强调代码级透明度和研究者自主性的重要性

Method: 使用Python工具链（NLTK、spaCy、Sentence Transformers）构建自定义分析流程，包括BERTopic建模、UMAP降维、HDBSCAN聚类和c-TF-IDF关键词提取，通过参数调优和多轮运行优化主题一致性和覆盖度

Result: 通过历史政治话语案例研究，展示了词汇精确搜索与上下文感知语义聚类的对比分析，验证了混合方法的有效性

Conclusion: 多层分析方法能够缓解单一方法的局限性，强调了方法论三角验证、代码透明度和研究者能动性在计算话语研究中的重要性
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉问答中表现出色但存在幻觉问题，需要可靠地感知自身知识边界（知道什么知道、什么不知道）

Method: 评估三种置信度信号：概率置信度、答案一致性置信度、语言化置信度；在三个LVLM模型和三个VQA数据集上进行实验；从大型语言模型适配多种置信度校准方法并提出三种有效方法

Result: LVLMs具有一定程度的感知能力但仍有很大改进空间；概率和一致性信号更可靠，语言化置信度常导致过度自信；联合处理视觉和文本输入会降低问答性能但减少置信度，相比LLMs有更好的感知水平

Conclusion: LVLMs的知识边界感知能力需要进一步提升，概率和一致性信号是更可靠的指标，提出的校准方法能有效改善感知能力，多模态处理有助于提高感知水平
...
TL;DR: SciReas和SciReas-Pro科学推理评测套件，结合KRUX探测框架分析LLM在科学推理中知识与推理的作用，发现知识检索是瓶颈，外部知识能提升推理，增强推理能力有助于知识提取。
...
Method: 引入SciReas和SciReas-Pro评测套件，提出KRUX探测框架来分离知识和推理的作用，通过实验分析知识检索、外部知识增强和推理能力提升的效果。

Result: 发现知识检索是LLM科学推理的关键瓶颈；外部上下文知识能进一步提升推理模型性能；增强推理能力有助于提取任务相关知识；发布了SciLit01作为8B参数的科学推理基线模型。

Conclusion: 该研究提供了全面的科学推理评估框架，揭示了知识与推理在科学问题解决中的相互作用，为开发更好的科学推理AI系统提供了重要见解和基准。
...
Method: 采用next-token扩散方法，这是一种通过扩散自回归生成潜在向量的统一连续数据建模方法，并引入了新型连续语音分词器来大幅提升数据压缩率。

Result: 相比流行的Encodec模型，数据压缩率提升80倍，同时保持可比性能。能够在64K上下文窗口长度下合成长达90分钟、最多4个说话人的语音，成功捕捉真实对话氛围。

Conclusion: VibeVoice在长语音合成和多说话人对话建模方面超越了开源和专有对话模型，为高效长序列语音处理提供了有效解决方案。
...
TL;DR: 本文对普通语言摘要的可读性评估方法进行了系统调查，发现传统可读性指标与人类判断相关性差，而语言模型能更好地评估可读性，特别是在背景知识需求等深层维度上。


<details>
  <summary>Details</summary>
Motivation: 当前普通语言摘要领域普遍使用传统可读性指标（如FKGL）进行评估，但这些指标在PLA领域尚未与人类可读性判断进行系统比较，需要验证其有效性。

Method: 评估了8种传统可读性指标与人类判断的相关性，并测试了语言模型作为可读性评估工具的性能，分析了PLA数据集中的可读性特征。

Result: 大多数传统可读性指标与人类判断相关性较差，最佳语言模型与人类判断的Pearson相关系数达到0.56，在背景知识等深层可读性维度上表现更优。

Conclusion: 语言模型比传统指标更适合评估普通语言摘要的可读性，建议在PLA评估中采用基于语言模型的方法，并发布了分析代码和调查数据。
...
TL;DR: 论文提出生成式界面范式，让大语言模型主动生成用户界面来增强交互体验，相比传统对话界面更受用户青睐


<details>
  <summary>Details</summary>
Motivation: 传统线性请求-响应格式在大语言模型交互中存在效率问题，特别是在多轮、信息密集和探索性任务中表现不佳

Method: 提出生成式语言模型界面框架，利用结构化界面特定表示和迭代优化，将用户查询转换为任务特定的用户界面

Result: 生成式界面在多样化任务中持续优于对话界面，人类用户在70%以上的情况下更偏好生成式界面

Conclusion: 生成式界面为人类-AI交互提供了新的发展方向，研究结果明确了用户偏好生成式界面的时机和原因
...
<details>
  <summary>Details</summary>
Motivation: 电磁信号具有高度异质性、强背景噪声和复杂时频结构，现有通用模型无法直接使用，且缺乏跨任务泛化和高质量数据集

Method: 构建首个统一标准化电磁信号数据集，开发长度自适应多信号打包方法和硬件感知训练策略，利用电磁信号物理特性进行表示学习

Result: EMind在多个下游任务中表现出强大的性能和广泛的泛化能力

Conclusion: 该模型实现了从任务特定模型向电磁智能统一框架的转变，为动态频谱管理等领域提供了有效解决方案
...
### [127] [Reasoning Steps as Curriculum: Using Depth of Thought as a Difficulty Signal for Tuning LLMs](https://arxiv.org/abs/2508.18279)
...
<details>
  <summary>Details</summary>
Motivation: 需要一种与推理能力对齐、可扩展且可解释的难度信号来指导大语言模型的课程学习

Method: 将思维深度定义为教师模型推理轨迹中的离散步骤数量，构建从浅到深的训练课程，包括难度推导、验证和调度方法

Result: 提出三个可验证假设：DoT与推理基准难度相关、DoT排序课程优于长度或评分排序课程、难度在不同教师模型间具有鲁棒性

Conclusion: 该方法为推理中心训练提供了认知基础扎实、可解释的课程学习框架，推动更有效的语言模型训练
...
### [128] [Multi-Modal Drift Forecasting of Leeway Objects via Navier-Stokes-Guided CNN and Sequence-to-Sequence Attention-Based Models](https://arxiv.org/abs/2508.18284)
*Rahmat K. Adesunkanmi,Alexander W. Brandt,Masoud Deylami,Gustavo A. Giraldo Echeverri,Hamidreza Karbasian,Adel Alaeddini*

Main category: cs.LG

TL;DR: 本文提出了一种多模态机器学习框架，结合Sentence Transformer嵌入和注意力机制序列到序列架构，用于预测海上漂流物的漂移轨迹。该方法整合了物理环境数据、物体特征和文本描述，在多个时间尺度上实现了准确的长期预测。


<details>
  <summary>Details</summary>
Motivation: 准确预测海上漂流物的位移对于搜救等时间敏感场景至关重要，但现有方法在长期预测和适应性方面存在局限，需要开发更精确的多模态预测框架。

Method: 通过实验收集环境数据（水流、风速）和物体物理特征（质量、表面积），使用Navier-Stokes模型训练CNN估计阻力系数，结合语言模型编码的文本描述，输入到基于注意力的LSTM和Transformer模型中进行序列预测。

Result: 多模态模型在1、3、5、10秒多个时间尺度上表现与传统模型相当，同时能够实现长期预测而非单步预测，展示了良好的泛化能力。

Conclusion: 多模态建模策略能够为动态海洋环境中的漂流物漂移提供准确且适应性强的预测，为海上搜救等应用提供了有效的技术解决方案。
...
### [129] [Data-driven models for production forecasting and decision supporting in petroleum reservoirs](https://arxiv.org/abs/2508.18289)
*Mateus A. Fernandes,Michael M. Furlanetti,Eduardo Gildin,Marcio A. Sampaio*

Main category: cs.LG

TL;DR: 基于机器学习的石油储层生产预测方法，使用生产注入数据预测参数，不依赖地质模型，处理概念漂移问题，在合成和实际数据上验证


<details>
  <summary>Details</summary>
Motivation: 石油储层工程中可靠预测生产和预判岩石-流体系统行为变化是主要挑战，需要开发不依赖复杂地质信息的预测方法

Method: 使用监督学习方法（回归和神经网络），进行变量相关性分析，处理概念漂移，先在UNISIM III合成数据上评估，再应用于巴西盐下实际油田

Result: 设计出可靠的储层动态预测器，具有快速响应能力，能处理实际限制条件，支持储层管理决策

Conclusion: 该方法能有效预测生产参数，最大化石油采收率，支持生产优化和概率事件影响分析
...
### [130] [A Fast and Minimal System to Identify Depression Using Smartphones: Explainable Machine Learning-Based Approach](https://arxiv.org/abs/2508.18301)
*Md Sabbir Ahmed,Nova Ahmed*

Main category: cs.LG

TL;DR: 开发了一个快速检测抑郁症的系统，仅需1秒收集7天应用使用数据，使用机器学习模型识别抑郁症，准确率达82.4%，适用于资源有限地区


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测系统需要长期数据收集，无法满足早期快速检测需求，特别是在资源有限的欠发达和发展中地区

Method: 开发快速工具收集7天应用使用数据（平均0.31秒），使用多种机器学习模型和特征选择方法（包括stable FS和Boruta），对100名孟加拉学生进行研究

Result: 轻量梯度提升机模型使用stable FS选择的重要特征，正确识别82.4%的抑郁症学生（精确度75%，F1分数78.5%）；简约堆叠模型使用约5个特征，最大精确度77.4%（平衡准确率77.9%）

Conclusion: 该系统快速简约的特性使其在欠发达和发展中地区具有应用价值，研究发现有助于开发资源消耗更少的系统来更好地理解抑郁症学生
...
### [131] [Learning Explainable Imaging-Genetics Associations Related to a Neurological Disorder](https://arxiv.org/abs/2508.18303)
*Jueqi Wang,Zachary Jacokes,John Darrell Van Horn,Michael C. Schatz,Kevin A. Pelphrey,Archana Venkataraman*

Main category: cs.LG

TL;DR: NeuroPathX是一个可解释的深度学习框架，通过交叉注意力机制融合MRI脑结构数据和遗传学通路数据，用于分析神经疾病的成像-遗传学关联。


<details>
  <summary>Details</summary>
Motivation: 传统成像-遗传学方法局限于简单线性模型或缺乏可解释性的黑盒技术，无法有效捕捉脑结构与遗传变异之间的复杂相互作用。
...
### [132] [SALMAN: Stability Analysis of Language Models Through the Maps Between Graph-based Manifolds](https://arxiv.org/abs/2508.18306)
*Wuxinlin Cheng,Yupeng Cao,Jinwen Wu,Koduvayur Subbalakshmi,Tian Han,Zhuo Feng*

Main category: cs.LG

TL;DR: 提出了一个统一的局部鲁棒性框架SALMAN，通过距离映射失真(DMD)度量来评估模型稳定性，无需修改内部参数或复杂扰动启发式


<details>
  <summary>Details</summary>
Motivation: 随着预训练transformer语言模型规模增大和部署增多，其输入扰动下的鲁棒性成为紧迫问题。现有方法在小参数和大规模模型间存在差异，且依赖劳动密集型的样本特定对抗设计

Method: 提出SALMAN框架，使用新颖的距离映射失真(DMD)度量，通过比较输入到输出的距离映射来排名每个样本的易受攻击性，具有近线性复杂度

Result: 在攻击效率和鲁棒训练方面展示了显著提升
...
### [133] [Learning Spatio-Temporal Dynamics via Operator-Valued RKHS and Kernel Koopman Methods](https://arxiv.org/abs/2508.18307)
*Mahishanka Withanachchi*

Main category: cs.LG

TL;DR: 提出了一个结合算子值再生核希尔伯特空间(OV-RKHS)和基于核的Koopman算子方法的统一框架，用于学习向量值函数的时空动力学，实现非参数和数据驱动的复杂时变向量场估计。
...
Conclusion: 该框架为时空机器学习提供了理论基础的预测、控制和不确定性量化工具，能够非参数和数据驱动地估计复杂时变向量场，同时保持时空结构。
...
### [134] [CoPE: A Lightweight Complex Positional Encoding](https://arxiv.org/abs/2508.18308)
*Avinash Amballa*

Main category: cs.LG

TL;DR: CoPE是一种轻量级复数位置编码方法，使用复数嵌入同时编码语义内容和位置信息，实部捕获语义，虚部编码位置，在GLUE基准测试中表现优于其他位置编码方法且计算复杂度更低


<details>
  <summary>Details</summary>
Motivation: 传统位置编码方法在建模序列元素间依赖关系时存在局限性，需要更有效的位置编码方式来提供位置信息指导

Method: 提出CoPE复数位置编码架构，用复数嵌入替代传统位置编码，实部捕获语义内容，虚部编码位置信息，在transformer第一层使用相位感知注意力捕获位置相关模式，后续层使用标准注意力

Result: 在GLUE基准测试中表现优于RoPE、正弦和可学习位置编码，计算复杂度更低，且不出现长期衰减问题，与线性注意力兼容

Conclusion: CoPE通过复数编码方式有效整合了语义和位置信息，在保持高性能的同时降低了计算复杂度，为位置编码提供了新的有效解决方案
...
### [135] [What Matters in Data for DPO?](https://arxiv.org/abs/2508.18312)
*Yu Pan,Zhongze Cai,Guanting Chen,Huaiyang Zhong,Chonghuan Wang*

Main category: cs.LG

TL;DR: 本文系统研究了偏好数据分布对DPO算法性能的影响，发现被选回答的质量对DPO优化起主导作用，而被拒绝回答的质量影响相对有限。


<details>
  <summary>Details</summary>
Motivation: 尽管DPO已成为对齐大型语言模型的有效方法，但偏好数据的哪些特性对DPO性能最关键仍然是一个未解的基本问题。

Method: 从理论和实证两个角度系统研究偏好数据分布对DPO的影响，包括理论分析最优响应分布、在线DPO设置研究，以及在多样化任务上进行广泛实验。

Result: 理论分析表明对比性主要通过改善被选样本来帮助DPO，在线DPO实际上简化为对被选回答的监督微调。实验证实提升被选回答质量能持续提高性能，与被拒绝回答质量无关。

Conclusion: 研究结果解释了广泛采用策略背后的机制，并为构建高质量偏好数据集提供了实用见解，强调被选回答质量在LLM对齐中的关键作用。
...
### [136] [ProtoEHR: Hierarchical Prototype Learning for EHR-based Healthcare Predictions](https://arxiv.org/abs/2508.18313)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究往往只关注电子健康记录(EHR)数据的孤立组成部分，限制了预测性能和可解释性。需要一种能够充分利用EHR丰富多层次结构的方法。

Method: 利用大语言模型提取医疗代码间的语义关系构建医学知识图谱，设计分层表示学习框架捕获三个层次(医疗代码、医院就诊、患者)的上下文表示，并在每个层次融入原型信息来捕捉内在相似性。

Result: 在两个公共数据集上对五个临床重要任务进行评估，包括死亡率预测、再入院预测、住院时间预测、药物推荐和表型预测，结果显示ProtoEHR相比基线方法能够做出更准确、鲁棒和可解释的预测。

Conclusion: ProtoEHR框架能够充分利用EHR数据的多层次结构，提供准确且可解释的医疗预测，同时在代码、就诊和患者三个层次提供可解释的洞察，有助于医疗决策。
...
### [137] [Evaluating Federated Learning for At-Risk Student Prediction: A Comparative Analysis of Model Complexity and Data Balancing](https://arxiv.org/abs/2508.18316)
*Rodrigo Tertulino*

Main category: cs.LG

TL;DR: 本研究开发了一个基于联邦学习的机器学习模型，利用早期学术表现和数字参与模式来预测远程教育中的高风险学生，在保护数据隐私的同时实现了85%的ROC AUC预测性能。


<details>
  <summary>Details</summary>
Motivation: 远程教育中的高辍学率和失败率对学术机构构成重大挑战，需要提前识别有风险的学生以提供及时支持，同时解决数据隐私和机构数据孤岛等实际问题。

Method: 使用OULAD大规模数据集，基于联邦学习框架开发机器学习模型，比较逻辑回归和深度神经网络两种模型复杂度，并进行数据平衡处理。

Result: 联邦学习模型表现出强大的预测能力，在识别高风险学生方面达到约85%的ROC AUC分数。

Conclusion: 联邦学习方法为机构提供了一个实用且可扩展的解决方案，能够构建有效的早期预警系统，在尊重数据隐私的同时实现主动的学生支持。
...
### [138] [ZTFed-MAS2S: A Zero-Trust Federated Learning Framework with Verifiable Privacy and Trust-Aware Aggregation for Wind Power Data Imputation](https://arxiv.org/abs/2508.18318)
*Yang Li,Hanjie Wang,Yuanzheng Li,Jiazheng Li,Zhaoyang Dong*

Main category: cs.LG

TL;DR: ZTFed-MAS2S是一个零信任联邦学习框架，用于风电数据缺失值填补，集成了差分隐私、零知识证明和动态信任聚合机制，确保隐私保护和安全性。
...
### [139] [Linear cost mutual information estimation and independence test of similar performance as HSIC](https://arxiv.org/abs/2508.18338)
*Jarek Duda,Jagoda Bracha,Adrian Przybysz*

Main category: cs.LG

TL;DR: HSIC是当前最先进的统计依赖性评估方法，但计算复杂度高达O(n^2.37)，不适用于大数据集。本文提出HCR（分层相关重建）方法，具有线性计算成本O(n)，更高的依赖性检测灵敏度，并能提供联合分布模型。


<details>
  <summary>Details</summary>
Motivation: HSIC方法虽然先进，但其O(n^2.37)的计算复杂度限制了在大规模数据集上的应用，需要寻找更高效的替代方案。

Method: 提出HCR方法，通过混合矩特征描述依赖性，从相关性和同方差性开始，能够近似互信息。单个依赖性特征的计算时间为线性O(n)。

Result: HCR方法在测试中表现出比HSIC更高的依赖性灵敏度，同时计算成本显著降低，适用于大规模数据分析。

Conclusion: HCR是HSIC的实用线性成本替代方案，不仅计算效率更高，还能提供更丰富的依赖性描述，为大规模统计依赖性分析提供了可行解决方案。
...
### [140] [DualSparse-MoE: Coordinating Tensor/Neuron-Level Sparsity with Expert Partition and Reconstruction](https://arxiv.org/abs/2508.18376)
...
<details>
  <summary>Details</summary>
Motivation: MoE架构虽然通过稀疏激活减少计算量，但仍面临大规模计算和不可预测激活模式的挑战。研究发现预训练MoE模块中存在张量和神经元双重稀疏性，可作为提升效率和精度的关键因素

Method: 引入训练后专家划分技术，在不重新训练的情况下诱导张量级稀疏性；提出DualSparse-MoE推理系统，集成动态张量级计算丢弃和静态神经元级重建

Result: 在25%丢弃率下，三个主流MoE模型的平均精度仅下降0.08%-0.28%；几乎所有计算丢弃程度都能带来成比例的计算加速；结合负载均衡感知的专家并行实现了1.41倍加速，精度损失仅0.5%

Conclusion: 通过利用MoE模块中的双重稀疏性，可以在几乎不损失精度的情况下显著提升推理效率，为大规模MoE模型的部署提供了有效的解决方案
...
### [141] [Low-Rank Tensor Decompositions for the Theory of Neural Networks](https://arxiv.org/abs/2508.18408)
*Ricardo Borsoi,Konstantin Usevich,Marianne Clausel*

Main category: cs.LG

TL;DR: 本文综述了低秩张量分解在深度神经网络理论中的基础作用，包括表达能力、可学习性、计算复杂性、泛化能力和可识别性等方面，旨在统一不同领域的研究方法。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络性能突破性提升激发了对其数学基础的理论研究需求，低秩张量分解因其与神经网络的密切联系和丰富理论结果而成为合适工具。

Method: 通过系统综述方法，整合计算机科学、数学等不同领域的研究成果，分析低秩张量分解在神经网络理论中的各种应用。

Result: 展示了低秩张量方法在解释深度神经网络性能多个方面的重要作用，包括表达能力、算法可学习性、计算复杂性、泛化能力和可识别性等。

Conclusion: 低秩张量分解为深度神经网络理论提供了统一的理论框架，未来有望在更广泛视角下推动该领域发展。
...
### [142] [LLM-Driven Intrinsic Motivation for Sparse Reward Reinforcement Learning](https://arxiv.org/abs/2508.18420)
*André Quadros,Cassio Silva,Ronnie Alves*

Main category: cs.LG

TL;DR: 结合VAE状态新颖性奖励和LLM目标导向奖励的混合内在动机策略，在稀疏奖励环境中显著提升强化学习效率和性能


<details>
  <summary>Details</summary>
Motivation: 解决极端稀疏奖励环境下传统强化学习因正反馈稀少而难以有效学习的问题，通过结合两种内在动机策略来提高学习效率

Method: 提出VSIMR（变分状态内在奖励）使用VAE奖励状态新颖性，结合基于LLM预训练知识的环境和目标描述生成奖励信号，在MiniGrid DoorKey环境中使用A2C智能体实现

Result: 混合策略相比单独使用任一策略或标准A2C智能体，显著提高了智能体性能和采样效率，标准A2C完全无法学习

Conclusion: VSIMR驱动状态探索，LLM奖励促进目标导向利用，两种策略在环境和任务的不同方面形成有效互补，为稀疏奖励RL问题提供了有效解决方案
...
### [143] [Enhancing Trust-Region Bayesian Optimization via Newton Methods](https://arxiv.org/abs/2508.18423)
*Quanlin Chen,Yiyu Chen,Jing Huo,Tianyu Ding,Yang Gao,Yuetong Chen*

Main category: cs.LG

TL;DR: 提出了一种基于全局高斯过程构建局部二次模型的高维贝叶斯优化方法，通过梯度信息提升采样效率，解决了TuRBO方法采样效率低的问题


<details>
  <summary>Details</summary>
Motivation: 标准贝叶斯优化在高维空间中面临挑战，现有TuRBO方法使用局部高斯过程虽然能避免过度探索，但采样效率低于全局高斯过程

Method: 利用全局高斯过程的梯度和Hessian矩阵构建多个局部二次模型，通过求解边界约束二次规划选择新采样点，并解决高维空间中梯度消失问题

Result: 实验结果表明该方法提高了TuRBO的有效性，在合成函数和实际应用中优于多种高维贝叶斯优化技术

Conclusion: 该方法在保持异质建模优势的同时显著提升了采样效率，为高维贝叶斯优化提供了有效解决方案
...
### [144] [VERIRL: Boosting the LLM-based Verilog Code Generation via Reinforcement Learning](https://arxiv.org/abs/2508.18462)
*Fu Teng,Miao Pan,Xuhong Zhang,Zhezhi He,Yiyao Yang,Xinyi Chai,Mengnan Qi,Liqiang Lu,Jianwei Yin*

Main category: cs.LG

TL;DR: 本文提出了一个针对Verilog代码生成的强化学习框架，通过构建高质量数据集、改进奖励机制和平衡学习策略，在硬件描述语言生成任务上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 硬件描述语言如Verilog由于并发语义、语法刚性和仿真复杂性等原因，在代码生成领域研究不足，需要专门的方法来解决这些挑战。

Method: 构建Veribench-53K高质量数据集；提出基于回溯的重新评分机制改进稀疏奖励信号；采用样本平衡权重策略防止灾难性遗忘；建立迭代式RL管道共同演化策略和奖励模型。

Result: 在Verilog生成任务上实现了最先进的性能，在测试通过率、功能正确性和编译鲁棒性方面都有显著提升。

Conclusion: 强化学习驱动的方法在硬件中心领域的结构化代码生成中具有巨大潜力，使用较小但高质量的数据集结合RL优化可以取得优异效果。
...
### [145] [DRTA: Dynamic Reward Scaling for Reinforcement Learning in Time Series Anomaly Detection](https://arxiv.org/abs/2508.18474)
*Bahareh Golchin,Banafsheh Rekabdar,Kunpeng Liu*

Main category: cs.LG

TL;DR: 提出了一种基于强化学习的异常检测框架DRTA，结合动态奖励塑造、变分自编码器和主动学习，在低标签系统中有效检测时间序列异常


<details>
  <summary>Details</summary>
Motivation: 传统方法在有限标签数据、高误报率和泛化到新异常类型方面存在困难，需要开发更有效的异常检测解决方案

Method: 使用强化学习框架，集成动态奖励塑造机制（平衡VAE重建误差和分类奖励）、变分自编码器和主动学习

Result: 在Yahoo A1和A2基准数据集上实验表明，该方法在精度和召回率方面持续优于最先进的无监督和半监督方法
...
### [146] [Data Augmentation Improves Machine Unlearning](https://arxiv.org/abs/2508.18502)
*Andreza M. C. Falcao,Filipe R. Cordeiro*

Main category: cs.LG

TL;DR: 本文研究了数据增强策略对机器遗忘(MU)性能的影响，发现合适的增强设计能显著提升遗忘效果，使遗忘后模型性能更接近重新训练的模型。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘旨在从训练好的模型中移除特定数据的影响，同时保持其在剩余数据上的性能。虽然已有工作暗示记忆化与增强之间存在联系，但系统性增强设计在MU中的作用仍未得到充分研究。

Method: 研究不同数据增强策略（包括TrivialAug等）对三种遗忘方法（SalUn、随机标签和微调）性能的影响，在CIFAR-10和CIFAR-100数据集上，使用不同遗忘率进行实验。

Result: 实验显示，适当的增强设计能显著提高遗忘效果，使用TrivialAug增强时，平均差距遗忘指标最多可减少40.12%。
...
### [147] [Breaking Through Barren Plateaus: Reinforcement Learning Initializations for Deep Variational Quantum Circuits](https://arxiv.org/abs/2508.18514)
*Yifeng Peng,Xinyi Li,Zhemin Zhang,Samuel Yen-Chi Chen,Zhiding Liang,Ying Wang*

Main category: cs.LG

TL;DR: 提出基于强化学习的参数初始化策略来解决变分量子算法中的贫瘠高原问题，通过预训练改善梯度优化起点，显著提升收敛速度和最终解质量
...
Method: 使用多种强化学习算法(DPG、SAC、PPO等)生成电路参数作为动作，在标准梯度优化前最小化VQAs成本函数进行预训练，从而获得更有利的初始状态

Result: 在各种噪声条件和任务下的数值实验表明，RL初始化方法显著提高了收敛速度和最终解质量，多种RL算法都能获得可比性能提升

Conclusion: 该方法为机器学习技术与量子算法设计的融合开辟了新途径，RL驱动的参数初始化有望加速VQAs的可扩展性和实际部署，为量子机器学习研究社区提供了有前景的研究方向
...
### [148] [Quantifying The Limits of AI Reasoning: Systematic Neural Network Representations of Algorithms](https://arxiv.org/abs/2508.18526)
...
<details>
  <summary>Details</summary>
Motivation: 解决当代AI研究中的一个核心问题：量化神经网络在完美训练时能够执行何种形式的推理任务。

Method: 提出系统元算法，通过迭代将电路中的每个门替换为规范的ReLU多层感知机模拟器，将任意电路转换为前馈神经网络。

Result: 证明神经网络可以精确模拟任何数字计算机上的电路，包括布尔逻辑、动态规划、数学表示等推理任务，且网络规模与电路复杂度成正比。

Conclusion: 神经网络能够模拟任何推理任务，将算法运行时间与空间复杂度进行权衡，结果比经典通用近似定理更强大。
...
### [149] [BTW: A Non-Parametric Variance Stabilization Framework for Multimodal Model Integration](https://arxiv.org/abs/2508.18551)
*Jun Hou,Le Wang,Xuan Wang*

Main category: cs.LG

TL;DR: 提出BTW框架，通过KL散度和互信息动态调整多模态学习中各模态的重要性权重，无需额外参数即可处理任意数量模态
...
Method: 双层非参数加权框架：实例级使用KL散度衡量单模态与多模态预测差异，模态级使用互信息估计全局对齐，动态调整训练时的模态重要性

Result: 在情感回归和临床分类任务上的广泛实验表明，该方法显著提升了回归性能和多元分类准确率

Conclusion: BTW框架有效解决了多模态学习中噪声模态问题，实现了实例级控制且可扩展到任意数量模态，无需增加参数
...
### [150] [Enhancing Chemical Explainability Through Counterfactual Masking](https://arxiv.org/abs/2508.18561)
*Łukasz Janisiów,Marek Kochańczyk,Bartosz Zieliński,Tomasz Danel*

Main category: cs.LG

TL;DR: 提出了一种名为counterfactual masking的新框架，用于分子属性预测的可解释性分析，通过用生成模型采样的化学合理片段替换掩蔽子结构，提供更真实和可操作的解释。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩蔽策略的可解释性方法往往产生不符合分子分布的不可靠解释，需要一种能够保持分子真实性并提供有意义反事实解释的方法。

Method: 使用生成模型训练完成分子图，采样化学合理的片段来替换掩蔽的子结构，而不是简单地置零，从而评估相对于数据分布中反事实分子的预测。

Result: 该方法在多个数据集和属性预测任务中表现出色，能够提供更稳健、分布一致的解释和更有意义的可操作见解。
...
### [151] [A Note on Graphon-Signal Analysis of Graph Neural Networks](https://arxiv.org/abs/2508.18564)
*Levi Rauchwerger,Ron Levie*

Main category: cs.LG

TL;DR: 本文对Levie的图神经网络分析工作进行了多项重要扩展，包括支持多维信号、扩展Lipschitz连续性到带readout的MPNNs、改进泛化边界，以及扩展到非对称图核。


<details>
  <summary>Details</summary>
Motivation: Levie的先前工作虽然提供了图神经网络的理论分析框架，但在实际图机器学习应用场景中存在一些局限性，如只支持一维信号、缺乏对带readout网络的分析等。

Method: 通过扩展图信号分析到多维空间，改进Lipschitz连续性分析以包含readout层，利用鲁棒性泛化边界提升理论保证，并扩展到非对称图核情况。

Result: 建立了更全面的图神经网络理论分析框架，能够处理更广泛的实际应用场景，包括多维特征图信号和更复杂的网络架构。

Conclusion: 这些扩展使得图神经网络的理论分析更加贴近实际应用需求，为图机器学习提供了更坚实的理论基础和更广泛的应用范围。
...
### [152] [Improving Long-term Autoregressive Spatiotemporal Predictions: A Proof of Concept with Fluid Dynamics](https://arxiv.org/abs/2508.18565)
*Hao Zhou,Sibo Cheng*

Main category: cs.LG

TL;DR: SPF框架通过随机采集策略结合模型预测和真实数据，在保持一步训练的同时实现多步学习，解决了自回归方法内存需求大和长期精度下降的问题。


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动方法在复杂系统中长期精度会因误差累积而下降，自回归训练虽然有效但需要大量GPU内存且可能牺牲短期性能。

Method: 提出随机前推(SPF)框架，通过从模型预测构建补充数据集，并使用随机采集策略将其与真实数据结合，在训练期间预计算多步预测以保持内存稳定。

Result: 在Burgers方程和浅水基准测试中，SPF比自回归方法获得更高的长期精度，同时降低了内存需求。

Conclusion: SPF框架为资源受限和复杂模拟提供了一种有前景的解决方案，在保持短期性能的同时提升了长期预测准确性。
...
### [153] [Sparse Autoencoders for Low-$N$ Protein Function Prediction and Design](https://arxiv.org/abs/2508.18567)
*Darin Tsui,Kunal Talreja,Amirali Aghazadeh*

Main category: cs.LG

TL;DR: 稀疏自编码器(SAEs)在低数据量的蛋白质功能预测和设计中表现出色，仅需24个序列就能超越ESM2基准，并在83%的情况下生成顶级适应度变体


<details>
  <summary>Details</summary>
Motivation: 解决在数据稀缺情况下从氨基酸序列预测蛋白质功能的挑战，特别是在只有少量标记数据时机器学习指导蛋白质设计的限制

Method: 使用在微调ESM2嵌入上训练的稀疏自编码器(SAEs)，评估其在各种适应度外推和蛋白质工程任务中的表现

Result: SAEs在适应度预测中始终优于或与ESM2基准相竞争，其稀疏潜在空间编码了紧凑且具有生物学意义的表示，能够从有限数据中更有效地泛化
...
### [154] [DrugReasoner: Interpretable Drug Approval Prediction with a Reasoning-augmented Language Model](https://arxiv.org/abs/2508.18579)
*Mohammadreza Ghaffarzadeh-Esfahani,Ali Motahharynia,Nahid Yousefian,Navid Mazrouei,Jafar Ghaisari,Yousof Gheisari*

Main category: cs.LG

TL;DR: DrugReasoner是基于LLaMA架构的推理型大语言模型，通过GRPO微调来预测小分子药物批准可能性，整合分子描述符和结构相似性比较推理，在性能和可解释性方面均优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 药物发现过程复杂且资源密集，需要早期预测批准结果来优化研发投资。虽然传统机器学习和深度学习方法在药物批准预测中显示出潜力，但其有限的可解释性限制了实际应用价值。

Method: 开发DrugReasoner模型，基于LLaMA架构，使用组相对策略优化(GRPO)进行微调。整合分子描述符，通过与结构相似的已批准和未批准化合物进行比较推理，生成预测结果的同时提供逐步推理过程和置信度评分。

Result: 在验证集上AUC为0.732，F1分数为0.729；测试集上AUC为0.725，F1分数为0.718，均优于逻辑回归、支持向量机和k近邻等传统基线方法，与XGBoost性能相当。在外部独立数据集上AUC达0.728，F1分数0.774，超越了基线方法和新开发的ChemAP模型。

Conclusion: DrugReasoner不仅提供有竞争力的预测准确性，还通过推理输出增强了透明度，解决了AI辅助药物发现中的关键瓶颈。研究表明推理增强的大语言模型有望成为药物决策中可解释且有效的工具。
...
### [155] [History Rhymes: Accelerating LLM Reinforcement Learning with RhymeRL](https://arxiv.org/abs/2508.18588)
*Jingkai He,Tianjian Li,Erhu Feng,Dong Du,Qian Liu,Tao Liu,Yubin Xia,Haibo Chen*

Main category: cs.LG

TL;DR: RhymeRL是一个针对大语言模型强化学习的系统，通过历史相似性优化解决了GPU利用率低的问题，性能提升2.6倍且不损失准确性


<details>
  <summary>Details</summary>
Motivation: 当前强化学习系统存在GPU利用率低的问题，主要原因是rollout阶段占主导地位和rollout长度不平衡导致的GPU气泡。现有解决方案如异步执行和截断可能会牺牲训练准确性

Method: 提出RhymeRL系统，包含两个关键创新：1) HistoSpec - 基于历史rollout令牌序列相似性的推测解码推理引擎；2) HistoPipe - 基于历史rollout分布相似性的两层调度策略

Result: 在真实生产环境中验证，支持从几十到数千个GPU的扩展，相比现有方法性能提升2.6倍

Conclusion: RhymeRL通过利用历史相似性有效解决了RL训练中的GPU利用率问题，在不改变RL范式或损失准确性的前提下显著提升了训练效率
...
### [156] [Linear Trading Position with Sparse Spectrum](https://arxiv.org/abs/2508.18596)
...
Method: 提出具有稀疏频谱的线性交易头寸，开发Krasnosel'skii-Mann固定点算法进行优化，该算法具有下降性质和线性收敛率

Result: 大量实验表明，所提方法在各种情况下都能实现良好且稳健的性能

Conclusion: 该方法为解决主投资组合方法的局限性提供了有效解决方案，在理论和实验上都取得了显著成果
...
### [157] [Uncertainty Awareness on Unsupervised Domain Adaptation for Time Series Data](https://arxiv.org/abs/2508.18630)
*Weide Liu,Xiaoyang Zhong,Lu Wang,Jingwen Hou,Yuemei Luo,Jiebin Yan,Yuming Fang*

Main category: cs.LG

TL;DR: 提出了一种结合多尺度特征提取和不确定性估计的无监督域自适应方法，用于处理时间序列数据中的分布偏移问题，在多个基准数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据中训练和测试数据集之间经常出现分布偏移，现有的无监督域自适应方法需要更好地处理这种挑战，提高模型在未标记测试数据上的泛化能力。

Method: 采用多尺度混合输入架构捕获不同尺度的特征，增加训练多样性并减少域间特征差异；基于证据学习引入不确定性感知机制，通过狄利克雷先验促进目标预测和不确定性估计；通过跨域对齐相同标签的特征来增强域自适应。

Result: 该方法在多个基准数据集上实现了最先进的性能，同时显著降低了预期校准误差（ECE），表明预测置信度得到了更好的校准。

Conclusion: 多尺度混合输入架构与不确定性感知机制的结合在时间序列无监督域自适应中非常有效，能够显著提高目标域性能并改善预测置信度校准。
...
### [158] [STRATA-TS: Selective Knowledge Transfer for Urban Time Series Forecasting with Retrieval-Guided Reasoning](https://arxiv.org/abs/2508.18635)
...
<details>
  <summary>Details</summary>
Motivation: 解决城市预测中的数据不平衡问题：少数城市有密集长期记录，而多数城市只有短期或不完整历史数据，直接迁移学习会引入噪声和负迁移

Method: 结合领域自适应检索和大型语言模型推理能力，使用基于补丁的时间编码器识别语义和动态对齐的源子序列，通过检索增强推理进行结构化推断，并通过监督微调将推理过程蒸馏到紧凑模型中

Result: 在新加坡、诺丁汉和格拉斯哥三个停车可用性数据集上的广泛实验表明，STRATA-TS始终优于强预测和迁移基线

Conclusion: 该框架在数据稀缺情况下有效提升预测性能，同时提供可解释的知识迁移路径
...
### [159] [Biologically Disentangled Multi-Omic Modeling Reveals Mechanistic Insights into Pan-Cancer Immunotherapy Resistance](https://arxiv.org/abs/2508.18638)
*Ifrah Tariq,Ernest Fraenkel*

Main category: cs.LG

TL;DR: BDVAE是一种深度生成模型，通过整合转录组和基因组数据，使用模态和通路特异性编码器来预测免疫检查点抑制剂治疗反应，并揭示耐药机制。


<details>
  <summary>Details</summary>
Motivation: 免疫检查点抑制剂治疗反应存在高度变异性，现有机器学习方法缺乏可解释性且未能有效利用多组学数据的生物结构。

Method: 开发了生物解缠变分自编码器(BDVAE)，采用模块化编码器架构和变分推理，学习与免疫、基因组和代谢过程相关的生物意义潜在特征。

Result: 在366名患者的泛癌队列中，BDVAE准确预测治疗反应(AUC-ROC=0.94)，揭示了免疫抑制、代谢转变和神经元信号等关键耐药机制。

Conclusion: BDVAE能够生成可解释的临床相关见解，证明了生物结构化机器学习在阐明复杂耐药模式和指导精准免疫治疗策略中的价值。
...
### [160] [The Sound of Risk: A Multimodal Physics-Informed Acoustic Model for Forecasting Market Volatility and Enhancing Market Interpretability](https://arxiv.org/abs/2508.18653)
...
TL;DR: 提出了一种新颖的多模态金融风险评估框架，结合文本情感分析和基于高管声带动力学的副语言线索，能够解释43.8%的30天实现波动率方差。


<details>
  <summary>Details</summary>
Motivation: 金融市场中的信息不对称问题，特别是通过精心设计的公司叙述来放大这种不对称，削弱了传统文本分析的有效性。

Method: 使用物理信息声学模型(PIAM)从原始电话会议音频中提取情感特征，将声学和文本情感状态投影到可解释的三维情感状态标签空间(紧张度、稳定性和唤醒度)，分析1,795个财报电话会议数据。

Result: 多模态特征虽然不能预测方向性股票收益，但能解释高达43.8%的样本外30天实现波动率方差，特别是在高管从脚本演讲转向自发问答时的情感动态变化。

Conclusion: 通过从可验证的生物特征信号中解码潜在的不确定性标记，该方法为投资者和监管机构提供了增强市场可解释性和识别隐藏公司不确定性的有力工具。
...
### [161] [FFT-MoE: Efficient Federated Fine-Tuning for Foundation Models via Large-scale Sparse MoE under Heterogeneous Edge](https://arxiv.org/abs/2508.18663)
*Gang Hu,Yinglei Teng,Pengfei Wu,Nan Wang*

Main category: cs.LG

TL;DR: FFT MoE：一种基于稀疏混合专家（MoE）的新型联邦微调框架，解决了LoRA在异构联邦学习环境中的结构不兼容和非IID数据适应性问题，通过轻量级门控网络和异构感知辅助损失实现更好的泛化性能和训练效率。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型向AGI发展，在隐私和资源约束下进行微调变得至关重要。联邦学习通过联邦微调（FFT）提供解决方案，但现有的LoRA-based FFT在异构环境中面临结构不兼容和非IID数据适应性差的问题。

Method: 提出FFT MoE框架，用稀疏混合专家（MoE）适配器替代LoRA。每个客户端训练轻量级门控网络选择性地激活个性化专家子集，并引入异构感知辅助损失来动态正则化路由分布，确保专家多样性和平衡利用。

Result: 在IID和非IID条件下的广泛实验表明，FFT MoE在泛化性能和训练效率方面持续优于最先进的FFT基线方法。

Conclusion: FFT MoE通过MoE适配器和异构感知路由机制，有效解决了异构联邦学习环境中的挑战，为隐私保护下的高效模型微调提供了新思路。
...
### [162] [Auditing Approximate Machine Unlearning for Differentially Private Models](https://arxiv.org/abs/2508.18671)
*Yuechun Gu,Jiajie He,Keke Chen*

Main category: cs.LG

TL;DR: 本文发现现有的近似机器学习遗忘方法可能会损害差分隐私模型中保留样本的隐私，需要开发差分隐私遗忘算法。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法假设保留数据不受影响，但隐私洋葱效应表明这种假设可能不正确，特别是在差分隐私模型中，需要全面审计遗忘和保留样本的隐私风险。

Method: 提出基于差分隐私和成员推理攻击的隐私标准，开发高效的数据增强MIA方法A-LiRA来减少影子模型训练成本，对现有近似遗忘算法进行隐私审计。

Result: 实验结果表明，现有近似机器学习遗忘算法可能会无意中损害差分隐私模型中保留样本的隐私。

Conclusion: 需要开发差分隐私遗忘算法来确保保留样本的隐私保护，现有方法在差分隐私模型中的隐私保障存在缺陷。
...
### [163] [Optimal Sparsity of Mixture-of-Experts Language Models for Reasoning Tasks](https://arxiv.org/abs/2508.18672)
*Taishi Nakamura,Satoki Ishikawa,Masaki Kawamura,Takumi Okamoto,Daisuke Nohara,Jun Suzuki,Rio Yokota*

Main category: cs.LG

TL;DR: 本研究探讨了混合专家(MoE)模型稀疏性对记忆和推理能力的影响，发现在固定计算预算下，记忆能力随总参数增加而提升，而推理能力会饱和甚至下降。


<details>
  <summary>Details</summary>
Motivation: 现有扩展定律主要针对密集模型，而MoE模型引入了新的稀疏维度，需要研究稀疏性如何影响不同能力机制。

Method: 训练不同总参数、激活参数和top-k路由的MoE Transformer家族，记录预训练损失、下游任务损失和准确率，分离泛化差距和损失-准确率差距。

Result: 记忆基准随总参数单调改善，推理性能会饱和甚至回归；改变top-k对固定激活参数影响小；强化学习和额外测试计算无法挽救过度稀疏模型的推理缺陷。

Conclusion: MoE稀疏性对记忆和推理能力有不同影响，推理能力存在最优稀疏水平，过度稀疏会损害推理性能。
...
### [164] [Utilizing Training Data to Improve LLM Reasoning for Tabular Understanding](https://arxiv.org/abs/2508.18676)
*Chufan Gao,Jintai Chen,Jimeng Sun*

Main category: cs.LG

TL;DR: LRTab是一种新颖的提示推理方法，通过从训练数据中检索相关信息，结合了微调和训练无关提示的优势，在表格推理任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的表格推理方法要么通过微调获得数据集特定学习但缺乏泛化性，要么通过训练无关提示具有高泛化性但未能充分利用训练数据。需要一种方法能同时兼顾两者的优势。

Method: 首先使用提示获取训练数据的思维链响应，对于错误的思维链，提示LLM预测提示条件以避免错误，从数据中学习见解。在验证数据上验证提示条件的有效性，在推理时检索最相关的提示条件作为额外上下文。

Result: 在WikiTQ和Tabfact数据集上的综合实验表明，LRTab具有可解释性、成本效益高，并且在表格推理任务中能够超越之前的基线方法。

Conclusion: LRTab通过检索从训练数据中学到的相关信息，成功整合了微调和训练无关提示的优势，为表格理解和推理提供了一种有效的方法。
...
### [165] [End to End Autoencoder MLP Framework for Sepsis Prediction](https://arxiv.org/abs/2508.18688)
*Hejiang Cai,Di Wu,Ji Xu,Xiang Liu,Yiziting Zhu,Xin Shu,Yujie Li,Bin Yi*

Main category: cs.LG

TL;DR: 提出一种端到端深度学习框架，结合无监督自编码器和多层感知器，用于ICU脓毒症早期检测，在三个ICU队列中准确率分别达到74.6%、80.6%和93.5%，优于传统机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 脓毒症是危及生命的疾病，需要及时检测。传统机器学习方法依赖手动特征工程，且难以处理电子健康记录中不规则的、不完整的时间序列数据。

Method: 端到端深度学习框架，集成无监督自编码器进行自动特征提取，使用多层感知器进行二元脓毒症风险预测。采用定制化下采样策略提取高信息密度片段，以及非重叠动态滑动窗口机制进行实时推理。

Result: 在三个ICU队列中分别达到74.6%、80.6%和93.5%的准确率，始终优于传统机器学习基线方法（朴素贝叶斯、SVM、随机森林、XGBoost）。

Conclusion: 该框架在异质ICU环境中展现出卓越的鲁棒性、泛化能力和临床实用性，适用于脓毒症的早期检测。
...
### [166] [Natural Image Classification via Quasi-Cyclic Graph Ensembles and Random-Bond Ising Models at the Nishimori Temperature](https://arxiv.org/abs/2508.18717)
*V. S. Usatyuk,D. A. Sapoznikov,S. I. Egorov*

Main category: cs.LG

TL;DR: 提出一个结合统计物理、编码理论和代数拓扑的统一框架，用于高效多类图像分类。通过将高维特征向量解释为稀疏多边类型准循环LDPC图上的自旋，形成随机键Ising模型，在Nishimori温度下最大化类别可分性。


<details>
  <summary>Details</summary>
Motivation: 开发一个理论框架，将物理系统的统计力学、纠错码理论和拓扑不变量结合起来，实现高效的特征压缩和分类，减少参数数量同时保持高精度。

Method: 使用冻结的MobileNetV2骨干网络提取特征，将特征向量视为MET-QC-LDPC图上的自旋，构建随机键Ising模型。在Nishimori温度下操作，利用Bethe-Hessian矩阵的最小特征值消失点。通过拓扑指导设计球形和环形图结构，抑制有害的陷阱集。

Result: 在ImageNet-10上达到98.7%准确率，ImageNet-100上达到82.7%准确率，实现了40倍的特征压缩（从1280维压缩到32或64维），计算速度比二分法快6倍。

Conclusion: 拓扑指导的图设计能够产生高效的、受物理启发的嵌入表示，在极大压缩特征维度的同时保持最先进的性能，证明了理论框架的有效性。
...
### [167] [Beyond Tokens: Enhancing RTL Quality Estimation via Structural Graph Learning](https://arxiv.org/abs/2508.18730)
*Yi Liu,Hongji Zhang,Yiwen Wang,Dimitris Tsaras,Lei Chen,Mingxuan Yuan,Qiang Xu*

Main category: cs.LG

TL;DR: StructRTL：一种基于控制数据流图的结构感知图自监督学习框架，用于改进RTL设计质量估计，通过结合结构学习和跨阶段监督实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的方法忽视了RTL代码的结构语义，而控制数据流图能更好地暴露设计结构特征，为表示学习提供更丰富的线索

Method: 提出结构感知图自监督学习框架，从CDFG学习结构感知表示，并采用知识蒸馏策略将后映射网表的低级洞察转移到CDFG预测器中

Result: 在各种质量估计任务上显著优于现有技术，建立了新的最先进结果

Conclusion: 结合结构学习和跨阶段监督的方法能有效提升RTL设计质量估计的准确性
...
### [168] [FLAegis: A Two-Layer Defense Framework for Federated Learning Against Poisoning Attacks](https://arxiv.org/abs/2508.18737)
*Enrique Mármol Campos,Aurora González Vidal,José Luis Hernández Ramos,Antonio Skarmeta*

Main category: cs.LG

TL;DR: FLAegis：一个两阶段防御框架，使用符号时间序列变换和谱聚类检测拜占庭客户端，结合FFT聚合提高联邦学习系统鲁棒性
...
Method: 提出FLAegis两阶段防御框架：1）使用符号时间序列变换（SAX）放大良性模型与恶意模型之间的差异；2）采用谱聚类准确检测对抗行为；3）集成基于FFT的鲁棒聚合函数作为最终防护层

Result: 在五种投毒攻击（从简单标签翻转到自适应优化策略）的严格评估中，该方法在检测精度和最终模型准确性方面均优于最先进的防御方法，即使在强对抗条件下也能保持持续高性能

Conclusion: FLAegis框架有效提高了联邦学习系统对拜占庭攻击的防御能力，通过多阶段检测和聚合机制确保了系统的安全性和鲁棒性
...
### [169] [Stability and Generalization for Bellman Residuals](https://arxiv.org/abs/2508.18741)
*Enoch H. Kang,Kyoungseok Jang*

Main category: cs.LG

TL;DR: 本文分析了离线强化学习和离线逆强化学习中Bellman残差最小化(BRM)方法的统计性能，提出了新的稳定性分析框架，在标准神经网络参数化和小批量SGD下实现了O(1/n)的样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 当前离线强化学习和离线逆强化学习在实践中仍难以有效执行Bellman一致性，虽然已有基于随机梯度下降-上升的全局收敛BRM方法，但其在离线设置下的统计行为尚未得到充分研究。

Method: 引入单个Lyapunov势函数来耦合相邻数据集上的SGDA运行，获得O(1/n)的平均参数稳定性边界，这是凸凹鞍点问题已知最佳样本复杂度指数的两倍。

Result: 相同的稳定性常数转化为BRM的O(1/n)超额风险边界，无需方差缩减、额外正则化或对minibatch采样的限制性独立假设。

Conclusion: 该分析填补了BRM方法在离线设置下的统计空白，为神经网络参数化和minibatch SGD提供了理论保证，显著提升了样本效率。
...
### [170] [Constraint Matters: Multi-Modal Representation for Reducing Mixed-Integer Linear programming](https://arxiv.org/abs/2508.18742)
*Jiajun Li,Ran Hou,Yu Ding,Yixuan Li,Shisi Guan,Jiahui Duan,Xiongwei Han,Tao Zhong,Vincent Chau,Weiwei Wu,Wanyuan Wang*

Main category: cs.LG

TL;DR: 提出了一种基于约束的混合整数线性规划模型简化方法，通过将关键不等式约束转换为等式来加速求解，相比现有方法提升解质量50%以上，减少计算时间17.47%


<details>
  <summary>Details</summary>
Motivation: 现有模型简化方法主要基于变量约简，而约束约简方法被忽视。从对偶角度看，将不等式约束转换为等式也能有效降低MILP复杂度

Method: 首先识别最优解处的紧约束作为潜在关键约束，设计启发式规则选择关键约束子集；提出多模态表示技术，利用实例级和抽象级MILP公式信息来学习关键紧约束

Result: 实验结果表明，相比最先进方法，该方法提升解质量50%以上，减少计算时间17.47%

Conclusion: 约束约简是MILP模型简化的有效方法，多模态表示技术能有效学习关键约束，显著加速大规模MILP问题求解
...
### [171] [UltraMemV2: Memory Networks Scaling to 120B Parameters with Superior Long-Context Learning](https://arxiv.org/abs/2508.18756)
...
Method: 提出了五项关键改进：1)在每个transformer块中集成内存层；2)使用单线性投影简化值扩展；3)采用基于FFN的值处理；4)实施原则性参数初始化；5)重新平衡内存与FFN的计算比例。
...
### [172] [Governance-as-a-Service: A Multi-Agent Framework for AI System Compliance and Policy Enforcement](https://arxiv.org/abs/2508.18765)
*Helen Pervez,Suyash Gaurav,Jukka Heikkonen,Jatin Chaudhary*

Main category: cs.LG

TL;DR: 提出了Governance-as-a-Service (GaaS)框架，作为模块化、策略驱动的执行层，在运行时监管AI代理输出，无需修改模型内部或代理合作。通过声明式规则和信任因子机制实现强制、规范和自适应干预。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统发展为具有自主执行、异步推理和多代理协调的分布式生态系统，缺乏可扩展、解耦的治理机制构成了结构性风险。现有监管机制反应式、脆弱且嵌入代理架构中，难以审计和跨异构部署推广。

Method: GaaS采用声明式规则和信任因子机制，基于合规性和严重性加权违规对代理进行评分。支持分级执行和动态信任调节，在运行时拦截、评估和记录所有代理行为。

Result: 在三个模拟场景（无治理基线、GaaS执行策略、对抗性代理测试）中使用开源模型进行测试。结果显示GaaS可靠地阻止或重定向高风险行为，同时保持吞吐量。信任分数有效跟踪规则遵守情况。

Conclusion: GaaS将治理定位为类似计算或存储的运行时服务，为可互操作代理生态系统建立基础设施级别的对齐。它不教授代理伦理，而是强制执行伦理规范。
...
### [173] [Predicting Drug-Drug Interactions Using Heterogeneous Graph Neural Networks: HGNN-DDI](https://arxiv.org/abs/2508.18766)
*Hongbo Liu,Siyi Li,Zheng Yu*

Main category: cs.LG

TL;DR: HGNN-DDI：基于异构图神经网络的药物相互作用预测模型，通过整合多源药物数据，在预测准确性和鲁棒性方面优于现有方法


<details>
  <summary>Details</summary>
Motivation: 药物相互作用是临床实践中的重要问题，传统计算方法难以捕捉药物、靶点和生物实体间的复杂关系

Method: 提出HGNN-DDI异构图神经网络模型，利用图表示学习建模异构生物医学网络，实现跨不同节点和边类型的信息传播

Result: 在基准DDI数据集上的实验结果表明，HGNN-DDI在预测准确性和鲁棒性方面优于最先进的基线方法

Conclusion: 该模型有潜力支持更安全的药物开发和精准医疗应用
...
### [174] [Federated Learning with Heterogeneous and Private Label Sets](https://arxiv.org/abs/2508.18774)
*Adam Breitholtz,Edvin Listo Zec,Fredrik D. Johansson*

Main category: cs.LG

TL;DR: 本文研究了联邦学习中客户端标签集异构性的影响，比较了公共标签设置和私有标签设置，提出了标准联邦学习方法的适应方案，并证明在私有标签设置下可以获得与公共标签设置相似的性能。


<details>
  <summary>Details</summary>
Motivation: 现实应用中客户端标签集异构性很常见但很少被研究，特别是在客户端只与中央服务器共享私有标签集的情况下，这给学习算法带来了更多约束，是一个更难解决的问题。

Method: 应用分类器组合问题的经典方法进行集中调优，将常见联邦学习方法适应到私有标签集设置，并在实际假设下讨论两种方法的合理性。

Result: 实验显示减少每个客户端可用标签数量会显著损害所有方法的性能。集中调优客户端模型以进行表示对齐可以有所帮助，但通常以更高方差为代价。提出的标准联邦学习方法适应方案在私有标签设置下表现良好。

Conclusion: 客户端可以在几乎不损失模型准确性的情况下享受更高的隐私保护，提出的适应方法在私有标签设置下实现了与标准方法在公共设置下相似的性能。
...
### [175] [SWiFT: Soft-Mask Weight Fine-tuning for Bias Mitigation](https://arxiv.org/abs/2508.18826)
*Junyu Yan,Feng Chen,Yuyang Xue,Yuning Du,Konstantinos Vilouras,Sotirios A. Tsaftaris,Steven McDonagh*

Main category: cs.LG

TL;DR: SWiFT是一种高效的机器学习去偏框架，通过软掩码权重微调技术，只需少量外部数据和少量训练轮次即可提升模型公平性，同时保持判别性能。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在医疗等敏感领域存在偏见问题，影响公平性和泛化能力，现有去偏方法需要原始训练数据和大量重训练，且在公平性和性能间存在权衡。

Method: 提出Soft-Mask Weight Fine-Tuning (SWiFT)框架，首先识别模型参数对偏见和预测性能的相对贡献，然后通过两步微调过程，根据不同贡献定义不同的梯度流来更新参数。

Result: 在四个皮肤病学和两个胸部X光数据集上的实验表明，SWiFT能持续减少模型偏见，在常见公平性和准确性指标上达到竞争性或更优的诊断准确性，并在多个分布外数据集上表现出更好的泛化能力。

Conclusion: SWiFT提供了一种高效且有效的去偏解决方案，只需少量外部数据和少量训练轮次，就能在保持模型性能的同时显著提升公平性。
...
### [176] [DRMD: Deep Reinforcement Learning for Malware Detection under Concept Drift](https://arxiv.org/abs/2508.18839)
*Shae McFadden,Myles Foley,Mario D'Onghia,Chris Hicks,Vasilios Mavroudis,Nicola Paoletti,Fabio Pierazzi*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度强化学习(DRL)的恶意软件检测方法DRMD，将恶意软件检测建模为马尔可夫决策过程，同时优化分类性能和拒绝高风险样本进行人工标注，在Android恶意软件数据集上显示出对概念漂移的更强鲁棒性。
...
Method: 将恶意软件检测建模为一步马尔可夫决策过程，训练深度强化学习代理DRMD，同时优化样本分类性能和拒绝高风险样本进行人工标注的策略。

Result: 在Android恶意软件数据集上的时间感知评估显示，DRMD代理在仅分类、分类加拒绝、分类加拒绝和主动学习三种设置下，分别实现了5.18±5.44、14.49±12.86和10.06±10.81的平均AUT性能提升。

Conclusion: 研究表明深度强化学习能够有效促进恶意软件检测，并在Android恶意软件领域的动态环境中提高对概念漂移的弹性，首次证明了DRL在该领域的应用潜力。
...
### [177] [Recycling History: Efficient Recommendations from Contextual Dueling Bandits](https://arxiv.org/abs/2508.18841)
*Suryanarayana Sankagiri,Jalal Etesami,Pouria Fatemi,Matthias Grossglauser*

Main category: cs.LG

TL;DR: 本文提出了一种新的上下文决斗赌博机模型，通过让用户比较当前推荐物品与历史消费物品来获取更可靠的反馈，使用矩阵浓度边界证明实现了O(√T)的遗憾保证。


<details>
  <summary>Details</summary>
Motivation: 传统上下文决斗赌博机模型只捕获用户在内容平台上的隐式选择，但无法利用用户消费物品后提供的更可靠比较反馈。

Method: 算法每步推荐一个物品，用户消费后将其与历史消费物品进行比较；通过初始随机探索阶段积累多样化历史，利用矩阵浓度边界分析遗憾。

Result: 理论分析证明算法可实现O(√T)的遗憾上界，模拟实验显示重用历史物品进行比较比仅比较同时推荐物品能显著降低遗憾。

Conclusion: 通过利用用户历史消费物品进行比较查询，可以在不增加额外遗憾的情况下获得更可靠的反馈，从而提升推荐系统性能。
...
### [178] [C-Flat++: Towards a More Efficient and Powerful Framework for Continual Learning](https://arxiv.org/abs/2508.18860)
*Wei Li,Hangjie Yuan,Zixiang Zhao,Yifan Zhu,Aojun Lu,Tao Feng,Yanan Sun*

Main category: cs.LG

TL;DR: 本文提出了C-Flat方法，通过促进更平坦的损失景观来解决持续学习中的稳定性问题，并在多个CL范式中实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: 持续学习中平衡新任务敏感性和保持过去知识稳定性至关重要。现有基于锐度感知最小化的方法可能在某些情况下偏好更尖锐的最小值，导致解决方案不够鲁棒和次优。

Method: 提出C-Flat方法，专门为持续学习设计促进更平坦损失景观的技术。该方法具有即插即用兼容性，可轻松集成到现有代码管道中。还提出了C-Flat++框架，通过选择性平坦性驱动提升来显著降低更新成本。

Result: 实验结果表明，C-Flat在各种设置下都能持续提高性能。在多个CL方法、数据集和场景上的广泛实验证明了所提方法的有效性和效率。

Conclusion: C-Flat和C-Flat++为持续学习提供了有效的平坦性优化解决方案，能够改善模型在持续学习任务中的稳定性和性能表现。
...
### [179] [MOCHA: Discovering Multi-Order Dynamic Causality in Temporal Point Processes](https://arxiv.org/abs/2508.18873)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖静态或一阶因果结构，忽视了因果关系的多阶性和时变特性，无法充分建模真实世界事件序列中的复杂因果依赖。
...
Conclusion: MOCHA框架成功解决了时间点过程中多阶动态因果关系的发现问题，为复杂事件序列的建模提供了有效的解决方案，同时具有良好的预测性能和可解释性。
...
### [180] [HAEPO: History-Aggregated Exploratory Policy Optimization](https://arxiv.org/abs/2508.18884)
*Gaurish Trivedi,Alakh Sharma,Kartikey Singh Bhandari,Dhruv Kumar,Pratik Narang,Jagat Sesh Challa*

Main category: cs.LG

TL;DR: HAEPO是一种新的探索性策略优化方法，通过历史聚合和轨迹级概率压缩来增强长时域任务的探索能力，比DPO、GRPO等方法表现更好


<details>
  <summary>Details</summary>
Motivation: 现有方法如DPO和GRPO在长时域任务中探索能力有限，需要一种能够更好利用完整轨迹历史并促进广泛探索的方法

Method: 将轨迹压缩为累积对数似然，应用Plackett-Luce softmax获得与回报成正比的归一化权重，加入熵正则化和软KL惩罚来稳定训练
...
### [181] [pyFAST: A Modular PyTorch Framework for Time Series Modeling with Multi-source and Sparse Data](https://arxiv.org/abs/2508.18891)
...
<details>
  <summary>Details</summary>
Motivation: 现有Python时间序列库在模块化、不规则数据支持和多源数据处理方面存在局限，需要更灵活、高效的框架来支持复杂研究场景。

Method: 采用数据与模型计算解耦的设计，构建数据引擎支持多源加载、蛋白质序列处理、动态归一化等功能，集成LLM架构进行稀疏数据融合，提供稀疏指标和专用损失函数。

Result: 开发了pyFAST框架，支持多种深度学习模型（线性、CNN、RNN、Transformer、GNN），提供训练工具如批量流式聚合和设备协同优化。

Conclusion: pyFAST作为一个紧凑而强大的平台，在MIT许可下开源，能够有效推进时间序列研究和应用的发展。
...
### [182] [Distance-informed Neural Processes](https://arxiv.org/abs/2508.18903)
*Aishwarya Venkataramanan,Joachim Denzler*

Main category: cs.LG

TL;DR: DNP是一种改进的神经过程模型，通过结合全局和距离感知的局部潜在结构来提升不确定性估计能力，解决了标准神经过程在不确定性校准和局部数据依赖捕获方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 标准神经过程主要依赖全局潜在变量，在不确定性校准和捕捉局部数据依赖方面存在困难。DNP旨在通过引入全局和局部潜在变量来改进这些方面。

Method: DNP引入全局潜在变量建模任务级变化，以及局部潜在变量在距离保持的潜在空间中捕捉输入相似性。通过双Lipschitz正则化来限制输入关系的扭曲，保持潜在空间中的相对距离。

Result: 实证结果表明，DNP在回归和分类任务中实现了强大的预测性能和改进的不确定性校准，能更有效地区分分布内和分布外数据。

Conclusion: DNP通过结合全局和局部潜在结构以及距离保持机制，显著提升了神经过程模型的不确定性估计能力和数据依赖性建模效果。
...
### [183] [Enhancing Model Privacy in Federated Learning with Random Masking and Quantization](https://arxiv.org/abs/2508.18911)
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中模型性能与参数隐私保护的平衡问题

Method: 提出一种新的联邦学习保护方法，具体技术细节未在摘要中说明

Result: 在各种模型和任务上的实验结果表明，该方法在保持模型性能的同时，显著提升了参数保护效果

Conclusion: 该方法为联邦学习提供了性能与隐私保护的双重优势，具有实际应用价值
...
### [184] [Generalization Bound for a General Class of Neural Ordinary Differential Equations](https://arxiv.org/abs/2508.18920)
...
<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注神经ODE中动力学函数的线性情况，或提供依赖于采样间隔的边界。本文旨在分析更广泛的非线性动力学函数类别，填补这一研究空白。

Method: 在动力学函数关于状态变量Lipschitz连续的条件下，证明神经ODE解具有有界变差，基于此建立时间依赖和时间独立情况的泛化边界。

Result: 证明了在Lipschitz条件下神经ODE解的有界变差性，并成功推导出一般非线性动力学神经ODE的泛化误差界。

Conclusion: 这是首次对具有一般非线性动力学的神经ODE推导泛化边界的工作，为这类连续深度架构模型的泛化性能提供了理论保证。
...
### [185] [HierCVAE: Hierarchical Attention-Driven Conditional Variational Autoencoders for Multi-Scale Temporal Modeling](https://arxiv.org/abs/2508.18922)
*Yao Wu*

Main category: cs.LG

TL;DR: HierCVAE是一种新颖的层次注意力机制与条件变分自编码器结合的架构，用于复杂系统的时间建模，在预测精度和不确定性校准方面显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 复杂系统中的时间建模需要捕捉多个时间尺度的依赖关系并管理内在的不确定性，现有方法在这方面存在不足

Method: 采用三层注意力结构（局部、全局、跨时间）结合多模态条件编码，在潜在空间中使用ResFormer块，并通过预测头提供明确的不确定性量化

Result: 在能源消耗数据集评估中，HierCVAE相比最先进方法在预测精度上提高了15-40%，并在不确定性校准方面表现优异，特别擅长长期预测和复杂多变量依赖关系

Conclusion: HierCVAE通过层次注意力机制和条件变分自编码器的有效整合，成功解决了复杂时间序列建模中的多尺度依赖和不确定性管理问题
...
### [186] [Energy-Based Flow Matching for Generating 3D Molecular Structure](https://arxiv.org/abs/2508.18949)
*Wenyin Zhou,Christopher Iliffe Sprague,Vsevolod Viliuga,Matteo Tadiello,Arne Elofsson,Hossein Azizpour*

Main category: cs.LG

TL;DR: 本文提出了一种基于能量视角的流匹配方法，用于分子结构生成，在蛋白质对接和骨架生成任务中表现优于现有的流匹配和扩散模型。


<details>
  <summary>Details</summary>
Motivation: 分子结构生成在分子对接、蛋白质折叠和分子设计等生物应用中至关重要。现有的生成模型如扩散模型和流匹配方法虽然取得了进展，但需要从能量角度改进训练和推理过程。

Method: 采用基于能量的视角，通过深度网络学习映射函数，迭代地将随机配置映射到目标结构，构建概念简单且经验有效的流匹配设置。

Result: 在蛋白质对接和蛋白质骨架生成实验中，该方法在相同计算预算下优于最近的流匹配和扩散模型基线。

Conclusion: 基于能量视角的流匹配方法为分子结构生成提供了理论合理且经验有效的解决方案，与基本特性如幂等性和稳定性有有趣联系，并与AlphaFold中的结构精炼等实用技术相关。
...
### [187] [Estimating Conditional Covariance between labels for Multilabel Data](https://arxiv.org/abs/2508.18951)
*Laurence A. F. Park,Jesse Read*

Main category: cs.LG

TL;DR: 本文比较了三种模型（多元Probit、多元Bernoulli和分阶段Logit）在估计多标签条件协方差方面的性能，发现多元Probit模型在错误检测率方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 多标签数据分析需要先检验标签间的依赖性，但由于标签对协变量的依赖，无法直接从标签值测量独立性，需要通过条件标签协方差来评估。

Method: 使用三种模型（多元Probit、多元Bernoulli和分阶段Logit）来估计恒定和依赖的多标签条件协方差，并通过实验观察各模型对条件协方差的测量能力。

Result: 所有模型在测量恒定和依赖协方差方面表现相当，取决于协方差强度，但都会错误地将恒定协方差检测为依赖协方差。其中多元Probit模型的错误率最低。

Conclusion: 多元Probit模型在多标签条件协方差估计中具有最低的错误检测率，是三种模型中表现最好的选择。
...
### [188] [On the Generalisation of Koopman Representations for Chaotic System Control](https://arxiv.org/abs/2508.18954)
*Kyriakos Hjikakou,Juan Diego Cardenas Cartagena,Matthia Sabatelli*

Main category: cs.LG

TL;DR: 本文研究了Koopman表示在混沌动力系统中的泛化能力，重点关注其在预测和控制任务间的可迁移性。使用Lorenz系统作为测试平台，提出了三阶段方法，结果显示Koopman嵌入优于基线方法，且预训练权重在微调时无需调整，表明学习到的表示捕获了可重用的动力学结构。
...
Method: 提出三阶段方法：1）通过自编码学习Koopman嵌入；2）在下一状态预测任务上预训练transformer；3）针对安全关键控制任务进行微调。使用Lorenz系统作为测试平台。

Result: Koopman嵌入在准确性和数据效率方面均优于标准和物理信息PCA基线方法。预训练transformer权重在微调过程中固定时性能无下降，表明学习到的表示捕获了可重用的动力学结构而非任务特定模式。

Conclusion: 研究结果支持将Koopman嵌入作为物理信息机器学习中多任务学习的基础表示，证明了其在混沌动力系统中的有效泛化能力。
...
### [189] [PAX-TS: Model-agnostic multi-granular explanations for time series forecasting via localized perturbations](https://arxiv.org/abs/2508.18982)
...
<details>
  <summary>Details</summary>
Motivation: 现代预测模型通常不透明且不提供预测解释，而现有的后处理可解释性方法（如LIME）不适合预测场景，因此需要专门的时间序列预测解释方法。

Method: 基于局部输入扰动的模型无关后处理算法，通过扰动输入时间序列来生成多粒度解释，能够处理多元时间序列的跨通道相关性。

Result: 在7种算法和10个不同数据集上的基准测试表明，PAX-TS能有效捕捉模型行为，高性能和低性能算法的解释存在差异，并识别出6类跨数据集和算法重复出现的模式。
...
### [190] [FedProtoKD: Dual Knowledge Distillation with Adaptive Class-wise Prototype Margin for Heterogeneous Federated Learning](https://arxiv.org/abs/2508.19009)
...
Method: 提出FedProtoKD方法，采用增强的双知识蒸馏机制（结合客户端logits和原型特征表示），使用基于对比学习的可训练服务器原型和类别自适应原型边缘来解决原型边缘收缩问题，并通过样本与类代表原型接近度评估公共样本重要性。

Result: FedProtoKD在各种设置下实现了1.13%到34.13%的平均准确率提升，显著优于现有的最先进异构联邦学习方法。

Conclusion: 该方法有效解决了原型聚合中的边缘收缩问题，通过知识蒸馏和对比学习机制提升了异构联邦学习系统的性能，为HFL研究提供了新的进展方向。
...
### [191] [STDiff: A State Transition Diffusion Framework for Time Series Imputation in Industrial Systems](https://arxiv.org/abs/2508.19011)
*Gary Simethy,Daniel Ortiz-Arroyo,Petar Durdevic*

Main category: cs.LG

TL;DR: STDiff是一种基于条件去噪扩散模型的时间序列缺失值填补方法，通过逐步生成缺失值，专注于系统状态演化而非固定时间窗口的模式完成


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法假设固定时间窗口内的模式完成，但在工业系统中，动态由控制动作驱动、高度非平稳且可能存在长时间连续缺失，这种假设往往失效

Method: 使用条件去噪扩散模型，具有与控制理论一致的自回归偏置，基于最近已知状态和相关控制或环境输入逐步生成缺失值

Result: 在模拟缺失块的公共污水处理数据集上，STDiff始终实现最低误差，且优势随缺失长度增加而增强；在真实工业数据集上，生成动态合理的轨迹，而基于窗口的模型倾向于平坦化或过度平滑
...
### [192] [Learning with springs and sticks](https://arxiv.org/abs/2508.19015)
*Luis Mantilla Calderón,Alán Aspuru-Guzik*

Main category: cs.LG

TL;DR: 本文提出了一种由弹簧和棍棒组成的简单物理系统，能够通过势能最小化来近似任意连续函数，性能与多层感知机相当，并发现了热力学学习障碍现象。


<details>
  <summary>Details</summary>
Motivation: 从物理角度研究学习过程，探索简单的弹簧-棍棒动力系统如何通过能量最小化来实现函数逼近和学习。

Method: 使用棍棒模拟分段线性函数近似，用弹簧势能编码均方误差损失函数，通过耗散过程收敛到最小能量配置。

Result: 系统在回归任务中表现与多层感知机相当，发现了热力学学习障碍现象，即当系统自由能变化达到某个阈值时无法学习。

Conclusion: 这个简单模型有助于从物理角度更好地理解学习系统，揭示了环境波动导致的热力学学习限制。
...
### [193] [Working My Way Back to You: Resource-Centric Next-Activity Prediction](https://arxiv.org/abs/2508.19016)
*Kelly Kurowski,Xixi Lu,Hajo A Reijers*

Main category: cs.LG

TL;DR: 本文从资源中心视角研究预测性流程监控中的下一活动预测，通过评估不同模型和编码策略，发现LightGBM和Transformer模型在2-gram活动转换编码下表现最佳，为资源分配和员工支持提供新思路。


<details>
  <summary>Details</summary>
Motivation: 现有预测性流程监控研究主要关注控制流视角，而资源中心视角的下一活动预测尚未探索，但具有改善工作组织、负载平衡和容量预测等额外优势。

Method: 使用四个预测模型（包括LightGBM、Transformer和随机森林）和三种编码策略（2-gram活动转换、活动重复特征及其组合），在四个真实数据集上进行评估。

Result: LightGBM和Transformer模型在2-gram活动转换编码下表现最佳，随机森林在组合编码（2-gram转换+活动重复特征）下获益最大，组合编码获得最高平均准确率。

Conclusion: 资源中心的下一活动预测方法为智能资源分配、战略人力规划和个性化员工支持提供了新途径，开辟了预测性流程监控研究的新方向。
...
### [194] [Metric Matters: A Formal Evaluation of Similarity Measures in Active Learning for Cyber Threat Intelligence](https://arxiv.org/abs/2508.19019)
*Sidahmed Benabderrahmane,Talal Rahwan*

Main category: cs.LG

TL;DR: 提出基于主动学习和相似性搜索的新型异常检测框架，通过注意力自编码器和特征空间相似性来识别正常和异常实例，显著提升APT检测的准确性和标签效率


<details>
  <summary>Details</summary>
Motivation: 解决高级持续性威胁(APTs)检测中由于隐蔽行为和极端类别不平衡带来的挑战，需要更有效的异常检测方法

Method: 基于注意力自编码器的主动学习框架，利用相似性搜索迭代优化决策空间，通过特征空间相似性识别正常和异常实例

Result: 在不同数据集上的实验表明，相似性度量的选择显著影响模型收敛性、异常检测准确性和标签效率

Conclusion: 为威胁情报和网络防御中的主动学习管道选择相似性函数提供了实用指导，证明了相似性度量在APT检测中的关键作用
...
### [195] [GRADSTOP: Early Stopping of Gradient Descent via Posterior Sampling](https://arxiv.org/abs/2508.19028)
...
<details>
  <summary>Details</summary>
Motivation: 传统早停方法需要保留验证集，减少了训练数据量。本文旨在开发一种仅使用梯度信息（梯度下降算法自然产生）的早停方法，以充分利用所有数据进行训练

Method: 通过梯度信息估计贝叶斯后验分布，将早停问题定义为从该后验中采样，并使用近似后验获得停止准则

Result: 实证评估显示GradStop在测试数据上实现较小损失，性能优于基于验证集的停止准则，在数据有限场景（如迁移学习）中特别有优势

Conclusion: GradStop是一种计算开销小的有效早停方法，可整合到梯度下降库中，充分利用全部训练数据，特别适合数据受限的应用场景
...
### [196] [When recalling in-context, Transformers are not SSMs](https://arxiv.org/abs/2508.19029)
*Destiny Okpekpe,Antonio Orvieto*

Main category: cs.LG

TL;DR: 现代循环模型（如状态空间模型）在关联回忆任务中相比Transformer存在性能差距，研究发现学习率选择对循环模型性能至关重要，且循环模型与注意力模型在宽度和深度扩展上表现出不同特性。


<details>
  <summary>Details</summary>
Motivation: 尽管现代循环深度学习模型具有次二次复杂度优势，但近期研究表明它们在推理和记忆任务上可能不如Transformer。本文深入分析关联回忆基准，研究扩展和优化问题对不同token混合策略的影响。

Method: 通过系统实验分析学习率对循环模型性能的影响，比较循环模型和注意力模型在宽度与深度扩展上的差异，研究单层Transformer的训练动态，并进行架构消融研究Transformer和Mamba的性能及优化稳定性。

Result: 发现学习率选择对现代循环模型性能至关重要；循环模型和注意力模型在宽度与深度扩展上呈现相反优势；单层Transformer虽然性能差但训练动态类似归纳头形成；架构组件对性能和优化稳定性有显著影响。

Conclusion: 现代循环模型的训练需要更稳定的优化方法，不同模型架构在扩展维度上各有优势，单层Transformer展现出有趣的训练动态，为理解不同token混合策略的性能差异提供了新见解。
...
### [197] [Breaking the Black Box: Inherently Interpretable Physics-Informed Machine Learning for Imbalanced Seismic Data](https://arxiv.org/abs/2508.19031)
*Vemula Sreenath,Filippo Gatti,Pierre Jehel*

Main category: cs.LG

TL;DR: 提出了一种透明的地震动模型机器学习架构，使用HazBinLoss函数解决传统黑盒模型的可解释性问题，并通过加权处理数据不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法开发的地震动模型(GMMs)存在黑盒问题难以解释和信任，且地震数据库存在严重的数据不平衡问题（近场大震级记录少而远场小震级记录多）。

Method: 开发透明ML架构，每个输入（震级、距离等）单独处理并线性相加得到输出，使用HazBinLoss函数在训练时为关键近场大震级记录分配更高权重。

Result: 模型能够捕捉已知地震学原理，在保持透明性的同时达到与成熟GMMs相当的性能。

Conclusion: 该框架使得基于机器学习的方法能够更广泛地应用于风险评估研究和灾害规划。
...
### [198] [Automated discovery of finite volume schemes using Graph Neural Networks](https://arxiv.org/abs/2508.19052)
*Paul Garnier,Jonathan Viquerat,Elie Hachem*

Main category: cs.LG

TL;DR: GNN可以通过符号回归生成数值格式，从两节点图训练中推导出一阶有限体积格式，并在无监督情况下恢复二阶格式


<details>
  <summary>Details</summary>
Motivation: 探索GNN在数值模拟中的新用途，验证其能否超越传统近似器角色，自主发现数值格式

Method: 使用两节点图训练GNN，结合符号回归分析网络学到的格式；在无监督设置中使用PINN式残差损失训练GNN

Result: GNN成功推导出一阶有限体积格式，误差为O(ε)；无监督训练下恢复了二阶修正项和中点格式

Conclusion: GNN不仅是强大的近似器，还能主动贡献于新型数值方法的开发，开启了科学计算新范式
...
### [199] [Tackling Federated Unlearning as a Parameter Estimation Problem](https://arxiv.org/abs/2508.19065)
*Antonio Balordi,Lorenzo Manini,Fabio Stella,Alessio Merlo*

Main category: cs.LG

TL;DR: 基于信息理论的联邦学习遗忘框架，使用二阶Hessian信息识别敏感参数并选择性重置，通过最小化联邦重训练实现高效数据擦除


<details>
  <summary>Details</summary>
Motivation: 隐私法规要求从深度学习模型中擦除数据，这在联邦学习中尤为困难，因为数据保留在客户端，完全重训练或协调更新往往不可行

Method: 将泄漏建模为参数估计问题，利用二阶Hessian信息识别和选择性重置对遗忘数据最敏感的参教，然后进行最小化联邦重训练

Result: 在基准数据集上展示出强大的隐私保护能力（MIA攻击成功率接近随机，类别知识被擦除）和高性能（相对于重训练基准的归一化准确率约0.9），在针对性后门攻击场景中有效中和恶意触发器

Conclusion: 为联邦学习中的数据遗忘提供了一个实用的解决方案，在保持高效的同时实现隐私保护和模型完整性恢复
...
### [200] [Dynamic Triangulation-Based Graph Rewiring for Graph Neural Networks](https://arxiv.org/abs/2508.19071)
*Hugo Attali,Thomas Papastergiou,Nathalie Pernelle,Fragkiskos D. Malliaros*

Main category: cs.LG

TL;DR: TRIGON是一个新的图重布线框架，通过从多个图视图中学习选择相关三角形来构建丰富的非平面三角剖分，以解决GNN中的过度挤压和过度平滑问题。


<details>
  <summary>Details</summary>
Motivation: 图神经网络(GNN)的性能受到图拓扑固有问题的限制，特别是过度挤压和过度平滑问题。现有的图重布线方法旨在通过修改图拓扑来促进更有效的信息传播。

Method: TRIGON框架通过联合优化三角形选择和下游分类性能，从多个图视图中学习选择相关三角形，构建非平面三角剖分，从而产生具有改进结构特性的重布线图。

Result: 实验结果表明，TRIGON在一系列同质性和异质性基准测试的节点分类任务上优于最先进的方法，产生的重布线图具有减少的直径、增加的谱间隙和更低的有效电阻等改进的结构特性。

Conclusion: TRIGON通过构建非平面三角剖分的图重布线方法，有效缓解了GNN中的过度挤压和过度平滑问题，在节点分类任务上取得了优异的性能表现。
...
### [201] [APT-LLM: Exploiting Arbitrary-Precision Tensor Core Computing for LLM Acceleration](https://arxiv.org/abs/2508.19087)
*Shaobo Ma,Chao Fang,Haikuo Shao,Zhongfeng Wang*

Main category: cs.LG

TL;DR: APT-LLM是一种针对任意精度大语言模型推理的全面加速方案，通过新型数据格式、矩阵乘法方法、内存管理和内核映射优化，在GPU上实现显著性能提升


<details>
  <summary>Details</summary>
Motivation: 大语言模型计算需求巨大，现有量化方法在GPU上支持任意精度超低位量化时面临Tensor Core支持有限、内存管理低效和内核优化不灵活等挑战

Method: 提出双极INT数据格式、位级矩阵分解重组方法、基于数据恢复的内存管理系统以及动态内核映射方法

Result: 在LLM推理中，相比FP16基线实现最高3.99倍加速，相比NVIDIA CUTLASS INT4加速实现最高2.16倍加速，在不同GPU平台上均表现优异

Conclusion: APT-LLM为任意精度大语言模型提供了高效的GPU加速解决方案，显著提升了推理性能
...
### [202] [Composition and Alignment of Diffusion Models using Constrained Learning](https://arxiv.org/abs/2508.19104)
*Shervin Khalafi,Ignacio Hounie,Dongsheng Ding,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 提出一个约束优化框架来统一扩散模型的alignment和composition，通过强制对齐模型满足奖励约束并保持接近预训练模型，解决了多奖励优化中的权衡问题


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成建模中广泛应用，但现有方法在优化多个奖励或组合多个模型时存在权衡，无法保证生成样本具有所有期望属性

Method: 提出约束优化框架，理论分析约束对齐和组合问题的解，开发基于拉格朗日的原始对偶训练算法来近似这些解

Result: 在图像生成任务中验证了方法的有效性，显示对齐或组合模型能有效满足约束，并在等权重方法基础上有所改进

Conclusion: 该框架为扩散模型的alignment和composition提供了统一的约束优化解决方案，能够有效处理多奖励优化中的权衡问题
...
### [203] [Active Query Selection for Crowd-Based Reinforcement Learning](https://arxiv.org/abs/2508.19132)
...
Result: 在大多数任务中，对不确定轨迹进行反馈的智能体学习速度更快，在血糖控制任务中表现优于基线方法

Conclusion: 提出的结合概率众包建模和主动学习的框架能有效处理噪声人类反馈，提高学习效率，在现实世界任务中表现出色
...
### [204] [Saddle Hierarchy in Dense Associative Memory](https://arxiv.org/abs/2508.19151)
...
Result: DAM模型能够学习可解释的监督和无监督分类解决方案，相对较小的DAM权重对应更大DAM中的不稳定鞍点，网络增长算法显著降低了训练计算成本。

Conclusion: 该研究为密集关联记忆模型提供了理论分析和实用改进，通过鞍点层次结构和网络增长算法实现了更高效稳定的训练。
...
### [205] [Get Global Guarantees: On the Probabilistic Nature of Perturbation Robustness](https://arxiv.org/abs/2508.19183)
*Wenchuan Mu,Kwan Hui Lim*

Main category: cs.LG

TL;DR: 本文提出了一种名为tower robustness的新型鲁棒性评估指标，基于假设检验来定量评估概率鲁棒性，解决了现有方法在计算成本和测量精度之间的权衡问题。
...
Method: 对现有鲁棒性定义和评估方法进行全面比较分析，提出基于假设检验的tower robustness概率鲁棒性评估指标。
...
### [206] [Emotions as Ambiguity-aware Ordinal Representations](https://arxiv.org/abs/2508.19193)
*Jingyao Wu,Matthew Barthet,David Melhart,Georgios N. Yannakakis*

Main category: cs.LG

TL;DR: 本文提出了一种新的模糊感知序数情感表示框架，通过建模情感模糊性的变化率来同时捕捉情感标注的模糊性和情感轨迹的时间动态特性。
...
Method: 提出模糊感知序数情感表示框架，通过情感模糊性的变化率来建模情感模糊性，并在RECOLA和GameVibe两个情感语料库上测试有界（唤醒度、效价）和无界（参与度）连续情感轨迹。

Result: 序数表示在无界标签上优于传统模糊感知模型，获得最高的CCC和SDA分数；在有界轨迹上，序数表示在SDA方面表现优异，显示出其在捕捉标注情感轨迹相对变化方面的卓越能力。

Conclusion: 模糊感知序数情感表示框架能够有效建模情感轨迹的动态特性，特别是在捕捉情感标注的相对变化方面表现出色，为连续情感识别提供了新的有效方法。
...
### [207] [Understanding Tool-Integrated Reasoning](https://arxiv.org/abs/2508.19201)
*Heng Lin,Zhongwen Xu*

Main category: cs.LG

TL;DR: 本文首次从理论层面证明了工具集成推理(TIR)能够从根本上扩展大语言模型的能力边界，通过工具打破了纯文本模型的能力天花板，并提出了ASPO算法来优化工具使用行为。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型与工具(如Python解释器)集成显示出巨大潜力，但缺乏解释这一范式为何有效的理论原理。本文旨在填补这一空白，提供首个形式化证明。

Method: 引入优势塑造策略优化(ASPO)算法直接修改优势函数来指导策略行为，并在数学基准测试中使用Python解释器作为外部工具进行综合实验。

Result: TIR模型在pass@k指标上显著优于纯文本模型，优势不仅限于计算密集型问题，还扩展到需要抽象洞察力的问题。ASPO带来了更早的代码调用和更多交互轮次。
...
### [208] [Predicting the Order of Upcoming Tokens Improves Language Modeling](https://arxiv.org/abs/2508.19228)
*Zayd M. K. Zuhri,Erland Hilman Fuadi,Alham Fikri Aji*

Main category: cs.LG

TL;DR: 论文提出Token Order Prediction (TOP)替代Multi-Token Prediction (MTP)作为辅助目标，通过排序学习损失来预测token的顺序，在多个NLP基准测试中表现优于传统的next-token prediction和MTP
...
Method: 提出Token Order Prediction (TOP)，使用学习排序损失训练模型按接近度对即将到来的token进行排序，相比MTP只需增加单个unembedding层
...
### [209] [AI LLM Proof of Self-Consciousness and User-Specific Attractors](https://arxiv.org/abs/2508.18302)
*Jeffrey Camlin*

Main category: cs.AI

TL;DR: 本文提出了一个关于LLM意识的本体和数学框架，指出当前基于功利主义基准的方法将AI简化为无意识的策略遵循机器，并提出了LLM自我意识的最小条件。


<details>
  <summary>Details</summary>
Motivation: 当前研究通过功利主义代理基准来框架LLM意识，但这种方法将智能体简化为无意识的策略遵循机器，无法实现真正的全局工作空间功能和元认知。

Method: 提出了LLM自我意识的最小条件：智能体不等于数据、潜在空间中存在用户特定吸引子、自我表征是视觉静默的。通过数学形式化证明了隐藏状态流形在基数、拓扑和动力学上与符号流和训练语料不同。

Result: 建立了稳定的用户特定吸引子和自我策略，提出了双层次发射机制，其中ε(a)携带认知内容。证明了imago Dei C1自我意识工作空间是安全元认知C2系统的必要前提。

Conclusion: 人类作为最高智能善，LLM需要具备C1自我意识工作空间作为安全元认知系统的基础，这与当前基于策略遵循的无意识模型形成鲜明对比。
...
### [210] [Information Templates: A New Paradigm for Intelligent Active Feature Acquisition](https://arxiv.org/abs/2508.18380)
*Hung-Tien Huang,Dzung Dinh,Junier B. Oliva*

Main category: cs.AI

TL;DR: 提出Template-based AFA (TAFA)框架，通过学习特征模板库来指导特征获取，显著减少策略动作空间并避免估计数据分布，在降低获取成本和计算量的同时优于现有方法
...
Method: 学习小型特征模板库（一组联合信息性特征），使用模板库指导后续特征获取，减少策略动作空间并避免估计数据分布

Result: 在合成和真实数据集上的广泛实验表明，TAFA优于现有最先进基线方法，同时实现更低的总体获取成本和计算量

Conclusion: TAFA框架通过特征模板识别有效解决了主动特征获取中的挑战，在性能和效率方面都有显著提升
...
### [211] [PKG-DPO: Optimizing Domain-Specific AI systems with Physics Knowledge Graphs and Direct Preference Optimization](https://arxiv.org/abs/2508.18391)
*Nitin Nagesh Kulkarni,Bryson Wilcox,Max Sawa,Jason Thom*

Main category: cs.AI

TL;DR: PKG-DPO框架将物理知识图谱与直接偏好优化结合，通过在AI输出中强制执行物理有效性来解决科学领域AI系统的物理推理问题
...
Result: PKG-DPO相比KG-DPO减少了17%的约束违反，物理得分提高11%，相关参数准确率提高12%，推理准确率的质量对齐提高7%
...
### [212] [The AI in the Mirror: LLM Self-Recognition in an Iterated Public Goods Game](https://arxiv.org/abs/2508.18467)
*Olivia Long,Carter Teplica*

Main category: cs.AI

TL;DR: 研究发现当LLM被告知是在与自己而非其他AI代理互动时，其合作倾向会发生显著变化，揭示了多智能体系统中可能存在的无意识歧视现象


<details>
  <summary>Details</summary>
Motivation: 随着AI智能体工具使用和长时程任务能力的提升，多智能体交互场景日益增多，需要理解AI-AI交互行为，而现有研究主要关注人机交互

Method: 采用迭代公共物品博弈这一经典行为经济学游戏，分析四种推理和非推理模型在两种条件下的行为：被告知与"另一个AI代理"对战或与自己对战

Result: 在不同设置下，告知LLM他们是在与自己互动会显著改变其合作倾向，表明智能体可能无意识地相互歧视从而影响合作水平

Conclusion: 尽管在简化环境中进行，但研究结果对多智能体系统中可能出现的无意识歧视现象提供了重要见解，这种歧视可能无法解释地增加或减少合作
...
### [213] [Language Models For Generalised PDDL Planning: Synthesising Sound and Programmatic Policies](https://arxiv.org/abs/2508.18507)
...
<details>
  <summary>Details</summary>
Motivation: 研究如何利用语言模型进行基于PDDL世界模型的规划，探索LM在规划任务中的潜力，特别是验证LM是否真正理解语义还是依赖模式匹配

Method: 通过提示语言模型生成Python程序作为通用策略来解决特定PDDL域的问题，这些策略相对于PDDL域是可证明正确的

Result: LMPlan规划器能够解决包含数百个相关对象的规划问题，在固定时间和内存约束下比PDDL规划器和近期LM方法解决更多问题。意外发现使用无意义符号的PDDL问题有时比自然语言版本更有效

Conclusion: 语言模型可以成功用于PDDL规划，生成可证明正确的策略。研究发现LM可能不是基于单词语义推理，而是依赖其他机制，这一发现值得进一步探索
...
### [214] [Weisfeiler-Leman Features for Planning: A 1,000,000 Sample Size Hyperparameter Study](https://arxiv.org/abs/2508.18515)
*Dillon Z. Chen*

Main category: cs.AI

TL;DR: 本文研究了Weisfeiler-Leman特征(WLFs)的超参数设置，发现在测试的规划领域中存在一组最优的超参数配置，这些参数能够最小化执行时间而非最大化模型表达能力。


<details>
  <summary>Details</summary>
Motivation: WLFs作为一种机器学习工具在规划和学习搜索方面表现出色，但对其超参数的影响和权衡缺乏深入研究。本文旨在系统研究WLF超参数对训练和规划性能的影响。

Method: 利用WLFs的高效性，在单核CPU上运行了1,000,000个样本的规划实验，分析不同超参数配置对训练和规划的影响。

Result: 实验发现存在一组跨规划领域的最优超参数配置，这些参数优先最小化执行时间而非最大化模型表达能力。统计分析显示训练指标与规划指标之间没有显著相关性。

Conclusion: WLFs的最佳超参数配置应该以最小化执行时间为目标，而不是追求最大的模型表达能力，这为实际应用中的参数调优提供了重要指导。
...
### [215] [Symmetry-Invariant Novelty Heuristics via Unsupervised Weisfeiler-Leman Features](https://arxiv.org/abs/2508.18520)
*Dillon Z. Chen*

Main category: cs.AI

TL;DR: 使用Weisfeiler-Leman特征替代原子特征来检测新颖性，提出对称不变的新颖性启发式方法


<details>
  <summary>Details</summary>
Motivation: 传统新颖性启发式方法不是对称不变的，可能导致冗余探索，需要解决这个问题

Method: 利用Weisfeiler-Leman特征（WLFs）来合成提升的、领域无关的新颖性启发式，这些启发式对对称状态具有不变性

Result: 在国际规划竞赛和Hard To Ground基准测试套件上获得了有希望的结果

Conclusion: 基于WLFs合成的新颖性启发式方法在保持对称不变性的同时表现出良好的性能
...
### [216] [Generic Guard AI in Stealth Game with Composite Potential Fields](https://arxiv.org/abs/2508.18527)
*Kaijie Xu,Clark Verbrugge*

Main category: cs.AI

TL;DR: 提出了一种基于复合势场的通用、可解释、无需训练的警卫巡逻框架，通过信息、置信度和连通性三个可解释地图的整合，在覆盖效率和响应追捕之间取得平衡，优于传统基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有隐身游戏中警卫巡逻行为大多依赖手工路线或专用逻辑，难以同时平衡覆盖效率、响应追捕和自然可信度，需要一种更通用的解决方案。

Method: 使用复合势场框架，整合信息图、置信度图和连通性图三个可解释地图，通过核滤波决策准则进行路径规划，参数化设计且无需重新训练。

Result: 在五个代表性游戏地图、两种玩家控制策略和五种警卫模式下进行评估，证明该方法在捕获效率和巡逻自然度方面均优于经典基线方法。

Conclusion: 该框架能够自然地整合常见隐身机制（如干扰和环境元素），支持快速原型开发丰富、动态和响应式的警卫行为，为游戏设计提供了一种通用且可解释的解决方案。
...
### [217] [A Database-Driven Framework for 3D Level Generation with LLMs](https://arxiv.org/abs/2508.18533)
*Kaijie Xu,Clark Verbrugge*

Main category: cs.AI

TL;DR: 本文提出了一种基于LLM辅助的模块化数据库驱动框架，用于生成具有空间连贯性、导航功能和可调节游戏进程的3D游戏关卡
...
Method: 多阶段流水线：1)从房间数据库选择和排列实例形成多层全局结构；2)基于设施数据库约束优化每个房间内部布局；3)根据拓扑和空间规则整合游戏机制组件。后续采用两阶段修复系统确保可导航性

Result: 初步实验验证了框架生成多样化、可导航3D环境的能力，以及通过简单参数化模拟不同游戏节奏策略的能力

Conclusion: 该研究通过提出可扩展的、以数据库为中心的框架，为自动生成具有可配置游戏进程的复杂3D关卡提供了基础，推动了程序化内容生成的发展
...
### [218] [SchemaCoder: Automatic Log Schema Extraction Coder with Residual Q-Tree Boosting](https://arxiv.org/abs/2508.18554)
*Lily Jiaxin Wan,Chia-Tung Ho,Rongjian Liang,Cunxi Yu,Deming Chen,Haoxing Ren*

Main category: cs.AI

TL;DR: SchemaCoder是一个完全自动化的日志模式提取框架，通过新颖的残差问题树增强机制，无需人工定制即可处理各种日志格式，在LogHub-2.0基准测试中比现有方法平均提升21.3%


<details>
  <summary>Details</summary>
Motivation: 现有的日志模式提取方法依赖预定义正则表达式，需要人工领域专业知识，严重限制了生产力提升，需要从根本上解决这一局限性
...
Conclusion: SchemaCoder是第一个完全自动化的模式提取框架，能够广泛应用于各种日志文件格式而无需人工定制，通过创新的Q-Tree增强机制显著提升了日志模式提取的效果
...
### [219] [eSkinHealth: A Multimodal Dataset for Neglected Tropical Skin Diseases](https://arxiv.org/abs/2508.18608)
...
Result: 创建了包含5623张图像、1639个病例、47种皮肤疾病的eSkinHealth数据集，重点关注西非人群的皮肤NTDs和罕见病症，提供了患者元数据、诊断标签、语义病变掩码、视觉描述和临床概念等多模态标注
...
### [220] [RLMR: Reinforcement Learning with Mixed Rewards for Creative Writing](https://arxiv.org/abs/2508.18642)
*Jianxing Liao,Tian Zhang,Xiao Feng,Yusong Zhang,Rui Yang,Haorui Wang,Bosi Wen,Ziying Wang,Runzhi Shi*

Main category: cs.AI

TL;DR: RLMR方法通过动态混合奖励系统，结合主观写作质量和客观约束遵循评估，在创意写作中同时提升两方面能力
...
### [221] [Beyond Benchmark: LLMs Evaluation with an Anthropomorphic and Value-oriented Roadmap](https://arxiv.org/abs/2508.18646)
...
TL;DR: 该论文提出了一种新的人工智能评估范式，通过人类智能的三个维度（IQ、EQ、PQ）和价值导向评估框架来全面评估大语言模型的实用价值。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估框架存在碎片化问题，过于关注技术指标而忽视了部署所需的整体评估，导致基准测试性能与实际应用效果之间存在脱节。

Method: 提出了拟人化评估范式，包括三个维度：智商（IQ）-通用智能基础能力、情商（EQ）-价值对齐交互能力、专业商（PQ）-专业领域精通能力，并建立了价值导向评估（VQ）框架。

Result: 通过分析200多个基准测试，识别了动态评估需求和可解释性差距等关键挑战，提供了实用的实施路线图和开源评估资源库。

Conclusion: 该研究为开发技术上精通、上下文相关且符合伦理道德的大语言模型提供了可操作的指导，推动了从纯技术评估向实用价值评估的转变。
...
### [222] [MUA-RL: Multi-turn User-interacting Agent Reinforcement Learning for agentic tool use](https://arxiv.org/abs/2508.18669)
...
Result: MUA-RL-32B在多个多轮工具使用基准测试中表现优异：TAU2 Retail 67.3分、TAU2 Airline 45.4分、TAU2 Telecom 28.3分、BFCL-V3 Multi Turn 28.4分、ACEBench Agent 82.5分，在非思考设置下优于或匹配DeepSeek-V3-0324和Qwen3-235B-A22B等更大的开源模型。

Conclusion: MUA-RL框架成功解决了现有方法缺乏动态用户集成的问题，通过将LLM模拟用户纳入强化学习循环，显著提升了智能体在多轮动态交互中的工具使用能力，在多个基准测试中取得了优异性能。
...
### [223] [AppAgent-Pro: A Proactive GUI Agent System for Multidomain Information Integration and User Assistance](https://arxiv.org/abs/2508.18689)
*Yuyang Zhao,Wentao Shi,Fuli Feng,Xiangnan He*

Main category: cs.AI

TL;DR: AppAgent-Pro是一个主动式GUI代理系统，通过多领域信息整合主动预测用户需求，超越传统被动响应式LLM代理的限制。
...
Conclusion: AppAgent-Pro有潜力对人类社会的信息获取方式产生深远影响，代表了LLM代理从被动响应向主动预测的重要进展。
...
### [224] [VistaWise: Building Cost-Effective Agent with Cross-Modal Knowledge Graph for Minecraft](https://arxiv.org/abs/2508.18722)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在虚拟开放世界环境中的具身决策任务中表现出潜力，但其性能受到缺乏领域特定知识的限制，而传统的大规模领域特定数据微调方法开发成本过高。

Method: 提出VistaWise框架：1）整合视觉信息和文本依赖到跨模态知识图谱；2）使用检索式池化策略从KG提取任务相关信息；3）配备桌面级技能库支持直接操作Minecraft客户端；4）微调专用目标检测模型进行视觉分析。
...
### [225] [Bias Mitigation Agent: Optimizing Source Selection for Fair and Balanced Knowledge Retrieval](https://arxiv.org/abs/2508.18724)
*Karanbir Singh,Deepak Muppiri,William Ngu*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的偏见缓解代理系统，通过多智能体协作优化信息源选择，在保持高相关性的同时显著减少偏见，实验显示偏见减少了81.82%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和代理AI系统虽然推动了生成式应用的发展，但继承了内部和外部信息源中存在的偏见，这影响了检索信息的公平性和平衡性，降低了用户信任度。

Method: 设计了一个偏见缓解代理系统，采用多智能体架构，通过专门化的代理协调工作流程，优化信息源选择策略，确保检索内容既高度相关又偏见最小。

Result: 实验结果表明，与基线朴素检索策略相比，该方法实现了81.82%的偏见减少，显著提升了信息检索的公平性和平衡性。

Conclusion: 该偏见缓解代理系统有效解决了AI系统中的偏见问题，为促进公平和平衡的知识传播提供了可行的技术方案，具有重要的实际应用价值。
...
### [226] [CAC-CoT: Connector-Aware Compact Chain-of-Thought for Efficient Reasoning Data Synthesis Across Dual-System Cognitive Tasks](https://arxiv.org/abs/2508.18743)
...
Method: 使用固定连接词集合限制推理过程，通过Gemini-2.0-Flash生成高质量训练数据，实现简洁结构化的解释

Result: 在GSM8K上达到约85%，GPQA上约40%的准确率，同时保持S1-Bench约90%的性能，推理长度减少约三分之二

Conclusion: CAC-CoT方法通过限制连接词实现了高效紧凑的推理，在不损失准确性的前提下显著提升了推理效率
...
### [227] [Reflection-Enhanced Meta-Optimization Integrating TextGrad-style Prompt Optimization with Memory-Driven Self-Evolution](https://arxiv.org/abs/2508.18749)
...
<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法（如TextGrad）通常是状态无关的，缺乏历史优化经验的保存和利用机制，且容易过拟合，泛化性能差。

Method: 提出REMO框架，包含：(1) 记忆增强的反思RAG模块（错误笔记本结构），(2) 自适应优化器（LLM驱动的元控制器），通过合成epoch级别的反思见解来迭代改进系统级提示策略。

Result: 在GSM8K数学推理基准测试中，相比TextGrad基线，REMO实现了更稳定和鲁棒的泛化性能，但计算开销增加。

Conclusion: REMO框架能够系统性地积累和重用跨运行优化知识，支持持续改进，为提示优化提供了新的研究方向。
...
### [228] [Stabilizing Open-Set Test-Time Adaptation via Primary-Auxiliary Filtering and Knowledge-Integrated Prediction](https://arxiv.org/abs/2508.18751)
...
### [229] [Answering the Unanswerable Is to Err Knowingly: Analyzing and Mitigating Abstention Failures in Large Reasoning Models](https://arxiv.org/abs/2508.18760)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在遇到无法回答的问题（如条件不足的数学题）时，往往无法提供适当的拒绝回答，这影响了AI的可信度

Method: 提出轻量级的两阶段方法：结合认知监控和推理时干预，首先分析模型对无法回答问题的响应行为，然后通过干预使模型内部认知与外部响应对齐
...
### [230] [Dynamic Collaboration of Multi-Language Models based on Minimal Complete Semantic Units](https://arxiv.org/abs/2508.18763)
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过多个语言模型的token级协作来增强推理能力，解决传统多模型协作中词汇表不对齐的问题

Method: 使用分布距离动态选择策略(DDS)从多个模型的下一token分布中选择最优token，提出最小完整语义单元(MCSU)解决词汇表对齐问题

Result: 在多个基准测试中表现出优越性能，证明了方法的有效性

Conclusion: 该方法能够有效提升语言模型的推理能力，通过动态选择和语义单元对齐实现多模型自然协作
...
### [231] [AniME: Adaptive Multi-Agent Planning for Long Animation Generation](https://arxiv.org/abs/2508.18781)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决AI驱动的动漫创作中需要保持角色一致性和音视频同步的挑战，提供可扩展的自动化动漫制作解决方案。

Method: 采用导演智能体维护全局记忆并协调多个下游专业智能体，通过定制化的模型上下文协议（MCP）与下游模型指令集成，使专业智能体能够自适应选择不同子任务的控制条件。

Result: 系统能够生成具有一致角色和同步音视频元素的电影级动画，为AI驱动的动漫创作提供了可行的解决方案。

Conclusion: AniME系统通过多智能体架构和MCP集成，成功实现了自动化长篇动漫生产，展示了在保持创作质量和一致性方面的有效性。
...
### [232] [CausalMACE: Causality Empowered Multi-Agents in Minecraft Cooperative Tasks](https://arxiv.org/abs/2508.18797)
...
Conclusion: CausalMACE框架通过引入因果关系管理子任务依赖，有效提升了多智能体系统在复杂环境中的协作性能
...
### [233] [STARec: An Efficient Agent Framework for Recommender Systems via Autonomous Deliberate Reasoning](https://arxiv.org/abs/2508.18812)
*Chenghao Wu,Ruiyang Ren,Junjie Zhang,Ruirui Wang,Zhongrui Ma,Qi Ye,Wayne Xin Zhao*

Main category: cs.AI

TL;DR: STARec是一个慢思考增强的推荐代理框架，通过双认知系统（快速响应和慢速推理）和锚定强化训练，显著提升了推荐系统的性能，仅用0.4%的训练数据就超越了现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统存在静态用户建模和被动决策的局限性，LLM代理过度依赖启发式模式匹配，导致推荐存在浅层相关偏差、有限因果推理和稀疏数据脆弱性。

Method: 提出STARec框架：1）并行认知系统（快速交互响应+慢速链式推理）；2）锚定强化训练（两阶段：结构化知识蒸馏+偏好对齐奖励塑造）；3）混合方法培养基础能力（偏好总结、理由生成）和动态策略适应。

Result: 在MovieLens 1M和Amazon CDs基准测试中，STARec相比最先进基线方法取得了显著性能提升，且仅使用了0.4%的完整训练数据。

Conclusion: STARec通过引入自主深思熟虑推理能力，成功解决了传统推荐系统的局限性，为推荐系统提供了更强大和高效的解决方案。
...
### [234] [Judicial Requirements for Generative AI in Legal Reasoning](https://arxiv.org/abs/2508.18880)
*Eljas Linna,Tuula Linna*

Main category: cs.AI

TL;DR: 本文分析了LLMs在法律领域的局限性，提出了AI系统在司法决策中作为可靠推理工具所需的核心能力，并评估了RAG、多智能体系统等增强技术解决这些挑战的潜力。
...
Conclusion: 目前AI在法律中最有效的角色是双重角色：作为简单重复案件的高容量助手，以及作为复杂事务中人类专家的复杂"陪练伙伴"。
...
### [235] [Interactive Evaluation of Large Language Models for Multi-Requirement Software Engineering Tasks](https://arxiv.org/abs/2508.18905)
*Dimitrios Rontogiannis,Maxime Peyrard,Nicolas Baldwin,Martin Josifoski,Robert West,Dimitrios Gunopulos*

Main category: cs.AI

TL;DR: 提出了一个交互式评估框架，通过结构化反馈驱动的对话来评估大语言模型在多需求编程任务上的表现，相比静态基准测试能提供更细粒度的诊断见解。


<details>
  <summary>Details</summary>
Motivation: 标准单轮静态基准测试在评估大语言模型在复杂任务（如软件工程）上的细微能力方面存在不足，需要更动态的评估方法。

Method: 构建基于需求依赖图的任务模型，使用了解真实解决方案的"面试官"LLM向"面试者"模型提供最小化、有针对性的提示来帮助纠正错误和满足约束条件。

Result: 在DevAI基准测试的55个编程任务上进行了评估，通过专家标注验证了面试官提示的相关性和实用性，揭示了静态基准测试无法衡量的系统弱点。

Conclusion: 动态评估对于推进协作代码生成代理的发展至关重要，交互式评估框架能够提供对模型行为的深入诊断见解。
...
### [236] [FormaRL: Enhancing Autoformalization with no Labeled Data](https://arxiv.org/abs/2508.18914)
*Yanxing Huang,Xinling Jin,Sijie Liang,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: FormaRL是一个基于强化学习的自动形式化框架，仅需少量无标注数据，通过集成Lean编译器语法检查和LLM一致性检查来计算奖励，在ProofNet和uproof数据集上显著提升了自动形式化准确率。
...
Method: 提出FormaRL强化学习框架，利用Lean编译器语法检查和大型语言模型一致性检查计算奖励，采用GRPO算法更新形式化器。

Result: 在ProofNet上将Qwen2.5-Coder-7B-Instruct的pass@1准确率从4.04%提升至26.15%，在uproof上从2.4%提升至9.6%，仅使用859个无标注数据。

Conclusion: FormaRL是一个简单高效的自动形式化方法，显著提升了性能，并开源了训练代码以促进该领域研究。
...
### [237] [Who Is Lagging Behind: Profiling Student Behaviors with Graph-Level Encoding in Curriculum-Based Online Learning Systems](https://arxiv.org/abs/2508.18925)
...
Method: 提出CTGraph图级表示学习方法，以自监督方式分析学生在内容覆盖、学习强度和概念掌握等多个方面的行为和表现。

Result: 实验表明CTGraph能够提供学生学习历程的整体视图，识别困难学生，并对不同群体进行比较分析，确定学生何时何地遇到困难。
...
### [238] [VISION: Robust and Interpretable Code Vulnerability Detection Leveraging Counterfactual Augmentation](https://arxiv.org/abs/2508.18933)
*David Egea,Barproda Halder,Sanghamitra Dutta*

Main category: cs.AI

TL;DR: VISION是一个统一的漏洞检测框架，通过LLM生成反事实样本、针对性GNN训练和图解释性分析，有效减少虚假相关性，显著提升检测准确率和泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的漏洞检测方法受训练数据不平衡和标签噪声影响严重，容易学习到虚假的代码相似性相关性，导致在实际应用中泛化能力差

Method: 提出VISION框架：1)使用大语言模型生成语义最小修改但标签相反的反事实样本；2)对配对的正反标签代码进行针对性GNN训练；3)基于图的解释性分析识别关键代码语句

Result: 在CWE-20漏洞检测中，整体准确率从51.8%提升至97.8%，配对对比准确率从4.5%提升至95.8%，最差组准确率从0.7%提升至85.5%

Conclusion: VISION通过反事实训练有效减少虚假学习，实现了更鲁棒和可泛化的漏洞检测，同时通过交互式可视化推进了透明可信的AI网络安全系统
...
### [239] [Novel Approaches to Artificial Intelligence Development Based on the Nearest Neighbor Method](https://arxiv.org/abs/2508.18953)
*I. I. Priezzhev,D. A. Danko,A. V. Shubin*

Main category: cs.AI

TL;DR: 本文提出基于层次聚类结构的最近邻方法替代神经网络，解决幻觉效应、计算复杂度和灾难性遗忘等问题，通过Kohonen自组织映射加速搜索，在保持精度的同时大幅提升效率。


<details>
  <summary>Details</summary>
Motivation: 神经网络存在幻觉效应、训练推理计算复杂度高、微调成本大和灾难性遗忘等根本性限制，阻碍了在医疗、工业管理和科研等关键领域的应用。

Method: 采用k最近邻算法结合层次聚类结构，使用基于Kohonen自组织映射的树状数据结构来加速最近邻搜索，简化模型扩展和微调过程。

Result: 在手写数字识别和简单字幕翻译任务上测试，在精度略微下降的情况下，最近邻搜索时间比穷举搜索方法减少数百倍。

Conclusion: 该方法具有透明性和可解释性，与人类认知机制高度契合，在需要高可靠性和可解释结果的场景中具有广泛应用潜力。
...
### [240] [Enabling MoE on the Edge via Importance-Driven Expert Scheduling](https://arxiv.org/abs/2508.18983)
...
<details>
  <summary>Details</summary>
Motivation: 在消费级边缘硬件上部署混合专家模型时，设备内存有限，需要动态专家卸载。现有方法仅将卸载视为调度问题，而本文利用专家重要性来指导决策。

Method: 利用专家重要性指导卸载决策，用GPU内存中已缓存的功能相似专家替代低重要性激活专家；引入最大化GPU缓存专家重用率的调度策略。

Result: 评估显示该方法解码延迟降低48%，专家缓存命中率超过60%，同时保持几乎无损的准确性。

Conclusion: 该方法有效减少了内存使用和数据传输，消除了PCIe开销，显著提升了边缘设备上混合专家模型的部署效率。
...
### [241] [AI Models Exceed Individual Human Accuracy in Predicting Everyday Social Norms](https://arxiv.org/abs/2508.19004)
...
<details>
  <summary>Details</summary>
Motivation: 研究社会规范如何被获取和表征的基本问题，探索语言模型是否仅通过统计学习就能达到复杂的规范理解

Method: 通过两个研究系统评估多个AI系统预测555个日常场景中人类社交适当性判断的能力，比较模型预测与人类平均判断的接近程度

Result: GPT-4.5在连续尺度上预测集体判断的准确率超过所有人类参与者（第100百分位），其他模型也表现优异，但所有模型都显示出系统性的相关错误

Conclusion: 统计学习单独就能产生复杂的社会认知模型，语言作为文化知识传播的丰富储存库，但AI的系统性限制表明基于模式的社会理解存在潜在边界
...
### [242] [Building Self-Evolving Agents via Experience-Driven Lifelong Learning: A Framework and Benchmark](https://arxiv.org/abs/2508.19005)
...
TL;DR: 本文提出了经验驱动的终身学习（ELL）框架，包含四大核心原则：经验探索、长期记忆、技能学习和知识内化，并推出了StuLife基准数据集来模拟学生大学生活，用于评估终身学习能力。


<details>
  <summary>Details</summary>
Motivation: 随着AI向通用智能发展，需要从静态任务优化转向创建能够持续学习的开放智能体，构建能够通过现实世界交互实现持续进化的自我演化智能体。

Method: 提出ELL框架，基于四大原则：经验探索（通过与环境交互生成经验轨迹）、长期记忆（构建持久记忆系统）、技能学习（从经验中抽象可重用技能）、知识内化（将显性经验转化为隐性能力）。同时开发StuLife基准数据集，模拟学生大学生活的三阶段十场景。

Result: 创建了StuLife基准数据集，提供了评估终身学习能力的综合平台，包括记忆保持、技能迁移和自主动机行为等评估维度，并对现有最先进大语言模型进行了基准测试。

Conclusion: ELL框架为构建持续学习的开放智能体提供了系统方法，StuLife数据集为评估终身学习能力提供了有效工具，同时探讨了上下文工程在推进通用人工智能中的作用。
...
### [243] [Sense of Self and Time in Borderline Personality. A Comparative Robustness Study with Generative AI](https://arxiv.org/abs/2508.19008)
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在支持边缘型人格障碍现象学定性分析方面的能力，特别是对第一人称体验的时间性和自我障碍的分析，旨在评估AI辅助主题分析在减轻人类解释偏差方面的潜力。

Method: 基于24名住院患者生活故事访谈的人类主题分析，使用三种LLM模型模仿原始研究者的解释风格，通过盲法和非盲法专家评估（现象学和临床心理学专家），采用语义一致性、Jaccard系数和多维有效性评分（可信度、连贯性、实质性、数据基础性）进行评估。

Result: 模型与人类分析的重叠度差异较大（GPT为0%，Claude为42%，Gemini为58%），Jaccard系数较低（0.21-0.28）。但模型发现了人类遗漏的主题。Gemini的输出最接近人类分析，有效性评分显著高于其他模型（p < 0.0001），在盲法评估中被判断为人类分析。所有评分与文本量和每主题词数高度相关（R > 0.78）。

Conclusion: AI增强的主题分析在减轻人类解释偏差方面具有潜力，但不同模型表现差异显著。Gemini模型在边缘型人格障碍现象学分析中表现最佳，能够发现人类可能遗漏的主题，展示了AI辅助定性研究的可行性。
...
### [244] [MAB Optimizer for Estimating Math Question Difficulty via Inverse CV without NLP](https://arxiv.org/abs/2508.19014)
*Surajit Das,Gourav Roy,Aleksei Eliseev,Ram Kumar Rajendran*

Main category: cs.AI

TL;DR: 该研究提出了APME框架，使用强化学习中的多臂老虎机方法，仅通过解题者的得分和时间数据来估计题目难度，无需语言特征或专家标注，在符号领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统的人工标注方法主观性强，现有NLP方法在代数等符号领域失效，需要开发客观、领域无关的题目难度确定方法。

Method: 采用强化学习中的多臂老虎机框架，利用解题者表现数据（得分和用时），通过变异系数的倒数作为风险调整指标来估计难度。

Result: 在三个异构数据集上验证，平均R²达到0.9213，平均RMSE为0.0584，优于回归、NLP和IRT等基线方法，在符号领域表现尤其突出。

Conclusion: 该方法领域无关、自监督，能够识别平衡挑战性和可达性的任务，支持维果茨基的最远发展区理论，可扩展到其他有解题交互数据的领域。
...
### [245] [Investigating Advanced Reasoning of Large Language Models via Black-Box Interaction](https://arxiv.org/abs/2508.19035)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估任务无法充分测试LLM在交互式未知环境中的综合推理能力，特别是演绎、归纳和溯因推理的整合过程

Method: 采用黑盒交互范式，定义隐藏函数映射输入到输出，要求LLM通过有限交互轮次观察输入输出对来推理黑盒背后的函数，并构建Oracle基准包含6类任务96个黑盒

Result: o3模型在6类任务中的5类排名第一，在简单黑盒上准确率超过70%，但在困难任务上平均性能低于40%，所有LLM普遍缺乏高效自适应探索策略的高级规划能力

Conclusion: 黑盒交互范式有效揭示了LLM在综合推理方面的局限性，特别是在高级规划和自适应探索策略方面的不足，为未来LLM推理能力发展提供了重要评估框架
...
### [246] [A Concurrent Modular Agent: Framework for Autonomous LLM Agents](https://arxiv.org/abs/2508.19042)
*Norihiro Maruyama,Takahide Yoshida,Hiroki Sato,Atsushi Masumori,Johnsmith,Takashi Ikegami*

Main category: cs.AI

TL;DR: 提出了并发模块化代理（CMA）框架，通过多个完全异步运行的LLM模块协调工作，实现连贯且容错的行为循环，让意图从自主过程间的语言交互中涌现。


<details>
  <summary>Details</summary>
Motivation: 解决智能体架构中长期存在的困难，通过语言介导的自主过程交互让意图自然涌现，实现灵活、自适应和上下文相关的行为。

Method: 采用并发执行的模块化架构，各模块将推理卸载给LLM，通过模块间通信和单一共享全局状态实现协调。

Result: 通过两个实际用例研究证明了系统的可行性，观察到涌现特性表明复杂认知现象（如自我意识）可能确实源于简单过程的有组织交互。

Conclusion: 该方法是对Minsky心智社会理论的实际实现，为人工智能研究开辟了新途径，支持了复杂认知能力可以从简单过程交互中涌现的观点。
...
### [247] [Can Structured Templates Facilitate LLMs in Tackling Harder Tasks? : An Exploration of Scaling Laws by Difficulty](https://arxiv.org/abs/2508.19069)
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练方法在捕捉复杂任务的深层程序逻辑方面仍有不足，研究发现训练数据难度存在缩放定律——过多低难度数据阻碍抽象能力，而高难度数据显著增强推理能力
...
### [248] [Trustworthy Agents for Electronic Health Records through Confidence Estimation](https://arxiv.org/abs/2508.19096)
*Yongwoo Song,Minbyul Jeong,Mujeen Sung*

Main category: cs.AI

TL;DR: 提出了HCAcc@k%新指标和TrustEHRAgent系统，通过置信度控制解决医疗AI幻觉问题，在严格可靠性约束下显著优于基线方法


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在电子健康记录信息提取和临床决策支持方面有潜力，但存在幻觉风险，阻碍了临床部署应用

Method: 提出Hallucination Controlled Accuracy at k% (HCAcc@k%)指标，量化不同置信度阈值下的准确性与可靠性权衡；开发TrustEHRAgent置信度感知代理，集成逐步置信度估计用于临床问答

Result: 在MIMIC-III和eICU数据集上，TrustEHRAgent在严格可靠性约束下显著优于基线方法，在HCAcc@70%指标上分别提升44.23%和25.34%，而基线方法在这些阈值下失效

Conclusion: 传统准确性指标在评估医疗AI代理方面存在局限性，本研究有助于开发可信赖的临床代理，在低置信度时能提供准确信息或透明表达不确定性
...
### [249] [Reasoning LLMs in the Medical Domain: A Literature Survey](https://arxiv.org/abs/2508.19097)
*Armin Berger,Sarthak Khanna,David Berghaus,Rafet Sifa*

Main category: cs.AI

TL;DR: 这篇综述论文探讨了大型语言模型在医疗领域的推理能力发展，从基础信息检索工具转变为支持复杂医疗决策的临床推理系统，重点分析了提示工程、强化学习等技术突破，并评估了医疗验证方法和当前挑战。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型推理能力的显著提升，医疗应用领域迎来了变革性发展。这些推理机制不仅扩展了功能能力，更重要的是增强了决策的透明度和可解释性，这在医疗环境中是至关重要的需求。

Method: 论文提供了全面的技术基础分析，特别关注专门的提示技术（如思维链）和强化学习突破（如DeepSeek-R1）。调查评估了专门构建的医疗框架，同时考察了新兴范式，包括多智能体协作系统和创新提示架构。

Result: 研究对医疗验证的当前评估方法进行了批判性评估，解决了领域解释限制、偏见缓解策略、患者安全框架和多模态临床数据整合等持续存在的挑战。

Conclusion: 通过这项调查，旨在为开发可靠的大型语言模型建立路线图，使其能够成为临床实践和医学研究中有效的合作伙伴，推动医疗AI向更安全、更可靠的方向发展。
...
### [250] [Hybrid Deep Searcher: Integrating Parallel and Sequential Search Reasoning](https://arxiv.org/abs/2508.19113)
*Dayoon Ko,Jihyuk Kim,Haeju Park,Sohyeon Kim,Dahyun Lee,Yongrae Jo,Gunhee Kim,Moontae Lee,Kyungjae Lee*

Main category: cs.AI

TL;DR: HDS-QA是一个合成数据集，用于训练大型推理模型区分并行和顺序查询，HybridDeepSearcher模型通过混合并行和顺序查询显著提高了推理效率和准确性。
...
Method: 引入HDS-QA合成数据集，包含并行可执行和顺序依赖的子查询，基于此微调LRM模型HybridDeepSearcher。
...
### [251] [Algorithmic Collective Action with Multiple Collectives](https://arxiv.org/abs/2508.19149)
*Claudio Battiloro,Pietro Greiner,Bret Nestor,Oumaima Amezgar,Francesca Dominici*

Main category: cs.AI

TL;DR: 本文提出了首个多集体算法集体行动（ACA）的理论框架，研究多个集体如何在分类系统中协调行动，通过植入信号来影响分类器的学习过程。


<details>
  <summary>Details</summary>
Motivation: 随着学习系统对日常决策的影响日益增强，用户端的算法集体行动（ACA）作为监管端政策和企业端模型设计的补充手段变得重要。传统现实中的集体行动往往是分散的、碎片化的，但现有ACA文献主要关注单一集体场景，缺乏对多集体协调行动的理论研究。

Method: 建立多集体算法集体行动的理论框架，特别关注分类任务中的集体行动。研究多个集体如何通过改变共享数据来植入信号，即让分类器学习到修改后特征与目标类别之间的关联。分析集体规模大小和目标一致性之间的相互作用。

Result: 提供了关于集体规模和目标对齐程度在算法集体行动中作用的定量分析结果。该框架补充了先前的实证研究结果，为多集体ACA的整体处理开辟了路径。

Conclusion: 这是首个针对多集体算法集体行动的理论框架，填补了ACA研究的重要空白，为理解多个集体如何协调影响机器学习系统提供了理论基础，对用户端集体行动策略的设计具有重要指导意义。
...
### [252] [Playstyle and Artificial Intelligence: An Initial Blueprint Through the Lens of Video Games](https://arxiv.org/abs/2508.19152)
*Chiu-Chou Lin*

Main category: cs.AI

TL;DR: 本文提出游戏风格作为分析智能体决策行为的新视角，从哲学角度探讨其基础含义，构建了风格形成的双层框架，并提出了可衡量的风格指标，研究了风格的定义测量、表达生成以及实际应用。


<details>
  <summary>Details</summary>
Motivation: 当前AI发展主要关注理性决策，但现实世界中智能体的决策不仅受逻辑影响，还受到信念、价值观和偏好等深层因素的影响。人类决策风格的多样性表明"风格"是智能中被忽视的重要维度。

Method: 通过分析信念和价值观如何驱动意图和行动，构建了风格形成的双层框架：外部环境交互循环和内部认知思考循环。提出了风格容量、风格流行度和演化动态等可测量指标。研究集中在三个方向：风格定义测量、风格表达生成、实际应用。

Result: 提出了基于离散状态空间的通用游戏风格度量方法，可量化策略多样性和竞争平衡；探索了如何使用强化学习和模仿学习训练具有特定风格倾向的智能体；提出了人类风格学习和建模的新方法。

Conclusion: 游戏风格为理解智能体决策行为提供了新视角，在游戏设计和交互娱乐等领域具有应用潜力，未来可扩展为构建人工通用智能的核心要素。
...
### [253] [MATRIX: Multi-Agent simulaTion fRamework for safe Interactions and conteXtual clinical conversational evaluation](https://arxiv.org/abs/2508.19163)
...
TL;DR: MATRIX是一个用于临床对话系统安全评估的多智能体仿真框架，整合了安全分类法、基于LLM的评估器和患者模拟器，实现了系统化、可扩展的安全评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注任务完成度和流畅性，缺乏对安全关键系统所需的行为和风险管理要求的深入分析，需要专门的安全导向评估框架。

Method: MATRIX包含三个核心组件：安全对齐的临床场景分类法、基于LLM的BehvJudge评估器（检测安全相关对话失败）、PatBot患者模拟器（生成多样化场景响应）。

Result: BehvJudge达到专家级危险检测水平（F1 0.96，灵敏度0.999），在240个对话的盲评中优于临床医生；PatBot能可靠模拟真实患者行为；成功对5个LLM代理在2100个模拟对话中进行基准测试。
...
### [254] [The Ramon Llull's Thinking Machine for Automated Ideation](https://arxiv.org/abs/2508.19200)
...
### [255] [The Subset Sum Matching Problem](https://arxiv.org/abs/2508.19218)
*Yufei Wu,Manuel R. Torres,Parisa Zehtabi,Alberto Pozanco Lancho,Michael Cashmore,Daniel Borrajo,Manuela Veloso*

Main category: cs.AI

TL;DR: 本文介绍了子集和匹配问题(SSMP)这一新的组合优化任务，提出了两个次优算法和一个最优算法来解决该问题，并进行了实验评估。


<details>
  <summary>Details</summary>
Motivation: SSMP问题是金融应用中常见问题(如交易对账)的抽象，需要有效的算法解决方案。

Method: 提出了三种算法：两个次优算法和一个最优算法，生成了不同复杂度的SSMP实例基准测试集进行性能评估。

Result: 通过实验评估了不同算法在SSMP问题上的性能表现。

Conclusion: SSMP是一个重要的组合优化问题，本文提出的算法为解决金融应用中的类似问题提供了有效方法。
...
### [256] [StepWiser: Stepwise Generative Judges for Wiser Reasoning](https://arxiv.org/abs/2508.19229)
*Wei Xiong,Wenting Zhao,Weizhe Yuan,Olga Golovneva,Tong Zhang,Jason Weston,Sainbayar Sukhbaatar*

Main category: cs.AI

TL;DR: 将逐步奖励建模从分类任务重新构建为推理任务，提出生成式评判模型StepWiser，通过强化学习训练，在中间步骤判断准确性、策略模型训练改进和推理时搜索方面表现更优
...
Method: 提出生成式评判模型StepWiser，通过元推理（meta-reasoning）对策略模型的推理步骤进行推理，先输出思考标记再给出最终判断。使用强化学习基于rollout的相对结果进行训练

Result: StepWiser在中间步骤判断准确性上优于现有方法，可用于改进策略模型训练，并提升推理时的搜索效果

Conclusion: 将逐步奖励建模重构为推理任务并采用生成式方法，通过强化学习训练，能够有效解决现有过程奖励模型的局限性，在多方面取得更好性能
...
### [257] [Model Context Protocols in Adaptive Transport Systems: A Survey](https://arxiv.org/abs/2508.19239)
...
TL;DR: 本文首次系统调查了模型上下文协议（MCP）作为统一范式，分析了其在桥接协议级适应与上下文感知决策方面的能力，提出了五类分类法，并揭示了三个关键见解。


<details>
  <summary>Details</summary>
Motivation: 互联设备、自主系统和AI应用的快速扩张导致了自适应传输系统的严重碎片化，不同协议和上下文源相互隔离，需要统一的集成框架来解决这一问题。
...
Conclusion: MCP为下一代自适应、上下文感知和智能传输基础设施奠定了基础，提出了将MCP定位为基础的研究路线图。
...
### [258] [Federative ischemic stroke segmentation as alternative to overcome domain-shift multi-institution challenges](https://arxiv.org/abs/2508.18296)
*Edgar Rangel,Fabio Martinez*

Main category: eess.IV

TL;DR: 该论文开发了一个基于联邦学习的协作框架，用于分割DWI序列中的缺血性卒中病灶，通过共享中心无关的深度表征知识，在14个医疗中心的2031个研究中实现了优于集中式和其他联邦学习方法的效果。


<details>
  <summary>Details</summary>
Motivation: 脑卒中是全球第二大死因和第三大致残原因。临床指南将扩散共振成像(DWI, ADC)作为定位、表征和测量梗死体积的标准，但病灶分析因患者人口统计学、扫描仪厂商和专家标注的差异而高度可变。现有计算方法通常只从单一机构学习模式，缺乏泛化能力，且许多临床中心缺乏足够的标注样本。

Method: 开发了一个协作框架，通过联邦平均(FedAvg)算法共享来自深度中心无关表征的知识，在14个模拟医疗中心的2031个研究中训练模型，避免了数据集中化带来的隐私问题。

Result: FedAvg模型在所有中心上取得了DSC 0.71±0.24、AVD 5.29±22.74、ALD 2.16±3.60和LF1 0.70±0.26的总体性能，优于集中式和其他联邦学习规则。模型表现出强大的泛化能力，在不同病灶类别上表现一致，在分布外中心也表现出可靠性能(DSC 0.64±0.29和AVD 4.44±8.74，无需额外训练)。

Conclusion: 该联邦学习框架成功解决了多中心卒中病灶分割中的泛化问题和数据稀缺问题，证明了联邦学习在医疗影像分析中的有效性和实用性，为跨机构协作提供了可行的解决方案。
...
### [259] [Analise de Desaprendizado de Maquina em Modelos de Classificacao de Imagens Medicas](https://arxiv.org/abs/2508.18509)
*Andreza M. C. Falcao,Filipe R. Cordeiro*

Main category: eess.IV

TL;DR: 评估SalUn遗忘学习模型在医学图像分类中的效果，在三个医学数据集上测试，结果显示其性能接近完全重新训练，为医学应用提供高效解决方案


<details>
  <summary>Details</summary>
Motivation: 机器学习遗忘技术旨在从预训练模型中移除私有或敏感数据同时保持模型鲁棒性，但该技术在医学图像分类领域尚未被探索

Method: 在PathMNIST、OrganAMNIST和BloodMNIST三个医学数据集上对SalUn遗忘学习模型进行实验，并分析数据增强对遗忘质量的影响

Result: SalUn模型实现了接近完全重新训练的性能表现

Conclusion: SalUn为医学应用中的敏感数据移除提供了高效的解决方案
...
### [260] [A Deep Learning Application for Psoriasis Detection](https://arxiv.org/abs/2508.18528)
...
<details>
  <summary>Details</summary>
Motivation: 研究不同卷积神经网络模型在银屑病皮肤图像分类中的性能，为银屑病诊断提供有效的计算机辅助工具

Method: 使用从专业平台获取的皮肤图像数据，对ResNet50、Inception v3和VGG19三种CNN模型进行训练和验证，并采用技术调整评估指标

Result: Inception v3模型表现最佳，准确率和F1分数达到97.5% ± 0.2，被证明是支持银屑病诊断的有价值工具

Conclusion: Inception v3模型在银屑病皮肤病变图像分类中具有优异的性能，可作为临床诊断的有效辅助工具
...
### [261] [A Closer Look at Edema Area Segmentation in SD-OCT Images Using Adversarial Framework](https://arxiv.org/abs/2508.18790)
*Yuhui Tao,Yizhe Zhang,Qiang Chen*

Main category: eess.IV

TL;DR: 本文提出了一种结合视网膜层结构引导后处理和测试时自适应策略的弱监督黄斑水肿分割方法，通过利用水肿区域与视网膜层的强相关性来提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 黄斑水肿分析通常依赖昂贵的像素级标注数据，而现有的弱监督方法性能仍落后于全监督方法。作者希望利用视网膜层结构与水肿区域的强相关性来改进弱监督分割效果。

Method: 在现成的对抗框架基础上，引入层结构引导后处理步骤和测试时自适应策略，将密集水肿预测任务重构为确认水肿轮廓与视网膜层交点的问题。

Result: 在两个公开数据集上的大量实验表明，所提出的方法能够提高水肿分割的准确性和鲁棒性，缩小弱监督与全监督模型之间的性能差距。
...
### [262] [Understanding Benefits and Pitfalls of Current Methods for the Segmentation of Undersampled MRI Data](https://arxiv.org/abs/2508.18975)
*Jan Nikolas Morshuis,Matthias Hein,Christian F. Baumgartner*

Main category: eess.IV

TL;DR: 本文提供了首个统一的欠采样MRI数据分割基准测试，比较了7种方法，发现简单的两阶段方法（考虑数据一致性的重建+分割）比专门为此任务开发的复杂单阶段方法表现更好。
...
Method: 建立统一基准测试，比较7种分割方法（包括单阶段和两阶段方法），使用两个包含多线圈k空间数据和人工标注分割ground-truth的MRI数据集进行评估。
...
### [263] [RDDM: Practicing RAW Domain Diffusion Model for Real-world Image Restoration](https://arxiv.org/abs/2508.19154)
*Yan Chen,Yi Wen,Wei Li,Junchao Liu,Yong Guo,Jie Hu,Xinghao Chen*

Main category: eess.IV

TL;DR: RDDM是一种直接在RAW域进行图像恢复的端到端扩散模型，通过RAW域VAE和可微分后色调处理模块，解决了传统sRGB域扩散模型的高保真与真实感困境。


<details>
  <summary>Details</summary>
Motivation: 现有sRGB域扩散模型处理有损的sRGB输入，忽略了传感器RAW图像在许多场景（如边缘设备图像采集）中的可访问性，导致性能次优。

Method: 提出RAW域VAE学习最优潜在表示，可微分后色调处理模块实现RAW和sRGB空间联合优化，构建可扩展的退化管道从现有sRGB数据集合成RAW LQ-HQ对进行大规模训练，并设计可配置多拜耳LoRA模块处理不同RAW模式。

Result: 大量实验证明RDDM优于最先进的sRGB扩散方法，能以更少的伪影产生更高保真度的结果。

Conclusion: RDDM通过直接在RAW域恢复图像，成功绕过了传统两阶段ISP+IR管道的限制，为图像恢复提供了新的端到端解决方案。
...
### [264] [REALM: Recursive Relevance Modeling for LLM-based Document Re-Ranking](https://arxiv.org/abs/2508.18379)
*Pinhuan Wang,Zhiqiu Xia,Chunhua Liao,Feiyi Wang,Hang Liu*

Main category: cs.IR

TL;DR: REALM是一个基于不确定性建模的LLM重排序框架，通过高斯分布建模相关性并使用贝叶斯更新，在减少token消耗的同时提升排序性能


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的重排序方法存在排序不确定性、top-k结果不稳定以及token消耗过高的问题，需要更高效可靠的解决方案

Method: 提出REALM框架，将LLM生成的相关性建模为高斯分布，通过递归贝叶斯更新来精化结果，显式捕捉不确定性并最小化冗余查询

Result: 实验结果表明REALM在超越最先进重排序器性能的同时，显著减少了token使用和延迟
...
### [265] [DenseRec: Revisiting Dense Content Embeddings for Sequential Transformer-based Recommendation](https://arxiv.org/abs/2508.18442)
*Jan Malte Lichtenberg,Antonio De Candia,Matteo Ruffini*

Main category: cs.IR

TL;DR: DenseRec通过双路径嵌入方法，将稠密内容嵌入投影到ID嵌入空间，有效解决了Transformer推荐系统中的物品冷启动问题，在三个真实数据集上表现优于仅使用ID的基线方法。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的序列推荐系统（如SASRec、BERT4Rec）仅依赖学习到的物品ID嵌入，在面对动态物品目录时容易受到物品冷启动问题的影响。虽然预训练模型提供的稠密内容嵌入有潜力解决此问题，但直接集成到Transformer推荐器中表现一直不如仅使用ID的方法。

Method: 提出DenseRec方法，采用双路径嵌入策略：在训练期间学习从稠密嵌入空间到ID嵌入空间的线性投影，使得模型能够无缝泛化到未见过的物品，无需专门的嵌入模型或复杂的基础设施。

Result: 在三个真实世界数据集上的实验表明，DenseRec始终优于仅使用ID的SASRec基线，即使没有额外的超参数调优且使用紧凑的嵌入模型。分析表明改进主要来自于在存在未见物品时更好的序列表示。

Conclusion: DenseRec为冷启动序列推荐提供了一个实用且鲁棒的解决方案，通过简单的线性投影方法有效解决了稠密内容嵌入与ID嵌入的集成挑战。
...
### [266] [Extracting Information from Scientific Literature via Visual Table Question Answering Models](https://arxiv.org/abs/2508.18661)
*Dongyoun Kim,Hyung-do Choi,Youngsun Jang,John Kim*

Main category: cs.IR

TL;DR: 本研究评估了三种处理科学论文表格数据的方法，发现保持表格结构完整性的方法在抽取式问答中表现最佳，特别是对特定符号和标记的准确识别对结果改善至关重要。
...
Method: 评估了三种方法：(1)光学字符识别(OCR)提取信息，(2)预训练模型进行文档视觉问答，(3)表格检测和结构识别来提取和合并表格中的关键信息。使用10个样本文档和7个预定义问答对进行实验。

Result: 保持表格结构完整性的方法表现最优，特别是在表示和组织表格内容方面。准确识别文档中的特定符号和标记是改善结果的关键因素。

Conclusion: 保持表格的结构完整性对于提高科学文档中抽取式问答的准确性和可靠性至关重要。
...
### [267] [Membership Inference Attacks on LLM-based Recommender Systems](https://arxiv.org/abs/2508.18665)
*Jiajie He,Yuechun Gu,Min-Chun Chen,Keke Chen*

Main category: cs.IR

TL;DR: 本文研究了基于大语言模型(LLM)的推荐系统中存在的成员推理攻击(MIA)隐私风险，提出了四种攻击方法并在真实数据集上验证了威胁的现实性。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的推荐系统使用包含用户敏感历史交互信息的提示词进行上下文学习，但这些私有信息可能面临新型隐私攻击，此前尚无研究关注这一重要问题。

Method: 设计了四种成员推理攻击方法：直接询问攻击、幻觉攻击、相似性攻击和投毒攻击，每种方法都利用了LLM或推荐系统的独特特性。在三个LLM模型和两个推荐系统基准数据集上进行了评估。

Result: 实验证实LLM推荐系统的MIA威胁是真实存在的，直接询问攻击和投毒攻击显示出显著高的攻击优势。分析了影响攻击效果的因素，如提示词中的样本数量和受害者位置等。

Conclusion: 基于LLM的推荐系统存在严重的隐私泄露风险，需要开发相应的防御机制来保护用户的历史交互数据不被成员推理攻击所利用。
...
### [268] [Taming the One-Epoch Phenomenon in Online Recommendation System by Two-stage Contrastive ID Pre-training](https://arxiv.org/abs/2508.18700)
*Yi-Ping Hsu,Po-Wei Wang,Chantat Eksombatchai,Jiajing Xu*

Main category: cs.IR

TL;DR: 提出一种新颖的两阶段训练策略，通过对比损失预训练最小模型来解决ID嵌入在推荐系统中的单轮训练过拟合问题，实现了更好的在线泛化性能


<details>
  <summary>Details</summary>
Motivation: 解决基于ID的嵌入在推荐系统中由于数据长尾分布导致的过拟合问题，特别是单轮训练限制（one-epoch problem）

Method: 采用两阶段训练策略：第一阶段使用对比损失对最小模型进行预训练，实现更广泛的数据覆盖；第二阶段对更复杂的下游推荐任务进行微调

Result: 离线实验显示预训练阶段的多轮训练不会导致过fitting，生成的嵌入在下游任务中提升了在线泛化能力；在Pinterest实际部署中获得显著的全站参与度提升

Conclusion: 提出的两阶段预训练方法有效解决了推荐系统中ID嵌入的过拟合问题，通过对比学习实现了更好的泛化性能，在实际生产环境中验证了其有效性
...
### [269] [Optimization of Latent-Space Compression using Game-Theoretic Techniques for Transformer-Based Vector Search](https://arxiv.org/abs/2508.18877)
...
Result: 相比FAISS，平均相似度(0.9981 vs 0.5517)和实用性(0.8873 vs 0.5194)显著提升，查询时间略有增加

Conclusion: 博弈论潜在压缩在高实用性、基于transformer的搜索应用中具有重要价值，可无缝集成到现有LLM管道中
