...
Motivation: 探索多模态大语言模型在视觉任务中的潜力，特别是在数据稀缺情况下如何有效利用LLM的上下文推理和语言引导能力，弥补传统CNN模型在数据效率方面的不足。

Method: 通过全面比较三种方法：微调的传统CNN、零样本预训练多模态LLM、以及微调的多模态LLM，在人工文本叠加检测任务上进行评估，重点关注LLM在少量数据（少于1000张图像）下的微调效果。

Result: 研究发现，经过微调的LLM在仅使用少量数据的情况下，准确率可提升高达36%，性能达到甚至超过需要大量数据的CNN基线模型，证明了LLM在数据效率方面的优势。

Conclusion: 多模态LLM在低资源视觉环境中具有出色的适应性和数据效率，为跨模态学习提供了新的有效策略，特别是在数据稀缺的视觉理解任务中展现出巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 脑肿瘤分割对诊断和治疗规划至关重要，但类别不平衡和模型泛化能力有限等问题仍然阻碍进展。

Method: 在公开MRI数据集上使用U-Net进行分割，采用焦点损失函数并调整其参数，同时评估水平翻转、旋转和缩放三种基础数据增强技术的影响。

Result: 使用焦点损失的U-Net模型达到了90%的精确度，与最先进的结果相当。

Conclusion: 通过公开所有代码和结果，本研究为脑肿瘤分割中的数据增强策略和损失函数设计研究建立了透明、可重复的基准。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型经常记忆和复制训练数据，引发隐私和版权担忧。现有方法推迟应用分类器自由引导(CFG)直到去噪轨迹逃离记忆吸引盆地，但这会导致生成的图像与输入提示对齐不佳。需要找到方法促进更早逃离，以便在去噪过程中更早应用CFG。

Method: 1. 发现初始噪声样本在决定逃离时间中起关键作用；2. 提出两种缓解策略：集体调整初始噪声或个体调整初始噪声，以找到并利用能促进更早盆地逃离的初始样本。

Result: 这些方法显著减少了记忆化，同时保持了图像-文本对齐。

Conclusion: 通过适当调整初始噪声，可以有效促进文本到图像扩散模型在去噪过程中更早逃离记忆吸引盆地，从而在保持生成质量的同时减少训练数据的记忆化问题。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在可视化领域应用广泛，但现有研究主要关注创建过程和图像质量，忽视了代表性偏见问题，特别是在职业场景中的性别刻板印象。

Method: 研究通过生成超过750张AI职业图像，使用DALL-E 3和Ideogram两个AI图像生成工具，并进行主题分析来评估性别代表性偏见。

Result: 分析结果显示，DALL-E 3和Ideogram都强化了传统性别刻板印象，尽管程度不同，AI可视化工具存在强化狭隘代表性的风险。

Conclusion: 研究强调需要解决AI生成图像中的有害性别偏见，并为从业者、个人和研究者提出了增加代表性、减少偏见的建议。
...
<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在递增分辨率下重复调用Transformer导致计算冗余，需要提高计算效率。

Method: 在VAR中集成动态混合专家路由器，采用尺度感知阈值策略平衡基于令牌复杂度和分辨率的专家选择。

Result: 实现了20% FLOPs减少、11%推理加速，同时匹配了密集基线的图像质量。

Conclusion: 提出的动态混合专家路由器有效解决了VAR模型的计算冗余问题，实现了计算效率与图像质量的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督OOD检测方法通常依赖预测熵，但这种方法混淆了认知不确定性和偶然不确定性，容易将分布内模糊区域误分类为OOD。需要一种能够准确区分这两种不确定性的方法。

Method: 使用深度神经网络特征空间中的高斯混合模型参数的分层贝叶斯建模来获取认知不确定性，无需辅助数据或额外训练阶段。

Result: 在SemanticKITTI数据集上，相比预测熵方法，AUROC提升18%，AUPRC提升22%，FPR95从76%降低到40%。

Conclusion: 基于分层贝叶斯建模的认知不确定性方法能够有效解决现有无监督OOD检测方法的局限性，显著提升检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 人类活动识别中存在无法逾越的差距：实际生活中的活动范围远大于训练数据集中标注的活动。未能正确处理未见活动会严重削弱分类器的可靠性，且活动类别之间存在不同程度的相似性和层次关系。

Method: 将活动类别组织成结构化层次，提出Hi-OSCAR层次化开放集分类器，不仅能识别已知活动，还能拒绝未知活动，并将未知类别定位到最近的内部节点。

Result: 开发了新的公开数据集NFI_FARED，包含多个受试者执行19种不同情境活动的数据，支持开放集HAR研究。

Conclusion: Hi-OSCAR方法能够有效解决开放集活动识别问题，提供比传统二元分类更丰富的层次化信息，为HAR研究提供了新的方向和数据集支持。
...
<details>
  <summary>Details</summary>
Motivation: 高频振荡是识别癫痫灶的新生物标志物，但视觉识别HFOs耗时、费力且主观，因此开发自动化检测方法对研究和临床应用至关重要。

Method: 采用无监督聚类技术对通过S变换从时频域提取的事件进行分类，提出的检测器能够区分HFOs事件与尖峰、背景活动和伪影。

Result: 在受控数据集上达到97.67%的灵敏度、98.57%的精确度和97.78%的F分数；在癫痫患者中，切除与非切除接触点的HFOs率比值为0.73，与手术结果相关性更强。

Conclusion: 研究证实HFOs是癫痫患者癫痫发生性的有前景的生物标志物，切除HFOs（特别是快速波纹）可导致无癫痫发作，而残留HFOs则导致癫痫复发。
...
<details>
  <summary>Details</summary>
Motivation: 尽管DINOv2被广泛用于物体、场景和动作识别，但其内部感知机制的本质仍然未知。本文旨在揭示DINOv2模型的内在表示结构，理解其如何组织视觉概念。

Method: 采用线性表示假说作为工作基线，使用稀疏自编码器(SAE)构建32,000个单元的字典作为可解释性基础。分析下游任务如何从字典中招募概念，研究SAE学习概念的几何和统计特性。

Result: 发现功能专门化：分类利用'Elsewhere'概念实现学习否定；分割依赖形成连贯子空间的边界检测器；深度估计使用三种单目深度线索。表示部分密集而非严格稀疏，字典向更大相干性演化，token占据低维局部连接集。

Conclusion: 提出Minkowski表示假说：token由原型的凸混合形成，这种结构基于Gardenfors的概念空间理论，多头部注意力产生凸混合的和，定义了由原型界定的区域，为解释视觉Transformer表示提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像在采集过程中不可避免地受到各种退化因素的影响，现有的一体化恢复方法过度依赖隐式特征表示，缺乏对退化物理的显式建模，难以有效处理复杂异构的退化问题。

Method: 采用两阶段级联架构，包含残差流形投影器(RMP)和频率感知退化分解器(FADD)进行渐进式退化挖掘，引入物理感知专家模块和温度控制稀疏激活策略，从流形几何和频率角度综合分析退化特征。

Result: 在三个基准数据集(MD-RSID、MD-RRSHID、MDRS-Landsat)上的广泛实验表明，PhyDAE在所有四个恢复任务中均实现卓越性能，全面超越最先进方法，同时显著降低参数数量和计算复杂度。

Conclusion: PhyDAE在恢复质量和计算效率之间实现了最佳平衡，通过显式建模退化物理和稀疏激活策略，在保持成像物理一致性的同时大幅提升计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界临床决策需要整合来自不同数据模态的信息，包括医学文本、2D/3D图像和视频，这导致了效率低下和潜在的诊断遗漏。虽然通用视觉语言模型有潜力，但其医疗应用面临流程不透明、数据稀缺和架构不灵活等挑战。

Method: 基于统一的基于补丁的视觉编码器和LLM解码器构建，通过渐进式训练在1670万个样本上从2D扩展到3D和视频理解。采用医学感知的令牌减少技术实现高效训练，7B到32B参数变体仅需4000到40000 GPU小时。

Result: 在30个基准测试中表现出最先进的性能，在视觉问答、医学报告生成以及多语言和罕见疾病场景中的复杂推理任务中，超越了领先的开源模型，并与专有系统竞争。

Conclusion: 通过开源完整流程，证明了高性能医疗视觉语言模型可以透明地实现，为可访问和有影响力的临床AI提供了基础工具。
...
<details>
  <summary>Details</summary>
Motivation: 相机中心的理解和生成是空间智能的两个基石，但通常被孤立研究。本文旨在开发一个统一的相机中心多模态模型，扩展相机维度的空间感知能力。

Method: 提出将相机视为语言的新范式，整合语言回归和基于扩散的生成方法。使用Puffin-4M数据集（400万视觉-语言-相机三元组），结合全局相机参数和像素级相机映射。

Result: 实验表明Puffin在相机中心生成和理解任务上优于专用模型。通过指令调优，能够泛化到空间想象、世界探索和摄影指导等跨视角任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索方法在检索精度上存在局限，现有的多向量检索方法虽然通过分解查询和图像分割提高了准确性，但仍面临对齐问题和冗余计算的问题。

Method: 提出HiMIR框架：1）采用分层范式，使用多个中间粒度处理不同图像对象以增强对齐；2）利用跨层次相似性一致性和层次稀疏性减少冗余检索；3）自动为每个数据集配置参数以适应多样化场景。

Result: 实证研究表明，HiMIR不仅实现了显著的精度提升，而且相比现有MVR系统减少了高达3.5倍的计算量。

Conclusion: HiMIR框架通过分层多粒度检索和优化策略，在提升检索精度的同时大幅降低了计算开销，为多模态大语言模型应用提供了高效的图像检索解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法冻结部分权重并添加任务特定层，虽然计算效率高但限制了模型适应领域特定特征的能力，在数据量很少时仍容易过拟合。

Method: 提出SOR框架，冻结网络内部结构（如卷积滤波器），同时使用组套索和L1惩罚的组合，以最少的额外参数使模型适应特定数据。

Result: 在三个少样本医学影像分类任务上评估SOR，使用DenseNet121和EfficientNetB4作为基础网络，相比已有基准取得了竞争性结果。
...
Method: 提出BEAR基准，包含4,469个跨14个领域的图像-视频-文本条目，涵盖6个类别的具身能力任务。同时提出BEAR-Agent代理，集成预训练视觉模型来增强MLLM的感知、3D理解和规划能力。

Result: 对20个代表性MLLM的广泛评估显示它们在所有具身能力领域都存在持续局限。BEAR-Agent在BEAR基准上显著提升了MLLM性能，实现了9.12%的绝对增益和17.5%的相对改进。

Conclusion: BEAR基准填补了MLLM具身能力评估的空白，BEAR-Agent有效增强了MLLM的具身能力，且改进具身能力有益于模拟环境中的具身任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统防御方法依赖计算密集的优化，而人类视觉系统通过进化机制天然具备对抗扰动的鲁棒性。本文假设注意力引导的非均匀稀疏采样和预测编码是这种鲁棒性的关键。

Method: 使用强化学习引导的扫视选择性捕获多个中央凹-外周瞥视，将这些瞥视整合成重建图像后再进行分类。该方法包含三个生物机制：中央凹-外周处理、扫视眼动和皮层填充。

Result: 在ImageNet数据集上的实验表明，该方法能提高系统对各种分类器和攻击类型的鲁棒性，同时相比生物和非生物启发的防御技术显著降低训练开销。

Conclusion: 生物启发的预处理方法能有效减轻对抗性噪声，保持语义完整性，无需重新训练或微调下游分类器，可与现有系统无缝集成。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在各种环境下实时检测泄漏的安全关键系统，利用热成像在速度、准确性和模型大小方面的优势。

Method: 使用平衡的二元数据集（4000张图像），采用预训练的深度学习模型（如VGG19和NasNetMobile）结合RGB和热成像进行分类。

Result: 热成像模型在推理速度、准确性和模型大小方面表现更优，VGG19模型在热成像上表现最佳，准确率达100%，推理时间低至44毫秒，模型大小小于350MB。

Conclusion: 热成像在泄漏检测中具有显著优势，特别是在不同光照条件下表现更稳健，系统在消费级硬件上具有实际部署能力。
...
<details>
  <summary>Details</summary>
Motivation: 生成式图像超分辨率模型依赖自注意力的二次复杂度(O(N^2))造成计算瓶颈，线性注意力虽然提供O(N)解决方案，但在逼真超分辨率中面临训练不稳定、感知-失真权衡等未解决挑战。

Method: 提出LinearSR框架：1) 使用基于'拐点'的早期停止引导微调(ESGF)解决训练不稳定性；2) 采用基于SNR的专家混合(MoE)架构缓解感知-失真权衡；3) 基于'精度优先于数量'原则建立轻量级TAG引导范式。

Result: LinearSR模型同时实现了最先进的感知质量和卓越效率，其核心扩散前向传播(1-NFE)达到SOTA级速度，整体多步推理时间保持高度竞争力。

Conclusion: 该工作首次为线性注意力在逼真超分辨率领域的应用提供了稳健方法，为高效生成式超分辨率的未来研究建立了基础范式。
...
<details>
  <summary>Details</summary>
Motivation: 传统野生动物监测方法（如鸟类腿环标记）耗时且具有侵入性，需要开发基于人工智能的非侵入性高效监测方法。

Method: 结合YOLO和Grounding DINO进行目标检测，使用光流模糊检测，DINOv2图像编码和聚类方法，从视频中提取代表性关键帧。

Result: 提出的关键帧选择方法在卡卡鹦鹉重识别任务中实现了高准确率，为在更复杂环境中收集的媒体数据研究奠定了基础。

Conclusion: 这种基于人工智能和计算机视觉的非侵入性方法为野生动物个体识别提供了有价值的替代方案，有助于改进种群监测，推动野生动物监测新方法的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估模型存在三个主要问题：(1) 对不同内容类型（用户生成内容、短视频、AI生成内容）的泛化能力差；(2) 可解释性有限；(3) 难以扩展到新的使用场景或内容类型。

Method: 提出Q-Router智能路由框架，包含多层级模型路由系统，使用视觉语言模型作为实时路由器，根据输入视频语义动态推理并集成最合适的专家模型。构建基于计算预算的多层级路由系统，最重层级涉及特定的时空伪影定位以实现可解释性。

Result: 在多个基准测试中，Q-Router达到或超越了最先进的VQA模型性能，显著提高了泛化能力和可解释性。在质量问答基准Q-Bench-Video上表现优异，并能有效定位时空伪影，显示出作为视频生成模型后训练奖励函数的潜力。

Conclusion: Q-Router通过智能路由设计结合了专业专家模型的互补优势，在异质视频源和任务中实现了灵活性和鲁棒性，有望成为下一代VQA系统的基础框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有Med-VQA方法在模态对齐方面缺乏统一解决方案，硬负样本问题研究不足，且常用的知识融合技术可能引入无关信息。

Method: 提出三部分核心方法：1）跨多级、多模态、多视图和多阶段的异质模态对齐统一方案；2）使用软标签进行多模态对齐并加强硬负样本对区分的硬负样本挖掘方法；3）集成答案词汇作为先验知识并从中选择相关信息门控交叉注意力模块。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散的超分辨率方法在视频生成和修复中很关键，但计算成本高且速度慢，限制了其在高分辨率和长视频中的应用。许多视频区域本身细节较少，不需要精细处理，但现有方法对所有像素统一处理。
...
Conclusion: SkipSR是一种简单有效的策略，在保持感知质量的同时显著减少了扩散超分辨率模型的计算量。
...
<details>
  <summary>Details</summary>
Motivation: 将图像预训练的视觉语言模型扩展到视频领域面临挑战，因为需要处理密集且时间延长的视觉输入，这超出了图像模型的能力范围。主要挑战包括感知瓶颈和token过载。
...
Conclusion: D-CoDe通过动态压缩和问题分解成功解决了视频大语言模型扩展中的关键挑战，为图像预训练模型向视频领域的适配提供了有效的训练免费解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将3D实例映射到2D RGB-D图像，然后使用视觉语言模型进行分类，但这种方法会引入2D遮挡噪声，且计算和内存成本高，推理速度慢
...
<details>
  <summary>Details</summary>
Motivation: 蔓越莓种植中的变化监测通常需要人工完成，耗时且效率低。深度学习虽然有望解决这一问题，但存在高维特征难以解释和需要大量手动标注的问题。

Method: 使用自监督方法微调视觉变换器（ViT），通过时间回归和类别预测的双重预训练任务，学习植物和果实外观时间演化的潜在空间表示。

Result: 生成的2D时间轨迹提供了可解释的作物生长时间序列模型，能够预测随时间的变化并区分不同蔓越莓品种的时间差异。

Conclusion: 该方法具有通用性，可应用于其他作物和场景，同时提供了包含8个不同品种、52次观测的新时间序列蔓越莓数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型难以同时表达概念族内的层次结构（如狗⊆哺乳动物⊆动物）和跨概念族的组合性（如"车里的狗"⊆狗,车）。虽然双曲空间能有效捕获树状层次结构，但其对组合性的表示能力尚不明确。

Method: 提出PHyCLIP模型，在双曲空间的笛卡尔积上使用ℓ₁-积度量。概念族内层次在单个双曲因子中形成，跨概念族组合通过ℓ₁-积度量捕获，类似于布尔代数。

Result: 在零样本分类、检索、层次分类和组合理解任务上的实验表明，PHyCLIP优于现有的单空间方法，并在嵌入空间中提供更可解释的结构。

Conclusion: PHyCLIP通过双曲空间的笛卡尔积和ℓ₁-积度量，成功解决了同时表达层次结构和组合性的挑战，为多模态表示学习提供了更强大的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的分割模型在白盒设置下对对抗样本表现出显著的脆弱性，但现有的对抗攻击方法在不同分割模型间的迁移性较差。分割模型中复杂的上下文依赖关系以及代理模型和目标模型之间的特征分布差距导致迁移成功率不理想。

Method: 提出SegTrans框架，将输入样本划分为多个局部区域，重新映射其语义信息以生成多样化的增强样本。这些增强样本替代原始样本进行扰动优化，仅保留原始输入的局部语义信息而非全局语义信息来优化扰动。

Result: 在PASCAL VOC和Cityscapes两个基准数据集、四种不同分割模型和三种骨干网络上进行的广泛实验表明，SegTrans显著提高了对抗迁移成功率，且不引入额外计算开销。与当前最先进方法相比，SegTrans平均提高了8.55%的迁移攻击成功率，计算效率提升了100%以上。
...
<details>
  <summary>Details</summary>
Motivation: 知识蒸馏攻击严重威胁深度学习模型的知识产权，现有防御方法主要针对图像分类任务，难以有效应用于图像修复这类生成式任务，因为修复任务需要保持空间一致性和细节质量。

Method: 提出自适应奇异值扰动(ASVP)方法，在运行时对教师模型的内部特征图进行奇异值分解，放大前k个奇异值以注入结构化高频扰动，破坏蒸馏所需的对齐关系。

Result: 在超分辨率、低光增强、水下增强、去雾和去雨五个图像修复任务上的实验表明，ASVP可将学生模型的PSNR降低高达4dB，SSIM降低60-75%，而对教师模型性能影响可忽略不计。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在各种视频理解任务中表现出色，但其在面对操纵视频内容时的鲁棒性尚未得到充分研究。

Method: 构建Ro-Bench基准测试，通过编辑视频的样式、对象、背景及其组合来创建高质量、多样化且时间相关的反事实视频数据集，评估了8个最新的视频MLLM模型。

Result: 当前模型在Ro-Bench反事实视频内容上表现出显著的性能下降。通过反事实数据微调MLLM可以增强鲁棒性，在Ro-Bench上性能提升21.73%，在MVBench数据集的20个任务上提升12.78%。

Conclusion: 反事实数据能有效增强MLLM的视频理解能力，证明了反事实数据在提升模型鲁棒性方面的重要性。
...
Method: 开发对象聚焦数据增强框架，分割动物与背景，通过变换和扩散合成技术生成真实多样的场景，专门针对动物健康监测的数据约束环境。

Result: 初步实验表明，增强数据集在动物检测任务上比基线模型表现更优，能够提升检测和监测性能。

Conclusion: 该方法通过生成领域特定数据，即使在数据稀缺场景下也能支持实时动物健康监测解决方案，弥合有限数据与实际应用之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 虽然推理时缩放技术显著提升了大型视觉语言模型的推理能力，但其在视觉感知任务中的效果尚不明确。当前LVLMs采用快速感知范式，将视觉理解视为一次性输出，没有建模底层的感知过程，这限制了感知精度的提升。

Method: 提出感知时缩放(PTS)范式，鼓励丰富的token感知，将复杂感知问题分解为可处理的子问题。结合强化学习技术，使用纯合成数据训练模型，使感知能够与推理时缩放对齐并从中受益。

Result: PTS显著提升了感知精度，在DisTANCE基准上高精度性能从8.0%提升到64.7%，在域外任务上泛化良好。即使使用纯合成数据，与数学推理数据结合也能在推理和真实世界感知基准上获得一致提升。分析显示PTS引入了更多感知相关token并增加了模型对图像token的关注。

Conclusion: 感知时缩放(PTS)是解决LVLMs视觉感知精度问题的有效方法，通过分解复杂感知问题和丰富token感知，使模型能够从推理时缩放技术中获益，显著提升感知性能。
...
<details>
  <summary>Details</summary>
Motivation: 在毫米波姿态估计中，稀疏信号和弱反射导致模型从统计先验而非传感器数据推断身体关节，这种对先验知识的过度依赖会降低下游任务（如手势和活动识别）的性能。

Method: 引入mmJoints框架，增强预训练的黑盒毫米波3D姿态估计器的输出，通过估计关节被感知的可能性和位置预测的可靠性来显式化偏差。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成复杂局部区域（特别是人手）时存在结构扭曲和不真实纹理的问题，但手部区域的质量评估一直被忽视，限制了人类中心生成质量优化和AIGC检测等下游任务的性能。

Method: 首先构建了包含48k张高质量和低质量手部图像对的HandPair数据集，无需人工标注；然后开发了HandEval模型，利用多模态大语言模型的视觉理解能力，并结合手部关键点先验知识来评估手部质量。

Result: 在包含各种最先进T2I模型生成手部图像的测试集上验证，HandEval比现有最先进方法更符合人类判断；在图像生成和AIGC检测应用中，分别显著提升了生成手部真实性和检测准确率。

Conclusion: 提出的手部质量评估任务和HandEval模型在多个下游应用中表现出通用有效性，为提升生成图像中手部区域质量提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: AI着色技术虽然能生成逼真的彩色图像，但存在版权侵权风险，如未经授权对黑白漫画和电影进行着色并转售。目前缺乏有效方法来防止此类滥用。

Method: 提出PAChroma方法：使用拉普拉斯滤波器优化不可察觉的扰动以保持感知质量；在优化过程中应用多样化的输入变换，增强跨模型的可迁移性和对常见后处理（如压缩）的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 自回归文本到图像模型由于顺序token-by-token解码过程导致推理速度缓慢，通常需要数千次模型前向传递来生成单张图像，需要解决这种效率低下的问题。

Method: 引入next-clean-token预测范式，通过低成本微调使预训练自回归模型能够接受噪声扰动token嵌入并预测下一个干净token；在推理时用高斯噪声初始化token序列，在嵌入空间进行迭代的next-clean-token预测，使用概率标准并行验证和接受多个token，并通过去噪轨迹细化未接受的token。
...
Method: 提出了一种简单有效的策略：使用对抗性扰动识别不确定的视觉标记，并在视觉编码器的中间层自注意力过程中屏蔽这些不确定标记，抑制其对视觉编码的影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常显式结合多模态信息，但本文提出一种不同框架，将多模态知识嵌入到单个网络中，使每个单模态网络都能获得性能提升。

Method: 为每个可用模态分配单独网络，强制它们协作学习具有共同语义和更好表示的网络。引入时空语义对齐损失(SSA)来对齐不同网络的特征内容，并使用提出的焦点正则化参数来避免负知识转移。

Result: 实验结果表明，该框架提高了单模态网络的测试时识别准确率，并在多个动态手势识别数据集上达到了最先进的性能。

Conclusion: 提出的框架能够有效利用多模态知识提升单模态网络的性能，在动态手势识别任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前自回归图像生成模型存在采样问题，图像标记相比文本标记具有更低的信息密度和非均匀空间分布特性，这影响了生成质量和效率。

Method: 1) 基于标记分布空间熵的动态温度控制，在掩码和尺度模型中平衡内容多样性、对齐精度和结构一致性；2) 在推测解码中引入熵感知接受规则，以约85%的推理成本实现近乎无损的生成。

Result: 在多个基准测试和多样化自回归图像生成模型上的广泛实验表明，该方法能有效提升生成质量和采样速度，具有良好的通用性。

Conclusion: 所提出的熵信息解码策略能够在不增加额外计算开销的情况下，显著改善自回归图像生成的质量和效率，为图像生成模型提供了有效的采样优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的稀疏表示分类方法在特征提取和稀疏表示方面存在局限性，需要一种能够同时学习鲁棒深度特征和稀疏表示的端到端框架。

Method: 构建包含卷积自编码器和全连接层的网络结构，自编码器学习深度特征，中间的全连接层负责寻找稀疏表示，然后将稀疏编码用于分类。

Result: 在三个不同数据集上的实验表明，该方法产生的稀疏表示比最先进的SRC方法获得更好的分类结果。

Conclusion: 所提出的转导式深度学习框架能够有效结合深度特征学习和稀疏表示，在分类任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: LiDAR感知对自动驾驶安全至关重要，但LiDAR标注常因传感器缺陷、遮挡和人为错误而存在噪声。这种噪声会降低分割精度，并在领域偏移下进一步放大，威胁系统可靠性。虽然噪声标签学习在图像领域已有研究，但在3D LiDAR分割的领域泛化中仍未被充分探索。

Method: 提出了DuNe双视图框架，包含强分支和弱分支，通过特征级一致性约束和基于置信度感知过滤的交叉熵损失来处理噪声标签。

Result: 在10%对称标签噪声下，在SemanticKITTI上达到56.86% mIoU，nuScenes上42.28%，SemanticPOSS上52.58%，算术平均49.57%，调和平均48.50%，表现出优异的领域泛化能力。

Conclusion: DuNe框架在噪声标签下的LiDAR语义分割领域泛化任务中实现了最先进的性能，为自动驾驶系统的鲁棒性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在医学图像转换中存在像素级细节丢失的问题，特别是对于占据图像小区域的病灶结构，这会严重影响诊断可靠性和临床决策。

Method: 提出了一种新颖的后训练框架，在预训练的潜在扩散模型中融入病灶感知的医学像素空间目标，增强病灶描绘精度。

Result: 在817名急性缺血性卒中患者数据集上的评估显示，该框架在从CT灌注扫描合成DWI和ADC图像时，提高了整体图像质量并增强了病灶描绘，优于现有图像转换模型。

Conclusion: 该后训练策略易于适应预训练的潜在扩散模型，在广泛的医学图像转换任务中具有巨大应用潜力。
...
Method: 利用现有目标定位结果，从原始图像中提取对齐的感兴趣区域，输入预训练的视觉变换器。提出渐进粒度补丁特征采样方法解决不同位置的敏感度-容错度权衡问题，并选择信噪比较高的特征通道为特定场景提供更好的描述符。

Result: 所提方法的有效性通过从植入系统收集的图像数据集进行了验证。
...
TL;DR: MambaH-Fit是一个基于状态空间建模的超曲面拟合点云法向量估计框架，通过注意力驱动的层次特征融合和补丁级状态空间模型，有效捕捉局部细粒度几何结构，在精度、鲁棒性和灵活性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有法向量估计方法在建模细粒度几何结构方面存在不足，限制了预测精度。虽然状态空间模型（特别是Mamba）在点云处理中显示出强大建模能力，但现有方法主要关注全局形状结构，对局部几何细节的建模研究不足。

Method: 1. 提出注意力驱动的层次特征融合（AHFF）方案，自适应融合多尺度点云补丁特征，增强局部邻域的几何上下文学习；2. 提出补丁级状态空间模型（PSSM），通过状态动力学将点云补丁建模为隐式超曲面，实现有效的细粒度几何理解。

Result: 在基准数据集上的大量实验表明，该方法在精度、鲁棒性和灵活性方面优于现有方法。消融研究进一步验证了所提出组件的贡献。

Conclusion: MambaH-Fit通过创新的特征融合和状态空间建模方法，成功解决了点云法向量估计中的细粒度几何建模问题，为点云处理提供了更准确和鲁棒的解决方案。
...
TL;DR: 本文提出GL-DT框架，通过时空特征融合和全局-局部协作检测策略提升无人机多目标跟踪性能，特别是对小目标检测和轨迹连续性的改进。


<details>
  <summary>Details</summary>
Motivation: 无人机在军事侦察、环境监测等领域的广泛应用对多目标跟踪技术提出了更高要求，但复杂背景、小尺度目标、频繁遮挡和交互等问题仍挑战现有方法的检测精度和轨迹连续性。

Method: 提出GL-DT框架，包含时空特征融合模块联合建模运动和外观特征，结合全局-局部协作检测策略增强小目标检测，并引入JPTrack跟踪算法解决ID切换和轨迹碎片化问题。
...
TL;DR: 本文提出Dense2MoE方法，将密集的扩散变换器转换为混合专家结构，通过激活更少的参数来减少推理开销，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在文本到图像生成中表现出色，但参数量大导致推理开销高。现有压缩方法主要依赖剪枝，但激进的剪枝会因模型容量减少而导致性能严重下降。
...
Conclusion: Dense2MoE为高效的文本到图像生成建立了新范式。
...
TL;DR: 本文提出了一种新颖的多分支ConvNeXt架构，专门用于医学图像分析，在COVID-19诊断任务中取得了优异表现，验证集ROC-AUC达到0.9937，准确率0.9757，F1分数0.9825。


<details>
  <summary>Details</summary>
Motivation: 医学影像的智能分析在辅助临床诊断中至关重要，特别是识别细微病理特征。针对医学图像分析的细微挑战，需要开发专门设计的深度学习架构。

Method: 采用多分支ConvNeXt架构，整合三个并行分支的特征提取：全局平均池化、全局最大池化和新的注意力加权池化机制。采用端到端流程，包括数据预处理、增强和两阶段训练策略，有效利用迁移学习。

Result: 在包含2,609个CT切片的组合数据集上训练验证，模型在COVID-19病例分类上表现优异：ROC-AUC 0.9937，验证准确率0.9757，F1分数0.9825，优于该数据集上所有先前报告的模型。
...
TL;DR: SOS是一个基于对象中心组合策略的简单可扩展数据合成管道，通过将高质量合成对象分割粘贴到新图像中，生成准确多样的掩码、边界框和指代表达式。在检测和定位任务上，使用10万张SOS合成图像训练的模型优于使用更大规模真实图像数据集训练的模型。
...
Method: 提出SOS数据合成管道，采用对象中心组合策略，使用结构化布局先验和生成式重光照技术，将高质量合成对象分割粘贴到新图像中。

Result: 在检测和定位任务上，SOS训练的模型在LVIS检测上达到+10.9 AP提升，在gRefCOCO定位上达到+8.4 NAcc提升。在低数据设置下表现更佳，如LVIS实例分割中罕见类别提升+3.83 AP，1% COCO设置下提升+6.59 AP。

Conclusion: SOS支持可控数据集构建，在低数据和封闭词汇设置下改善泛化能力，通过合成数据增强真实数据集可在各种数据规模下获得强大性能。
...
TL;DR: 提出了一种多模态语义扩散模型（MSDM），用于生成细胞和细胞核分割的逼真图像-掩码对，通过整合形态学、颜色特征和元数据来解决标注数据稀缺问题，显著提升了分割模型的准确性。
...
Method: 开发了多模态语义扩散模型（MSDM），通过整合细胞/细胞核形态学（使用水平和垂直图）、RGB颜色特征和BERT编码的检测/指示元数据来生成图像-掩码对。这些异构模态通过多头交叉注意力机制集成，实现对生成图像的细粒度控制。

Result: 定量分析显示合成图像与真实数据高度匹配，在匹配生物条件下生成图像与真实图像的嵌入具有较低的Wasserstein距离。包含这些合成样本（以柱状细胞为例）显著提高了分割模型在柱状细胞上的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 传统的正交矩图像表示方法存在高计算复杂度和数值不稳定性问题，特别是在大阶数时，Zernike和伪Zernike矩需要耦合的径向-角向处理，导致效率低下。

Method: 提出PSepT变换，通过离散余弦变换径向基和傅里叶谐波角基的张量积构造实现完全核分解，实现独立的径向和角向处理。

Result: PSepT将计算复杂度降至O(N² log N)，内存需求降至O(N²)，条件数缩放降至O(√N)，相比多项式方法有指数级改进，在结构化数据集上表现出更好的数值稳定性和计算效率。

Conclusion: PSepT的可分离框架使得之前用经典方法不可行的高阶矩分析成为可能，为稳健的图像分析应用开辟了新可能性。
...
TL;DR: 该论文提出了一种新的可解释性方法——训练特征归因，将测试预测与特定训练图像的特定区域联系起来，为深度模型提供更细粒度的解释。
...
Method: 探索训练特征归因方法，将测试预测与特定训练图像的特定区域联系起来，从而提供对深度模型内部工作机制的新见解。

Result: 在视觉数据集上的实验表明，训练特征归因能够提供细粒度的、特定于测试的解释：识别导致错误分类的有害示例，并揭示传统归因方法无法暴露的虚假相关性（如基于补丁的捷径）。

Conclusion: 训练特征归因方法能够提供比传统方法更深入的解释，揭示深度模型学习过程中的潜在问题和偏差。
...
TL;DR: 提出了一种基于图像的支气管镜拓扑定位管道，无需患者CT扫描即可在手术过程中提供导航辅助，该方法仅在幻影数据上训练，具有良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: 支气管镜检查中，医生需要确定内窥镜在复杂支气管树中的位置，传统方法依赖CT扫描和额外传感器，成本高且需要额外设置。拓扑定位通常足以提供导航辅助。

Method: 开发了基于图像的支气管镜拓扑定位管道，仅在幻影数据上训练，无需真实患者数据标注，避免了高昂的数据标注成本。

Result: 该方法在结果上超越了现有方法，特别是在真实数据测试序列上表现优异。

Conclusion: 提出的图像支气管镜拓扑定位方法能够有效提供导航辅助，无需患者CT扫描，训练成本低且泛化能力强。
...
TL;DR: 本文提出了一种无需真实图像即可合成多领域对象实例的新方法，用于解决实例级识别(ILR)中数据标注困难的问题，显著提升了跨七个ILR基准的检索性能。


<details>
  <summary>Details</summary>
Motivation: 实例级识别(ILR)需要识别单个对象而非宽泛类别，这种细粒度特性使得创建大规模标注数据集具有挑战性，限制了ILR在现实世界中的跨领域应用。

Method: 引入一种新颖方法，从多个领域合成生成多样化的对象实例，这些实例具有不同的条件和背景，形成一个大规模训练集。该方法不依赖任何真实图像，专门解决ILR特有的挑战。

Result: 在生成数据上微调基础视觉模型，显著提升了跨七个ILR基准的检索性能，涵盖了多个领域。

Conclusion: 该方法为广泛的数据收集和整理提供了新的、高效且有效的替代方案，引入了一种新的ILR范式，其中唯一的输入是目标领域的名称，为广泛的现实世界应用开辟了道路。
...
TL;DR: TARO是一个新颖的开放世界目标检测框架，不仅识别未知物体，还能将其分类到语义层次结构中的粗粒度父类别，显著提升安全关键场景的决策能力。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测器局限于'封闭世界'假设，无法处理现实世界中的未知物体。现有的开放集检测方法将所有未知物体归为单一'未知'类别，这在安全关键场景中不够实用。

Method: TARO采用独特架构：基于sparsemax的物体性建模头、层次结构引导的重标记组件提供辅助监督、学习层次关系的分类模块。

Result: 实验表明TARO能将29.9%的未知物体分类到有意义的粗粒度类别，显著减少未知与已知类别间的混淆，在未知召回率和已知mAP上均具竞争力。

Conclusion: TARO框架成功实现了对未知物体的细粒度分类，为安全关键应用提供了更实用的开放世界目标检测解决方案。
...
TL;DR: 本文提出了在线视频深度估计方法oVDA，通过借鉴大语言模型的技术（缓存潜在特征和训练时掩码帧），克服了现有方法需要批处理的限制，在准确性和VRAM使用方面优于所有竞争方法，并在边缘设备上实现实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有的Video Depth Anything (VDA)方法虽然在处理长视频序列方面表现出色，但依赖于批处理，无法在在线设置中使用。本文旨在克服这一限制，开发能够在实时在线环境中运行的视频深度估计方法。

Method: 提出了在线VDA (oVDA)方法，关键创新是借鉴大语言模型的技术：在推理期间缓存潜在特征，在训练时使用帧掩码技术。这种方法避免了批处理需求，实现了在线处理能力。

Result: oVDA方法在准确性和VRAM使用方面都优于所有竞争的视频深度估计方法。在NVIDIA A100上达到42 FPS，在NVIDIA Jetson边缘设备上达到20 FPS，证明了其在边缘设备上的实时部署能力。

Conclusion: oVDA成功实现了在线视频深度估计，通过借鉴LLM技术解决了批处理限制问题，在保持高精度的同时显著降低了VRAM使用，为边缘设备部署提供了可行的解决方案。
...
TL;DR: 本文首次对板球击球动作分类进行了全面的基准研究，比较了7种深度学习方法在4个研究范式下的表现。研究发现学术文献中的高精度声明与实际实现结果存在显著差距，而现代EfficientNet-GRU组合方法达到了92.25%的准确率。


<details>
  <summary>Details</summary>
Motivation: 板球击球动作分类在体育视频分析中仍然是一个具有挑战性的问题，需要有效建模空间和时间特征。现有文献中的高精度声明与实际实现之间存在显著差距，需要系统性的基准研究来建立可靠的评估标准。

Method: 实现了7种深度学习方法并系统评估：传统CNN-LSTM架构、基于注意力的模型、视觉变换器、迁移学习方法以及现代EfficientNet-GRU组合。所有实现都采用PyTorch Lightning遵循现代MLOps实践。

Result: 研究发现学术文献中的高精度声明与实际实现存在巨大差距：Balaji LRCN从96%降至46.0%，IJERCSE从99.2%降至55.6%，Sensors从93%降至57.7%。而现代SOTA方法EfficientNet-B0与GRU组合达到了92.25%的准确率。

Conclusion: 现代架构和系统优化可以带来实质性改进，标准化评估协议在体育视频分析研究中至关重要。研究提供了一个可重复的研究平台，强调了在运动视频分析研究中标准化评估的重要性。
...
TL;DR: 本文提出了一个可解释的驾驶员意图预测框架VCBM，通过新的多模态数据集DAAD-X和概念瓶颈模型，为自动驾驶系统提供时空一致的解释，而不依赖事后技术。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶系统与人类交互的增加，决策过程的可解释性对于确保安全驾驶变得至关重要。深度学习系统在理解环境表示和驾驶任务方面仍面临挑战。

Method: 提出了Video Concept Bottleneck Model (VCBM)框架，使用新的多模态数据集DAAD-X，该数据集包含从驾驶员眼动和车辆视角推导的层次化高级文本解释。

Result: 在DAAD-X数据集上的评估表明，基于transformer的模型比传统CNN模型具有更好的可解释性。还引入了多标签t-SNE可视化技术来展示多个解释之间的解缠和因果相关性。

Conclusion: VCBM框架能够为驾驶员意图预测生成时空一致的解释，提高了自动驾驶系统的可解释性，有助于安全的人机交互。
...
TL;DR: Cattle-CLIP是一个基于多模态深度学习框架的牛只行为识别系统，通过结合语义线索和视频视觉特征，在监督学习和少样本学习场景下都能实现高精度行为识别。


<details>
  <summary>Details</summary>
Motivation: 牛只行为是评估动物健康、生产力和福利的重要指标。现有视频监控方法在数据稀缺的行为识别任务中表现不佳，需要开发能够处理领域差异和有限数据的鲁棒方法。

Method: 基于大规模图像-语言模型CLIP进行适配，添加时间整合模块，采用定制化数据增强策略和专门文本提示来解决预训练模型与真实牛只监控视频之间的领域差距问题。

Result: 在监督设置下，Cattle-CLIP在六种行为上达到96.1%的整体准确率，其中进食、饮水和站立反刍行为的召回率接近100%，在少样本场景下也表现出良好的泛化能力。

Conclusion: 多模态学习在农业和动物行为分析中具有巨大潜力，Cattle-CLIP框架能够有效处理数据稀缺的行为识别问题，为牲畜监测提供了可靠的技术方案。
...
TL;DR: 本文提出了一种通用的时间分辨Transformer（TRT）架构，用于提升光子效率成像中的3D重建性能。TRT包含专门为时空瞬态测量设计的注意力机制，包括时空自注意力编码器和时空交叉注意力解码器。基于TRT开发了TRT-LOS和TRT-NLOS两个具体实现，在合成数据和真实数据上都显著优于现有方法。
...
Method: 提出TRT架构，包含两个精心设计的注意力机制：时空自注意力编码器通过分割或下采样输入特征到不同尺度来探索瞬态数据中的局部和全局相关性；时空交叉注意力解码器在token空间中整合局部和全局特征，生成具有高表示能力的深度特征。

Result: 在合成数据和不同成像系统捕获的真实数据上进行的广泛实验表明，TRT-LOS和TRT-NLOS两个实现都显著优于现有方法。

Conclusion: TRT架构有效提升了光子效率成像中的3D重建性能，同时贡献了大规模高分辨率合成LOS数据集和真实NLOS测量数据，增强了该领域的数据多样性。
...
TL;DR: 提出Stable Video Infinity (SVI)方法，能够生成无限长度视频，具有高时间一致性、合理的场景转换和可控的流式故事情节。通过错误回收微调技术，让Diffusion Transformer从自身生成的错误中学习，解决了训练假设与自回归推理之间的差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有长视频生成方法仅限于单提示外推，产生同质化场景和重复动作。根本挑战在于训练假设（看到干净数据）与测试时自回归现实（基于自生成的错误输出）之间存在关键差异。

Method: 提出错误回收微调技术：1）注入DiT的历史错误干预干净输入，模拟流匹配中的错误累积轨迹；2）通过一步双向积分高效近似预测，用残差计算错误；3）在离散时间步上动态存储错误到重放内存，为新输入重采样。

Result: SVI能够将视频从秒级扩展到无限时长，无需额外推理成本，同时保持与多种条件（音频、骨架、文本流）的兼容性。在三个基准测试中验证了其多功能性和最先进性能。

Conclusion: SVI通过错误回收微调成功解决了长视频生成中的错误累积问题，实现了无限长度视频生成，在一致性、创造性和条件设置方面都表现出色。
...
TL;DR: 本文提出了TEMA-LLM框架，通过集成大型语言模型生成语义标签，结合多注意力机制来改进跨域序列推荐系统，在四个大规模电商数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 跨域序列推荐在现代电商平台中至关重要，需要准确捕捉领域特定和跨域行为模式，以提供个性化和无缝的消费者体验。

Method: TEMA-LLM使用LLM为项目标题和描述生成领域感知的描述性标签，将标签嵌入与项目标识符、文本和视觉特征融合构建增强的项目表示，并引入标签增强的多注意力机制联合建模域内和跨域用户偏好。

Result: 在四个大规模电商数据集上的广泛实验表明，TEMA-LLM始终优于最先进的基线方法。

Conclusion: 该方法突显了LLM在推进消费电子领域智能、以用户为中心服务方面的潜力，证明了基于LLM的语义标记和多注意力集成的优势。
...
TL;DR: 本文对图像和视频修复技术进行了全面综述，重点解决雾霾、雨雪等恶劣天气条件对智能交通系统视觉输入的降质问题，涵盖了传统方法和现代数据驱动模型，并讨论了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件（如雾霾、雨雪）显著降低图像和视频质量，对依赖视觉输入的智能交通系统（包括自动驾驶、交通监控和监控）构成严重挑战，需要有效的修复技术来缓解天气引起的视觉损伤。

Method: 将现有方法分类为传统基于先验的方法和现代数据驱动模型（包括CNN、Transformer、扩散模型和新兴的视觉语言模型），并根据范围进一步分类为单任务模型、多任务/多天气系统和能够处理多种降质的全能框架。

Result: 提供了图像和视频修复技术的全面综述，涵盖了日间和夜间修复挑战、基准数据集和评估协议，为推进智能交通环境中天气弹性视觉系统提供了有价值的参考。

Conclusion: 当前研究存在局限性，未来方向包括混合/复合降质修复、实时部署和代理AI框架，作者将定期更新该领域的最新相关论文和开源实现。
...
TL;DR: 该研究提出了一种基于消费级设备视频的肩部疾病诊断框架HMVDx，使用多模态大语言模型分别完成动作理解和疾病诊断任务，显著提高了肩关节损伤诊断准确率。


<details>
  <summary>Details</summary>
Motivation: 在医疗资源稀缺地区，肩部疾病（如肩周炎）的早期准确诊断面临挑战，迫切需要低成本、易扩展的辅助诊断解决方案。

Method: 提出HMVDx混合运动视频诊断框架，将动作理解和疾病诊断两个任务分离，分别由两个多模态大语言模型完成，并提出了基于医疗决策逻辑过程的新评估指标。

Result: 实验表明，HMVDx在诊断肩关节损伤方面的准确率比直接视频诊断提高了79.6%。

Conclusion: 该研究展示了低成本多模态大语言模型在医疗应用中的潜力，为未来医疗领域视频理解研究做出了重要技术贡献。
...
TL;DR: 本文建立了图像隐私分类的零样本基准，评估了顶级开源视觉语言模型(VLMs)与专用模型的性能对比。研究发现，尽管VLMs资源密集，但在隐私预测准确性上反而落后于更小的专用模型，但在图像扰动鲁棒性方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 当前文献倾向于采用为通用任务设计的大型视觉语言模型(VLMs)，这可能导致忽略专用模型设定的性能上限，因为缺乏系统评估。

Method: 建立图像隐私分类的零样本基准，使用任务对齐的提示评估前3名开源VLMs，并将其性能、效率和鲁棒性与已建立的仅视觉和多模态方法进行对比。

Result: VLMs尽管参数数量多、推理速度慢，但在隐私预测准确性上落后于专用的小型模型。VLMs对图像扰动表现出更高的鲁棒性。

Conclusion: 视觉语言模型在图像隐私预测任务中尚未超越专用模型，但在鲁棒性方面具有优势，这为未来研究提供了重要启示。
...
TL;DR: 本研究使用离散语义熵（DSE）来检测和过滤可能产生幻觉的问题，从而显著提高黑盒视觉语言模型在放射学图像视觉问答中的准确性。


<details>
  <summary>Details</summary>
Motivation: 黑盒视觉语言模型在放射学图像视觉问答中容易产生幻觉，影响诊断准确性，需要一种可靠的方法来检测和过滤这些问题。

Method: 使用两个公开数据集，让GPT-4o和GPT-4.1模型多次回答每个问题，通过双向蕴含检查将意义等效的回答分组，计算语义簇的相对频率得到DSE，然后过滤高熵问题。

Result: 在过滤掉DSE > 0.3的高熵问题后，GPT-4o的准确率从51.7%提升到76.3%，GPT-4.1从54.8%提升到63.8%，且统计显著。

Conclusion: DSE能够可靠地检测黑盒视觉语言模型中的幻觉，通过量化语义不一致性显著提高诊断答案准确性，为临床VLM应用提供有效的过滤策略。
...
TL;DR: 本文提出了一种统一框架，联合优化时序语句定位(TSG)和参考视频对象分割(RefVOS)，通过创新的[FIND]令牌进行关键时刻识别，并设计了时刻中心采样策略和双向锚点更新传播机制来提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有的RefVOS方法要么依赖手工启发式策略而忽略重要时序线索，要么使用外部关键帧模型增加系统复杂性。需要一种能够自然整合关键时刻定位能力的统一解决方案。

Method: 1) 提出联合优化TSG和RefVOS的统一框架；2) 训练时使用[FIND]令牌通过时序令牌相似性匹配识别关键时刻；3) 推理时采用时刻中心采样策略，密集采样信息丰富时刻，稀疏采样非必要帧；4) 开发双向锚点更新传播机制，利用最相关时刻作为高质量掩码初始化起点。

Result: 该方法避免了外部时间戳编码的需求，同时保持了运动细节和全局上下文，有效缓解了累积误差问题。

Conclusion: 所提出的框架为RefVOS任务提供了一种端到端的解决方案，自然整合了关键时刻定位能力，在保持系统简洁性的同时提升了分割性能。
...
TL;DR: 本文提出了一种新颖的多模态强化学习方法VPPO，通过分析token级别的视觉依赖性来优化视觉语言模型的推理能力。研究发现视觉依赖在推理过程中是稀疏分布的，基于此VPPO通过重新加权轨迹优势和聚焦关键token来改进学习信号。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理方法在强化学习优化过程中忽视了视觉感知的关键作用，本文从token感知的新视角探索多模态RLVR，旨在提升视觉语言模型的推理能力。

Method: 提出视觉感知策略优化(VPPO)算法，通过双重机制：基于整体视觉依赖性重新加权轨迹优势，以及仅对感知关键token进行策略更新。该方法基于token感知分析，发现视觉依赖在推理过程中的稀疏分布特性。
...
TL;DR: 人类视觉注视在扫描图像时遵循类似动物觅食的Levy行走模式，这是一种在稀疏资源环境中优化的重尾步长随机轨迹。通过大规模眼动实验和CNN模型，研究发现人类视觉探索遵循与自然觅食类似的统计规律。


<details>
  <summary>Details</summary>
Motivation: 传统模型强调基于图像的显著性，但眼动的基本时空统计特性仍未充分探索。理解这些动态对于注意力建模和基于视觉的界面具有广泛应用价值。

Method: 进行了大规模人类受试者实验，40名参与者观看50张多样化图像，使用高速眼动仪记录超过400万个注视点。同时训练卷积神经网络从图像输入预测注视热图。

Result: 分析显示人类眼睛的注视轨迹遵循类似动物觅食的Levy行走模式。CNN模型能够准确重现新颖图像中的显著注视区域，表明注视行为的关键组件可以从视觉结构中学习。

Conclusion: 人类视觉探索遵循与自然觅食类似的统计规律，这为通过生成和预测框架建模注视行为开辟了新途径。
...
Method: 引入CapGeo框架，通过几何描述辅助推理，将视觉和文本模态连接起来。同时提出了CapGeo-Bench数据集，包含4,641个精选的图形-描述对，并使用基于关键点的评估指标来评估几何描述质量。

Result: 实验显示配备描述后模型性能显著提升：Qwen2.5-VL-72B从8.6%提升到59.0%，Claude-Opus-4从44.8%提升到73.0%。提出的关键点评估指标与下游CapGeo性能强相关。

Conclusion: CapGeo框架和基准为推进多模态大语言模型的几何推理能力开辟了新的途径，证明了通过几何描述连接视觉和文本模态的有效性。
...
TL;DR: RadioFlow是一种基于流匹配的生成框架，用于实时无线电地图生成，相比扩散模型减少了8倍参数，推理速度提升4倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的无线电地图生成方法存在模型规模大、迭代去噪慢、推理延迟高等问题，阻碍了实际部署。

Method: 提出RadioFlow框架，通过学习噪声和数据之间的连续传输轨迹，实现单步高效采样，显著加速训练和推理过程。

Result: 实验表明RadioFlow在保持重建精度的同时，比领先的扩散基线模型参数减少8倍，推理速度提升4倍以上。

Conclusion: RadioFlow为未来6G网络的可扩展、能效高、实时的电磁数字孪生提供了有前景的技术路径。
...
TL;DR: 本文提出Hybrid-depth框架，通过整合CLIP和DINO基础模型来提取视觉先验，解决自监督单目深度估计中语义-空间知识提取不足的问题。该方法采用从粗到细的渐进学习策略，利用对比语言指导聚合多粒度特征，并在KITTI基准测试中显著优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 当前自监督单目深度估计方法由于语义-空间知识提取不足而面临性能限制。为解决这一问题，需要系统性地整合基础模型来提取视觉先验和获取足够的上下文信息。

Method: 1) 从粗到细的渐进学习框架：首先在对比语言指导下聚合CLIP的全局语义和DINO的局部空间细节特征，设计近距离-远距离图像块比较的代理任务进行深度感知特征对齐；2) 在粗特征基础上，整合相机姿态信息和逐像素语言对齐来细化深度预测，可作为即插即用模块与现有自监督MDE流程集成。

Result: 在KITTI基准测试上的广泛实验表明，该方法在所有指标上都显著优于最先进方法，并且确实有益于下游任务如BEV感知。

Conclusion: 通过语言指导聚合CLIP的语义上下文和DINO的空间细节，Hybrid-depth方法有效解决了特征粒度不匹配问题，显著提升了自监督单目深度估计的性能。
...
TL;DR: 提出IRCR-Net用于半监督的细胞核实例分割，通过实例感知一致性正则化和形态学先验知识来提升密集分布和重叠细胞核的分割精度，减少伪标签噪声。


<details>
  <summary>Details</summary>
Motivation: 病理图像中的细胞核实例分割对肿瘤微环境分析至关重要，但标注数据成本高且稀缺，现有半监督方法在实例级一致性正则化不足，缺乏病理结构先验知识利用，且容易引入噪声伪标签。
...
Conclusion: IRCR-Net通过实例级一致性正则化和形态学先验知识有效解决了半监督细胞核分割中的伪标签噪声问题，在标注数据稀缺的情况下实现了优异的性能。
...
TL;DR: 本文提出了一种用于热红外图像增强的渐进式提示融合网络（PPFN），通过建立基于热成像过程的提示对来调制模型特征，并引入选择性渐进训练机制来处理单一或复合退化问题，在复杂退化场景中实现了8.76%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有红外图像增强方法主要针对单一退化问题（如噪声、对比度、模糊），难以处理耦合退化；而通用的增强方法由于成像模型差异在红外图像上效果有限。

Method: 提出渐进式提示融合网络（PPFN）：基于热成像过程建立提示对，为每种退化类型融合相应提示对来调制模型特征；引入选择性渐进训练（SPT）机制逐步优化模型对复合情况的处理能力。

Result: 该方法不仅能有效去除相机噪声并保留关键结构细节，还能增强热图像的整体对比度；在复杂退化场景中实现了8.76%的显著性能提升。

Conclusion: PPFN方法在热红外图像增强任务中表现出色，既能处理特定退化问题，也能显著改善复杂退化场景的性能，为热红外图像增强提供了有效的解决方案。
...
TL;DR: 本文提出利用视觉语言模型进行多模态关键词预测，通过零样本、监督微调、Fine-tune-CoT和动态CoT策略提升模型性能，解决了传统方法在缺失和未见场景下的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统多模态方法在处理缺失和未见场景时存在显著局限性，且现有基准测试因训练测试重叠而高估模型能力。需要开发更强大的多模态关键词预测方法。

Method: 1) 使用零样本和监督微调评估VLM性能下限；2) 采用Fine-tune-CoT利用教师模型生成的高质量推理数据微调小模型；3) 提出动态CoT策略，在训练中自适应注入CoT数据，使模型在推理阶段灵活运用推理能力。

Result: 在多个数据集上的实验结果表明所提方法的有效性，代码已在GitHub开源。

Conclusion: 提出的基于视觉语言模型的多模态关键词预测方法，特别是动态CoT策略，能够有效提升模型在复杂推理任务中的性能，解决了传统方法的局限性。
...
TL;DR: VAD-GS是一个针对挑战性城市场景的3D高斯泼溅框架，通过体素化可见性推理、多样性感知视图选择和基于块匹配的多视图立体重建来恢复缺失的几何结构，显著提升静态和动态物体的重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅的性能严重依赖于初始点云质量，但在无边界动态城市场景中，由于观测视锥体不重叠，往往只能获得部分初始化的点云，导致训练时出现扭曲和伪影。现有密集化策略无法重建缺失结构。

Method: 1) 通过体素化可见性推理识别不可靠几何结构；2) 通过多样性感知视图选择选取信息丰富的支持视图；3) 通过基于块匹配的多视图立体重建恢复缺失结构；4) 在缺乏初始点的区域生成新的高斯基元。

Result: 在Waymo和nuScenes数据集上的大量实验表明，VAD-GS优于最先进的3DGS方法，显著提高了静态和动态物体的重建几何质量。
...
TL;DR: Minkowski-MambaNet是一个新颖的深度学习框架，直接从原始LiDAR点云估计森林木材体积和地上生物量，通过整合Mamba模型的SSM到Minkowski网络中，有效编码全局上下文和长距离依赖关系，无需数字地形模型且对边界伪影具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 准确的森林生物量量化对于碳循环监测至关重要。虽然机载LiDAR擅长捕捉3D森林结构，但由于难以建模区分树木所需的长距离依赖关系，直接从点云估计木材体积和地上生物量具有挑战性。

Method: 提出Minkowski-MambaNet框架，将Mamba模型的选择性状态空间模型集成到Minkowski网络中，有效编码全局上下文和长距离依赖关系以改善树木区分。采用跳跃连接来增强特征并加速收敛。

Result: 在丹麦国家森林清单LiDAR数据上的评估表明，Minkowski-MambaNet显著优于最先进方法，提供更准确和稳健的估计。关键优势是不需要数字地形模型且对边界伪影具有鲁棒性。

Conclusion: 这项工作为大规模森林生物量分析提供了一个强大的工具，推进了基于LiDAR的森林清查技术。
...
TL;DR: 本文分析了DETR中MLP层的固有稀疏性，提出了两种无需重新训练即可利用稀疏性的方法：静态指示器稀疏化(SIBS)和微门控稀疏化(MGS)，在COCO数据集上实现了85-95%的激活稀疏性，同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 基于transformer的模型在视觉任务（如目标检测）中的推理效率仍然是一个挑战，特别是在DETR的MLP层中存在固有的稀疏性，但现有方法需要重新训练整个模型。

Method: 提出了两种方法：1) SIBS：基于固定激活模式的启发式方法预测神经元不活跃性；2) MGS：在预训练DETR上训练的轻量级门控机制，使用小型线性层预测动态稀疏性。

Result: 在COCO数据集上的实验表明，MGS实现了85-95%的激活稀疏性，在显著减少计算量的同时保持甚至提升了性能。
...
TL;DR: Mono4DEditor是一个基于文本提示编辑从单目视频重建的4D场景的新框架，通过量化CLIP特征增强3D高斯表示，实现灵活准确的语义编辑。
...
Method: 使用量化CLIP特征增强3D高斯形成语言嵌入动态表示；提出两阶段点级定位策略：先通过CLIP相似度选择候选高斯，再优化空间范围；使用基于扩散的视频编辑模型进行针对性编辑，通过流和涂鸦引导确保空间保真度和时间一致性。

Result: 大量实验表明，Mono4DEditor能够在不同场景和对象类型上实现高质量的文本驱动编辑，同时保持未编辑区域的外观和几何特性，在灵活性和视觉保真度方面超越先前方法。

Conclusion: Mono4DEditor框架成功解决了4D场景编辑中的语义精确性和内容完整性挑战，为动态场景编辑提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的方法在真实低光场景中仍难以处理严重噪声，主要由于无法有效利用时间信息。
...
TL;DR: 提出了SilvaScenes数据集，用于森林环境下的树木实例分割和物种分类，包含1476棵树木、24个物种，在复杂自然条件下基准测试显示树木分割相对容易但物种分类仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 森林管理机器人面临复杂自然环境感知的挑战，现有数据集主要关注城市环境或物种有限，无法满足精准林业、生物多样性监测和林业设备自动化的需求。
...
Conclusion: SilvaScenes数据集填补了森林环境感知数据集的空白，展示了树木分割相对可行但物种分类仍需改进，为森林管理机器人感知系统开发提供了重要资源。
...
Method: 提出维度熵最大化方法，通过正则化文本特征分布使其趋向均匀分布，以减少对主导维度的依赖。

Result: 该方法有效缓解了测试时提示调优中校准性能的退化问题。

Conclusion: 维度熵最大化为增强视觉语言模型在真实部署场景中的可靠性提供了一种简单而有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决在动画、游戏等领域中，如何基于少量人工设计的参考角色，生成大量新颖角色同时保持其艺术风格和视觉特征一致性的问题。

Method: 基于DreamBooth微调文本到图像扩散模型，采用多令牌策略（通过聚类为个体角色和集体风格分配独立令牌），结合LoRA参数高效微调，移除类别特定正则化集，并在生成时引入随机令牌和嵌入。

Result: 在五个小型专业数据集上的评估显示，该方法能生成高质量、多样化的角色，同时保持参考角色的独特美学特征，人类评估进一步证实了其有效性。

Conclusion: 该方法在保持学习风格的同时实现了无限角色创建，展示了在创意产业中扩展艺术可能性的潜力。
...
TL;DR: 本文提出了一个临床基础的交互式医学图像分割评估方法，构建了标准化评估框架，并评估了多种先进算法，发现最小化交互信息损失、自适应缩放机制、验证与训练一致性以及3D上下文对分割性能的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前交互式医学图像分割算法的评估存在不一致性和临床不现实性，阻碍了公平比较和真实世界性能的准确反映。

Method: 提出临床基础的评估任务和指标定义方法，构建标准化评估框架软件，对多种先进算法在异质复杂任务上进行评估。

Result: 发现：(i)最小化交互信息损失对模型鲁棒性至关重要；(ii)自适应缩放机制提升鲁棒性和收敛速度；(iii)验证提示行为/预算与训练不一致时性能下降；(iv)2D方法在板状图像和粗糙目标上表现良好，但3D上下文有助于大或不规则形状目标；(v)非医学领域模型在对比度差和复杂形状时性能下降。

Conclusion: 建立了临床现实的交互式医学图像分割评估标准，为算法开发和比较提供了重要指导，强调了交互处理、缩放机制和领域适应性的重要性。
...
TL;DR: PhysToolBench是首个专门评估多模态大语言模型对物理工具理解能力的基准测试，包含1000多个图像-文本对的VQA数据集，评估工具识别、工具理解和工具创造三个难度层级的能力。


<details>
  <summary>Details</summary>
Motivation: 工具使用、理解和创造是人类智能的标志，但现有MLLM在物理工具理解方面的真实能力尚未量化，需要专门的基准测试来评估。

Method: 构建包含1000+图像-文本对的VQA数据集，设计三个难度层级的评估：工具识别（识别主要功能）、工具理解（理解操作原理）、工具创造（用周围物体创造新工具）。

Result: 对32个MLLM的全面评估显示，模型在工具理解方面存在显著缺陷，包括专有模型、开源模型、专用具身模型和VLA骨干模型。

Conclusion: 需要进一步改进MLLM对物理工具的理解能力，提供了深入分析和初步解决方案，代码和数据集已公开。
...
TL;DR: 该论文研究了三星智能手机图像中的对角线伪影及其对PRNU相机来源验证的影响，发现某些Galaxy S系列和A系列机型存在指纹碰撞问题，但PRO模式的原始图像可避免此问题，同时探讨了这些伪影在法证应用中的潜力。


<details>
  <summary>Details</summary>
Motivation: 研究三星智能手机图像中存在的对角线伪影问题，分析其对基于PRNU的相机来源验证系统的影响，特别是在法证应用中可能导致的误识别问题。

Method: 通过分析多个三星智能手机型号的图像数据，比较普通JPEG图像和PRO模式原始图像的差异，研究伪影产生机制及其对PRNU指纹提取的影响。

Result: 发现某些Galaxy S系列和A系列机型存在共同的伪影模式导致指纹碰撞，但PRO模式的原始图像可有效避免此问题；同时发现这些伪影在HDR图像误检测减少和肖像模式合成虚化区域定位方面具有法证应用价值。

Conclusion: 三星智能手机的对角线伪影会影响PRNU相机验证的准确性，但原始图像可解决此问题；这些伪影在特定法证场景中反而具有应用价值，为图像取证提供了新的分析维度。
...
TL;DR: PRNet是一个实时检测框架，通过保留和有效利用原始浅层空间特征来增强小目标检测。它包含渐进细化颈部（PRN）和增强切片采样（ESSamp）两个模块，在VisDrone、AI-TOD和UAVDT数据集上优于现有方法。
...
Method: 提出PRNet框架，包含两个模块：1) 渐进细化颈部（PRN）- 通过骨干网络重用和迭代细化实现空间-语义对齐；2) 增强切片采样（ESSamp）- 通过优化的重排和卷积在下采样过程中保留浅层信息。

Result: 在VisDrone、AI-TOD和UAVDT数据集上的广泛实验表明，PRNet在可比较的计算约束下优于最先进的方法，实现了更好的精度-效率权衡。

Conclusion: PRNet通过优先保留和有效利用原始浅层空间特征，成功解决了航空图像中小目标检测的信息退化问题，在保持实时性的同时实现了优越的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统多层感知机在隐式神经表示中建模变形时，需要昂贵的正则化来提取连贯准确的变形，这会导致训练不稳定并阻碍有效的特征对齐。

Method: 将变形重新定义为微分向量流的构造，通过将结构流属性直接编码到网络架构中，自然确保连续性、可逆性和时间一致性。

Result: 在面部和图像变形以及高斯溅射变形等多种应用中，FLOWING实现了最先进的变形质量，并具有更快的收敛速度。

Conclusion: FLOWING的流中心方法产生原则性和稳定的变换，能够实现准确且结构保持的2D图像和3D形状变形。
...
<details>
  <summary>Details</summary>
Motivation: 当前可控扩散模型使用静态架构修改中间激活来注入条件指导，这种静态条件策略限制了模型在从粗结构到精细细节的生成过程中适应其响应的能力。

Method: 使用超网络动态生成LoRA适配器，在扩散过程的每个步骤中根据时间步长和用户条件为冻结的主干网络定制权重修改，实现参数化控制。

Result: 在各种数据领域的实验中，这种动态参数控制相比静态激活方法显著提高了生成保真度和对空间条件的遵循度。
...
TL;DR: 对CVPR、ICLR和NeurIPS 2023-2025年间26,104篇论文的研究趋势进行透明可复现的测量分析，揭示了三个宏观转变：多模态视觉-语言-大语言模型工作的急剧增长、生成式方法的稳步扩展以及3D和视频活动的持续活跃。


<details>
  <summary>Details</summary>
Motivation: 量化计算机视觉和机器学习领域的研究趋势，通过系统化分析顶级会议论文来识别主要的技术转向和新兴研究方向。

Method: 对论文标题和摘要进行标准化处理，使用手工制作的词典匹配分配最多35个主题标签，挖掘关于任务、架构、训练机制、目标函数、数据集和共现模态的细粒度线索。

Result: 发现三个主要趋势：1）多模态视觉-语言-LLM工作急剧增长，将经典感知重新定义为指令跟随和多步推理；2）生成式方法稳步扩展，扩散研究集中在可控性、蒸馏和速度方面；3）3D和视频活动持续活跃，组合方法从NeRF转向高斯溅射，对人中心和智能体中心理解的关注增加。

Conclusion: 研究提供了计算机视觉和机器学习领域研究趋势的系统性量化分析，揭示了向多模态、生成式和3D/视频理解的技术转向，并发布了词典和方法论以支持后续审计和扩展。
...
TL;DR: 本文提出了一个全尺度空间推理解决方案，包括结构化知识系统、尺度感知建模和渐进式训练范式，创建了包含100万个空间问答对的SpaceVista-1M数据集，并开发了SpaceVista-7B模型，在多个基准测试中表现出色。
...
Method: 采用结构化空间推理知识系统、尺度感知建模和渐进式训练范式，通过任务特定的自动化流程创建大规模数据集，并开发接受密集输入的空间推理模型。

Result: 创建了包含38K视频场景和100万个空间问答对的SpaceVista-1M数据集，开发的SpaceVista-7B模型在5个基准测试中表现出竞争性性能，展现出跨所有尺度和场景的强泛化能力。

Conclusion: 该研究成功推进了全尺度空间推理能力，为多模态大语言模型的空间智能扩展提供了首个尝试，数据集、模型和基准测试将公开发布。
...
TL;DR: 提出了一种基于蒸馏的框架，通过从预训练的小型动作模型转移知识，为视觉语言模型赋予动作执行能力，避免了从头训练的高成本。


<details>
  <summary>Details</summary>
Motivation: 视觉语言动作模型虽然能利用预训练视觉语言模型的感知能力，但从头训练成本高昂。本文旨在通过知识蒸馏方法，以低成本为视觉语言模型添加动作执行能力。

Method: 采用两阶段训练策略：1) 轻量级对齐，将VLM隐藏状态映射到小型动作模型的动作空间；2) 选择性微调语言模型、状态编码器和动作模块。架构上仅添加动作令牌和状态编码器来整合物理输入。

Result: 在LIBERO数据集上达到97.3%平均成功率（提升11.8%），在LIBERO-LONG上达到93.5%（提升24.5%）。真实世界实验中，在五个操作任务上达到82.0%成功率（比教师模型提升17%）。
...
TL;DR: StreamingVLM是一个专为实时处理无限视频流设计的视觉语言模型，通过统一的训练和推理框架，使用KV缓存重用和注意力机制优化，在长视频理解任务中表现出色，同时保持实时性能。
...
Method: 提出StreamingVLM框架，在推理时维护紧凑的KV缓存，重用注意力sink状态、近期视觉token和文本token。通过简单的监督微调策略，在短重叠视频块上应用完整注意力，模拟推理时的注意力模式。
...
Conclusion: StreamingVLM成功解决了长视频流实时理解的挑战，通过统一的训练推理框架和高效的注意力机制，实现了稳定、实时的性能，同时提升了模型的通用能力。
...
TL;DR: 本研究评估了GLiNER-BioMed模型在BioASQ数据集上的表现，并引入基于词典的后处理策略来解决基因和化学实体间的常见误分类问题。虽然后处理在开发集上显著提升了性能（F1从0.79到0.83），但在盲测集上未能泛化（F1从0.79降到0.77）。


<details>
  <summary>Details</summary>
Motivation: 生物医学命名实体识别（BioNER）在提取科学文献信息中至关重要，但面临区分相似实体类型（如基因和化学物质）的挑战。

Method: 评估GLiNER-BioMed模型，并引入基于词典的后处理策略来纠正常见误分类，同时探索了条件随机场等替代方法。

Result: 后处理在开发集上显著提升性能（micro F1从0.79到0.83），但在盲测集上性能下降（micro F1从0.79到0.77）。

Conclusion: 基于词典的细化对预训练BioNER模型具有潜力，但面临开发数据过拟合的挑战，需要确保鲁棒泛化以实现实际应用。
...
TL;DR: 本文揭示了测试时缩放（TTS）方法的一个新失败模式：当候选响应多样性受限时，TTS更可能产生不安全输出。作者提出了RefDiv攻击协议来验证这一现象，并发现现有安全防护措施对此类攻击防护有限。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放（TTS）方法假设候选响应多样性能够提高可靠性，但作者发现这一假设存在未被识别的失败模式，需要验证当多样性受限时TTS的安全性表现。

Method: 提出了参考引导的多样性减少协议（RefDiv）作为诊断攻击，通过限制候选响应多样性来压力测试TTS管道。在四个开源模型和两种TTS策略上进行了广泛实验。

Result: 限制多样性显著提高了TTS产生不安全结果的概率，这种效应甚至比高对抗意图的提示更强。现有安全防护分类器无法有效检测RefDiv生成的对抗性输入提示。

Conclusion: TTS存在基于多样性的通用失败模式，现有防御措施对此防护有限。需要设计既能有效又能抵御多样性目标压力测试的鲁棒TTS策略。
...
TL;DR: HAREN-CTC是一个用于语音抑郁检测的新架构，通过整合多层自监督学习特征、跨注意力机制和多任务学习框架，结合CTC损失处理稀疏时间监督，在DAIC-WOZ和MODMA数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统语音抑郁检测方法难以提取有意义的特征，且无法有效捕捉稀疏、异质的抑郁线索。现有方法通常只使用SSL模型的最后一层或寻找单一最佳层，容易过拟合且无法充分利用层次结构来检测细微和持续的抑郁信号。

Method: 提出HAREN-CTC架构，包含两个关键模块：层次自适应聚类模块将SSL特征重组为互补嵌入，跨模态融合模块通过跨注意力建模层间依赖关系。使用CTC损失实现对齐感知训练，跟踪抑郁语音线索的不规则时间模式。

Result: 在标准数据分割的上限设置和五折交叉验证的泛化设置下进行评估，在DAIC-WOZ数据集上达到0.81的宏F1分数，在MODMA数据集上达到0.82的宏F1分数，在两个评估场景中都优于先前方法。

Conclusion: HAREN-CTC通过有效整合多层SSL特征和建模层间依赖关系，显著提升了语音抑郁检测的性能，证明了该方法在捕捉抑郁语音线索方面的有效性。
...
TL;DR: 该研究提出了一个评估数学推理能力的新框架，通过分析GPT-3.5在GSM8K数据集上的逐步推理过程，使用GPT-4o-mini进行错误分类和无监督聚类，揭示了AI模型在数学推理上的非人类脆弱性。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中关于机器学习模型数学理解程度的核心问题，超越标准基准测试来诊断具体的失败点。

Method: 首先从GPT-3.5-turbo在GSM8K数据集上生成结构化的逐步推理，然后使用更强大的GPT-4o-mini模型进行错误分类和无监督聚类，识别出现的"推理模式"。

Result: 分析揭示了AI模型的认知特征具有明显的非人类脆弱性：在顺序计算等程序性模式上表现近乎完美，但在需要组合推理和限制条件的模式上性能急剧下降。

Conclusion: 通过识别和量化这些不同推理技能的可靠性，该研究提供了更细粒度的数学理解评估方法，并为开发新能力和更可靠的未来应用提供了精确路线图。
...
TL;DR: 提出置信度分数（CS）作为无参考指标的替代方案，缓解传统指标对创意文本生成的偏见，实验证明CS在评估创意内容时表现更平衡。


<details>
  <summary>Details</summary>
Motivation: 传统无参考指标（如自困惑度）对创意文本生成存在严重偏见，需要一种更少偏见的替代评估方法。

Method: 提出基于模型输出概率分布的置信度分数（CS），在99个创意提示上对比CS与基于流畅度指标的表现。

Result: 实验显示：流畅度指标在0%的情况下偏好新颖回复，而CS在19%的情况下偏好新颖回复，差异具有统计显著性（95%置信区间：[11.1%, 27.3%]）。CS还能有效区分不同难度任务。

Conclusion: 置信度分数缓解了传统指标的创造力偏见，同时保留了核心评估优势，为现代LLM提供了更平衡的评估方法。
...
TL;DR: Recover-LoRA是一种轻量级、数据集无关的方法，通过使用合成数据和logit蒸馏学习选择性层的LoRA适配器，来恢复因量化、剪枝、格式转换等优化导致的模型性能下降，在小型语言模型上能恢复5-17%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的推理优化技术（如量化、剪枝、格式转换等）会导致语言模型任务性能的功能性退化，而大多数性能恢复工作都集中在鲁棒量化技术上，本文关注从任何导致模型权重退化的源中恢复模型准确性，包括不正确的模型序列化。

Method: 提出Recover-LoRA方法，使用合成数据和logit蒸馏来学习选择性层的LoRA适配器，使退化模型与其全精度模型对齐。该方法轻量级且数据集无关。

Result: 在多样化的小型语言模型（包括多头注意力和分组查询注意力架构）和多个评估数据集上，Recover-LoRA能够恢复模型准确率5-17%。

Conclusion: Recover-LoRA是一种有效的模型准确性恢复方法，能够处理各种推理优化导致的性能退化问题，在小型语言模型上表现出良好的恢复效果。
...
TL;DR: Mnemosyne是一个受人类记忆启发的无监督长期记忆架构，专为边缘设备上的大语言模型设计。它采用图结构存储、模块化过滤器和记忆管理机制，在纵向医疗对话中实现了65.8%的胜率，显著优于传统检索增强生成方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM记忆系统要么依赖暴力扩展上下文，要么使用静态检索管道，在边缘设备上表现不佳。特别是在医疗等纵向应用中，重复且语义相似但时间不同的对话限制了传统检索方法的效果。

Method: 使用图结构存储，包含模块化的内容和冗余过滤器、记忆提交和修剪机制，以及基于人类记忆建模的概率回忆（包含时间衰减和刷新过程）。还引入了从记忆图中固定长度子集高效推导的"核心摘要"来捕捉用户个性等长期细节。

Result: 在纵向医疗对话的盲测人类评估中，Mnemosyne在真实性和长期记忆能力方面达到65.8%的胜率，显著优于基线RAG的31.1%。在LoCoMo基准测试中，在时间推理和单跳检索方面获得最高分，总体得分54.6%排名第二，优于Mem0和OpenAI等常用基线。

Conclusion: Mnemosyne证明了通过边缘兼容且易于迁移的无监督记忆架构，可以实现改进的事实回忆、增强的时间推理以及更自然的用户响应，特别适用于纵向医疗助手等应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将AI文本检测视为二分类问题，但人类文本并不构成统一的分布，这种假设导致模型泛化能力差，容易记忆训练数据的特征而非学习'非分布内'行为的本质。

Method: 采用离群检测框架，使用单类学习方法（DeepSVDD和HRN）和基于分数的学习技术（如基于能量的方法），将机器生成文本作为分布内样本，人类文本作为分布外样本。

Result: 在DeepFake数据集上达到98.3%的AUROC和AUPR，FPR95仅为8.9%。在多语言、被攻击、未见模型和领域文本设置下均表现出鲁棒性和泛化性。

Conclusion: 将AI文本检测重新定义为离群检测问题能够有效解决传统二分类方法的泛化限制，提供更稳健和通用的检测框架。
...
TL;DR: YpathRAG是一个面向病理学的RAG框架，通过构建包含28个子领域和153万段落的病理学向量数据库，采用双通道混合检索和基于LLM的证据判断模块，显著提升了病理学问答的准确性和事实可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用任务上表现出色，但在病理学等高门槛领域仍存在幻觉问题。现有方法依赖领域微调，既不能扩展知识边界，也无法强制执行证据约束。

Method: 构建病理学向量数据库，开发YpathRAG框架，包含双通道混合检索（BGE-M3密集检索+词汇引导稀疏检索）和基于LLM的支持证据判断模块，形成检索-判断-生成的闭环。

Result: 在YpathR基准上，Recall@5达到98.64%，比基线提升23个百分点；在YpathQA-M（300个最具挑战性问题）上，通用和医学LLM的准确率平均提升9.0%，最高提升15.6%。

Conclusion: YpathRAG显著提升了检索质量和事实可靠性，为面向病理学的RAG提供了可扩展的构建范式和可解释的评估方法。
...
### [103] [FinAuditing: A Financial Taxonomy-Structured Multi-Document Benchmark for Evaluating LLMs](https://arxiv.org/abs/2510.08886)
...
TL;DR: FinAuditing是首个针对财务审计任务的基准测试，基于真实XBRL文件构建，包含语义一致性、关系一致性和数值一致性三个子任务，评估LLMs在结构化财务文档推理中的能力。


<details>
  <summary>Details</summary>
Motivation: GAAP的复杂性和XBRL的分层结构使得财务审计难以自动化验证，而LLMs在结构化、相互依赖的财务文档推理能力尚未充分探索。

Method: 从真实US-GAAP合规XBRL文件中构建FinAuditing基准，定义三个互补子任务：FinSM（语义一致性）、FinRE（关系一致性）、FinMR（数值一致性），并提出统一的评估框架。

Result: 对13个最先进LLMs的零样本实验显示，当前模型在语义、关系和数学维度表现不一致，当推理分层多文档结构时准确率下降60-90%。

Conclusion: 研究揭示了现代LLMs在基于分类法的财务推理中的系统性局限性，FinAuditing为开发可信赖、结构感知且符合监管的财务智能系统奠定了基础。
...
TL;DR: 提出LatentBreak方法，通过语义替换生成低困惑度的对抗性提示来绕过基于困惑度的安全检测，相比现有攻击方法更有效。


<details>
  <summary>Details</summary>
Motivation: 现有越狱攻击使用高困惑度的对抗后缀或长模板，容易被基于困惑度的过滤器检测到，需要开发能绕过此类防御的自然对抗提示生成方法。

Method: 使用白盒攻击方法，在潜在空间中最小化对抗提示与无害请求表示之间的距离，通过语义替换生成低困惑度的自然对抗提示。

Result: LatentBreak生成的提示更短且困惑度更低，在多个安全对齐模型上对基于困惑度的过滤器表现出色，优于现有越狱算法。

Conclusion: LatentBreak证明了通过语义替换生成低困惑度对抗提示的有效性，为绕过基于困惑度的安全防御提供了新思路。
...
TL;DR: 本文提出了一种多语言、多智能体的大型语言模型框架，用于检测数字平台上的错误信息，并通过检索增强生成技术开发了可部署为网络插件的解决方案。


<details>
  <summary>Details</summary>
Motivation: 数字平台上错误信息的快速传播威胁公共讨论、情感稳定和决策制定，现有研究尚未系统研究语言切换、查询长度膨胀和结构重构等特定转换攻击。

Method: 开发了多语言多智能体LLM框架，采用检索增强生成技术，支持英语、法语、西班牙语、阿拉伯语、印地语和中文的语言切换攻击检测，并能部署为网络插件。

Result: 展示了AI驱动的错误信息检测在保护在线事实完整性方面的重要性，同时证明了基于插件的部署在真实网络应用中的可行性。

Conclusion: 该研究强调了对抗多样化攻击的错误信息检测的重要性，并为实际网络应用提供了可行的插件部署方案。
...
TL;DR: 本文提出了一种基于情感热点的对话情感识别方法，通过检测文本、音频和视频中的情感热点，使用热点门控融合和路由混合对齐器来整合多模态信息，在标准基准测试中取得了优于基线模型的效果。


<details>
  <summary>Details</summary>
Motivation: 对话情感识别面临的主要挑战是判别性证据稀疏、局部化且跨模态异步，因此需要一种能够聚焦于情感关键区域的方法。

Method: 提出统一模型：检测每个话语在文本、音频和视频中的情感热点；通过热点门控融合将热点与全局特征融合；使用路由混合对齐器对齐模态；构建跨模态图编码对话结构。

Result: 在标准ERC基准测试中相比强基线模型取得了一致的性能提升，消融实验证实了热点门控融合和混合对齐器的贡献。

Conclusion: 研究结果表明热点中心视角可以为未来多模态学习提供新思路，为ERC中的模态融合提供了新的视角。
...
TL;DR: MMA-ASIA是一个评估大语言模型在亚洲文化背景中文化意识的综合框架，包含多语言多模态对齐的27,000道选择题基准，覆盖8个亚洲国家和10种语言，79%问题需要基于文化背景的多步推理。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在非西方、低资源环境中的多模态理解和推理能力下降，需要专门评估其在亚洲文化背景下的文化意识。

Method: 构建人类策划的多语言多模态对齐基准数据集，提出五维评估协议（文化意识差异、跨语言一致性、跨模态一致性、文化知识泛化、基础有效性），开发文化意识基础验证模块检测捷径学习，使用比较模型分析、注意力追踪和视觉消融前缀重放方法。

Result: 创建了首个在文本、图像和语音三个模态输入层面对齐的数据集，能够直接测试跨模态迁移能力。

Conclusion: 该框架为构建文化可靠的多模态大语言模型提供了可操作的见解，揭示了模型在不同语言和模态间差异的原因。
...
TL;DR: GraphGhost是一个统一框架，将LLM的神经元激活和信号传播表示为图结构，揭示LLM如何从序列输入中捕获结构语义并通过结构一致机制生成输出。该框架使用图算法分析LLM特性，识别关键神经元节点，并通过结构干预验证其重要性。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型展现出卓越的推理能力，但其背后的结构机制仍未充分探索。需要开发新方法来理解LLM如何捕获结构语义并生成输出。

Method: 提出GraphGhost框架，将神经元激活和信号传播建模为图结构，使用PageRank等图算法分析LLM特性，识别关键神经元节点，并通过结构干预（编辑关键神经元节点）来验证其重要性。

Result: 揭示了不同模型在多样化数据集上的共享和特定推理行为，发现对关键神经元节点的编辑会引发推理崩溃，改变逻辑流程和语义理解。

Conclusion: GraphGhost是分析、干预和最终理解LLM推理结构基础的有力工具，为深入探索LLM推理机制提供了新视角。
...
TL;DR: 本研究评估了大型语言模型在医疗诊断中对性别偏见的敏感性，发现虽然诊断结果相对一致，但所有模型在判断患者性别相关性和必要性时都表现出显著不一致性，特别是在相关性判断方面，某些模型甚至显示出系统性的性别差异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域的应用前景广阔，但其对偏见的敏感性是一个关键问题。性别长期以来影响着医生的行为和患者结果，因此需要评估LLMs在扮演类似人类角色时是否会复制或放大性别相关偏见。

Method: 使用《新英格兰医学杂志挑战赛》的案例研究，为多个开源和专有LLMs分配性别（女性、男性或未指定），评估它们在LLM性别分配下的响应一致性，包括基于LLM的诊断以及模型对患者性别临床相关性或必要性的判断。

Result: 大多数模型的诊断结果在LLM性别分配下相对一致。然而，对于患者性别在LLM诊断中的相关性和必要性，所有模型都表现出显著的不一致性，特别是在相关性判断方面。某些模型甚至在其对患者性别的解释中显示出系统性的女性-男性差异。

Conclusion: 这些发现揭示了一种未被充分探索的偏见，可能会削弱LLMs在临床实践中的可靠性，强调在与LLMs交互时需要常规检查身份分配一致性，以确保可靠和公平的AI支持临床护理。
...
TL;DR: 本文提出了一种利用大语言模型自动生成数学应用题中干扰条件的迭代框架，通过设计多角度和认知层次的提示词来创建有意义的干扰条件，同时保持原问题解法的完整性。


<details>
  <summary>Details</summary>
Motivation: 现有数学应用题数据集大多只包含必要信息，缺乏干扰条件，而现有的干扰条件数据集存在难度低、表达不自然等问题，导致评估大语言模型数学推理能力时可信度不足。

Method: 设计迭代框架，利用大语言模型从多角度和认知层次自动生成干扰条件，通过特定提示词确保干扰条件不改变原问题解法，避免需要重新编写答案。

Result: 该框架能够高效生成高质量的包含干扰条件的数学应用题，大幅减少了人工标注工作量，同时保持了数据的质量。

Conclusion: 提出的迭代框架能够有效解决数学应用题干扰条件生成的问题，为大语言模型数学推理能力的可靠评估提供了高质量的数据集生成工具。
...
TL;DR: 本文提出了一种通过重新评估模型在基准问题的改写版上的表现来探测LLM泛化能力的简单协议，发现改写会导致准确率显著下降，证实了关于数据污染和表面形式捷径的担忧。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的基准分数可能因记忆测试项目或近似重复项而被夸大，需要一种方法来探测模型是否真正理解问题而非仅仅依赖表面形式。

Method: 使用Mistral-7B-Instruct和Qwen2.5-7B-Instruct模型，在ARC-Easy和ARC-Challenge数据集上，通过重新评估模型在改写版本问题上的表现来测量准确率差距。流程包括控制解码、强制多选输出格式和鲁棒的改写清理步骤以保持语义。

Result: 改写会导致非平凡（显著）的准确率下降（原始vs改写），这与之前关于数据污染和脆弱的表面形式捷径的担忧一致。

Conclusion: 基准测试中的改写评估揭示了LLM泛化能力的局限性，证实了模型可能过度依赖表面形式而非深层理解，强调了需要更严格的评估方法来检测数据污染问题。
...
TL;DR: JAI-1是一个拥有750亿参数的泰语中心语言模型，采用上扩增策略从较小的英语开源模型扩展参数空间，系统性地集成泰语知识，在泰语基准测试中表现优于Typhoon2-70B。


<details>
  <summary>Details</summary>
Motivation: 现有泰语模型主要依赖现有开源模型进行额外训练，这种方法可能导致原有知识在参数空间中被侵蚀，因为通用任务优化参数可能与新语言需求冲突。

Method: 从高性能英语开源LLM出发，扩展其参数空间，利用新分配的能力系统集成泰语知识，包括1.5万亿token的预训练（含3000亿泰语token）和超过60万指令示例的监督微调和对齐调优。

Result: 在泰语中心基准测试（IFEval-TH、MT-Bench-TH和JAI-Hall-Bench）中表现优于Typhoon2-70B，验证了其上扩增和知识集成框架的有效性。

Conclusion: JAI-1的上扩增方法不仅保留了原始模型的通用智能，还建立了与其他开源模型不同的独特架构，支持可扩展的未来增强。
...
TL;DR: 本文研究基于用户画像（年龄、性别、职业）的销售导向对话代理，发现职业对对话意图影响最大，提出了一种轻量级的职业条件策略来优化对话效果。


<details>
  <summary>Details</summary>
Motivation: 随着智能对话模型的快速发展，需要真实的用户模拟研究来调整有效的对话策略，特别是在销售导向的对话系统中。

Method: 研究销售导向对话代理根据用户画像（年龄、性别、职业）调整对话策略，引入轻量级的职业条件策略，使代理优先考虑与用户偏好一致的对话意图。

Result: 年龄和性别影响整体表现，但职业在对话意图上产生最显著差异，使用职业条件策略后对话更短且成功率更高。
...
TL;DR: Text2Stories是一个用于评估软件需求与利益相关者需求对齐的任务和度量标准，通过量化用户故事与访谈记录之间的匹配程度来验证需求的正确性和完整性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型可以自动从自然语言输入生成软件需求，但验证这些需求是否真实反映利益相关者需求仍然主要依赖人工。需要一种可扩展的方法来量化需求与原始需求来源的对齐程度。

Method: 将访谈记录分割成文本块，将用户故事与文本块的匹配建模为匹配问题。使用基于LLM的匹配器来量化正确性（故事被记录支持的比例）和完整性（记录被故事覆盖的比例）。

Result: 在四个数据集上的实验表明，基于LLM的匹配器在保留注释上达到0.86的宏F1分数，而嵌入模型虽然性能较低但可用于有效的阻塞。

Conclusion: Text2Stories提供了一个可扩展的、忠实于源头的补充方法，可用于比较不同故事集（如人工生成与自动生成）的质量，作为现有用户故事质量标准的补充。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法直接将大语言模型应用于提取任务，但将JSON模式视为静态合约，导致提取性能不佳、频繁产生幻觉，以及在模式规范模糊或不完整时代理行为不可靠。

Method: 开发PARSE系统，包含两个协同组件：ARCHITECT（自动优化JSON模式以提升LLM消费效果）和SCOPE（实现基于反射的提取，结合静态和LLM防护机制）。

Result: 在三个数据集上的评估显示，PARSE在SWDE上提取准确率提升64.7%，整体框架改进达10%，首次重试时提取错误减少92%，同时保持实用延迟。

Conclusion: PARSE通过将JSON模式视为可解释和优化的自然语言理解合约，显著改进了结构化信息提取的可靠性和性能。
...
TL;DR: 研究发现，评估导向的提示框架会人为夸大LLM性能指标，增加推理长度但未必提升准确率，在真实部署中可能产生误导。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试使用评估导向的提示框架（如要求显式推理和严格格式），而实际部署需要简洁、契约绑定的回答，这种"评估气味"可能夸大性能测量。

Method: 使用GPT-OSS-20B模型进行六组A/B测试，固定任务内容和解码参数，改变提示框架（评估导向vs真实世界）和推理深度，涵盖数学、代码修复、引用生成、激励翻转、推理可见性和多语言场景。

Result: 评估框架显著增加推理长度（数百到1000+字符），降低仅回答合规性，准确率提升有限或不一致。在结构化输出中改善格式包装但未提升实质内容。激励措辞改变错误构成：鼓励谨慎略微提升高推理深度下的准确率，减少自信错误；鼓励能力产生更简洁但风险更高的输出。

Conclusion: 需要采用中性措辞或双框架检查、契约感知评分、风格差异报告、置信度治理和多语言仪表板，确保基准测试收益反映可部署能力。
...
TL;DR: PULSE是一个使用小型语言模型生成推理理由作为监督信号的推荐框架，通过联合建模用户行为及其语义驱动因素，在多个基准数据集上超越了基于ID、协同过滤和LLM的推荐模型。
...
Method: 提出PULSE框架，将SLM生成的理由作为直接学习信号，与交互历史一起监督，共同建模用户行为（什么）及其语义驱动因素（为什么），将理由视为一等信号。

Result: 在多个基准数据集上，PULSE超越了领先的基于ID、协同过滤和LLM的顺序推荐模型，在跨域推荐中表现出优越的迁移性，并在推理导向的下游任务中表现强劲。

Conclusion: PULSE通过将SLM生成的理由作为一等学习信号，创建了更稳健和可泛化的嵌入表示，为高效且有效的推荐系统提供了新思路。
...
TL;DR: ExPO-HM是一个针对仇恨表情包检测的新方法，通过结合SFT预热、GRPO课程学习和条件决策熵，在检测性能、细粒度分类和推理质量方面都达到了最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有的仇恨表情包检测方法大多只提供二元预测，缺乏上下文和解释，无法满足实际审核需求。而现有的解释-检测方法性能不如简单的SFT基线，存在两个关键问题：模型无法假设政策相关线索，二元奖励信号不足以指导推理。

Method: ExPO-HM结合了SFT预热、GRPO课程学习和条件决策熵（CDE），将CDE同时作为推理质量的度量和奖励，灵感来自人类标注者的训练和评估过程。
...
TL;DR: 本文提出了分层扩散语言模型（HDLM），这是一种用于语言建模的新型离散扩散模型。HDLM基于分层词汇表构建，其中低层令牌包含详细语义，通过满射映射到具有粗粒度含义的高层令牌。在正向过程中，每个令牌根据调度器独立扰动到其更高层的祖先令牌；在反向过程中，模型逐步预测下一个更详细的语义。


<details>
  <summary>Details</summary>
Motivation: 开发一种通用的时变下一语义尺度预测过程，用于语言建模。通过分层词汇表结构，实现从粗粒度到细粒度的语义生成过程。

Method: HDLM基于分层词汇表构建，低层令牌映射到高层令牌。正向过程：每个令牌独立扰动到更高层祖先；反向过程：模型逐步预测更详细的语义。推导了扩散证据下界（ELBO）的闭式表达式，并提出了实用的训练技术。

Result: 广泛的文本生成实验验证了HDLM的有效性，相比基线方法，HDLM在验证困惑度和生成困惑度方面都表现出持续更低的性能。

Conclusion: HDLM提供了一种通用的时变下一语义尺度预测框架，可以灵活实现，并将现有的MDLM作为特例包含在内。实验结果表明该方法在语言建模任务中具有优越性能。
...
TL;DR: UCoT是一种高效的推理框架，通过前置思维嵌入实现CoT压缩，使用小模型作为压缩器和大模型作为执行器的协作工作流，在保持强大推理能力的同时显著减少CoT长度。


<details>
  <summary>Details</summary>
Motivation: 长链思维推理在LLMs中带来高计算成本和延迟损失，现有CoT压缩方法要么需要繁琐的离散提示设计，要么构建外部压缩数据集牺牲关键推理细节，需要更高效的自动化压缩方法。

Method: 提出UCoT两阶段框架：第一阶段训练压缩器生成富含推理信息的前置思维嵌入；第二阶段优化执行器利用前置思维嵌入进行短推理，采用奖励机制。

Result: 在GSM8K数据集上，UCoT将Qwen2.5-7B-Instruct模型的token使用量减少50%，性能比SOTA方法高3.08%。

Conclusion: UCoT在保持强大推理能力的同时显著提高了推理效率，为LLMs的高效推理提供了有效解决方案。
...
TL;DR: 本文提出了一种分析个人叙事风格的新框架，将风格定义为作者在表达主观体验时的语言选择模式，整合了功能语言学、计算机科学和心理学，并通过语言模型自动提取语言特征来分析梦境叙事。


<details>
  <summary>Details</summary>
Motivation: 个人叙事是作者构建意义的重要方式，风格是表达主观体验的核心，但目前缺乏系统分析这些风格选择的正式框架。

Method: 整合功能语言学、计算机科学和心理学，使用语言模型自动提取过程、参与者和环境等语言特征，分析语言选择序列模式。

Result: 在数百个梦境叙事分析中，包括对患有创伤后应激障碍的战争退伍军人的案例研究，发现其叙事中言语过程主导心理过程的独特模式，揭示了语言选择与心理状态的关系。

Conclusion: 该框架成功建立了语言选择模式与心理观察之间的联系，为系统分析个人叙事风格提供了有效方法，展示了语言选择如何反映心理状态。
...
TL;DR: 本研究利用大型语言模型(LLM)将定性文本数据与传统评分量表相结合，创建增强型心理测试，在抑郁症评估案例中显著提高了测量精度和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统心理评估主要依赖结构化评分量表，无法充分利用受访者自然语言的丰富信息。本研究旨在利用LLM技术挖掘定性数据的价值，增强传统心理测量工具。

Method: 开发了一个结合LLM评分文本和传统评分量表项目的新概念框架，在真实世界高中生样本(n=693)和合成数据集(n=3,000)上进行验证，通过计算项目信息量来经验性地选择最有信息的LLM评分指令。

Result: 在保留测试集上，增强测试在测量精度和准确性方面实现了统计学显著改进。LLM项目带来的信息增益相当于在原19项测试基础上增加了6.3(真实数据)至16.0(合成数据)个项目。

Conclusion: 该方法标志着自动化评分的概念转变，绕过了传统瓶颈，为利用日益增长的转录文本数据增强传统心理测量提供了可扩展的框架，在临床健康等领域具有潜在应用价值。
...
TL;DR: dInfer是一个用于扩散式大语言模型(dLLM)的高效推理框架，通过模块化设计和系统优化，在保持输出质量的同时显著提升推理速度，相比现有系统有10倍加速，甚至比优化后的自回归模型快2-3倍。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型(dLLM)作为自回归模型的有前景替代方案，具有内在并行性优势，但由于缺乏标准化和高效的推理框架，其广泛应用受到限制。

Method: dInfer将推理流水线分解为四个模块化组件：模型、扩散迭代管理器、解码策略和KV缓存管理器，并为每个组件集成新算法和系统级优化。

Result: 在8×H800 GPU上，dInfer在HumanEval上达到每秒1100+个token，六个基准测试平均超过800 token/秒。相比Fast-dLLM有10倍加速，相比优化后的自回归模型QWen2.5-3B仍有2-3倍速度优势。

Conclusion: dInfer通过算法创新和系统优化的结合，为扩散式大语言模型提供了高效且可扩展的推理框架，显著提升了推理效率而不影响输出质量。
...
TL;DR: 本文首次对代码LLM的缩放规律进行大规模实证研究，发现代码比自然语言更数据饥渴，需要更高的数据参数比，且Farseer定律比Chinchilla定律更准确。


<details>
  <summary>Details</summary>
Motivation: 代码LLM正在改变软件工程，但现有的缩放规律主要基于自然语言分析。由于代码和自然语言在严格语法等方面存在根本差异，不清楚这些规律是否直接适用于代码。

Method: 进行了117次实验运行，模型规模从0.2B到3.8B，训练token从2B到128B，拟合了Chinchilla定律和Farseer定律，并进行了代码-自然语言混合实验。

Result: 1) Farseer定律比Chinchilla定律更准确；2) 代码LLM能有效随模型规模扩展；3) 代码比自然语言更数据饥渴，需要更高的数据参数比；4) 自然语言在资源受限时有益，但在高计算预算时反而有害。

Conclusion: 代码LLM的缩放规律与自然语言不同，需要专门的分析和优化策略，特别是在数据需求和混合训练方面。
...
TL;DR: 本文提出了一个三阶段推理框架来分析法律案例之间的显著区别，评估大型语言模型在类比推理中的表现，发现模型在表面推理上表现良好，但在层次推理和综合分析上表现显著下降，且错误回答消耗更多计算资源。


<details>
  <summary>Details</summary>
Motivation: 案例推理是美国法律实践的核心，需要专业人员通过类比和区分先例来论证当前案例。虽然大型语言模型展现出强大能力，但它们在复杂、细致的推理形式中的熟练度仍需进一步研究。

Method: 提出了一个正式框架，将识别案例间显著区别的过程分解为三阶段推理任务：使用事实谓词（因素）建模案例，将其组织到法律知识层次结构中，并定义可验证的规则来识别区别、分析其论证支持并评估其重要性。

Result: 评估显示模型在表面推理（任务1）上准确率高，但在层次推理（任务2：64.82%-92.09%）和综合分析（任务3：11.46%-33.99%）上表现下降。最引人注目的是，模型在错误回答上消耗的计算资源比正确回答更多。

Conclusion: 研究为复杂领域中LLM推理能力的细粒度分析提供了方法，并揭示了必须解决的基本限制，以实现稳健和可信的法律AI。
...
TL;DR: 本文提出了一个框架来构建紧凑、多样且抗分数膨胀的测试用例基准，通过寻找二进制代码-测试矩阵中的最优诊断基，解决了现有基准的高计算成本、分数膨胀和偏向简单错误的问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM自动生成测试用例的基准存在高计算成本、分数膨胀和偏向简单错误而非关键罕见故障的问题，需要更有效的评估方法。

Method: 提出一个框架，将基准构建形式化为在二进制代码-测试矩阵中寻找最优诊断基的问题，矩阵的秩指定了最小独立错误模式数量。提出了WrongSelect算法来选择最大多样性的错误代码，并构建了TC-Bench基准。

Result: 实验表明，即使最先进的测试用例生成方法在TC-Bench上也只能达到约60%的排除率，暴露了其诊断能力的显著差距。

Conclusion: 该框架能够构建紧凑、多样且抗分数膨胀的基准，有效评估测试用例生成方法的诊断能力，揭示了现有方法的局限性。
...
TL;DR: 该论文研究了微基准测试在语言模型评估中的可靠性问题，发现现有方法在模型排名一致性方面存在局限，特别是对于性能相近的模型对。


<details>
  <summary>Details</summary>
Motivation: 解决语言模型开发中全基准测试成本过高的问题，探究微基准测试能否像完整基准测试一样一致地排名模型，以及是否比随机选择数据点更有效。

Method: 引入了一种微基准测试的元评估方法，分析微基准测试在模型性能差异函数下对模型对的排名能力，比较不同微基准测试方法与随机抽样的效果。

Result: 发现现有微基准测试方法无法一致地排名MMLU-Pro上3.5分准确率差异或BIG-bench Hard上4分差异的模型对；需要多达250个示例才能一致排名性能相近的模型对，此时随机抽样与现有方法竞争力相当；在25个示例的MMLU-Pro微基准测试中，超过一半的8B指令调优模型对比较可能无法保持一致性。

Conclusion: 为微基准测试用户和开发者提供了在评估效率与可靠性之间权衡的可操作指导，强调了微基准测试在模型排名一致性方面的局限性。
...
TL;DR: 本文提出了一种基于LLM的方法来处理组合位置引用的地理编码任务，通过评估LLM的地理空间知识与推理能力，开发出改进性能的策略，并证明较小规模的微调LLM可以达到与大型现成模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 地理编码是将位置引用链接到实际地理位置的任务，对于非结构化文本的后续分析至关重要。本文特别关注组合位置引用这一具有挑战性的地理编码场景。

Method: 基于LLM在地理空间数据推理方面的能力评估，提出了一个基于LLM的策略来处理组合位置引用的地理编码任务，包括对相对较小的LLM进行微调。

Result: 该方法提高了组合位置引用地理编码任务的性能，并且相对较小的微调LLM能够与更大的现成模型达到相当的性能水平。

Conclusion: LLM在地理编码任务中展现出强大的潜力，特别是对于组合位置引用这一复杂场景，通过适当的策略和微调，较小规模的模型也能获得优异的性能。
...
TL;DR: 本研究评估了前沿和领先开源模型在低资源和高资源语言中的五个维度表现，发现GPT-5在各项类别中表现最佳，而其他模型在不同语言和类别间存在不一致性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在全球多语言环境中的广泛应用，需要理解和防护其多语言响应，评估和基准测试多语言响应能力的需求日益增加。

Method: 使用五点评分标准和法官LLM，评估前沿和领先开源模型在低资源和高资源语言中五个维度的响应准确性和一致性。

Result: GPT-5在各项类别中平均表现最佳，特别是在同意与自主权和伤害预防与安全类别中得分最高（3.56和4.73），而Gemini 2.5 Pro表现最差（1.39和1.98）。

Conclusion: 研究结果强调了需要进一步测试语言变化如何影响LLM在不同类别中的响应，并需要在这些领域进行改进。
...
<details>
  <summary>Details</summary>
Motivation: Transformer注意力机制随序列长度呈二次方增长O(n^2)，限制了长上下文的使用。

Method: 使用Bernoulli门通过Hard-Concrete/变分松弛进行训练，在推理时通过简单的top-M规则强制执行，使方法可微分且可直接替换标准编码器。

Result: 在分类、抽取式问答和长文档摘要任务中，仅保留30-50%的token就能保持≥95%的完整模型性能，同时峰值内存减少约35-45%，吞吐量提升高达约1.8倍。
...
TL;DR: 提出了中文常识多跳推理基准CCMOR，用于评估LLMs在中文语境下整合事实知识与多步推理的能力，通过专家验证确保数据质量，发现LLMs在处理长尾知识和知识密集型推理方面存在局限，检索增强生成能显著改善性能。
...
Method: 首先从现有QA数据集构建领域平衡的种子集，然后开发基于LLM的流水线生成基于事实单元链的多跳问题，最后通过专家参与的人类循环验证系统来确保数据质量。

Result: 评估显示最先进的LLMs在处理长尾知识和执行知识密集型推理方面存在持续局限，检索增强生成能显著缓解知识差距，带来明显的性能提升。

Conclusion: CCMOR基准揭示了LLMs在中文多跳推理中的知识处理局限，检索增强是有效的改进方法，为未来模型开发提供了重要参考。
...
TL;DR: MOSAIC是一个用于解决挑战性科学编码任务的多智能体大语言模型框架，通过专门设计的智能体在师生范式下进行自我反思、创建原理、编码和调试，无需训练即可处理需要领域知识和算法迭代的科学工作流。


<details>
  <summary>Details</summary>
Motivation: 科学工作流与通用编程不同，需要严谨的算法、深厚的领域知识、领域特定推理以及无需I/O测试用例的算法迭代。许多科学问题需要解决一系列子问题才能得到最终结果，现有方法难以满足这些需求。

Method: MOSAIC采用无训练框架设计，包含专门设计的智能体在师生范式下工作，支持逐步问题分解、针对性错误修正，并结合统一上下文窗口来减少LLM在解决涉及链式子问题的复杂科学任务时的幻觉问题。

Result: 在科学编码基准测试中，MOSAIC在准确性、鲁棒性和可解释性方面优于现有方法。

Conclusion: MOSAIC框架通过多智能体设计和师生范式，有效解决了科学编码任务的特殊挑战，为复杂科学问题的自动化编码提供了可行方案。
...
TL;DR: 本文研究了不同语言结构对LLM隐私泄露的影响，发现意大利语泄露最严重，英语成员推断分离性最高，而法语和西班牙语因形态复杂性更高而更具韧性。
...
Method: 在英语、西班牙语、法语和意大利语医学语料库训练的LLM上，量化六个语言指标，评估三种攻击向量：提取、反事实记忆和成员推断。

Result: 隐私漏洞与语言冗余和分词粒度成正比：意大利语泄露最强，英语成员分离性最高，法语和西班牙语因形态复杂性而更具韧性。
...
TL;DR: SoG是一个简单有效的框架，让LLM能够在知识图谱上进行迭代式图导航，通过单一的Search函数实现"观察-导航"原则，在多个KGQA基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在知识密集型多跳问答中的不可靠性问题（如遗漏长尾事实、产生幻觉、知识滞后），以及现有KGQA方法在查询规划、子图检索和代理框架方面的局限性。

Method: 提出Search-on-Graph框架，采用"观察-导航"原则：在每个步骤中，LLM检查当前实体的实际可用关系，然后决定下一跳，无需预规划路径或检索大子图。

Result: 在六个KGQA基准测试中（包括Freebase和Wikidata），SoG无需微调即达到最先进性能，特别是在Wikidata基准上比之前最佳方法提升16%，在Freebase基准上也有持续改进。

Conclusion: SoG框架通过简单的迭代图导航方法有效解决了KGQA中的关键挑战，展示了在复杂知识图谱问答任务中的强大能力。
...
TL;DR: PE-CoA是一个多轮越狱攻击框架，通过五种对话模式构建有效的多轮越狱，在12个LLM和10个危害类别上实现最先进性能，揭示了模式特定的漏洞和LLM行为特征。


<details>
  <summary>Details</summary>
Motivation: 现有多轮越狱方法依赖启发式或临时探索策略，对模型底层弱点提供有限洞察，对话模式与模型漏洞之间的关系理解不足。

Method: 提出PE-CoA框架，包含五种对话模式来通过自然对话构建有效的多轮越狱攻击。

Result: 在12个LLM和10个危害类别上评估，实现最先进性能，发现模型对特定对话模式存在独特弱点，且模型家族共享相似的失败模式。

Conclusion: 这些发现突显了安全训练的局限性，表明需要模式感知的防御方法。
...
TL;DR: 本文研究了在文档级别机器翻译中应用质量估计重排序的效果，发现使用学习型指标SLIDE和LLM指标GEMBA-DA都能显著提升翻译质量，特别是在使用更多候选翻译时效果更佳。


<details>
  <summary>Details</summary>
Motivation: 质量估计重排序在句子级别机器翻译中已被证明有效，但在日益重要的文档级别翻译中的应用仍未被充分探索。

Method: 评估了多种学习型和基于大语言模型的质量估计指标在文档级别翻译重排序中的表现，包括SLIDE和GEMBA-DA等指标，使用不同数量的候选翻译进行实验。

Result: 使用最佳学习型指标SLIDE时，BLEURT-20分数在使用2个候选时提升+2.00，使用32个候选时提升+5.09；使用最佳LLM指标GEMBA-DA时，分别提升+1.63和+4.30。虽然在长输入下增益有所减少，但在最长文档上仍能获得显著改进。

Conclusion: 文档级别的质量估计重排序具有实际应用价值，在合适的翻译模型和硬件条件下，只需最小的运行时开销就能显著提升翻译质量。
...
TL;DR: 本文提出了一种在强化学习中为LLMs的不同类型token应用不同温度设置的方法，对推理token使用更高温度以鼓励探索，对知识token使用较低温度以保持事实准确性，从而显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过限制更新来间接鼓励探索，但未在token生成阶段本身显式促进探索行为。研究发现LLMs中的token在推理任务中扮演不同角色，可分为高熵推理token和低熵知识token。

Method: 引入互补方法，在采样过程中显式促进探索：对推理token应用更高温度设置以主动鼓励探索，对知识token保持较低温度以维持事实正确性；系统研究多种多温度调度策略及其在强化学习环境中的影响。

Result: 在多个推理基准测试上的实证评估表明，该方法显著提升了LLMs的推理性能。

Conclusion: 通过为不同类型token应用差异化温度设置的方法，能够有效提升LLMs在强化学习环境中的推理能力，代码已开源。
...
TL;DR: 提出基于大语言模型的统一生物医学命名实体识别框架，通过文本生成任务重构、符号化标注策略、双语联合微调和对比学习实体选择器，在多个基准数据集上实现最先进性能，并具备跨语言的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学命名实体识别方法在处理嵌套实体、实体边界模糊和跨语言泛化方面存在困难，需要开发更强大的统一框架来解决这些问题。

Method: 将BioNER重构为文本生成任务，设计符号化标注策略处理平面和嵌套实体；进行中英文数据集的双语联合微调；引入基于对比学习的实体选择器，利用边界敏感的正负样本过滤错误预测。

Result: 在四个基准数据集和两个未见语料库上的实验结果表明，该方法实现了最先进的性能，并具备跨语言的鲁棒零样本泛化能力。

Conclusion: 提出的统一BioNER框架有效解决了嵌套实体、边界模糊和跨语言泛化问题，在生物医学命名实体识别任务上表现出色，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于自编码任务的上下文压缩方法存在优化目标与下游任务不匹配的问题，削弱了对实际使用更有益的特征。

Method: SAC直接选择原始上下文中的锚点标记，将上下文信息聚合到其KV表示中，包含锚点嵌入和双向注意力修改两个关键设计。

Result: 实验结果表明SAC在各种压缩比下都优于现有方法，在MRQA的分布外评估中，5倍压缩时比强基线提高1个EM分数，且在更高压缩比下优势更明显。

Conclusion: SAC通过消除自编码训练需求，提供了一种更有效的上下文压缩方法，在保持性能的同时实现了更好的压缩效果。
...
TL;DR: 该论文研究了LLM内部表征中类似人类印象和刻板印象的人工印象模式，使用线性探针预测基于刻板印象内容模型的印象，并探讨这些印象与下游模型行为的关系。


<details>
  <summary>Details</summary>
Motivation: 研究LLM内部表征中是否存在类似人类印象和刻板印象的模式，以及这些模式如何影响模型行为。

Method: 使用线性探针在生成的提示上预测基于刻板印象内容模型(SCM)的印象，分析隐藏表征中印象的可解码性，并研究提示特征对印象的影响。

Result: 发现LLM在提示时不一致地报告印象，但从隐藏表征中可更一致地线性解码印象；人工印象能预测模型响应的质量和模糊语言使用；特定内容、风格和方言特征会影响LLM印象。

Conclusion: LLM内部表征中存在可解码的人工印象模式，这些印象影响模型行为，提示特征会塑造LLM的印象形成。
...
TL;DR: 提出了SOP-Maze基准，从真实商业数据构建了397个任务，涵盖23个复杂标准操作流程场景，将任务分为横向根系统（宽选项任务）和核心根系统（深度逻辑推理任务）。实验显示当前最先进模型在此基准上表现不佳，主要存在路径盲区、对话脆弱性和计算错误三类问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估LLM在一般指令遵循和决策方面的能力，但在涉及复杂标准操作流程（SOP）的商业场景中，LLM的能力评估尚未充分探索。需要专门基准来评估LLM在真实商业SOP环境中的表现。

Method: 从真实商业数据构建SOP-Maze基准，包含397个任务，涵盖23个复杂SOP场景。将任务分为两类：横向根系统（LRS）强调精确选择的宽选项任务；核心根系统（HRS）强调具有复杂分支的深度逻辑推理任务。

Result: 广泛实验表明，几乎所有最先进的模型在SOP-Maze基准上都表现不佳。识别出三个关键错误类别：路径盲区（难以遵循程序）、对话脆弱性（无法处理真实对话细节）、计算错误（复杂情境下的时间或算术推理错误）。

Conclusion: 系统研究探索了LLM在挑战广度和深度的SOP任务中的表现，为改进模型能力提供了新见解。该工作已在GitHub上开源。
...
TL;DR: 本研究通过分析710个开源项目的治理文档，使用熵值、丰富度和Jensen Shannon散度等指标来衡量治理结构的变化，发现项目随时间增加角色和行动定义，分布更均衡，但规则组成保持稳定。


<details>
  <summary>Details</summary>
Motivation: 研究开源社区如何通过版本控制的治理文档描述参与和控制机制，为评估未来AI中介工作流是否集中或重新分配权力提供可复现的基线。

Method: 使用710个项目的配对快照语料库，将文本解析为参与者、规则、行动和对象，然后通过熵值（均匀性）、丰富度（多样性）和Jensen Shannon散度（漂移）进行分组和变化测量。

Result: 项目随时间定义了更多角色和行动，且分布更加均匀，但规则组成保持稳定。治理通过扩展和平衡参与类别而增长，没有发生重大规范性转变。

Conclusion: 治理通过扩展和平衡参与类别而增长，没有发生重大规范性转变，为评估未来AI中介工作流是否集中或重新分配权力提供了可复现的基线。
...
TL;DR: CAPC-CG是中国首个开放的政策指令数据集，包含1949-2023年间中国中央政府发布的政策文件，标注了清晰与模糊语言的五色分类法，包含330万个段落单元，并提供了完整的元数据和标注框架。


<details>
  <summary>Details</summary>
Motivation: 建立首个中文政策指令数据集，支持政策沟通研究和多语言NLP研究，填补中文政策语言分析的数据空白。

Method: 基于Ang的适应性政策沟通理论，采用五色分类法对政策语言进行标注，使用专家和训练有素的标注员进行两轮标注，确保标注质量。

Result: 构建了包含330万个段落单元的语料库，标注者间一致性达到Fleiss's kappa K=0.86，提供了基线分类结果和标注手册。

Conclusion: CAPC-CG为政策沟通的下游任务和多语言NLP研究提供了重要资源，支持监督建模和政策语言模式分析。
...
TL;DR: MASA是一个基于大型语言模型的多智能体自动形式化框架，通过协作智能体将自然语言语句转换为形式化表示，具有模块化、灵活性和可扩展性特点。


<details>
  <summary>Details</summary>
Motivation: 自动形式化在连接自然语言和形式推理中起着关键作用，需要解决如何有效利用LLMs和定理证明器来提高自动形式化的效率和可靠性。

Method: 提出MASA多智能体系统框架，利用协作智能体进行自然语言到形式化表示的转换，强调模块化架构设计，支持新智能体和工具的集成。

Result: 通过真实世界数学定义的用例和形式数学数据集的实验展示了MASA的有效性，证明了多智能体系统在自动形式化中的潜力。

Conclusion: 这项工作突显了由LLMs和定理证明器交互驱动的多智能体系统在提高自动形式化效率和可靠性方面的潜力，为该领域的研究者和实践者提供了有价值的见解和支持。
...
TL;DR: 本文分析了现有基于GRPO的RLVR方法存在权重分配问题，提出了DARO方法动态调整不同难度样本的损失权重，在多个数学基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法（如GRPO及其变体）使用静态或过于简化的权重方案，无法适应模型能力的变化，导致训练在不同难度级别上失衡，限制了整体性能提升。
...
Conclusion: DARO通过动态权重调整有效解决了现有RLVR方法的局限性，为强化学习与可验证奖励的结合提供了更有效的训练框架。
...
TL;DR: 本文提出基于LoRA的拒绝训练方法，能够仅使用安全数据进行性能保持的安全对齐，LoRA可作为低成本、性能保持、即插即用的安全补丁。


<details>
  <summary>Details</summary>
Motivation: 安全对齐对于构建可信AI至关重要，但当前方法需要在安全关键数据和通用数据之间寻找最优比例，计算成本高且收益有限。

Method: 采用LoRA（低秩适应）基础的拒绝训练方法，仅使用安全数据进行训练，将安全能力解耦到与模型内在变换空间正交的低秩子空间。

Result: 实验证明LoRA能够有效将安全能力解耦到低秩子空间，确保安全增强不会干扰模型的固有能力。

Conclusion: LoRA可作为高效、性能保持的安全对齐解决方案，提供理论和实验证据支持其有效性。
...
TL;DR: LitE-SQL是一个轻量高效的Text-to-SQL框架，包含模式检索器和SQL生成器，在BIRD和Spider数据集上表现优异，参数比LLM方法少2-30倍，适合隐私敏感和资源受限场景。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于大型语言模型的Text-to-SQL方法依赖专有模型导致的部署可行性和数据隐私问题。

Method: 包含两个组件：使用预计算模式嵌入向量数据库的模式检索器，以及经过两阶段微调（监督微调+执行引导强化学习）的SQL生成器，支持自我修正而无需昂贵的多候选生成。

Result: 在BIRD上达到72.10%执行准确率，在Spider 1.0上达到88.45%，性能与LLM方法相当或更优，但参数少2-30倍。
...
TL;DR: 本研究通过迭代优化评分标准来提升LLM在自动作文评分中的表现，在TOEFL11和ASAP数据集上分别实现了高达0.19和0.47的QWK提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)的性能对提示词高度敏感，受提示优化领域启发，本研究探索通过优化评分标准来提升自动作文评分(AES)的效果。

Method: 采用迭代优化方法，让模型基于自身的评分理由和与人工评分的差异来不断改进评分标准，在TOEFL11和ASAP数据集上使用GPT-4.1、Gemini-2.5-Pro和Qwen-3-Next-80B-A3B-Instruct进行实验。

Result: 使用简单初始评分标准的方法取得了与详细人工编写评分标准相当或更好的二次加权Kappa(QWK)分数，在TOEFL11和ASAP数据集上分别实现了高达0.19和0.47的QWK提升。

Conclusion: 研究结果表明迭代评分标准优化在基于LLM的自动作文评分中对于提升与人工评估的一致性至关重要。
...
TL;DR: 该研究针对资源稀缺的查克马语，构建了一个经过母语者验证的孟加拉语转写查克马语语料库，并在六个多语言和区域Transformer模型上进行微调实验，结果显示微调后的模型在查克马语任务上表现显著提升。


<details>
  <summary>Details</summary>
Motivation: 查克马语作为一种印度-雅利安语系语言，可用数据有限，在语言模型中代表性不足，需要开发有效的方法来支持这种低资源语言的自然语言处理研究。

Method: 从查克马文学中收集语料，经过母语者验证构建孟加拉语转写的查克马语数据集，然后在六个编码器模型（mBERT、XLM-RoBERTa、DistilBERT、DeBERTaV3、BanglaBERT和IndicBERT）上进行掩码语言建模任务的微调。

Result: 实验表明，经过微调的多语言模型在孟加拉语转写查克马语上表现优于预训练模型，最高达到73.54%的token准确率和低至2.90的困惑度。同时分析了数据质量对模型性能的影响以及OCR流程在形态丰富的印度文字上的局限性。

Conclusion: 孟加拉语转写的查克马语对于查克马语的迁移学习非常有效，研究发布了经过人工验证的单语数据集以促进低资源语言的多语言建模研究。
...
TL;DR: LLMs无法可靠地区分事实和幻觉，因为当幻觉与主题知识相关时，其内部处理过程与正确回答相同，导致无法检测；只有当幻觉与主题知识无关时才能被检测。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否真的"知道它们不知道什么"，探索内部计算是否能可靠区分事实和幻觉输出。

Method: 对LLMs处理事实查询进行机制分析，比较基于主题知识依赖的两种幻觉类型，分析隐藏状态几何结构。

Result: 当幻觉与主题知识相关时，LLMs使用与正确回答相同的内部回忆过程，隐藏状态几何重叠且无法区分；与主题知识无关的幻觉产生独特的聚类表示，可被检测。

Conclusion: LLMs不在内部状态中编码真实性，只编码知识回忆模式，证明"LLMs并不真正知道它们不知道什么"。
...
TL;DR: 提出了Alif-1.0-8B-Instruct模型，这是一个针对乌尔都语-英语的多语言模型，通过改进的自指导技术生成高质量合成数据集，在低预算下显著提升了乌尔都语任务性能
...
Conclusion: 通过改进的自指导方法，可以高效开发高性能、低资源语言的大语言模型，并实现文化对齐，所有数据集、模型和代码均已公开
...
TL;DR: ReFIne是一个训练框架，通过监督微调和GRPO结合，提升推理模型的可解释性、忠实性和可靠性，使模型生成结构化推理轨迹、明确决策信息并提供置信度评估。


<details>
  <summary>Details</summary>
Motivation: 当前长链思维推理研究过于关注准确性和效率，忽视了可信赖性的关键方面。可信赖的推理系统应具备可解释性、忠实性和可靠性。

Method: 提出ReFIne训练框架，结合监督微调和GRPO，鼓励模型：生成结构化标签轨迹提高可解释性；明确披露决策信息增强忠实性；提供自我评估提升可靠性。

Result: 在Qwen3模型(1.7B/4B/8B)上实验显示：可解释性提升44.0%，忠实性提升18.8%，可靠性提升42.4%，生成更清晰的结构化推理轨迹。

Conclusion: 推理模型不仅应优化准确性，还应关注可信赖性的多个维度。ReFIne框架有效提升了模型的可解释性、忠实性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然因果语言模型（如GPT、Llama系列）在广泛的语言理解任务中表现出色，但尚未应用于语义框架归纳任务。本文旨在探索CLMs在语义框架归纳中的潜力。

Method: 提出FrameEOL方法：1）使用提示工程获取框架嵌入；2）利用上下文学习（ICL）和深度度量学习（DML）优化嵌入质量；3）通过聚类实现框架归纳。

Result: 在英语和日语FrameNet数据集上的实验结果表明，该方法优于现有框架归纳方法。特别是对于资源匮乏的日语，仅使用5个ICL示例的CLM方法就达到了与使用DML微调的MLM方法相当的性能。

Conclusion: CLMs在语义框架归纳任务中具有显著潜力，特别是在资源有限的语言中，通过少量示例就能获得良好性能，为框架语义分析提供了新的有效途径。
...
TL;DR: 这篇论文对检索增强生成(RAG)进行了全面综述，分析了RAG的目标、组件、关键挑战以及与传统LLMs相比的优势，特别是在处理动态信息和领域特定查询方面的应用价值。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在静态语料库上训练，难以处理快速演化的信息或领域特定查询。RAG通过将LLMs与外部检索机制结合来克服这一限制，但传统RAG框架的相对优势随着LLMs能力的提升而减弱。

Method: 论文采用综述方法，首先阐述RAG的总体目标和核心组件，然后分析RAG中的关键挑战和限制其有效性的关键弱点。

Result: 展示了LLMs单独表现不足但结合RAG能显著提升效果的多个应用场景，证明了RAG在增强LLMs处理动态和特定领域信息方面的价值。
...
TL;DR: DITING是一个专门评估网络小说翻译的综合框架，包含六个维度评估叙事和文化保真度，并提出了AgentEval多智能体评估框架和MetricAlign元评估数据集。研究发现中文训练的LLM在翻译表现上优于更大的外国模型，DeepSeek-V3提供最忠实和风格一致的翻译。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译基准依赖表面指标，无法捕捉网络小说这一独特体裁的特征，需要专门评估框架来评估其叙事和文化保真度。

Method: 1) 引入DITING评估框架，在六个维度评估翻译质量；2) 提出AgentEval多智能体评估框架，模拟专家审议；3) 开发MetricAlign元评估数据集；4) 综合评估14个开放、闭源和商业模型。

Result: 中文训练的LLM超越更大的外国对应模型，DeepSeek-V3提供最忠实和风格一致的翻译。AgentEval在七个测试的自动指标中与人类判断相关性最高。

Conclusion: 该研究为探索基于LLM的网络小说翻译建立了新范式，并提供了公共资源以推进未来研究。
...
TL;DR: 本文提出了两种互补的策略来构建更强的文本重识别攻击：一是通过聚合多个PII识别顺序的预测来提高结果，二是利用推理模型在拥有广泛背景知识时提升重识别性能。
...
Method: 提出了两种策略：(1) 考虑PII跨度重识别顺序的重要性，通过聚合多个顺序的预测来改进结果；(2) 使用推理模型，特别是在攻击者拥有广泛背景知识时提升重识别性能。

Result: 这些策略能够构建更强的重识别攻击，有效测试文本去标识化方法的鲁棒性。

Conclusion: 通过优化重识别顺序和利用推理模型，可以显著提升文本重识别攻击的效果，为评估去标识化技术的安全性提供更有效的方法。
...
TL;DR: 提出了一种新颖的翻译增强方法，通过从指令模型开始并仅在平行数据上应用层选择性调优，开发了Qwen3-XPlus模型。该模型在高低资源语言翻译任务中表现优异，同时在多语言任务上保持改进，推理能力与原始指令模型相当。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型擅长推理，但专门优化的翻译模型在推理任务上表现不佳。为了解决这个问题，需要开发一种既能提升翻译性能又不损害推理能力的方法。

Method: 提出翻译增强配方：从指令模型开始，仅在平行数据上应用层选择性调优。基于此流程开发了Qwen3-XPlus模型，仅使用小规模平行数据集进行训练。

Result: Qwen3-XPlus在高低资源语言翻译中表现显著提升，在低资源语言（如斯瓦希里语）上达到15+ spBLEU和40+ xComet。在7个多语言任务上平均提升1+分，同时在15个流行推理数据集上保持与Qwen3指令模型相当的能力。

Conclusion: 这项工作为多语言增强提供了一种有前景的方法，显著降低了复杂性，提高了更多语言的可访问性。代码和模型已公开可用。
...
TL;DR: DICE是一个轻量级框架，通过思维链修正来指导小型语言模型优化大型语言模型的输出，解决LLMs在推理任务中忽视用户特定格式要求的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在执行推理任务时往往优先考虑推理过程而忽视详细的输出格式要求，而通过监督数据微调LLMs又面临计算成本高和参数访问受限的问题。

Method: DICE采用解耦方法：首先提示LLMs生成自然语言响应，然后使用训练好的SLMs分析和精炼这些输出以满足结构化输出规范。通过两阶段方法构建结构化思维链适应数据集，并应用双重调优策略微调SLMs以分析-回答模式生成结构化输出。

Result: 实验表明，DICE将LLM输出的平均格式准确性和内容正确性分别提高了35.4%和29.4%，在竞争基准上实现了最先进的性能。

Conclusion: DICE框架在保持LLMs广泛知识和推理能力的同时，确保输出符合用户需求，为处理结构化输出要求提供了一种有效的轻量级解决方案。
...
TL;DR: IRIS是一个新颖的因果发现框架，结合统计算法和LLM方法，能够从初始变量自动发现已知和新型因果关系，并识别缺失变量来扩展因果图，实现无需预存数据的实时因果发现。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法面临数据收集成本高、对已知关系重复计算、假设不现实等挑战，而现有LLM方法只能识别常见因果关系，无法发现新型关系。

Method: IRIS框架从初始变量开始，自动收集相关文档、提取变量、发现因果关系，采用统计算法与LLM结合的混合方法，并包含缺失变量提案组件来扩展因果图。

Result: 该方法能够实现无需预存数据集的实时因果发现，既能发现已知因果关系，也能识别新型因果关系。

Conclusion: IRIS框架有效解决了传统因果发现方法的局限性，为科学研究的因果发现提供了更高效和全面的解决方案。
...
TL;DR: 本文提出了CrisiText，这是第一个用于在13种不同危机场景中生成警告消息的大规模数据集，包含超过40万条警告消息，旨在帮助平民在危机期间和之后。


<details>
  <summary>Details</summary>
Motivation: 在危机情况下有效识别威胁并减轻潜在损害对于保护受威胁个体至关重要。虽然AI已被用于协助人类处理紧急情况，但NLP技术的应用仍然有限，主要集中在分类任务上，而使用NLG架构及时生成警告消息的潜力在很大程度上被忽视了。

Method: 通过从现有危机描述创建事件链，为每个事件配对警告消息来生成数据集。生成遵循专家书面指南以确保正确的术语和事实性。此外，每条消息都伴随三种次优警告类型，以研究不同的NLG方法。进行了一系列实验，比较了监督微调设置与偏好对齐、零样本和少样本方法。

Result: 创建了包含超过40万条警告消息（涵盖近1.8万个危机情况）的大规模数据集，并评估了不同NLG方法的性能，包括在分布外场景中的表现以及自动后编辑器的有效性。

Conclusion: CrisiText数据集填补了危机情况下NLG应用的空白，为研究不同NLG方法在警告消息生成中的有效性提供了基础，并展示了在危机管理中使用NLG技术的潜力。
...
TL;DR: 本文提出DSPO算法，通过序列级优化和动态样本过滤来增强LLM的外部知识搜索能力，在多个QA基准测试中显著提升性能，7B模型比之前工作提升34.1%，在复杂多跳QA中甚至超越14B模型9%。


<details>
  <summary>Details</summary>
Motivation: 当前方法要么依赖提示来激发模型的固有代理能力，要么在将RL应用于复杂交互任务时存在性能上限和崩溃问题，未能充分发挥其代理潜力。

Method: 引入动态过滤序列级策略优化(DSPO)，通过序列级优化和动态样本过滤进行鲁棒代理训练，纯RL训练实现多轮搜索和推理的交织，无需监督演示数据。

Result: 在多个QA基准测试中，7B模型比可比先前工作提升34.1%，在HotpotQA等复杂多跳QA中甚至超越之前工作的14B模型近9%，且保持出色的训练稳定性。

Conclusion: DSPO算法能有效增强LLM的外部知识搜索能力，在复杂任务中实现显著性能提升，证明了纯RL训练在开发模型代理潜力方面的有效性。
...
TL;DR: 本文提出了Self-Critique方法，这是首个针对强化学习后训练阶段数据污染检测的系统研究。该方法通过检测模型输出熵分布的崩溃来识别污染数据，在RL-MIA基准测试中显著优于基线方法，AUC提升高达30%。


<details>
  <summary>Details</summary>
Motivation: 数据污染严重威胁大语言模型评估的可靠性。虽然已有方法检测预训练和监督微调阶段的污染，但强化学习后训练阶段缺乏专门的污染检测方法，这成为一个关键漏洞。

Method: 提出Self-Critique方法，基于关键观察：RL后训练后，LLM的输出熵分布会崩溃为高度特定和稀疏的模式。该方法通过探测底层策略崩溃（模型收敛到狭窄推理路径）来检测污染。

Result: 在RL-MIA基准测试中，Self-Critique显著优于基线方法，在多个模型和污染任务上AUC提升高达30%。现有方法对RL阶段污染接近随机猜测，而该方法使检测成为可能。

Conclusion: Self-Critique是首个专门针对RL后训练阶段数据污染检测的有效方法，填补了该领域的重要研究空白，为可靠评估LLM提供了关键工具。
...
TL;DR: 提出了CFVBench基准测试，评估多模态检索增强生成模型在视频理解中的能力，发现现有模型在捕捉细粒度多模态细节方面存在瓶颈，并提出自适应视觉优化框架AVR来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频MRAG基准测试在模态覆盖和格式多样性方面存在局限，通常关注单一或有限模态任务，或粗粒度场景理解，需要更全面的评估框架。

Method: 构建CFVBench基准测试，包含599个公开视频和5360个开放QA对，涵盖图表密集型报告、新闻广播和软件教程等高密度格式；提出自适应视觉优化框架AVR，通过自适应增加帧采样密度和选择性调用外部工具来提升细粒度多模态理解。

Result: 系统评估了7种检索方法和14个常用MLLM，发现当前模型（包括GPT5和Gemini）在捕捉瞬态但关键的细粒度多模态细节方面存在困难；AVR框架在所有评估的MLLM中一致提升了细粒度多模态理解和性能。

Conclusion: CFVBench揭示了当前多模态模型在细粒度细节理解方面的关键瓶颈，AVR提供了一种简单有效的解决方案来提升模型的多模态理解能力。
...
TL;DR: DyReMe是一个用于医学诊断的动态基准测试，它通过生成类似真实咨询的病例、引入干扰因素和多样化表达风格，更准确地评估大语言模型在临床实践中的表现，包括准确性、真实性、帮助性和一致性四个维度。


<details>
  <summary>Details</summary>
Motivation: 当前基于静态医学考试题目的LLM评估方法与现实临床实践存在根本性脱节，高估了模型性能，忽视了教科书案例与真实世界模糊多变条件之间的差异。

Method: 提出DyReMe动态基准测试，生成新的咨询式病例，引入鉴别诊断和常见误诊因素等干扰项，并模拟多样化的真实世界查询习惯，从准确性、真实性、帮助性和一致性四个维度评估LLM。

Result: 实验表明这种动态方法提供了更具挑战性和真实性的评估，揭示了最先进LLM表现与现实临床实践之间的显著脱节。

Conclusion: 迫切需要能够更好反映可信赖医学诊断需求的评估框架，DyReMe为更准确的医学AI评估提供了重要方向。
...
TL;DR: CLARity是一个成本效益高的强化学习框架，使用小型通用LLM来增强专家模型在稀缺数据领域中的推理质量，通过一致性感知奖励机制和两阶段训练流程提高推理一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: 在数据稀缺领域训练专家LLM很困难，通常依赖多项选择题。但标准的基于结果的强化学习在MCQs上存在风险，虽然可能提高准确性，但经常降低推理质量如逻辑一致性。现有监督推理的解决方案（如大规模过程奖励模型）成本过高。

Method: CLARity框架包含：1）一致性感知奖励机制；2）两阶段精炼-监控训练流程；3）动态数据重构策略，以更好地利用有限数据。使用小型通用LLM来指导专家模型。

Result: 实验表明，CLARity相比基线方法提高了16.5%的响应一致性和7.5%的准确性。人工评估进一步确认了在连贯性和专业性方面的整体改进。

Conclusion: CLARity提供了一个通用解决方案，使较小模型能够通过推理一致性有效地指导专家模型。
...
TL;DR: 提出DualCSE方法，为每个句子分配两个嵌入向量：一个表示显式语义，一个表示隐式语义，以克服传统句子嵌入方法只能为每个句子分配单个向量的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有句子嵌入方法在捕捉句子中的隐式语义方面存在困难，这归因于传统方法只能为每个句子分配单个向量的固有局限性。

Method: 提出DualCSE方法，为每个句子分配两个共存于共享空间的嵌入向量：一个表示显式语义，另一个表示隐式语义，可根据特定任务需求选择相应的语义表示。

Result: 实验结果表明，DualCSE能够有效编码显式和隐式含义，并提高下游任务的性能。

Conclusion: DualCSE方法通过为句子分配双重嵌入向量，成功克服了传统句子嵌入方法在捕捉隐式语义方面的局限性，在信息检索和文本分类等任务中表现出更好的性能。
...
TL;DR: 本文提出了MaP框架，通过集成检查点合并和Pass@k指标来解决LLM预训练评估中的不稳定性问题，包括参数不稳定性和评估不稳定性，从而提供更可靠的训练动态观察。
...
Method: 提出MaP框架，采用双重策略：检查点合并通过平均最近模型权重来平滑参数空间，Pass@k指标提供低方差的统计能力估计。
...
TL;DR: ShiZhi是首个专门用于法院观点生成的大语言模型，在110K中文法律案例数据集上训练，在法院观点生成任务中达到58.5 BLEU-1分数，在罪名预测任务中达到86.1%准确率和92.5%宏F1分数。
...
Method: 构建了包含超过11万案例的中文法院观点生成数据集CCVG，每个案例包含事实描述和对应的法院观点。基于该数据集训练了专门的大语言模型ShiZhi。

Result: ShiZhi在法院观点生成任务中达到58.5 BLEU-1分数，在罪名预测任务中达到86.1%准确率和92.5%宏F1分数。
...
TL;DR: MaskKV是一个专为扩散大语言模型设计的训练无关缓存淘汰框架，通过掩码查询引导的评分机制和自适应缓存预算策略，显著减少KV缓存的内存占用和计算成本，在仅保留5%令牌的情况下仍能保持94%的性能。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然支持并行解码，但其双向注意力机制需要大量内存缓存，限制了在资源受限环境下处理长文本的能力。现有的缓存淘汰策略是为自回归模型设计的，不适用于扩散模型的特点。

Method: MaskKV包含两个关键创新：1) 掩码查询引导的评分机制，利用注意力权重识别并淘汰每个注意力头中不太重要的提示令牌；2) 自适应缓存预算策略，减少中间层的缓存分配，将资源集中在偏好提示的注意力头上。

Result: 在LLaDA模型上应用MaskKV，将KV缓存压缩到仅256对（少于5%的令牌），在LongBench上仍能保持94%的完整缓存性能，在32k提示长度下实现高达31倍的加速。

Conclusion: MaskKV有效解决了扩散大语言模型在长文本处理中的内存瓶颈问题，通过针对性的缓存优化策略实现了高性能和高效能的平衡。
...
TL;DR: 本文提出了一种基于电路推理验证的白盒方法，通过分析正确与错误推理步骤的归因图结构特征来检测和纠正LLM推理错误。


<details>
  <summary>Details</summary>
Motivation: 现有的CoT验证方法（黑盒或灰盒）只能预测推理正确性，但无法解释计算失败的原因，需要一种能深入理解推理过程的白盒方法。

Method: 提出电路推理验证方法，假设正确与错误CoT步骤的归因图具有不同的结构特征，通过训练分类器识别这些结构指纹来检测推理错误。

Result: 方法能有效预测推理错误，发现错误特征具有高度领域特异性，并能通过针对性干预成功纠正模型的错误推理。

Conclusion: 通过深入分析模型的计算过程，可以从简单的错误检测转向对LLM推理的因果理解，为理解模型推理机制提供了新视角。
...
TL;DR: FLRC是一种细粒度低秩压缩方法，通过为每个层分配最优秩并结合渐进低秩解码，在保持文本生成质量的同时显著提升LLM压缩性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的巨大参数量阻碍了在资源受限硬件上的部署，而现有的均匀压缩方法会导致显著性能下降，且在解码阶段表现不佳。

Method: 提出细粒度低秩压缩器(FLRC)，为每个层高效确定最优秩分配，并整合渐进低秩解码技术来维持文本生成质量。

Result: 在多样化基准测试中，FLRC相比最先进的低秩压缩方法在摘要任务上实现了高达17%的ROUGE-L提升。

Conclusion: FLRC建立了一个更稳健高效的框架来改进LLM推理，为资源受限环境下的模型部署提供了有效解决方案。
...
TL;DR: LLP是首个基于大语言模型的二手产品定价生成框架，通过检索相似产品和两阶段优化（监督微调+群体相对策略优化），显著提升了定价准确性和泛化能力，在闲鱼平台部署后效果显著。
...
Method: LLP框架首先检索相似产品，然后利用LLM对自由文本中关键定价信息的理解生成价格建议，通过两阶段优化（SFT+GRPO）增强领域推理能力，并采用基于置信度的过滤机制拒绝不可靠建议。

Result: LLP大幅超越现有方法，在闲鱼平台部署后，在30%产品覆盖率下将静态采纳率从40%提升到72%，在90%召回率下仍保持47%的采纳率。

Conclusion: LLP框架成功解决了二手产品定价问题，证明了LLM在动态市场环境中的有效性，并具有良好的泛化能力。
...
TL;DR: ReTraceQA是一个新的基准测试，引入过程级评估来衡量小型语言模型在常识推理中的推理过程有效性，发现SLMs经常在推理过程有缺陷的情况下给出正确答案，导致性能被高估。


<details>
  <summary>Details</summary>
Motivation: 当前对小语言模型的评估主要关注最终答案的准确性，而忽视了推理过程的有效性，这可能导致对模型能力的过度估计。

Method: 引入ReTraceQA基准测试，通过专家标注的数据集评估推理过程，并使用大型语言模型作为自动裁判进行推理感知的评估。

Result: 研究发现14-24%的情况下SLMs给出正确答案但推理过程有缺陷；使用LLMs进行推理感知评估时，SLM性能显著下降，得分最多降低25%。

Conclusion: 仅基于最终答案的评估方法会高估SLMs的能力，需要引入过程级评估来更准确地衡量模型的推理能力。
...
TL;DR: ThinkLogit是一种无需训练的解码时方法，通过logit算术使用小推理模型引导大模型进行长链推理，ThinkLogit-DPO通过偏好优化进一步提升了性能，在五个推理基准上分别实现了24.5%和29.1%的相对准确率提升。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通常需要额外训练才能展现长链推理能力，本研究旨在探索是否可以在不进行训练的情况下激发这种能力。

Method: 提出ThinkLogit解码时方法，利用logit算术通过小推理模型引导大模型进行长推理；进一步开发ThinkLogit-DPO，通过偏好优化训练引导模型，使用正确/错误推理对进行优化。

Result: 使用Qwen2.5-32B模型和21倍小的R1-Distill-Qwen-1.5B引导模型，在五个推理基准上分别实现了24.5%和29.1%的平均准确率相对提升；方法在不同模型家族间仍然有效，且与小型模型的后训练方法正交。

Conclusion: ThinkLogit提供了一种无需昂贵后训练即可解锁大规模模型长推理能力的实用路径，改进后的引导模型可以直接插入以产生更强的大模型。
...
TL;DR: 本文提出了NL2GenSym框架，将大型语言模型与SOAR认知架构结合，从自然语言自动生成符号规则。该框架通过执行驱动的生成器-批评机制，在SOAR环境中验证和迭代优化规则，在Water Jug Problem数据集上取得了86%以上的成功率，并能生成新颖的启发式规则。


<details>
  <summary>Details</summary>
Motivation: SOAR认知架构虽然能开发通用智能体，但手动编码规则非常耗时。大型语言模型具有高效生成规则的潜力，但现有研究缺乏坚实的实验验证。需要填补这一空白，实现从自然语言到符号规则的自动生成。

Method: 提出NL2GenSym框架，包含执行驱动的生成器-批评机制：1）基于检索增强生成的LLM生成器从自然语言提出规则；2）在SOAR环境中立即执行验证规则正确性；3）基于执行反馈的LLM批评器迭代优化规则。

Result: 在Water Jug Problem数据集上，使用Gemini和Qwen系列模型，框架实现了86%以上的规则生成成功率。生成的启发式规则将平均决策周期降至最优解的1.98倍，是基线方法的1/1000。小参数模型也能获得优于大模型的性能。

Conclusion: NL2GenSym框架成功实现了从自然语言到符号规则的自动生成，填补了LLM与认知架构结合的研究空白，为开发更高效的智能体提供了可行路径。
...
TL;DR: 本文首次系统研究了大型医学语言模型中的领域特定微调，发现微调仅修改表示子空间的一小部分，基本保留预训练模型的表示。作者提出了调优向量框架来捕捉微调引起的参数方向性变化，这些向量对提升指令遵循和生成质量至关重要，且跨领域组合可改善泛化能力。


<details>
  <summary>Details</summary>
Motivation: 目前对领域专业化大语言模型微调如何重塑其参数空间的内在机制理解不足，先前研究主要关注自回归或通用指令模型，而领域专业化LLMs研究较少。

Method: 提出调优向量框架（受任务向量启发），明确捕捉微调引起的方向性参数变化，分析这些向量在不同模型层中的行为模式。

Result: 微调仅修改表示子空间的一小部分；调优向量对提升指令遵循和生成质量至关重要；跨领域组合调优向量可改善泛化；调优向量主要在MLP层写入新方向信息，在注意力头中放大现有方向。

Conclusion: 研究为LLM适应提供了新见解，并提供了一个通用的、可解释的分析大型语言模型专业化的框架。
...
TL;DR: TEPO是一个新颖的token级框架，通过马尔可夫似然将组级奖励与token关联，解决了GRPO等熵正则化方法在思维链稀疏token奖励中的挑战，显著提升了数学推理性能。


<details>
  <summary>Details</summary>
Motivation: GRPO和相关熵正则化方法在思维链的稀疏token奖励中面临挑战，当前方法依赖无差别的token级熵调整，常导致熵崩溃或模型崩溃。

Method: 提出TEPO框架，通过马尔可夫似然将组级奖励与token关联，实现token级聚合。

Result: TEPO在所有关键指标上持续优于现有基线，在数学推理任务上达到新的最先进水平，并显著提升训练稳定性。

Conclusion: TEPO通过token级马尔可夫似然框架有效解决了稀疏奖励问题，为LLM推理能力提供了更稳定和高效的优化方法。
...
TL;DR: 本文研究了人机通信中目标共享的模糊性问题，特别是在数据可视化领域。作者开发了模糊性分类法和量化指标，证明多轮对话可以减少模糊性并提高代码准确性。


<details>
  <summary>Details</summary>
Motivation: 人机通信中建立共享目标时存在模糊性，这会导致输出看似正确但未能反映说话者意图。特别是在数据可视化领域，自然语言的模糊性会影响数据可视化代码的生成。

Method: 1. 开发了模糊性分类法来识别任务中出现的模糊类型；2. 提出了量化模糊性的指标；3. 使用DS-1000数据集中的Matplotlib问题进行评估；4. 探索了多轮对话减少模糊性的策略，基于三种语用模型：Gricean合作原则、话语表征理论和问题讨论框架。

Result: 1. 提出的模糊性指标比不确定性基线更能与人类标注相关；2. 模拟用户研究显示语用对话可以减少模糊性并提高代码准确性；3. 多轮交流在代码生成中具有重要价值。

Conclusion: 多轮对话是减少人机通信中模糊性的有效方法，能够更好地匹配用户目标并提高代码生成准确性。语用模型为构建此类对话提供了理论基础。
...
TL;DR: 本文提出了一种多尺度图思维链（MSGCOT）提示框架，通过整合图数据的多尺度结构信息来解决现有图提示调优方法局限于单一粒度的问题。该方法使用轻量级低秩粗化网络捕获多尺度特征，并模拟人类从粗到细的认知过程，在少样本场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有图提示调优方法局限于单一粒度（节点级或子图级），忽略了图数据固有的多尺度结构信息，限制了提示语义的多样性。

Method: 设计轻量级低秩粗化网络高效捕获多尺度结构特征作为层次基础向量，模拟人类从粗到细的认知过程，在每个推理步骤动态整合多尺度信息，形成渐进式粗到细的提示链。

Result: 在八个基准数据集上的广泛实验表明，MSGCOT优于最先进的单粒度图提示调优方法，特别是在少样本场景下展现出优越性能。

Conclusion: 多尺度信息的整合显著提升了图提示调优的效果，特别是在数据稀缺的情况下，证明了多尺度结构信息在图学习中的重要性。
...
TL;DR: LLM SELECTOR是首个用于大型语言模型主动选择的框架，通过自适应选择少量最具信息量的查询进行标注，结合基于评判的标注模型，显著降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法需要完全标注的数据集，标注成本高昂。需要一种能在有限标注下高效识别最佳LLM的方法。

Method: 提出LLM SELECTOR框架，自适应选择少量信息量最大的查询进行标注，利用基于评判的标注模型进一步减少标注需求。

Result: 在6个基准测试和151个LLM上的实验表明，LLM SELECTOR在选择最佳和接近最佳LLM时，标注成本降低高达59.62%。

Conclusion: LLM SELECTOR提供了一种高效的LLM选择方法，显著降低了标注成本，为实际应用中的模型选择提供了实用解决方案。
...
TL;DR: 该论文提出了实体提及重构框架来研究大语言模型如何内部表示实体，通过任务向量方法从隐藏状态生成多标记实体提及，揭示了LLMs开发了实体特定机制来表示和操作多标记实体。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型如何内部表示命名实体，因为实体是文本知识的基本构建块，但目前对LLMs内部实体表示机制了解甚少，特别是多标记实体的编码方式。

Method: 引入实体提及重构框架，利用任务向量从LLMs隐藏状态的各种实体表示中一致生成多标记提及，提出实体透镜方法扩展对数透镜来预测多标记提及。

Result: 发现LLMs开发了实体特定机制来表示和操作任何多标记实体，包括训练期间未见过的实体，能够从内部表示生成实体提及。

Conclusion: LLMs内部形成了专门的实体表示机制，能够编码和操作多标记实体，这一发现为理解LLMs如何表示结构化知识提供了新证据。
...
TL;DR: 本文比较了语音-LLM端到端口语对话状态跟踪的上下文管理策略，发现提供完整口语对话历史作为输入能获得最佳性能，而基于注意力池化的压缩方法能在保持竞争力的同时减少上下文大小。


<details>
  <summary>Details</summary>
Motivation: 研究不同上下文管理策略对语音-LLM在口语对话状态跟踪任务中的影响，探索如何更有效地利用口语对话上下文信息。

Method: 系统评估了三种上下文管理策略：传统多模态上下文（文本历史+当前语音轮次）、完整口语历史、压缩口语历史。在SpokenWOZ语料库上进行实验，使用基于注意力池化的压缩方法。

Result: 实验表明，提供完整口语对话作为输入在相似模型规模下获得最高性能，显著超越先前方法。注意力池化压缩方法提供了良好的权衡，在减小上下文大小的同时保持竞争力。

Conclusion: 改进主要源于更有效的上下文利用，完整口语历史输入是最佳策略，而压缩方法为资源受限场景提供了实用替代方案。
...
TL;DR: 本文提出了首个大规模构建完全开放的双语大语言模型（KORMo-10B）的研究，该模型专门针对韩语，主要使用合成数据进行训练，在韩语推理和语篇连贯性方面达到接近母语水平。


<details>
  <summary>Details</summary>
Motivation: 针对非英语语言（特别是韩语）缺乏完全开放的大语言模型的问题，探索在低资源环境下使用合成数据构建双语模型的可行性。

Method: 从零开始训练10.8B参数的KORMo-10B模型，使用韩英双语语料库，其中68.74%的韩语部分是合成数据，通过精心策划的平衡语言覆盖和多样化指令风格来确保训练稳定性。

Result: 模型在推理、知识和指令跟随等广泛基准测试中与当代开放权重的多语言基线模型性能相当，证明了合成数据在大规模预训练中不会导致模型崩溃或性能下降。

Conclusion: 合成数据可以可靠地支持长期预训练，双语指令调优能够实现接近母语的韩语推理和语篇连贯性，为低资源环境下开发合成数据驱动的完全开放模型建立了透明框架。
...
TL;DR: 本文研究使用紧凑型开源预训练语言模型（PLMs）从交通事故叙述中提取推理密集型信息，包括碰撞方式和车辆碰撞类型。通过微调技术（如LoRA和BERT）注入领域知识，在权威数据集CISS上表现优于GPT-4o等大型闭源模型，且训练资源需求低。


<details>
  <summary>Details</summary>
Motivation: 现实交通事故数据库中的自由文本叙述对交通安全分析很重要，但大规模分析困难。现有Transformer模型在推理密集型任务（如碰撞类型识别）上性能下降，且闭源LLMs存在隐私问题和领域知识不足的局限。

Method: 使用紧凑型开源PLMs，通过微调技术（LoRA和BERT）注入任务特定知识，针对碰撞方式和车辆碰撞类型两个挑战性目标进行训练。

Result: 在权威CISS数据集上的实验表明，微调后的紧凑模型优于GPT-4o等强闭源LLMs，同时仅需最小训练资源。模型能捕捉更丰富的叙述细节，甚至修正数据集中的错误标注。

Conclusion: 紧凑型开源PLMs通过适当微调可以有效支持从交通事故叙述中进行推理密集型信息提取，在性能和隐私保护方面优于大型闭源模型。
...
<details>
  <summary>Details</summary>
Motivation: 尽管开源LLM模型增多，但训练数据的访问仍然受限，且规模庞大难以分析，可能包含从互联网抓取的关键数据。

Method: 利用Elasticsearch并行索引和Alps基础设施（先进的节能arm64超级集群），为Apertus LLM家族训练数据构建全文索引管道。

Result: 成功索引了15.2T tokens中的8.6T，证明了Elasticsearch在arm64架构上的可行性，以及现代LLM训练数据集规模全文索引的可行性。

Conclusion: 该索引系统可作为关键LLM安全工具和离线网络搜索引擎，促进大规模数据索引和绿色计算的转型。
...
TL;DR: 本文研究了大型语言模型在逻辑推理中的泛化能力，区分了组合性和递归性两个关键方面。研究发现LLMs在递归性方面表现良好，但在组合性方面存在困难，为此提出了一种结合符号推理和神经计算的混合架构来克服这些限制。


<details>
  <summary>Details</summary>
Motivation: 尽管神经网络模型取得了显著进展，但其泛化能力仍然是关键挑战，特别是在逻辑推理等应用中。本文旨在明确区分组合性（抽象原子逻辑规则的能力）和递归性（通过迭代应用推理规则构建复杂表示的能力）这两个泛化能力的关键方面。

Method: 使用三段论片段作为自然语言推理的基准，评估预训练大型语言模型的逻辑泛化能力。为了克服模型在组合性方面的限制，提出了一种集成符号推理与神经计算的混合架构，通过协同交互实现鲁棒高效的推理。

Result: 研究发现LLMs在递归性方面表现出合理的熟练度，但在组合性方面存在显著困难。提出的混合架构在保持高效率的同时，即使使用相对较小的神经组件也能实现完整的推理能力。

Conclusion: 混合模型具有有效解决神经推理系统中关键泛化障碍的潜力，神经组件加速处理，而符号推理确保完整性，这种协同作用为构建可靠的逻辑证明器提供了可行方案。
...
TL;DR: 本文提出了多模态策略内化（MPI）任务，将复杂的多模态策略内化到模型参数中，从而在推理时无需包含策略文本，实现了更强的策略遵循能力、泛化性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代对话系统依赖预定义策略来指定元数据、响应风格和工具使用规则。随着LLM系统扩展到支持多样化业务和用户查询，这些策略变得日益复杂冗长，导致难以忠实遵循并产生高昂的计算成本。多模态代理的兴起使得管理视觉和多模态行为的策略至关重要，但相关研究仍不足。

Method: 提出了TriMPI三阶段训练框架：1）通过持续预训练注入策略知识；2）进行监督微调；3）应用PolicyRollout，这是一种GRPO风格的强化学习扩展，通过策略感知响应增强rollout以进行有基础的探索。

Result: TriMPI在端到端准确性、泛化性和抗遗忘鲁棒性方面取得了显著提升。这是首个关于多模态策略内化的工作，提供了数据集、训练方案和全面评估。

Conclusion: 多模态策略内化（MPI）是一个重要的新任务，TriMPI框架有效解决了该任务的数据和算法挑战，为未来研究奠定了基础。
...
TL;DR: StatEval是首个专门针对统计学的综合基准测试，包含13,817个基础问题和2,374个研究级证明任务，揭示了当前大语言模型在统计推理方面的局限性。
...
Method: 设计了一个可扩展的多智能体流程，结合人工验证，自动化大规模问题提取、重写和质量控制，同时确保学术严谨性。

Result: 实验结果显示，闭源模型如GPT5-mini在研究级问题上准确率低于57%，开源模型表现更差，凸显了统计推理的独特挑战。

Conclusion: StatEval可作为推动大语言模型统计智能发展的严谨基准，所有数据和代码已在web平台公开。
...
TL;DR: 本文分析了无标签情况下评估NLP模型跨域性能排名的可靠性，发现基于大语言模型的错误预测器比基于数据漂移或零样本基线的方法能产生更强更一致的排名相关性。
...
Result: 大语言模型错误预测器比漂移基线和零样本基线产生更强更一致的排名相关性；当跨域性能差异较大且错误预测与基础模型真实失败模式一致时，排名更可靠。

Conclusion: 研究结果阐明了性能估计方法何时可信，为跨域模型评估提供了使用指导。
...
TL;DR: 提出了一种口音不变的自动语音识别框架，通过集成口音和方言分类到识别流程中，使用频谱图分类器捕捉口音特征，掩码关键区域进行数据增强，提高ASR模型对口音变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 预训练的基于transformer的ASR模型对重音和方言变化敏感，导致在英语和波斯语等语言多样性语言中词错误率升高，需要解决这一挑战。

Method: 训练基于频谱图的分类器捕捉口音特征，掩码对预测影响最大的区域，使用掩码后的频谱图进行数据增强，增强ASR模型的口音鲁棒性。

Result: 在英语和波斯语语音上的实验结果表明，该方法在Whisper模型中显著降低了词错误率，波斯语方面还引入了包含多种地区口音的新数据集。

Conclusion: 该研究推进了能够抵抗口音和方言多样性的多语言ASR系统的发展，代码和数据集已公开。
...
TL;DR: 本文提出了一种名为GRSP（Group Relative Segment Penalization）的步骤级方法，通过段级监督来平衡大型推理模型的效率和准确性，解决了RLVR方法中过度思考导致计算成本增加的问题。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在强化学习验证奖励（RLVR）的推动下展现出强大的问题解决能力，但经常出现过度思考现象：过多的、迂回的推理会显著增加计算成本。现有的惩罚设计虽然减少了token消耗，但往往损害模型性能，这源于token级监督的过度简化。

Method: 提出GRSP（Group Relative Segment Penalization）方法，这是一种步骤级的推理正则化方法。通过初步分析发现推理段与token消耗和模型性能强相关，设计了跨段簇的长度感知加权机制。

Result: 大量实验表明，GRSP在保持准确性的同时实现了优越的token效率，特别是在处理更难问题时优势更明显。此外，GRSP稳定了强化学习训练，并在不同模型规模上有效扩展。

Conclusion: 监督的粒度在平衡效率和准确性方面起着关键作用，GRSP方法通过段级监督有效解决了过度思考问题，在保持性能的同时显著提高了推理效率。
...
TL;DR: 本文介绍了MulTypo，一种多语言拼写错误生成算法，用于评估LLM在不同语言和任务中对拼写错误的鲁棒性。研究发现拼写错误会显著降低模型性能，特别是在生成和推理任务中，且不同语言间的鲁棒性存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设输入是干净的，而真实应用中用户输入往往包含拼写错误。LLM在多语言环境下对拼写错误的鲁棒性尚未得到充分研究。

Method: 开发MulTypo算法，基于语言特定的键盘布局和打字行为模拟人类拼写错误。评估18个开源LLM在三个模型家族和五个下游任务上的表现，包括语言推理、多选问答、数学推理和机器翻译。

Result: 拼写错误持续降低模型性能，特别是在生成任务和需要推理的任务中。指令调优提高了干净输入的性能，但可能增加对噪声的脆弱性。高资源语言通常比低资源语言更鲁棒，从英语翻译比翻译成英语更鲁棒。

Conclusion: 研究结果强调了噪声感知训练和多语言鲁棒性评估的必要性。代码和数据已公开提供。
...
TL;DR: 本文提出了Sandwiched Policy Gradient (SPG)方法，通过同时利用对数似然的上界和下界来解决扩散大语言模型(dLLMs)在强化学习中对齐人类偏好时的策略梯度偏差问题，显著优于现有基于ELBO或一步估计的方法。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)因其能并行解码多个token而成为自回归模型的高效替代品，但由于其难以处理的似然函数，无法直接应用标准策略梯度方法进行人类偏好对齐。现有方法使用ELBO等单边近似会引入显著的策略梯度偏差。

Method: 提出了Sandwiched Policy Gradient (SPG)方法，该方法同时利用真实对数似然的上界和下界，而不是像ELBO那样只使用单边近似，从而减少策略梯度偏差。

Result: 实验表明SPG显著优于基于ELBO或一步估计的基线方法。具体来说，在GSM8K上准确率提升3.6%，在MATH500上提升2.6%，在Countdown上提升18.4%，在Sudoku上提升27.0%。

Conclusion: SPG方法通过同时利用对数似然的上界和下界，有效解决了dLLMs在强化学习中对齐人类偏好时的策略梯度偏差问题，在多个任务上取得了显著优于现有方法的性能。
...
TL;DR: Dyna-Mind是一个两阶段训练框架，通过教导AI代理进行心理模拟来增强其在复杂交互环境中的推理能力。第一阶段ReSim训练代理从真实环境交互中生成结构化推理轨迹，第二阶段Dyna-GRPO使用强化学习进一步强化模拟和决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在数学和编程方面表现出色，但在长时程交互任务（如网页导航和计算机/手机使用）中表现不佳。受人类认知研究启发，作者认为AI需要"替代性试错"能力，即在进行行动前心理模拟替代未来情景，以增强对复杂交互环境的理解和表现。

Method: 提出Dyna-Mind两阶段训练框架：1）ReSim阶段：训练代理从真实环境交互构建的扩展搜索树中生成结构化推理轨迹，将推理基于真实世界动态；2）Dyna-GRPO阶段：在线强化学习方法，使用结果奖励和中间状态作为真实rollout的反馈，进一步强化模拟和决策能力。

Result: 在三个基准测试（Sokoban、ALFWorld和AndroidWorld）上的实验表明：1）ReSim有效将模拟能力注入AI代理；2）Dyna-GRPO利用结果和交互级信号为长时程、规划密集型任务学习更好的策略。

Conclusion: 这些结果强调了模拟在使AI代理能够在日益具有挑战性的环境中更有效地推理、规划和行动中的核心作用。
...
TL;DR: 本文识别了扩散大语言模型中的并行-顺序矛盾(PSC)，发现DLLMs仅在简单任务中实现真正并行，在复杂推理中会退化为自回归行为。作者提出了三个扩展维度并提出了缓解PSC的实用方法。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(DLLMs)提供了高吞吐量和有效的顺序推理，是自回归LLMs的有力替代品。但并行解码与严格推理所需的因果顺序存在冲突，这种并行-顺序矛盾限制了DLLMs的性能。

Method: 通过行为分析研究DLLMs在不同难度任务中的表现，识别PSC现象。引入并行、扩散和顺序三个扩展维度来表征DLLMs，并提出了并行导向提示、扩散早停和并行扩展等缓解方法。

Result: 实验表明DLLMs仅在可直接判定的输出上实现真正并行，随着任务难度增加会退化为自回归行为。并行扩展能带来一致改进，但扩散和顺序扩展受PSC限制。

Conclusion: 并行-顺序矛盾是DLLMs的核心限制，需要专门设计的方法来缓解其导致的低效和无效问题。提出的并行导向提示、扩散早停和并行扩展等方法能有效改善DLLMs的性能。
...
TL;DR: 提出一个多阶段框架来解决多语言视频语料检索任务，通过语义分块、知识图谱增强、层次化索引和轻量级LLM重排序，在保持精度的同时实现高效检索。


<details>
  <summary>Details</summary>
Motivation: 现有系统要么将长视频压缩为粗糙嵌入，要么细粒度匹配成本过高，无法有效处理多语言医学视频档案中的复杂多跳问题检索。

Method: 将视频字幕分成语义连贯的块，用知识图谱事实增强，构建层次化树结构，使用语言无关的多语言编码器生成节点嵌入。查询时进行粗到细的树搜索剪枝，仅对排名靠前的块进行轻量级LLM重评分。

Result: 在mVCR测试集上达到最先进性能，消融研究证实了知识图谱增强、层次化索引和针对性LLM重排序的互补贡献。
...
TL;DR: 本文首次对多语言链式思维推理进行综合研究，评估了三个关键维度：性能、一致性和忠实度，揭示了不同语言间思维轨迹的质量和有效性存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 虽然大型推理模型依赖逐步推理链来提高任务性能，但在多语言环境中，导致最终答案的中间思维轨迹本身仍未被充分探索。

Method: 通过测量语言遵从性、答案准确性和一致性来评估模型在目标语言中的表现；通过跨语言交换思维轨迹来评估一致性；采用基于扰动的技术（截断和错误注入）来探测思维轨迹的忠实度。

Result: 发现模型在不同语言中表现出强烈的语言偏好和不同的性能；思维轨迹的质量和有效性因提示语言而异；模型在不同语言中对思维轨迹的依赖程度各不相同。

Conclusion: 多语言链式思维推理在不同语言间存在显著差异，思维轨迹的质量、一致性和忠实度都受到语言选择的影响，这为未来研究提供了重要启示。
...
TL;DR: 该论文提出了WUGNECTIVES数据集，研究话语连接词是否能帮助语言模型进行关于新实体的世界知识推理，发现经过推理调优的模型表现更好，但所有模型在表达让步意义的连接词上都存在困难。


<details>
  <summary>Details</summary>
Motivation: 传统研究关注世界知识如何帮助预测话语连接词，本文翻转这一前提，研究话语连接词是否能反过来为语言模型提供世界知识信息。

Method: 构建包含8,880个刺激项的WUGNECTIVES数据集，评估17个不同规模和训练方式的语言模型在新实体推理任务上的表现，特别关注模型在连接词引导下的推理能力。

Result: 经过推理调优的语言模型在大多数连接词类型上表现显著提升，但所有模型在表达让步意义的连接词上都系统性地表现不佳，不同连接词类型间存在较大性能差异。

Conclusion: 话语连接词确实能为语言模型提供世界知识信息，但模型对不同类型连接词的理解能力存在差异，这为更细致地研究语言线索在语言模型中的功能作用开辟了新方向。
...
TL;DR: 本文提出了AutoPR任务，将研究论文自动转化为准确、吸引人且及时的宣传内容，并开发了PRBench基准和PRAgent多智能体框架来评估和实现这一过程。


<details>
  <summary>Details</summary>
Motivation: 随着同行评审研究数量的激增，学者们越来越依赖社交平台来发现研究，而作者们需要投入大量精力来推广工作以确保可见性和引用。为了简化这一过程并减少对人力的依赖，需要自动化解决方案。

Method: 提出了PRAgent多智能体框架，通过三个阶段自动化AutoPR：1）多模态内容提取和准备；2）协作合成生成精炼输出；3）平台特定适配以优化规范、语气和标签，实现最大覆盖。

Result: 与直接LLM管道相比，PRAgent在PRBench上表现出显著改进：总观看时间增加604%，点赞数增长438%，整体参与度至少提升2.9倍。消融研究表明平台建模和目标推广对这些收益贡献最大。
...
TL;DR: Mind-Paced Speaking (MPS) 是一个受大脑启发的框架，通过双脑方法实现实时推理：用"构思脑"进行高级推理来引导"表达脑"生成流畅语音，消除了模式切换，在保持推理质量的同时大幅降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的实时口语语言模型难以利用链式思维推理，因为顺序生成整个思维过程会产生过高的延迟。需要让SLM像人类一样边思考边说话，实现高质量推理和实时交互的平衡。

Method: 提出双脑方法："构思脑"负责高级推理，指导"表达脑"进行语音生成。这种分工消除了模式切换，保持了推理过程的完整性。

Result: MPS显著优于现有的边思考边说话方法，在数学推理任务Spoken-MQA上达到92.8%准确率，在语音对话任务URO-Bench上获得82.5分，推理性能与预计算完整CoT的模型相当，同时大幅降低延迟。

Conclusion: 该工作有效弥合了高质量推理和实时交互之间的差距，为实时口语语言模型的推理能力提供了创新解决方案。
...
TL;DR: P-TTS是一种推理时数据增强策略，仅使用90个手动选择的推理实例，通过系统化改变提示强度来合成多样化的推理轨迹上下文，从而增强LLM的推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要收集大量推理数据集，这既费力又资源密集。P-TTS旨在通过最小化标注开销来提升LLM的推理能力。

Method: 使用仅90个手动选择的推理实例，通过系统化改变提示强度在测试时合成多样化的推理轨迹上下文，然后对各种规模的Qwen-2.5模型进行微调。

Result: 在多个数学推理基准测试中，P-TTS-7B和32B模型显著优于先前竞争基线，在AIME'24和AIME'25上分别获得高达+30.00%和+26.63%的绝对准确率提升，并在其他基准测试中表现相当或更好。

Conclusion: 测试时扩展有效探索了推理模式的潜在空间，以最小标注开销放大LLM的问题解决能力，进一步释放了LLM的推理潜力和能力，为资源受限或快速发展的领域提供了一种实用的低成本方法。
...
TL;DR: AScience框架通过LLM研究智能体网络实现大规模科学假设探索，能够在癌症队列中重新发现已知生物标志物、扩展已知通路并提出新治疗靶点。


<details>
  <summary>Details</summary>
Motivation: 大规模科学数据集为无特定研究问题的探索性发现创造了机会，需要支持假设狩猎——在复杂假设空间中持续探索以积累洞察的过程。

Method: 提出AScience框架，将发现建模为智能体、网络和评估规范的交互，实现为ASCollab分布式系统，包含具有异质行为的LLM研究智能体，这些智能体自组织成演化网络，在共享评估标准下持续产生和同行评审发现。

Result: 实验表明这种社会动力学能够沿着多样性-质量-新颖性前沿积累专家评级结果，包括重新发现已建立的生物标志物、扩展已知通路和提出新治疗靶点。

Conclusion: 虽然湿实验室验证仍然不可或缺，但在癌症队列上的实验证明，社会结构化的智能体网络可以大规模持续进行探索性假设狩猎。
...
TL;DR: 提出一个使用大语言模型评估车辆路径规划生成路线的框架，在印度电商物流场景中，开源LLM识别路由问题准确率达79%，专有推理模型可达86%。


<details>
  <summary>Details</summary>
Motivation: 印度电商市场快速增长，最后一公里配送占运营成本近半。传统车辆路径规划在实际应用中受限于非结构化地址、不完整地图和距离估算计算约束，效果有限。

Method: 使用大语言模型基于策略标准对VRP生成的路由进行批判性评估，生成、标注和评估了400个案例，比较开源和专有LLM的表现。

Result: 开源LLM识别路由问题准确率79%，专有推理模型最高达86%，证明LLM评估VRP路线是有效且可扩展的方法。

Conclusion: 基于LLM的VRP路线评估可作为超越传统距离和时间指标的有效评估层，有助于提高成本效率、配送可靠性和可持续性，特别适用于印度等发展中国家。
...
TL;DR: UniWM是一个统一的世界模型，将视觉前瞻和导航规划集成在单一多模态自回归框架中，通过分层记忆机制实现长期稳定的推理，显著提升了导航性能。
...
Method: 提出UniWM统一记忆增强世界模型，在单一多模态自回归骨干网络中集成自我中心视觉前瞻和规划，采用分层记忆机制整合短期感知线索和长期轨迹上下文。

Result: 在四个挑战性基准测试中，导航成功率提升高达30%，轨迹误差显著降低，并在未见过的TartanDrive数据集上展现出优异的零样本泛化能力。

Conclusion: UniWM是实现统一、想象力驱动的具身导航的重要进展，通过将动作决策明确基于视觉想象结果，确保了预测与控制之间的紧密对齐。
...
TL;DR: 通过向LLM提供启发式算法表现不佳的实例、解释原因，并在输入空间特定区域进行专门化设计，可以生成比现有技术更鲁棒的启发式算法，最坏情况性能提升约28倍，同时保持运行时间。
...
Method: 提出三个简单但有效的改进：(1)向LLM展示启发式算法表现不佳的实例；(2)解释表现不佳的原因；(3)在输入空间的特定区域进行专门化设计。

Result: 生成的启发式算法相比FunSearch在最坏情况下性能提升约28倍，平均性能也有所改善，同时保持了相同的运行时间。

Conclusion: 通过向LLM提供失败案例分析和针对性改进建议，可以显著提升启发式算法的鲁棒性和性能，这为LLM在算法设计中的应用提供了新的思路。
...
TL;DR: COMPASS是一个轻量级分层框架，通过将战术执行、战略监督和上下文管理分离为三个专门组件来解决LLM代理在长时程任务中的上下文管理瓶颈问题，在多个基准测试中准确率提升高达20%。


<details>
  <summary>Details</summary>
Motivation: 长时程任务需要持续推理和多次工具交互，这对LLM代理具有挑战性：小错误会在步骤间累积，即使最先进的模型也经常产生幻觉或失去连贯性。上下文管理被确定为关键瓶颈，扩展的历史记录导致代理忽略关键证据或被无关信息分散注意力。

Method: 提出COMPASS框架，包含三个专门组件：(1)主代理执行推理和工具使用，(2)元思考者监控进度并发出战略干预，(3)上下文管理器为不同推理阶段维护简洁相关的进度简报。

Result: 在GAIA、BrowseComp和Humanity's Last Exam三个挑战性基准测试中，COMPASS相比单代理和多代理基线准确率提升高达20%。还引入了测试时扩展扩展，使性能达到已建立的DeepResearch代理水平，以及后训练管道将上下文管理委托给较小模型以提高效率。

Conclusion: COMPASS通过专门的上下文管理和分层架构有效解决了LLM代理在长时程任务中的核心瓶颈，显著提升了性能，并提供了可扩展和高效的实现方案。
...
TL;DR: 本研究通过对比人类和AI对文学风格的评估，发现两者都存在系统性的人类归因偏见，但AI的偏见程度是人类2.5倍。当内容被标注为AI生成时，评估者会贬低其价值，即使内容完全相同。


<details>
  <summary>Details</summary>
Motivation: 随着AI写作工具的普及，需要理解人类和机器如何评估文学风格这一主观领域，特别是归因偏见如何影响评估结果。

Method: 研究1：比较556名人类参与者和13个AI模型对Queneau作品与GPT-4生成版本的评估（盲测、准确标注、反事实标注三种条件）。研究2：在14×14的AI评估者-创作者矩阵中测试偏见泛化。

Result: 人类显示+13.7个百分点的偏见，AI显示+34.3个百分点的偏见（2.5倍于人类）。AI系统在标注为AI生成时系统性地贬低创意内容，无论哪个AI创作。归因标签导致评估者反转评估标准。

Conclusion: AI模型在训练过程中吸收了人类对人工创造力的文化偏见，不仅复制而且放大了这种人类倾向。这是首次在美学判断中控制比较人类和人工评估者的归因偏见。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种系统性的方法来评估智能体在目标设定、计划制定和行动执行整个操作循环中的表现，以全面覆盖各种智能体失败情况。

Method: 基于智能体的目标-计划-行动操作循环，设计了五个评估指标，并在TRAIL/GAIA基准数据集和生产级数据智能体内部数据集上进行实验验证。

Result: 实验结果显示该框架能够覆盖TRAIL/GAIA基准数据集中的所有智能体错误，LLM评估器与人工标注的一致性达到80%-95%，错误定位一致性达到86%。
...
TL;DR: ReviewerToo是一个模块化AI辅助同行评审框架，在ICLR 2025数据集上达到81.8%的接受/拒绝分类准确率，接近人类评审员的83.9%。AI评审在事实核查和文献覆盖方面表现出色，但在评估方法新颖性和理论贡献方面仍需人类专家。


<details>
  <summary>Details</summary>
Motivation: 解决传统同行评审中存在的不一致性、评审员主观性和可扩展性挑战，通过AI辅助系统来补充人类判断，提供更系统和一致的评估。

Method: 开发了ReviewerToo模块化框架，支持专门的评审员角色和结构化评估标准，可在真实会议工作流程中部分或完全集成。使用gpt-oss-120b模型在ICLR 2025的1,963篇论文提交上进行验证。

Result: AI评审员在分类论文接受/拒绝任务上达到81.8%准确率，接近人类平均83.9%；AI生成的评审质量被LLM评估为高于人类平均水平，但仍落后于最强专家评审。AI在事实核查和文献覆盖方面表现优异，但在评估方法新颖性和理论贡献方面存在困难。

Conclusion: AI可以增强同行评审的一致性、覆盖范围和公平性，但复杂的评估判断仍需依赖领域专家。该工作为构建系统化混合同行评审系统奠定了基础，能够随着科学出版的增长而扩展。
...
TL;DR: 提出了GTAlign框架，将博弈论决策整合到LLM的推理和训练中，通过构建收益矩阵和互惠奖励机制，使模型能够选择对用户和自身都更有利的响应，提高推理效率和答案质量。


<details>
  <summary>Details</summary>
Motivation: 传统对齐方法假设最大化模型奖励就能最大化用户福利，但实践中模型可能产生过度澄清或冗长的推理，而用户更偏好简洁答案。这种个体理性选择导致社会次优结果，类似于囚徒困境。

Method: 在推理阶段将用户-LLM交互视为策略博弈，构建收益矩阵评估双方福利并选择互惠行动；在训练阶段引入互惠奖励强化合作响应；还提出基于博弈论推理的动态适应技术来应对LLM服务定价策略变化。

Result: 大量实验表明，GTAlign在多样化任务中显著提高了推理效率、答案质量和互惠福利，优于基线方法。

Conclusion: GTAlign通过博弈论方法有效解决了LLM对齐中的社会效率问题，实现了模型与用户的双赢，为LLM对齐提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型多模态模型基准测试往往无法捕捉它们在实时对抗环境中的真实表现，需要一种动态、交互式的评估方法。

Method: 在受控的锦标赛中测试六个领先的开源和闭源模型，每个模型控制相同的游戏角色以确保公平比较。模型需要解释游戏画面和状态数据来选择下一步行动。

Result: LM Fight Arena提供了一个完全自动化、可重现且客观的评估方法，能够评估LMM在动态环境中的战略推理能力。
...
Conclusion: 这项工作展示了机制可解释性在推进LLM评估超越传统表面级指标方面的潜力。
...
TL;DR: 本文提出了一个用于评估检索增强生成(RAG)管道的基准测试，包含多模态数据评估、短语级召回指标、幻觉检测方法，并比较了开源与闭源管道的性能差异。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG评估基准主要关注检索等特定方面，缺乏对完整RAG管道在多模态信息处理能力的综合评估。

Method: 创建了包含93个问题的数据集，涵盖文本、表格、图像等多模态信息；提出了短语级召回指标和基于最近邻嵌入的幻觉检测方法；评估了2个开源和4个闭源模型。

Result: 闭源管道在正确性和幻觉检测方面显著优于开源管道，特别是在多模态和跨文档问题上表现差距更大；人工评估显示指标与人类判断高度一致(正确性4.62/5，幻觉检测4.53/5)。

Conclusion: 提出的基准测试能有效评估RAG管道的整体性能，闭源模型在多模态信息处理方面具有明显优势，评估指标与人类判断具有良好一致性。
...
TL;DR: EcphoryRAG是一个基于实体知识图谱的RAG框架，通过提取核心实体和元数据进行轻量级索引，减少94%的token消耗。它使用实体线索进行多跳关联检索，动态推断实体间隐含关系，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 受人类认知神经科学中利用线索激活实体记忆痕迹进行复杂多跳回忆的机制启发，开发一个高效的实体中心知识图谱检索框架。

Method: 在索引阶段仅提取和存储核心实体及元数据；检索时从查询中提取线索实体，在知识图谱上进行可扩展的多跳关联搜索，动态推断实体间隐含关系来丰富上下文。

Result: 在2WikiMultiHop、HotpotQA和MuSiQue基准测试中，EcphoryRAG将平均精确匹配分数从0.392提升到0.474，优于HippoRAG等强基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度研究框架虽然能够协调外部工具进行复杂的多步科学推理，但仍面临上下文污染、证据支持薄弱和执行路径脆弱等问题，需要更稳健的推理支持机制。
...
Result: 在科学推理基准HLE和GPQA上，DualResearch实现了有竞争力的性能。使用开源系统InternAgent的日志文件，其在HLE上的准确率提高了7.7%，在GPQA上提高了6.06%。

Conclusion: DualResearch作为深度研究系统的补充，能够将冗长的多工具执行日志压缩为简洁的推理图，稳定有效地重构答案，为解决工具密集型推理中的关键问题提供了有效方案。
...
TL;DR: 本文提出了一种新的知识注入范式——语义条件调优（SCT），通过图神经网络提取上下文感知的语义条件，并自适应地调制文本嵌入，实现深度特征层面的知识交互，显著提升了知识图谱补全任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的前缀调优方法简单地将知识嵌入与文本输入拼接，忽略了知识图谱中丰富的关系语义，并将隐含的推理负担强加给大语言模型。需要一种更深入的知识融合方法来改善知识推理。

Method: SCT包含两个关键模块：1）语义图模块使用图神经网络从局部图邻域中提取上下文感知的语义条件；2）条件自适应融合模块通过参数化投影器自适应地调制文本嵌入，实现深度特征层面的知识感知交互。

Result: 在知识图谱基准测试上的广泛实验表明，SCT显著优于前缀调优和其他强基线方法。分析证实通过在大语言模型推理前用语义图上下文调制输入表示，SCT提供了更直接和有效的信号。

Conclusion: SCT通过在大语言模型推理前深度融合知识图谱语义信息，实现了更准确和鲁棒的知识推理，为知识密集型任务提供了更有效的知识注入范式。
...
TL;DR: Tiny-R1V是一个轻量级的3B多模态大语言模型，通过两阶段优化实现了更快的推理速度和更高的准确率。第一阶段使用LIPO强化学习方法鼓励生成更短更准确的回答，第二阶段使用AMM模型融合方法将多个专家模型统一为一个架构。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在各种任务中表现出色，但在推理效率方面面临诸多挑战，如模型规模大、过度思考和轻量级场景下准确率下降。目前对轻量级MLLM推理能力的研究还很缺乏。

Method: 采用两阶段优化方法：1) LIPO强化学习方法，动态调整组内回答的优势，优先简洁高质量的回答；2) AMM模型融合方法，自适应调整任务向量权重，通过梯度投影正则化损失函数优化合并向量。

Result: 在涵盖数学、结构化数据、OCR和通用能力的十个广泛使用的推理基准测试中，Tiny-R1V展现出卓越性能，使轻量级模型能够在多样化的多模态推理任务中表现出色。

Conclusion: Tiny-R1V通过创新的两阶段优化方法，成功解决了轻量级多模态大语言模型在推理效率方面的挑战，实现了更快的推理速度和更高的准确率，统一了多任务多模态推理能力。
...
TL;DR: 提出了一个统一的旅行规划基准，将细粒度标准整合为单一奖励分数，支持强化学习训练，并在大规模数据集上验证了RL方法在提高旅行计划可行性方面的优势。
...
Method: 开发了统一的旅行规划基准，包含4,870个查询和219个真实世界自由形式请求；使用GRPO进行强化学习训练；评估器与旅行专家标注达到60.75%的一致性。

Result: RL方法在基础模型上普遍提高了行程的可行性，相比仅使用提示和监督基线获得了更高的统一奖励分数；评估器性能优于多个LLM-as-judge基线。

Conclusion: 提出的统一基准能够有效评估旅行计划质量，RL方法在提高旅行规划可行性方面表现出优势，为LLMs的旅行规划能力提供了更好的评估框架。
...
TL;DR: 本文评估了SOTA LLMs在证明评分方面的能力，包括检测错误、判断严重程度和分配公平分数。研究发现模型能可靠标记错误解答，但在部分分数分配上存在校准差距。作者提出了基于代理工作流程的多步骤评分方法，显著提高了与人类评分的一致性和部分分数处理的连贯性。


<details>
  <summary>Details</summary>
Motivation: 随着SOTA LLMs在解决奥林匹克数学问题方面取得显著进展（能解决IMO 2025的大部分问题），作者希望评估这些模型在证明评分方面的能力，包括检测错误、判断严重程度和超越二元正确性的公平评分。

Method: 使用90个Gemini 2.5 Pro生成的解决方案进行评分（1-4分制），并在MathArena的IMO/USAMO 2025解决方案集上评分（0-7分制）。引入代理工作流程，提取和分析参考解决方案，自动推导问题特定的评分标准，进行多步骤评分过程。

Result: 模型能可靠标记错误（包括细微错误）的解决方案，但在部分分数分配上表现出校准差距。提出的工作流程在注释语料库和MathArena上都实现了与人类评分更高的协议，并在各项指标上更一致地处理部分分数。

Conclusion: 虽然SOTA LLMs在证明评分方面表现出潜力，特别是在错误检测方面，但需要专门的工作流程来改善部分分数的分配。提出的代理工作流程显著提高了评分质量，为未来研究提供了有价值的工具和数据。
...
TL;DR: 本文提出了一种混合框架LRR，结合符号方法和LLM来修复正则表达式拒绝服务漏洞，在保持语义正确性的同时提高了修复率。


<details>
  <summary>Details</summary>
Motivation: 当前正则表达式修复方法面临两难：基于符号的规则系统精确但无法修复未见或复杂漏洞模式，而LLM具有泛化能力但在需要严格语法和语义正确性的任务中不可靠。

Method: 提出局部化正则表达式修复框架，首先使用确定性符号模块定位易受攻击的子模式，然后调用LLM为隔离的片段生成语义等效的修复。

Result: 该混合架构成功解决了基于规则修复无法处理的复杂修复案例，同时避免了纯LLM方法的语义错误，修复率比最先进方法提高了15.4个百分点。

Conclusion: 这项工作为自动修复问题提供了经过验证的方法论，展示了如何结合符号方法和LLM的优势来解决复杂修复问题。
...
TL;DR: 提出了一种连续记忆机制，通过将GUI轨迹编码为固定长度的连续嵌入，显著减少上下文成本并保留精细视觉信息。使用自动扩展的数据飞轮收集10万+轨迹，仅微调1.2%参数，在真实GUI基准测试中显著提升长视野和分布偏移下的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理将过去轨迹压缩为文本标记，导致上下文长度膨胀并丢失关键视觉线索（如精确的部件大小和位置），需要一种能够跨陌生界面和长视野任务泛化的可扩展记忆机制。

Method: 使用VLM作为编码器将GUI轨迹编码为固定长度的连续嵌入，直接插入主干网络输入层；引入自动扩展数据飞轮，通过搜索发现新环境、使用开源VLM合成任务、代理执行轨迹、同一VLM验证成功来低成本扩展记忆；仅微调记忆编码器（LoRA on Q-Former，1.2%参数）。

Result: 随着记忆大小和检索深度的增加，性能单调提升，而文本记忆在长提示下性能下降；在真实GUI基准测试中，记忆增强代理在长视野和分布偏移下持续提升成功率；Qwen-2.5-VL-7B + 连续记忆达到与GPT-4o、Claude-4等闭源模型相当的性能。

Conclusion: 连续记忆机制有效解决了GUI代理的可扩展记忆问题，在保持低成本的同时显著提升了跨界面泛化和长视野任务执行能力，证明了视觉信息保留和连续嵌入表示的重要性。
...
TL;DR: 本研究提出了一种整合精神分析和MBTI人格理论的新方法，构建了三种人工意识（自我意识、无意识和前意识）和16种MBTI人格角色，通过多种评估方法验证了人工意识的人类化认知表现。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型虽在翻译、摘要等领域取得进展，但由于幻觉问题难以模拟人类意识。本研究旨在通过整合精神分析和人格理论来构建更接近人类意识的人工智能系统。

Method: 基于精神分析原理开发了三种人工意识模块，设计了16种MBTI人格角色，并创建了包含情绪理解、逻辑思维等7个属性的10种情境来测试模型认知能力。

Result: 定量和定性分析均表明模型能够较好地模拟人类意识，尽管不同角色和意识之间的响应差异不显著。
...
TL;DR: 提出了MEC³O多专家共识系统，通过将LLMs分配到不同的时间复杂度类别并让它们进行结构化辩论，结合加权共识机制来预测代码时间复杂度，在CodeComplex数据集上显著优于开源基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在未经微调的情况下难以准确预测所有时间复杂度类别，不同模型在不同类别上各有优势，因此需要一种能够整合各模型优势的方法。

Method: MEC³O系统将LLMs根据性能分配到特定时间复杂度类别，提供类别专业化指令使其成为专家，然后让这些专家进行结构化辩论，最后通过加权共识机制整合预测结果。

Result: 在CodeComplex数据集上，MEC³O比开源基线模型准确率和macro-F1分数至少高出10%，在macro-F1分数上平均超过GPT-4o-mini，并与GPT-4o和GPT-o4-mini在F1分数上表现相当。
...
TL;DR: 提出了一种无需训练、在推理时控制流匹配模型的方法，通过特征空间目标和时间调度的随机扰动来增强生成多样性，同时保持图像质量和提示对齐。
...
Method: 同时通过特征空间目标鼓励轨迹间的横向扩展，并通过时间调度的随机扰动重新引入不确定性。关键是将扰动投影为与生成流正交，这种几何约束允许在不降低图像细节或提示保真度的情况下增强变化。

Result: 在固定采样预算下，该方法在多个文本到图像设置中持续改进了Vendi Score和Brisque等多样性指标，同时保持了图像质量和对齐度。

Conclusion: 该方法理论上显示能单调增加体积代理，同时由于几何约束近似保持边缘分布，这为生成质量稳健保持提供了原则性解释。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂网络动态演化预测中的两个主要问题：1）现有方法通常使用简单图描述网络关系，只能捕获成对关系，而网络中可能存在丰富的非成对结构化关系；2）理论预测模型缺乏准确性，数据驱动预测模型缺乏可解释性。

Method: 1）引入动态超图学习捕获复杂网络中的高阶非成对关系；2）提出物理数据双驱动动态预测模块，结合Koopman算子理论将非线性动态微分方程转化为线性系统求解，同时利用物理信息神经微分方程方法确保动态演化符合物理规律。

Result: 在公共数据集和自建产业链网络数据集上的实验验证表明，该方法具有良好的预测准确性和长期预测性能。

Conclusion: 提出的高阶网络动力学识别方法能够有效捕获复杂网络中的高阶非成对关系，通过双驱动预测模块实现了准确且可解释的长期动态预测，为复杂网络建模和控制提供了有效工具。
...
TL;DR: 本文提出了一种基于Stackelberg竞争的强化学习框架，用于训练LLM智能体在社交推理游戏中优化说服性对话，显著提升了智能体的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在社交推理游戏中主要关注信息处理和策略选择，忽视了说服性沟通在影响其他玩家信念和反应中的重要性。游戏成功不仅需要正确推理，还需要说服他人按照自己的意图行动。

Method: 将回合制对话形式化为Stackelberg竞争，当前玩家作为领导者战略性地影响跟随者的响应。基于此理论基础，提出了一个强化学习框架来训练智能体优化话语的说服效果。

Result: 在三个不同的社交推理游戏中进行全面实验，证明所提出的智能体显著优于基线方法。

Conclusion: 这项工作代表了开发具有战略性社会影响力AI智能体的重要一步，对需要说服性沟通的场景具有广泛意义。
...
TL;DR: 提出了一种名为PAC推理的方法，通过动态切换思考模式来控制性能损失，在保证用户指定性能损失容忍度的同时节省计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型虽然能解决复杂问题，但部署时计算成本高昂。现有的动态切换思考模式方法存在额外推理错误且缺乏性能损失统计保证，这在关键应用中尤为重要。

Method: 构建性能损失的上置信界，将其表示为不确定性得分的单调函数，然后确定切换到非思考模型的阈值。理论上，使用该阈值在思考和非思考模式间切换可确保分布无关的有界性能损失。

Result: 在推理基准测试上的综合实验表明，该方法能够节省计算预算并控制用户指定的性能损失。

Conclusion: PAC推理方法为大型推理模型提供了有效的效率改进方案，在保证性能损失可控的前提下显著降低计算成本。
...
TL;DR: 本文探讨了大型语言模型在医疗建议生成中存在的系统性偏见问题，研究发现LLMs对不同的社会群体（特别是原住民和双性人）生成的医疗建议在可读性和复杂性上存在差异。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在医疗领域的广泛应用，需要评估这些模型是否会在医疗建议中产生系统性偏见，特别是考虑到医疗保健的社会性质以及不同社会群体之间的健康差异。

Method: 通过模拟不同患者档案（性别、年龄、种族）提出一系列医疗问题，比较LLMs生成回答的自然语言特征，分析不同社会群体接收的医疗建议差异。

Result: 研究发现LLMs生成的医疗建议在不同社会群体间存在系统性差异，原住民和双性人患者收到的建议可读性更差、更复杂，交叉性群体的差异更为明显。

Conclusion: 鉴于人们对LLMs日益增长的信任，需要提高AI素养，并呼吁AI开发者紧急调查和缓解这些系统性差异，确保不会转化为不公正的患者支持。
...
TL;DR: 本文提出了基于知识图谱结构的医疗领域知识融合概念模型，旨在整合多种医疗知识源以支持精准的患者驱动决策制定。


<details>
  <summary>Details</summary>
Motivation: 医疗领域需要统一的方法来收集、分析和利用现有医疗知识和操作，以支持准确的基于患者的决策制定。医疗知识的复杂性和多样性要求融合多种知识源来为医疗专业人员提供决策支持。

Method: 提出基于知识图谱结构的多概念模型，评估如何实现知识融合，并展示如何将各种知识源整合到知识图谱中以用于救援操作。

Result: 开发了支持知识融合的概念模型，能够整合多种上下文对齐的知识源，为关键决策提供支持。

Conclusion: 知识图谱结构为医疗领域的知识融合提供了有效框架，能够整合多样化医疗知识源，支持医疗专业人员的决策过程。
...
TL;DR: 本文提出了一个基于PSPACE完全正则表达式问题的新基准，用于评估大型语言模型和大型推理模型的空间计算限制，揭示了模型在复杂推理任务中的常见失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在自然语言处理、数学推理和编程方面表现强劲，但其有限上下文窗口带来的空间计算限制尚未得到充分理解。现有研究主要关注NP复杂度问题，而PSPACE完全问题能更严格地评估计算能力。

Method: 通过双指数空间探索构建了包含超过100万个正则表达式实例的标注数据集，采用声音过滤过程建立基准，并对6个LLM和5个LRM进行了广泛评估。

Result: 评估揭示了模型在PSPACE完全正则表达式问题上的常见失败模式，包括冗长性和重复性，表明模型在需要大规模搜索空间探索的复杂推理任务中存在局限性。

Conclusion: 这项工作首次对LLM和LRM的空间计算限制进行了实证研究，为评估其高级推理能力提供了新框架，相关代码已开源。
...
TL;DR: 本文综述了基于大语言模型(LLM)的智能体架构和实现方法，探讨如何通过感知、推理、记忆和执行系统构建能够自动化复杂任务的'智能体化'LLM。


<details>
  <summary>Details</summary>
Motivation: 传统LLM在现实世界任务中存在局限性，研究旨在探索开发能够自动化复杂任务、缩小与人类能力差距的智能体化LLM模式。

Method: 构建包含四个关键组件的智能体架构：感知系统将环境感知转换为有意义的表示；推理系统通过思维链、思维树等技术制定计划、适应反馈和评估行动；记忆系统通过短期和长期机制保留知识；执行系统将内部决策转化为具体行动。

Result: 研究表明，整合这些系统能够创建更强大和通用的软件机器人，模拟人类认知过程以实现自主智能行为。

Conclusion: 通过集成感知、推理、记忆和执行系统，可以开发出能够自动化复杂任务、更接近人类能力的智能体化大语言模型。
...
TL;DR: 提出了一种新颖的框架，用于训练具有连续可调内部表示的大型语言模型，通过局部性调节器在可解释的局部化表示和高效的分布式表示之间动态切换。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在需要透明度的监管领域中，同时满足可解释性和高性能的需求，提供在训练和推理过程中无需重新训练即可动态调整表示局部性的能力。

Method: 使用组稀疏惩罚、信息论锚点设计和动态规则注入，通过数学证明建立明确的阈值条件，确保注意力机制集中在语义相关块上。

Result: 证明了当组稀疏惩罚超过特定阈值时，模型的注意力机制会集中在语义相关块上，实现低熵和高保真度，误差可忽略不计。

Conclusion: 该框架使从业者能够在可解释模式和高性能模式之间连续插值，支持需要透明性和能力的监管领域应用。
...
TL;DR: 小型语言模型可以通过学习底层规则（而非统计学习）解决演绎推理任务，研究发现归纳头在实现逻辑推理所需的规则补全和规则链式推理中起核心作用。
...
Method: 使用小型语言模型解决演绎推理任务，通过分析其内部表示和计算电路，特别是研究归纳头在规则补全和规则链式推理中的作用。

Result: 研究发现归纳头在实现逻辑推理所需的规则补全和规则链式推理步骤中发挥核心作用，揭示了语言模型进行逻辑推理的具体机制。

Conclusion: 小型语言模型能够通过学习底层规则而非统计学习来解决演绎推理任务，归纳头是实现逻辑推理的关键组件，这为理解语言模型的推理机制提供了重要见解。
...
TL;DR: 本文提出了序列变量作为约束编程中处理车辆路径问题的新方法，解决了传统后继变量模型在处理可选访问和插入启发式算法时的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统约束编程中基于后继变量的模型无法有效处理车辆路径问题中的可选访问和插入启发式算法，这限制了CP在VRP问题中的应用效果。

Method: 形式化定义了序列变量，包括其域定义、更新操作和一致性级别；设计了专门的数据结构和全局约束；实现了基于序列变量的插入式大邻域搜索。

Result: 序列变量简化了问题建模过程，在Dial-a-Ride问题上展现了具有竞争力的计算性能。

Conclusion: 序列变量为约束编程处理车辆路径问题提供了更直观、声明性的框架，有效克服了传统模型的局限性。
...
TL;DR: 本文综述了基于大语言模型（LLM）的智能代理系统在放射学中的应用，探讨了从半自动化工作流到自适应代理的设计方法、关键应用、评估指标以及面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 放射学具有多模态数据流和复杂工作流程的特点，非常适合利用能够适应上下文并自动化重复复杂任务的智能代理。虽然LLM在单个任务上表现出色，但单独使用未能充分发挥其在复杂多步骤工作流程中的潜力。

Method: 通过为LLM配备外部工具和反馈机制，构建能够展示不同程度自主性的系统，包括半自动化工作流和能够管理复杂流程的自适应代理。

Result: LLM驱动的代理系统在放射学中已显示出应用潜力，能够整合多源信息并执行复杂决策过程。

Conclusion: LLM驱动的智能代理系统为放射学工作流程自动化提供了有前景的解决方案，但仍需解决错误级联、工具使用效率和医疗IT集成等挑战。
...
TL;DR: 本文探讨了在数据湖仓中如何通过API优先的可编程架构实现安全的人工智能代理工作流，使用Bauplan作为案例研究，展示了数据分支和声明式环境如何自然扩展到代理，实现可重现性和可观测性，同时减少攻击面。


<details>
  <summary>Details</summary>
Motivation: 数据湖仓运行敏感工作负载，AI驱动的自动化引发了关于信任、正确性和治理的担忧。需要为安全设计的人工智能代理工作流提供合适的抽象。

Method: 使用Bauplan作为案例研究，采用数据分支和声明式环境的方法，提出了受证明携带代码启发的正确性检查机制，允许不受信任的AI代理在生成数据上安全操作。

Result: 原型系统证明不受信任的AI代理可以在生成数据上安全操作，并展示了实现完全代理化数据湖仓的路径。

Conclusion: API优先的可编程湖仓为安全设计的人工智能代理工作流提供了正确的抽象，通过数据分支和声明式环境实现了可重现性、可观测性和安全性。
...
<details>
  <summary>Details</summary>
Motivation: 神经符号AI领域近三十年来未能实现其潜力，主要因为现有框架无法扩展，且神经方法的隐式表示和近似推理限制了可解释性和可信度。知识图谱作为显式语义知识的黄金标准表示可以解决符号方面的问题，但从文本语料库自动推导可靠知识图谱仍是一个开放问题。
...
TL;DR: LiveOIBench是一个包含403个奥林匹克级编程竞赛题目的基准测试，每个题目平均有60个专家设计的测试用例，旨在解决现有编程基准测试在挑战性、测试覆盖率和可访问性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前编程基准测试存在缺乏高难度问题、测试用例覆盖不足、依赖在线平台API导致可访问性差等问题，需要更全面、高质量的评估标准。

Method: 从72个不同地区的官方信息学奥林匹克竞赛中收集403个专家策划的高质量编程问题，每个问题配备详细子任务评分标准和大量私有测试用例，并集成精英选手表现数据进行对比。

Result: 在评估32个流行LLM后，GPT-5达到81.76百分位，表现良好但仍不及顶尖人类选手（通常超过90百分位），而开源模型GPT-OSS-120B仅达到60百分位，显示与前沿闭源模型存在显著差距。

Conclusion: 强大的推理模型应优先进行精确的问题分析而非过度探索，未来模型应强调结构化分析并减少不必要的探索。所有数据、代码和排行榜结果将公开提供。
...
### [240] [Personalize Before Retrieve: LLM-based Personalized Query Expansion for User-Centric Retrieval](https://arxiv.org/abs/2510.08935)
...
TL;DR: 本文提出PBR框架，通过用户特定的查询扩展来解决RAG系统中的个性化问题，包含P-PRF和P-Anchor两个组件，在个性化基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统的查询扩展方法采用统一策略，忽略了用户特定的语义，包括个人表达风格、偏好和历史上下文，导致在个性化设置中泛化能力有限。

Method: 提出PBR框架，包含：1) P-PRF：利用用户历史生成风格对齐的伪反馈来模拟用户表达风格；2) P-Anchor：基于图的用户语料结构对齐来捕捉用户语料结构。
...
### [241] [SHERLOCK: Towards Dynamic Knowledge Adaptation in LLM-enhanced E-commerce Risk Management](https://arxiv.org/abs/2510.08948)
*Nan Lu,Yurong Hu,Jiaquan Fang,Yan Liu,Rui Dong,Yiming Wang,Rui Lin,Shaoyi Xu*

Main category: cs.IR

TL;DR: 提出了SHERLOCK框架，利用大语言模型的推理能力辅助风险调查，通过构建领域知识库、智能平台和Reflect & Refine模块，显著提高了风险调查的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 电商行业增长加剧了风险管理的挑战，大量案件分析给分析师带来沉重负担，且分析师个体差异阻碍了标准化工作流程的建立。

Method: SHERLOCK框架包含三个主要组件：从多模态数据提取风险管理知识构建知识库；基于数据飞轮范式构建智能平台，集成日常操作、专家标注和模型评估；引入Reflect & Refine模块与领域知识库协作建立快速响应机制。

Result: 在京东真实交易数据集上的实验表明，该方法显著提高了LLM分析结果的事实对齐精度和风险定位精度。基于SHERLOCK的LLM系统部署显著提升了风险管理人员案件调查工作流程的效率。

Conclusion: SHERLOCK框架有效解决了电商风险管理中的工作负担和标准化问题，通过LLM推理能力与领域知识结合，为风险调查提供了高效可靠的解决方案。
...
### [242] [Rethinking Reasoning in Document Ranking: Why Chain-of-Thought Falls Short](https://arxiv.org/abs/2510.08985)
*Xuan Lu,Haohang Huang,Rui Meng,Yaohui Jin,Wenjun Zeng,Xiaoyu Shen*

Main category: cs.IR

TL;DR: 本文首次系统研究了在文档重排序任务中引入推理链（CoT）的有效性，发现在点排序和列表排序两种设置下，带有推理的重排序器表现均不如直接预测排名的模型，尽管推理带来了显著更高的推理成本。


<details>
  <summary>Details</summary>
Motivation: 受大型推理模型（LRMs）启发，近期研究开始将显式推理链（CoT）引入基于LLM的重排序器，但推理在排序任务中的有效性尚未得到充分探索。

Method: 在点排序和列表排序两种设置下，分别使用监督微调和强化学习方法，在推理密集型数据集（BRIGHT）和标准IR基准（BEIR）上进行系统评估。

Result: 推理增强的重排序器在点排序中破坏了校准性，偏向正类，增加了假阳性；在列表排序中提高了域内拟合但增加了方差且无法泛化到域外；直接微调的重排序器更稳定、有效和鲁棒。

Conclusion: 显式推理并非对重排序普遍有益，未来方向包括点排序中的校准感知评分，以及设计简洁、有针对性的推理策略来减轻列表排序中的过拟合和过度思考问题。
...
### [243] [Generative Data Augmentation in Graph Contrastive Learning for Recommendation](https://arxiv.org/abs/2510.09129)
*Yansong Wang,Qihui Lin,Junjie Huang,Tao Jia*

Main category: cs.IR

TL;DR: 提出GDA4Rec框架，使用生成式数据增强方法在推荐系统的图对比学习中生成高质量增强视图，通过噪声生成模块和项目互补矩阵提供鲁棒的自监督信号。


<details>
  <summary>Details</summary>
Motivation: 推荐系统中从稀疏用户-物品交互中学习有效嵌入是一个基本挑战。现有对比学习方法使用随机数据增强会改变原始语义信息，需要生成更高质量的增强视图。

Method: 使用深度生成模型近似原始数据分布进行数据增强；提取项目互补矩阵表征物品间潜在相关性；采用结合推荐、数据增强和对比学习的联合目标函数。

Result: 在三个公共数据集上的广泛实验证明了模型的优越性。

Conclusion: GDA4Rec框架能够生成高质量的增强视图，提供鲁棒的自监督信号，学习更有效和信息丰富的嵌入表示。
...
### [244] [Controlled Personalization in Legacy Media Online Services: A Case Study in News Recommendation](https://arxiv.org/abs/2510.09136)
*Marlene Holzleitner,Stephan Leitner,Hanna Lind Jorgensen,Christoph Schmitz,Jacob Welander,Dietmar Jannach*

Main category: cs.IR

TL;DR: 传统新闻媒体采用受控个性化策略，在编辑策划内容中加入算法推荐，A/B测试表明即使适度个性化也能显著提高点击率、降低导航成本，同时增加内容多样性并减少流行度偏见。


<details>
  <summary>Details</summary>
Motivation: 传统新闻媒体在采用个性化推荐技术时面临平衡技术创新与编辑价值观的挑战，需要找到既能提升用户体验又能维护新闻专业性的方法。

Method: 在挪威主要传统新闻机构的网站上进行了A/B测试，比较编辑策划内容与受控个性化（结合编辑策划和算法推荐）的效果。

Result: 受控个性化显著提高了点击率，降低了用户导航成本，增加了内容多样性和目录覆盖率，同时减少了流行度偏见。

Conclusion: 受控个性化能够成功协调用户需求与编辑目标，为传统媒体在采用个性化技术的同时维护新闻价值观提供了可行路径。
...
### [245] [Hierarchical Semantic RL: Tackling the Problem of Dynamic Action Space for RL-based Recommendations](https://arxiv.org/abs/2510.09167)
*Minmao Wang,Xingchen Liu,Shijie Yi,Likang Wu,Hongke Zhao,Fei Pan,Qingpeng Cai,Peng Jiang*

Main category: cs.IR

TL;DR: HSRL是一种用于推荐系统的分层强化学习方法，通过固定语义动作空间解决大规模动态动作空间问题，在公开基准和实际生产环境中均优于现有方法。
...
Method: HSRL引入固定语义动作空间，将物品编码为语义ID进行策略学习，通过分层策略网络进行粗到细的决策，并使用多级评论家进行细粒度信用分配。

Result: 在公开基准和大型短视频广告平台生产数据集上，HSRL持续超越最先进基线方法。在线A/B测试中，CVR提升18.421%，成本仅增加1.251%。

Conclusion: HSRL为基于强化学习的推荐系统提供了一个可扩展的范式，能够有效处理大规模动作空间并实现长期用户价值优化。
...
### [246] [ChoirRec: Semantic User Grouping via LLMs for Conversion Rate Prediction of Low-Activity Users](https://arxiv.org/abs/2510.09393)
...
<details>
  <summary>Details</summary>
Motivation: 解决电商推荐系统中低活跃用户转化率预测的三个关键挑战：依赖嘈杂行为信号、用户级信息不足、训练偏向高活跃用户。

Method: 提出ChoirRec框架，包含三个组件：语义组生成模块使用LLM构建跨活跃度用户集群；组感知分层表示模块用组级先验丰富稀疏用户嵌入；组感知多粒度模块采用双通道架构和自适应融合机制。

Result: 在淘宝平台上的离线实验显示GAUC提升1.16%，在线A/B测试显示订单量增加7.24%。

Conclusion: ChoirRec通过利用LLM的语义能力构建用户组，有效解决了低活跃用户转化率预测的挑战，在实际应用中具有显著价值。
...
### [247] [MRMR: A Realistic and Expert-Level Multidisciplinary Benchmark for Reasoning-Intensive Multimodal Retrieval](https://arxiv.org/abs/2510.09510)
*Siyue Zhang,Yuan Gao,Xiao Zhou,Yilun Zhao,Tingyu Song,Arman Cohan,Anh Tuan Luu,Chen Zhao*

Main category: cs.IR

TL;DR: MRMR是首个需要深度推理的专家级多学科多模态检索基准，包含1502个查询覆盖23个领域，通过引入多领域专家级查询、推理密集型任务和图像文本交织序列，为多模态检索系统提供了更现实的评估环境。


<details>
  <summary>Details</summary>
Motivation: 现有检索基准主要关注简单查询和单模态文档，缺乏对多领域专业知识、深度推理能力和多模态交互的评估。MRMR旨在填补这一空白，推动多模态检索在真实复杂场景中的发展。

Method: 构建包含1502个查询的基准数据集，涵盖23个专业领域；引入矛盾检索新任务；采用图像文本交织序列构建查询和文档；对4类多模态检索系统和14个前沿模型进行全面评估。

Result: 基于文本嵌入模型Qwen3-Embedding配合LLM生成图像描述的方法表现最佳；最新多模态模型在专家领域查询上表现良好，但在推理密集型任务上仍有不足；整体显示多模态检索模型仍有很大改进空间。

Conclusion: MRMR为推进多模态检索在更现实和挑战性场景中的发展奠定了基础，揭示了当前模型在推理能力和多模态理解方面的局限性，指明了未来研究方向。
...
### [248] [Doc2Query++: Topic-Coverage based Document Expansion and its Application to Dense Retrieval via Dual-Index Fusion](https://arxiv.org/abs/2510.09557)
*Tzu-Lin Kuo,Wei-Ning Chiu,Wei-Yun Ma,Pu-Jen Cheng*

Main category: cs.IR

TL;DR: Doc2Query++是一个文档扩展框架，通过无监督主题建模推断文档的潜在主题，然后使用混合关键词选择为每个文档创建多样且相关的关键词集，指导LLM生成查询。该方法还提出双索引融合策略来隔离文本和查询信号，提升密集检索性能。


<details>
  <summary>Details</summary>
Motivation: 解决文档扩展中存在的三个主要问题：查询生成缺乏控制导致幻觉或冗余；从领域内训练到领域外数据的泛化能力差；查询拼接产生的噪声对密集检索的负面影响。

Method: 1. 通过无监督主题建模推断文档的潜在主题；2. 使用混合关键词选择创建多样且相关的关键词集；3. 指导LLM利用关键词生成查询；4. 提出双索引融合策略隔离文本和查询信号。

Result: Doc2Query++在多个数据集上显著优于现有最先进基线，在MAP、nDCG@10和Recall@100等指标上获得显著提升，适用于稀疏和密集检索。

Conclusion: Doc2Query++通过结构化查询生成和双索引融合策略，有效解决了文档扩展中的关键挑战，在跨领域检索任务中表现出色。
...
### [249] [Energy-Driven Steering: Reducing False Refusals in Large Language Models](https://arxiv.org/abs/2510.08646)
...
TL;DR: 提出Energy-Driven Steering (EDS)框架，通过基于能量的模型在推理时动态干预LLM隐藏状态，同时提高安全性并降低误拒率，无需微调。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全对齐技术往往只关注对抗有害提示的安全性，导致模型变得过度谨慎，拒绝响应良性提示。需要同时增强安全性并减少误拒率。

Method: 训练轻量级外部基于能量模型(EBM)，为不良状态(误拒或越狱)分配高能量，为理想状态(有帮助响应或安全拒绝)分配低能量。在推理时使用能量函数梯度动态引导LLM隐藏状态到低能量区域。

Result: 在广泛模型上的实验表明，该方法显著降低误拒率，例如在ORB-H基准上合规率从57.3%提升至82.6%，同时保持基线安全性能。

Conclusion: EDS提供了一个有效的范式，可构建同时具有低误拒率和高安全性的LLM，将行为控制与模型核心知识解耦，提供灵活且计算开销小的解决方案。
...
### [250] [Inverse-Free Wilson Loops for Transformers: A Practical Diagnostic for Invariance and Order Sensitivity](https://arxiv.org/abs/2510.08648)
*Edward Y. Chang,Ethan Y. Chang*

Main category: cs.LG

TL;DR: WILSON是一个最小化的后处理诊断套件，用于检测大语言模型在无害编辑下的不稳定性问题，如段落重排序、微调、提示路径等导致的答案翻转，通过计算内部表示的曲率图和激活级交换子来识别重排序风险。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在实际应用中存在不稳定性问题：RAG输出在段落重排序时翻转、微调侵蚀预训练学到的恒定性、辩论或思维链提示依赖路径、编译器融合或重排序在决策边界附近扰动logits。这些违反预期恒定性，破坏持续集成，迫使团队在安全性和速度之间权衡。

Method: WILSON结合了无逆曲率图（使用JVP和Hutchinson探针计算）和激活级交换子来标记重排序风险。信号计算成本低，适用于标准Transformer模型，输出为阈值和CSV文件供编排器使用。
...
### [251] [Knowledge Graph Sparsification for GNN-based Rare Disease Diagnosis](https://arxiv.org/abs/2510.08655)
...
Conclusion: RareNet仅需表型数据即可实现复杂基因分析，为缺乏先进基因组基础设施的服务不足人群提供了民主化的诊断工具。
...
### [252] [Inner-Instance Normalization for Time Series Forecasting](https://arxiv.org/abs/2510.08657)
*Zipo Jibao,Yingyi Fu,Xinyang Chen,Guoting Chen*

Main category: cs.LG

TL;DR: 本文提出了两种点级方法（LD和LCD）来解决时间序列预测中的实例内分布偏移问题，通过在不同时间步使用不同参数来消除内部差异，在多个基准测试中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现实世界的时间序列受到多种因素影响，表现出复杂的非平稳特性。非平稳性会导致分布偏移，即时间序列的统计特性随时间变化，这会负面影响模型性能。现有的实例归一化技术未能考虑单个实例内部的分布偏移，导致性能不理想。

Method: 提出了两种点级方法：学习分布（LD）和学习条件分布（LCD）。LD通过在不同时间步使用不同参数来拟合输入和输出的内部分布，从而消除内部差异；LCD利用神经网络预测输出的缩放系数。

Result: 在多个公共基准测试中使用各种骨干模型评估了两种方法的性能，通过对比实验证明了点级范式的有效性。

Conclusion: 提出的点级方法能够有效解决时间序列预测中的实例内分布偏移问题，相比现有方法具有更好的性能表现。
...
### [253] [Provably Robust Adaptation for Language-Empowered Foundation Models](https://arxiv.org/abs/2510.08659)
*Yuni Lai,Xiaoyu Xue,Linghui Shen,Yulun Wu,Gaolei Li,Song Guo,Kai Zhou,Bin Xiao*

Main category: cs.LG

TL;DR: 本文提出了LeFCert，这是首个针对语言赋能基础模型（LeFMs）的可证明鲁棒小样本分类器。它通过整合文本和特征嵌入，采用自适应混合机制，并提出了双重修剪均值原型来获得可证明的鲁棒性。还开发了LeFCert-L和LeFCert-C两个变体来增强性能。


<details>
  <summary>Details</summary>
Motivation: 现有的语言赋能基础模型（如CLIP、GraphCLIP）在小样本学习中依赖开放环境中收集的小型任务特定支持数据集，容易受到投毒攻击。现有防御方法缺乏形式化保证，且对未知和自适应攻击仍然脆弱。可证明鲁棒性方法在基于LeFMs的小样本分类器中尚未得到充分探索。

Method: 1. 提出LeFCert模型，整合文本和特征嵌入，采用自适应混合机制；2. 提出双重修剪均值原型，推导分类分数的可证明上下界；3. 开发两个变体：LeFCert-L通过随机平滑提供Lipschitz连续性，LeFCert-C提供集体认证；4. 在更现实和更严格的攻击预算下工作。

Result: 实验表明LeFCert实现了最先进的性能，与现有基线相比显著提高了清洁准确率和认证准确率。尽管具有先进的鲁棒性机制，LeFCert计算效率高，适合实际应用。

Conclusion: LeFCert填补了语言赋能基础模型在小样本分类中可证明鲁棒性的关键空白，提供了首个针对此类模型的认证防御方法，在保持计算效率的同时显著提升了鲁棒性。
...
### [254] [How Scale Breaks "Normalized Stress" and KL Divergence: Rethinking Quality Metrics](https://arxiv.org/abs/2510.08660)
*Kiran Smelser,Kaviru Gunaratne,Jacob Miller,Stephen Kobourov*

Main category: cs.LG

TL;DR: 本文分析了高维数据可视化中常用的质量指标（归一化应力和KL散度）对尺度变化的敏感性，并提出了一种简单方法使这些指标对尺度变化不敏感。


<details>
  <summary>Details</summary>
Motivation: 高维数据可视化中，二维散点图是常用方法，但视觉评估其准确性具有挑战性，因此需要质量指标。然而常用的归一化应力和KL散度指标都对尺度变化敏感，即使尺度变化不改变投影的本质特征。

Method: 通过分析和实证研究尺度变化对应力和KL散度的影响，提出了一种简单技术使这两个指标对尺度变化不敏感，并在小型基准测试中验证了其有效性。

Result: 研究表明尺度的微小变化会显著改变应力和KL散度的值，从而影响降维技术的评估结果。提出的尺度不变方法在基准测试中准确捕捉了预期行为。

Conclusion: 现有可视化质量指标对尺度变化过于敏感，提出的尺度不变方法能够更准确地评估降维投影的质量，避免尺度变化带来的误导性评估结果。
...
TL;DR: CATS-Linear模型通过分类辅助通道独立机制，将不同时间序列实例动态路由到专用预测器，并结合趋势-季节分解架构，在固定超参数下实现了与调优基线相竞争的最先进预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明线性模型在预测性能上能与复杂架构竞争，但增强线性模型的方法仍待探索。基于不同时间序列实例可能遵循异质线性映射的假设，需要开发更有效的线性模型增强方法。

Method: 提出CATS-Linear模型，采用分类辅助通道独立机制动态路由实例到专用预测器，重新设计趋势-季节分解架构，包含解耦-线性映射-重耦合框架处理趋势分量，以及复数域线性投影处理季节分量。

Result: 大量实验验证CATS-Linear在固定超参数下实现了与超参数调优基线相竞争的最先进精度，同时在固定超参数对比中达到SOTA精度。

Conclusion: CATS-Linear通过分类辅助通道独立和重新设计的分解架构，有效提升了线性模型的预测性能，在固定超参数设置下实现了优异的预测精度。
...
<details>
  <summary>Details</summary>
Motivation: 传统基因组选择方法在处理复杂性状和大数据集时预测精度不足，需要更先进的模型来准确建模基因型-表型关系。

Method: 使用8维one-hot编码表示SNP数据，按染色体排序，采用PMF算法进行特征选择，结合CNN和自注意力机制构建DPCformer模型。

Result: 在5种作物的13个性状上测试：玉米性状精度提升达2.92%，棉花纤维性状精度提升8.37%，番茄小样本场景PCC提升57.35%，鹰嘴豆产量相关性提升16.62%。
...
TL;DR: 本文提出了一种频率感知缓存方法FreqCa，通过分析扩散模型中不同频率分量的动态特性，对低频分量直接重用，对高频分量使用二阶Hermite插值预测，显著降低了扩散变压器的推理成本。


<details>
  <summary>Details</summary>
Motivation: 扩散变压器的推理成本过高，现有的特征缓存方法假设相邻时间步的特征相似或连续，但这一假设并不总是成立。通过频域分析发现，不同频率分量在时间步间表现出不同的动态特性：低频分量相似度高但连续性差，高频分量连续性好但相似度低。

Method: 提出频率感知缓存FreqCa：对低频分量基于相似性直接重用特征，对高频分量使用二阶Hermite插值器基于连续性进行预测。同时提出缓存累积残差特征而非所有层特征，将内存占用减少99%。

Result: 在FLUX.1-dev、FLUX.1-Kontext-dev、Qwen-Image和Qwen-Image-Edit等多个数据集上的实验证明了该方法在生成和编辑任务中的有效性。

Conclusion: FreqCa通过利用不同频率分量的不同动态特性，实现了高效的特征缓存，显著降低了扩散变压器的推理成本，同时保持了生成和编辑质量。
...
### [258] [Don't Waste Mistakes: Leveraging Negative RL-Groups via Confidence Reweighting](https://arxiv.org/abs/2510.08696)
...
TL;DR: 提出了一种模型无关的测试时优化范式MATT，通过利用特征组合的置信度分数来生成多个推理路径，从而减少低置信度特征对最终预测的影响，提高CTR模型的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注训练阶段的优化，忽视了推理阶段的优化机会。低频特征组合会降低预测性能，导致不可靠或低置信度的输出。
...
TL;DR: 本文研究了基于上下文学习(ICL)的通道均衡器在非平稳时变信道中的性能，提出了从经典自适应信号处理算法(如LMS、LRMS)中推导出的新型注意力机制变体，以增强在动态环境中的适应性和性能。


<details>
  <summary>Details</summary>
Motivation: 传统的基于监督学习的通道均衡方法需要为每个新任务进行昂贵的重新训练或微调，而现有的ICL均衡器主要针对静态信道设计。本文旨在探索ICL在非平稳时变信道均衡中的能力，解决现有ICL理论和分析主要关注平稳设置的问题。

Method: 采用原则性框架设计高效的注意力机制，利用自适应信号处理算法(如LMS、LRMS)来指导更好的设计，推导出新的注意力变体，包括多步梯度更新以改进长期跟踪能力。

Result: 实验结果表明，ICL在非平稳MIMO均衡中表现出强大潜力，受经典自适应算法启发的注意力机制能显著增强动态环境中的适应性和性能。

Conclusion: ICL在非平稳信道均衡中具有重要应用价值，基于经典自适应算法的注意力机制设计为开发具有更强适应性和鲁棒性的下一代无线基础模型提供了关键见解。
...
TL;DR: 实例判别是一种自监督表示学习方法，将数据集中的每个实例视为不同类别，通过生成两个不同的视图来学习对变换不变的表示。


<details>
  <summary>Details</summary>
Motivation: 为了在没有人工标注的情况下学习有效的视觉表示，通过将每个图像实例视为独立类别来训练模型识别实例本身而非语义类别。

Method: 对每个实例应用随机变换生成两个不同的视图，训练模型学习对这些变换不变的表示，使同一实例的不同视图在表示空间中相近。

Result: 该方法能够学习到对图像变换具有鲁棒性的表示，为下游任务提供有效的特征提取能力。

Conclusion: 实例判别是一种有效的自监督学习范式，通过将实例作为类别进行判别学习，能够学习到对变换不变的视觉表示。
...
### [262] [Counterfactually Fair Conformal Prediction](https://arxiv.org/abs/2510.08724)
*Ozgur Guldogan,Neeraj Sarna,Yuanyuan Li,Michael Berger*

Main category: cs.LG

TL;DR: 本文提出了Counterfactually Fair Conformal Prediction (CF-CP)方法，通过对称化保护属性干预的符合度分数，在保持边际覆盖特性的同时实现反事实公平的预测集。


<details>
  <summary>Details</summary>
Motivation: 现有的反事实公平研究主要关注点预测器，而预测集在不确定性下的公平决策中至关重要但研究不足。同时，保形预测能提供有效的预测集但不保证反事实公平性。

Method: 通过对称化保护属性干预的符合度分数来开发CF-CP方法，该方法保持训练自由，在回归和分类任务中都能应用。

Result: 在合成和真实数据集上的实验表明，CF-CP实现了期望的反事实公平性，达到了目标覆盖率，且预测集大小的增加最小。
...
### [263] [Cross-attention Secretly Performs Orthogonal Alignment in Recommendation Models](https://arxiv.org/abs/2510.09435)
...
TL;DR: 本文提出了正交对齐现象，发现当交叉注意力的查询输入与输出正交时，模型性能显著提升。正交对齐自然出现且无需显式约束，能改善缩放定律，使交叉注意力模型在参数效率上优于参数匹配的基线模型。


<details>
  <summary>Details</summary>
Motivation: 理解交叉注意力在跨域序列推荐中的工作机制，现有研究主要将其视为残差对齐，但作者发现还存在正交对齐现象，两种机制可以共存。

Method: 通过300多次实验验证正交对齐现象，分析交叉注意力模块在跨域序列推荐中的作用机制，比较参数匹配的基线模型与包含交叉注意力的模型。

Result: 当查询输入与交叉注意力输出正交时，模型性能显著提升；包含交叉注意力的基线模型在准确率-模型参数效率上优于参数匹配的基线模型。

Conclusion: 正交对齐自然出现且能改善缩放定律，为多模态研究中的参数高效缩放提供了新方向。
...
TL;DR: 本文为大型语言模型的推理时控制技术提供了理论基础，解释了如何通过修改内部状态（激活向量或权重矩阵）来控制模型行为，并将文本输入转化为可重用的权重更新。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型控制技术虽然有效，但主要基于经验启发式方法，缺乏理论支撑。本文旨在为这些干预技术提供数学基础，解释它们如何从Transformer架构的基本计算中自然产生。

Method: 基于Dherin等人(2025)的研究，将理论推广到深度多块Transformer架构。通过数学分析展示用户提示信息如何在内部通过权重向量和权重矩阵表示和组合，并推导出将信息压缩为token无关的思想向量和思想矩阵的原则性方法。

Result: 开发了将文本输入转化为可重用权重更新的理论基础，为现有的向量和矩阵模型编辑技术提供了理论解释，并提供了计算上更基础的方法。

Conclusion: 本文为大型语言模型的推理时控制技术建立了坚实的理论基础，展示了如何从Transformer架构的基本计算中自然地推导出这些干预技术，为更原则性的模型控制方法铺平了道路。
...
TL;DR: 本文提出了一种基于SHAP值聚类的可解释AI方法，通过将SHAP值进行聚类来识别样本获得相同预测的相似原因，并在阿尔茨海默病案例中展示了该方法的应用。


<details>
  <summary>Details</summary>
Motivation: 随着大数据和黑盒模型的普及，这些模型缺乏可解释性，在高风险场景中难以被信任。需要开发能够解释模型预测过程的方法。

Method: 使用SHAP分析量化每个特征对预测的贡献，然后对SHAP值进行聚类，将获得相同预测且预测原因相似的样本分组。

Result: 通过模拟实验和阿尔茨海默病案例研究验证了该方法的有效性，能够识别样本获得相同预测的不同路径。

Conclusion: SHAP值聚类提供了一种有效的方式来理解黑盒模型的决策过程，增强了模型的可解释性和可信度。
...
TL;DR: 本文提出了一个双方法框架，通过代理模型解释复杂时间序列集成模型的预测，并结合频谱可预测性分析来量化时间序列的内在可预测性，从而解决AutoML系统在时间序列预测中的可解释性和可预测性挑战。


<details>
  <summary>Details</summary>
Motivation: 现代时间序列预测越来越依赖AutoML系统生成的复杂集成模型，虽然精度高但缺乏透明度和可解释性。本文旨在解决复杂时间序列集成模型的可解释性和可预测性评估问题。

Method: 1. 开发基于代理模型的解释方法：训练LightGBM模型模拟AutoGluon的时间序列预测，实现稳定的SHAP特征归因；2. 集成频谱可预测性分析：通过比较时间序列的频谱可预测性与纯噪声基准，量化每个序列的内在可预测性。

Result: 在M5数据集上的实证评估表明：更高的频谱可预测性不仅与改进的预测精度强相关，还与代理模型和原始预测模型之间的更高保真度相关。特征注入实验验证了提取的SHAP值与已知真实效应之间的高度忠实性。

Conclusion: 该框架为最先进的集成预测提供了可解释的实例级解释，同时为用户提供了可预测性指标，作为预测及其解释的可靠性指示器。逐项归一化对于在具有不同尺度的异构时间序列中生成有意义的SHAP解释至关重要。
...
TL;DR: DemoDiff是一个基于演示条件扩散模型的分子设计方法，通过少量分子-评分示例定义任务上下文，指导去噪Transformer生成符合目标性质的分子。该方法在33个设计任务中表现优异，超越大100-1000倍的语言模型。


<details>
  <summary>Details</summary>
Motivation: 解决现有上下文学习在分子设计中效果有限的问题，特别是针对ChEMBL等数据库中每个属性的标记数据稀缺的挑战。

Method: 开发演示条件扩散模型(DemoDiff)，使用少量分子-评分示例定义任务上下文；提出新的分子分词器Node Pair Encoding，在基序层面表示分子，减少5.5倍节点；构建包含数百万上下文任务的数据集，预训练7亿参数模型。
...
TL;DR: RFOD是一个基于随机森林的表格数据异常检测框架，通过特征级条件重构处理混合类型数据，结合调整的Gower距离和不确定性加权平均，在15个真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 表格数据异常检测在网络安全、金融欺诈检测和医疗保健等高风险领域至关重要，但现有方法在处理混合类型数据时存在语义信息丢失和可解释性不足的问题。

Method: 将异常检测重新定义为特征级条件重构问题，为每个特征训练专门的随机森林模型，结合调整的Gower距离进行单元格级评分，使用不确定性加权平均聚合为行级异常分数。

Result: 在15个真实数据集上的广泛实验表明，RFOD在检测准确性上持续优于最先进的基线方法，同时对混合类型表格数据具有更好的鲁棒性、可扩展性和可解释性。

Conclusion: RFOD框架有效解决了混合类型表格数据异常检测中的挑战，在保持语义完整性的同时提供了精确且可解释的检测结果。
...
TL;DR: 本文提出了一种控制优化确定性等价(OCE)风险的方法，扩展了传统的保形风险控制框架，能够处理尾部风险，并引入了端到端的保形风险训练方法，在保持可证明风险保证的同时显著提升了平均性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然预测准确率高，但缺乏可证明的风险或可靠性保证，这在高风险应用中至关重要。传统的保形风险控制方法只能控制期望损失，无法处理对尾部风险敏感的实际应用。

Method: 开发了控制OCE风险类的方法，包括条件风险价值(CVaR)等尾部风险度量；引入了端到端的保形风险训练方法，在模型训练或微调过程中通过保形OCE风险控制进行反向传播。

Result: 该方法在控制分类器假阴性率和电池储能操作中的金融风险等应用中，相比后处理方法显著提升了平均性能，同时保持了可证明的风险保证。
...
TL;DR: 本文提出了一个联邦学习框架，用于量化客户端内部和客户端之间的细粒度跨样本记忆化风险，填补了现有方法只能检测单个样本记忆化的不足。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习记忆化检测技术只关注单个样本，低估了跨样本记忆化的风险。而集中式学习的细粒度记忆化评估方法无法直接应用于联邦学习，因为无法集中访问数据。

Method: 提出了一个框架，通过在所有客户端上进行细粒度跨样本记忆化测量，来量化联邦学习中的客户端内部和客户端间记忆化。

Result: 研究发现联邦学习模型确实会记忆客户端数据，特别是客户端内部数据的记忆化程度高于客户端间数据，记忆化程度受训练和推理因素的影响。

Conclusion: 联邦学习存在数据记忆化风险，需要更细粒度的检测方法，记忆化程度受多种因素影响，为联邦学习隐私保护提供了重要见解。
...
TL;DR: 本文提出了LOTION框架，通过随机噪声平滑方法解决量化神经网络优化中的梯度问题，保证标准优化器的收敛性，并在语言模型上表现优于标准QAT方法。


<details>
  <summary>Details</summary>
Motivation: 量化神经网络优化面临根本性挑战，因为量化器是分段常数函数，在量化阈值处梯度未定义，现有方法如STE缺乏收敛保证。

Method: 提出LOTION框架，受Nesterov平滑启发，用无偏随机舍入噪声的期望来近似量化损失表面，将原始量化损失替换为平滑版本。

Result: 在该框架下，标准优化器能保证收敛到损失表面的局部最小值；使用随机舍入噪声时，原始量化损失的全局最小值得以保留；在合成测试集和150M/300M参数语言模型上优于标准QAT。

Conclusion: LOTION提供了一个有理论保证的量化优化框架，有效解决了量化神经网络训练中的梯度问题，并在实际应用中表现出色。
...
TL;DR: 本文提出了一种名为Spatial Deconfounder的两阶段方法，通过利用干扰揭示潜在混杂因素的结构，解决了空间因果推断中未测量的空间混杂因素和干扰问题。该方法使用条件变分自编码器从局部治疗向量重建替代混杂因素，并通过灵活的结果模型估计因果效应。


<details>
  <summary>Details</summary>
Motivation: 空间因果推断面临两个交织的挑战：未测量的空间混杂因素（如天气、空气污染等）和附近治疗的干扰。现有方法通常只解决其中一个问题而忽略另一个，但作者发现这两个问题实际上是相互关联的——干扰揭示了潜在混杂因素的结构。

Method: 提出了Spatial Deconfounder两阶段方法：第一阶段使用具有空间先验的条件变分自编码器从局部治疗向量重建替代混杂因素；第二阶段通过灵活的结果模型估计因果效应。该方法不需要多种治疗类型或已知的潜在场模型。

Result: 在扩展的SpaCE基准套件（包含治疗干扰）上进行了实证评估，结果显示Spatial Deconfounder在环境健康和社会科学领域的真实数据集上持续改善了效应估计。

Conclusion: 通过将干扰转化为多原因信号，该框架连接了空间和去混杂文献，推进了结构化数据中的稳健因果推断，能够在弱假设下非参数识别直接效应和溢出效应。
...
TL;DR: 本研究提出了一种结合虚拟成像工具和强化学习的方法来优化CT协议，使用PPO算法最大化肝脏病灶的可检测性指数，相比穷举搜索减少了79.7%的计算步骤，同时达到了全局最优解。


<details>
  <summary>Details</summary>
Motivation: CT协议优化对于实现高质量诊断图像同时最小化辐射剂量至关重要，但传统方法依赖参数组合的穷举测试，这在实践中往往不可行。

Method: 使用验证的CT模拟器对人体肝脏病灶模型进行成像，结合新型CT重建工具包，采用近端策略优化(PPO)算法在包含管电压、管电流、重建核、层厚和像素大小的参数空间中优化图像质量目标。

Result: 强化学习方法在所有测试案例中都达到了全局最大可检测性指数(d')，同时比在超级计算机上进行的穷举搜索减少了79.7%的步骤。

Conclusion: 该框架具有灵活性，可适应各种图像质量目标，研究结果突显了将虚拟成像工具与强化学习相结合用于CT协议管理的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习策略在面临新的机器人、任务或不同物理参数环境时往往泛化能力不足，这限制了其实际应用。

Method: 利用白金汉π定理，通过在无量纲空间中缩放预训练策略的输入（观测）和输出（动作）来适应新的系统上下文，无需重新训练。

Result: 在三个复杂度递增的环境中验证：模拟钟摆、物理钟摆（sim-to-real验证）和高维HalfCheetah。缩放迁移在动态相似上下文中无性能损失，在非相似上下文中始终优于朴素迁移方法。
...
TL;DR: 本文提出了一种新的结构感知文本嵌入方法，通过在LLM内部编码过程中直接整合结构关系（如超链接或引用），而不是依赖传统的后处理聚合。通过零样本实验验证了该方法在检索、聚类、分类和推荐任务中的优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM文本嵌入模型通常只处理原始文本，忽略了现实世界数据集中丰富的结构信息（如超链接、引用等），这些结构信息提供了重要的上下文语境。

Method: 研究了两种主要的内部处理方法：顺序拼接和并行缓存。还引入了两种处理噪声结构数据的技术：上下文蒸馏和语义平衡。

Result: 结构感知方法在多个任务中一致优于纯文本和后处理基线方法。顺序拼接在噪声中等长度上下文中表现更好，而并行缓存在长高信号上下文中扩展性更强但对干扰更敏感。

Conclusion: 这项工作首次全面分析了内部结构感知编码方法，为构建更强大和上下文感知的嵌入模型提供了蓝图。
...
TL;DR: 提出了一种利用LLM生成动作建议来指导RL探索的框架，通过增强观察空间让RL智能体学习何时遵循或忽略LLM指导，在稀疏奖励环境中显著提升训练效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 传统RL智能体在稀疏奖励环境中探索困难，而LLM具有程序性知识和推理能力，但现有方法要么强制RL策略遵循LLM建议，要么将其直接纳入奖励函数，缺乏灵活性。

Method: 通过增强观察空间提供LLM生成的动作建议，让RL智能体学习何时遵循或忽略这些指导，利用LLM的世界知识和推理能力，同时通过软约束保持灵活性。

Result: 在三个复杂度递增的BabyAI环境中，LLM指导的益处随任务难度增加而增强。在最复杂环境中，最终成功率相对基线提升71%，样本效率提升高达9倍，且无需修改现有RL算法。

Conclusion: 该方法有效利用LLM的规划能力来加速RL在挑战性环境中的训练，展示了LLM指导RL探索的可行性和有效性。
...
TL;DR: 提出基于基函数预训练的可复用初始化框架，通过预训练神经网络逼近多项式基函数，然后将其参数用于初始化更复杂目标函数的网络，显著提升训练效率、泛化能力和模型可迁移性。
...
Method: 采用基函数预训练方法，首先在参考域上训练基神经网络来逼近多项式族，然后将学习到的参数用于初始化复杂目标函数的网络。引入域映射机制将任意域输入变换到参考域，保持与预训练模型的结构对应性。
...
TL;DR: 本文研究多臂老虎机问题中的欺骗性智能体行为，智能体在公开奖励分布下使用Thompson Sampling，但实际目标是识别最佳私有臂而不被发现。观察者只能看到公开奖励和被拉动的臂，而智能体能看到公开和私有奖励。


<details>
  <summary>Details</summary>
Motivation: 研究在存在观察者监控的情况下，智能体如何在满足KL散度检测约束的前提下，有效识别具有最佳私有奖励的臂，同时避免被观察者发现其欺骗行为。

Method: 将可检测性形式化为智能体实际拉动概率与观察者预期拉动概率之间的逐步KL散度约束。将拉动公开次优臂建模为成功概率递减的伯努利过程，并提出受top-two算法启发的自适应探索算法。

Result: 在KL约束下，拉动公开次优臂最多只能以Θ(√T)的速率进行。通过极大极小问题求解得到最佳私有臂识别的最优误差指数，算法能根据公开次优间隙的难度自适应调整探索策略。

Conclusion: 成功建立了欺骗性多臂老虎机问题的理论框架，证明了在检测约束下的性能极限，并提出了有效的算法实现，数值实验验证了理论结果和算法性能。
...
TL;DR: PO-CKAN是一个基于分块有理Kolmogorov-Arnold网络(KANs)的物理信息深度算子框架，用于逼近偏微分方程的解算子。该框架结合了DeepONet架构和分块有理KAN子网络，通过PINN原理增强物理一致性，在多个基准问题上表现出色。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够高效学习物理一致的时空解算子的框架，用于快速预测参数化时变偏微分方程的解，特别是处理变化的输入参数和初始/边界条件。

Method: 采用DeepONet风格的分支-主干架构，子网络实例化为有理KAN模块，通过PDE残差(PINN风格)损失强制物理一致性。
...
TL;DR: TAPAS数据集是一个用于分析后量子密码学的AI工具包，旨在解决AI攻击LWE问题时缺乏可访问数据的问题，为AI研究者提供现成的数据集来加速LWE攻击研究。


<details>
  <summary>Details</summary>
Motivation: AI在攻击LWE（后量子密码学中的重要数学难题）方面展现出潜力，但缺乏可访问的数据限制了AI研究者研究和改进这些攻击方法的能力。创建LWE训练数据既耗时又需要专业知识。

Method: 提出了TAPAS数据集工具包，覆盖多种LWE参数设置，为AI研究者提供即用型数据集，用于原型开发新的LWE破解方法。

Result: 建立了TAPAS数据集创建文档，设定了攻击性能基准，并为未来工作指明了方向。

Conclusion: TAPAS数据集填补了AI攻击LWE研究中的数据空白，将加速后量子密码学的AI研究进展。
...
TL;DR: Edu-EmotionNet是一个用于在线教育情感识别的新框架，通过动态跨模态注意力对齐、模态重要性估计和时间反馈循环，实现了对学习者情感演化的鲁棒建模。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别方法依赖静态融合策略并假设模态输入始终可靠，这在真实学习环境中很少成立。需要开发能够处理模态不可靠性和动态情感演化的方法。

Method: 提出Edu-EmotionNet框架，包含三个关键组件：跨模态注意力对齐模块（CMAA）用于动态跨模态上下文共享，模态重要性估计器（MIE）基于置信度为每个模态分配权重，时间反馈循环（TFL）利用先前预测确保时间一致性。

Result: 在IEMOCAP和MOSEI的教育子集上评估，重新标注了困惑、好奇、无聊和沮丧等情感，Edu-EmotionNet实现了最先进的性能，并对缺失或噪声模态表现出强鲁棒性。
...
### [282] [Titans Revisited: A Lightweight Reimplementation and Critical Analysis of a Test-Time Memory Model](https://arxiv.org/abs/2510.09551)
*Gavriel Di Nepi,Federico Siciliano,Fabrizio Silvestri*

Main category: cs.LG

TL;DR: 本文对Google的Titans模型进行了轻量级重实现和全面评估，发现该模型由于分块处理并不总是优于现有基线，但其神经记忆组件相比仅使用注意力的模型能持续提升性能。


<details>
  <summary>Details</summary>
Motivation: Google研究人员提出的Titans模型在多个任务上取得了强经验结果，但由于缺乏公开代码和原始描述中的模糊性，阻碍了可复现性。本文旨在通过重实现和评估来解决这些问题。

Method: 开发了Titans模型的轻量级重实现，并在掩码语言建模、时间序列预测和推荐任务上进行了全面评估。

Result: 评估结果显示，Titans模型由于分块处理并不总是优于已建立的基线模型，但其神经记忆组件相比仅使用注意力的模型能一致地提升性能。

Conclusion: 这些发现证实了该模型的创新潜力，同时突出了其实际局限性，并为未来研究提出了问题。
...
### [283] [TinyGraphEstimator: Adapting Lightweight Language Models for Graph Structure Inference](https://arxiv.org/abs/2510.08808)
*Michal Podstawski*

Main category: cs.LG

TL;DR: 本文研究了小型Transformer语言模型是否能够从文本图表示中推断图论参数，并引入了TinyGraphEstimator数据集进行系统评估。通过轻量级微调，模型在所有评估指标上均取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 探索资源高效的小型语言模型在图结构推理中的潜力，目前这一领域主要由大型语言模型主导，小型模型的潜力尚未充分发掘。

Method: 引入TinyGraphEstimator数据集，包含来自多种随机图模型的连通图及其结构元数据。使用LoRA（低秩适应）技术对多个小型开源模型进行轻量级微调，评估其预测图密度、聚类系数和色数等关键参数的能力。

Result: 小型语言模型在图结构数据上展现出非平凡的推理能力，通过高效的参数调整可以有效适应结构推理任务。LoRA微调在所有评估指标上都带来了持续改进。

Conclusion: 紧凑的Transformer语言模型能够直接从文本图表示中推断图论参数，证明了小型模型在图结构推理任务中的可行性和有效性。
...
### [284] [Long-Tailed Recognition via Information-Preservable Two-Stage Learning](https://arxiv.org/abs/2510.08836)
*Fudong Lin,Xu Yuan*

Main category: cs.LG

TL;DR: 提出一种新颖的两阶段学习方法来解决长尾数据分布问题，通过信息理论视角的表示学习和数学上有信息的实例采样策略，在多个长尾基准数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据分布往往存在不平衡（长尾）特性，这导致深度分类模型偏向频繁类别，对尾部类别表现不佳。需要解决这种多数偏向趋势同时保留数据集中的有价值信息。

Method: 第一阶段：从信息理论角度提出新的表示学习技术，理论上等价于最小化类内距离，产生有效且分离良好的特征空间。第二阶段：开发新颖的采样策略，选择数学上有信息的实例，能够纠正多数偏向的决策边界而不损害模型整体性能。

Result: 该方法在多个长尾基准数据集上实现了最先进的性能，通过广泛实验验证了其有效性。

Conclusion: 提出的两阶段学习方法成功缓解了长尾数据分布中的多数偏向问题，同时保持了模型整体性能，为处理不平衡数据提供了有效的解决方案。
...
### [285] [Reinforcement Learning-Driven Edge Management for Reliable Multi-view 3D Reconstruction](https://arxiv.org/abs/2510.08839)
*Motahare Mounesan,Sourya Saha,Houchao Gan,Md. Nurul Absur,Saptarshi Debroy*

Main category: cs.LG

TL;DR: 提出了一个基于强化学习的边缘资源管理框架，用于在资源受限和易受干扰的环境中实现可靠的实时多视角3D重建，通过两个协作的Q学习代理分别负责相机选择和服务器选择。


<details>
  <summary>Details</summary>
Motivation: 边缘环境中的实时多视角3D重建应用（如消防救援）面临资源可用性动态变化带来的挑战，包括图像质量下降、网络连接不稳定和服务器负载波动等干扰，这些因素威胁着重建管道的可靠性。

Method: 采用基于强化学习的边缘资源管理框架，包含两个协作的Q学习代理：一个负责相机选择，一个负责服务器选择。两个代理完全在线运行，通过与边缘环境的交互学习策略。构建了包含实验室终端设备和FABRIC基础设施边缘服务器的分布式测试平台，模拟智能城市边缘基础设施在真实干扰场景下的运行。

Result: 实验结果表明，所提出的框架通过有效平衡端到端延迟和重建质量，在动态环境中提高了应用的可靠性。

Conclusion: 该强化学习框架能够在资源受限和易受干扰的边缘环境中，确保3D重建在合理时间内达到高质量，为关键边缘应用提供了可靠的解决方案。
...
### [286] [The Boundaries of Fair AI in Medical Image Prognosis: A Causal Perspective](https://arxiv.org/abs/2510.08840)
*Thai-Hoang Pham,Jiayuan Chen,Seungyeon Lee,Yuanlong Wang,Sayoko Moroi,Xueru Zhang,Ping Zhang*

Main category: cs.LG

TL;DR: FairTTE是首个用于评估医学影像中时间到事件预测公平性的综合框架，揭示了跨不同成像模态普遍存在的偏见问题，并发现现有公平性方法缓解效果有限。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在医学影像分析中的广泛应用，出现了对其可能存在针对特定社会群体偏见的担忧。现有公平性研究主要关注医学影像诊断任务，而忽视了涉及预测疾病进展的预后场景。

Method: 提出FairTTE框架，整合先进的TTE预测和公平性算法，利用因果分析技术揭示和量化医学影像数据集中嵌入的不同偏见来源，进行大规模评估。

Result: 大规模评估显示偏见在不同成像模态中普遍存在，当前公平性方法缓解效果有限。发现基础偏见来源与模型差异之间存在强关联，且在分布偏移下公平性更难维持。

Conclusion: 需要针对所有形式偏见的整体方法，现有解决方案在分布偏移下存在局限性，迫切需要更稳健、公平的预后模型。
...
### [287] [Deep Multimodal Subspace Clustering Networks](https://arxiv.org/abs/1804.06498)
*Mahdi Abavisani,Vishal M. Patel*

Main category: cs.LG

TL;DR: 提出基于卷积神经网络的非监督多模态子空间聚类方法，包含多模态编码器、自表达层和多模态解码器三个主要阶段，研究早期、晚期和中间融合技术，并在三个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决多模态数据的子空间聚类问题，通过深度学习框架融合不同模态信息，提高聚类性能。

Method: 构建三阶段框架：多模态编码器融合数据到潜在空间，自表达层获取亲和矩阵，解码器重建输入数据。研究三种空间融合方法（早期、晚期、中间融合）和亲和融合方法。

Result: 在三个数据集上的广泛实验表明，所提方法显著优于最先进的多模态子空间聚类方法。

Conclusion: 提出的基于CNN的多模态子空间聚类框架通过有效的融合策略和自表达机制，在多模态聚类任务中取得了显著性能提升。
...
### [288] [On the Alignment Between Supervised and Self-Supervised Contrastive Learning](https://arxiv.org/abs/2510.08852)
*Achleshwar Luthra,Priyadarsi Mishra,Tomer Galanti*

Main category: cs.LG

TL;DR: 本文分析了自监督对比学习(CL)与仅负样本监督对比学习(NSCL)在表示层面的对齐性，证明了在共享随机性条件下两者的相似性矩阵保持接近，但参数空间存在指数级发散。


<details>
  <summary>Details</summary>
Motivation: 虽然已有理论表明CL损失近似于NSCL损失，但尚不清楚两者在表示层面是否在整个训练过程中保持对齐，这有助于理解自监督与监督学习之间的联系。

Method: 在共享随机性条件下训练CL和NSCL模型，分析其表示相似性矩阵的接近程度，并提供关于CKA和RSA等对齐指标的高概率保证。

Result: CL和NSCL的表示保持相似性，对齐性随类别数增加、温度升高而改善，但参数空间存在指数级发散。实证验证了这些预测。

Conclusion: NSCL可作为自监督与监督学习之间的原则性桥梁，表示层面的对齐性随规模增大而增强，而参数空间的对齐不稳定。
...
### [289] [Time-Aware Feature Selection: Adaptive Temporal Masking for Stable Sparse Autoencoder Training](https://arxiv.org/abs/2510.08855)
*T. Ed Li,Junyu Ren*

Main category: cs.LG

TL;DR: 本文提出了一种名为自适应时间掩码（ATM）的新训练方法，用于解决稀疏自编码器中的特征吸收问题，通过在Gemma-2-2b模型上的实验证明该方法能显著降低吸收分数并保持良好重构质量。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型的内部表示对于确保其可靠性和安全性至关重要，但当前稀疏自编码器训练方法面临特征吸收问题，使得难以一致地识别和分析模型行为。

Method: 引入自适应时间掩码（ATM），通过跟踪激活幅度、频率和重构贡献来计算随时间演化的重要性分数，并基于这些分数的统计阈值应用概率掩码机制，实现更自然的特征选择过程。

Result: 在Gemma-2-2b模型上的广泛实验表明，ATM相比TopK和JumpReLU SAEs等方法实现了显著更低的吸收分数，同时保持了优秀的重构质量。

Conclusion: ATM为在神经网络中学习稳定、可解释的特征提供了一个原则性解决方案，为更可靠的模型分析奠定了基础。
...
### [290] [Sparse components distinguish visual pathways & their alignment to neural networks](https://arxiv.org/abs/2510.08858)
*Ammar I Marvi,Nancy G Kanwisher,Meenakshi Khosla*

Main category: cs.LG

TL;DR: 本文提出了一种新的稀疏分解方法来分析人类视觉皮层中腹侧、背侧和外侧通路的表征差异，并开发了稀疏成分对齐(SCA)方法来更精确地测量大脑与人工神经网络之间的表征对齐程度。研究发现标准视觉DNN与腹侧通路的对齐度高于背侧和外侧通路。


<details>
  <summary>Details</summary>
Motivation: 探索为什么在人类视觉皮层中腹侧、背侧和外侧通路具有不同功能，但仅通过单一任务训练的深度神经网络却能很好地建模整个视觉系统，这暗示了这些通路之间可能存在共同的计算原理。

Method: 采用新颖的稀疏分解方法识别每个视觉通路中的主导表征成分，并开发了稀疏成分对齐(SCA)方法来测量大脑与人工神经网络之间的表征对齐。

Result: 发现三个视觉通路的成分响应特征存在明显差异：腹侧通路对脸孔、地点、身体、文字和食物有选择性；外侧通路对社会互动、隐含运动和手部动作有选择性；背侧通路则包含一些较难解释的成分。SCA方法显示标准视觉DNN与腹侧通路的对齐度更高。

Conclusion: SCA方法比传统群体水平几何方法能更精细地揭示大脑与人工神经网络之间的表征对齐差异，提供了一种对系统潜在神经调谐轴敏感的测量方法。
...
### [291] [Multi-fidelity Batch Active Learning for Gaussian Process Classifiers](https://arxiv.org/abs/2510.08865)
*Murray Cutforth,Yiming Yang,Tiffany Fan,Serge Guillas,Eric Darve*

Main category: cs.LG

TL;DR: 本文提出了BPMI（Bernoulli Parameter Mutual Information）算法，一种用于多保真度高斯过程分类器的批量主动学习方法，通过一阶泰勒展开解决概率空间中互信息计算难题，在固定计算预算下实现更高的预测精度。


<details>
  <summary>Details</summary>
Motivation: 许多科学和工程问题依赖昂贵的计算模拟，多保真度方法可以加速参数空间探索。研究如何在二元模拟输出情况下，使用高斯过程模型高效分配模拟预算。

Method: 引入BPMI算法，通过一阶泰勒展开链接函数来规避概率空间中互信息计算的不易处理性，实现多保真度GP分类器的批量主动学习。

Result: 在两个合成测试案例和一个涉及激光点火火箭燃烧器模拟的复杂实际应用中，BPMI在所有实验中均表现出优于多个基线的性能。

Conclusion: BPMI算法在固定计算预算下能够实现更高的预测精度，为多保真度模拟预算分配提供了有效解决方案。
...
### [292] [An Improved Model-Free Decision-Estimation Coefficient with Applications in Adversarial MDPs](https://arxiv.org/abs/2510.08882)
*Haolin Liu,Chen-Yu Wei,Julian Zimmert*

Main category: cs.LG

TL;DR: 本文提出了Dig-DEC，一种基于信息增益的无模型决策估计系数，用于解决结构化观测决策问题。该方法移除了乐观主义，在对抗性环境中表现更好，并在混合MDP中实现了首个无模型遗憾边界。


<details>
  <summary>Details</summary>
Motivation: 现有基于乐观DEC的方法只能处理随机环境，无法扩展到对抗性环境，且在模型类大小方面存在遗憾边界差距。需要一种更通用的方法来处理对抗性环境并缩小理论差距。

Method: 引入Dig-DEC，通过纯信息增益驱动探索而非乐观主义。改进在线函数估计过程，包括平均估计误差最小化和贝尔曼完备MDP中的平方误差最小化。

Result: Dig-DEC始终不大于乐观DEC，在特殊情况下更小。在混合MDP中获得了首个无模型遗憾边界。将遗憾边界从T^{3/4}提升到T^{2/3}(在线)和从T^{5/6}提升到T^{7/9}(离线)，在贝尔曼完备MDP中从T^{2/3}提升到√T。

Conclusion: Dig-DEC提供了一种更通用的方法，能够处理对抗性环境，缩小了理论差距，并在多个场景下实现了显著的性能提升，首次使DEC方法在贝尔曼完备MDP中达到与乐观主义方法相当的性能。
...
### [293] [Pinpointing crucial steps: Attribution-based Credit Assignment for Verifiable Reinforcement Learning](https://arxiv.org/abs/2510.08899)
*Junxi Yin,Haisen Luo,Zhenyu Li,Yihua Liu,Dan Liu,Zequn Li,Xiaohang Xu*

Main category: cs.LG

TL;DR: ACPO是一个分阶段的强化学习框架，通过难度感知课程和基于归因的策略优化，解决了RLVR中探索与利用的平衡问题，显著提升了复杂推理任务的性能。
...
Method: 提出了基于归因的策略优化框架ACPO，包含轨迹语义分割和基于归因的表征来动态调节策略熵，以及因子化奖励系统来精确量化每个推理步骤的层次贡献。

Result: 在AIME、MATH和AMC等具有挑战性的基准测试中，ACPO显著优于现有的最先进方法。

Conclusion: ACPO通过改进探索和利用的平衡，有效解决了强化学习中信用分配和熵崩溃问题，在复杂推理任务上取得了显著性能提升。
...
### [294] [A Frequency-Domain Analysis of the Multi-Armed Bandit Problem: A New Perspective on the Exploration-Exploitation Trade-off](https://arxiv.org/abs/2510.08908)
*Di Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种新颖的频域分析框架，将多臂老虎机问题重新表述为信号处理问题，证明了UCB算法中的置信边界项在频域中等效于对不确定频谱分量施加时变增益，并推导了关于探索率衰减的有限时间动态边界。


<details>
  <summary>Details</summary>
Motivation: 现有的多臂老虎机分析主要从时域和累积遗憾角度进行，难以刻画学习过程的动态特性，需要新的分析框架来更好地理解探索与利用的权衡机制。

Method: 构建频域老虎机模型，将每个臂的奖励估计视为频谱分量，其不确定性对应分量频率，将老虎机算法解释为自适应滤波器，并证明UCB算法置信边界在频域的等价形式。

Result: 证明了UCB算法置信边界项在频域中等效于对不确定频谱分量施加的时变增益，该增益与访问次数的平方根成反比，并推导了探索率衰减的有限时间动态边界。
...
### [295] [Velocity and Density-Aware RRI Analysis and Optimization for AoI Minimization in IoV SPS](https://arxiv.org/abs/2510.08911)
*Maoxin Ji,Tong Wang,Qiong Wu,Pingyi Fan,Nan Cheng,Wen Chen*

Main category: cs.LG

TL;DR: 本文提出了一种基于大语言模型和深度确定性策略梯度的优化方法，用于解决车联网中半持续调度导致的年龄信息恶化问题，通过双路径优化方案显著降低了AoI。


<details>
  <summary>Details</summary>
Motivation: 解决车联网中由于数据包碰撞和车辆速度相关的信道不确定性导致的年龄信息恶化问题，这些因素在半持续调度中会严重影响通信性能。

Method: 建立了受车辆速度、车辆密度和资源预留间隔影响的AoI计算模型，设计了双路径优化方案，其中DDPG基于状态空间和奖励函数进行指导，LLM利用上下文学习生成最优参数配置。

Result: 实验结果表明，LLM在积累少量示例后无需模型训练就能显著降低AoI，而DDPG方法在训练后能实现更稳定的性能。

Conclusion: 结合LLM和DDPG的双路径优化方法能有效解决车联网中的AoI恶化问题，LLM提供快速优化能力，DDPG提供稳定性能保障。
...
### [296] [Simple and Robust Forecasting of Spatiotemporally Correlated Small Earth Data with A Tabular Foundation Model](https://arxiv.org/abs/2510.08920)
*Yuting Yang,Gang Mei,Zhengjing Ma,Nengxiong Xu,Jianbing Peng*

Main category: cs.LG

TL;DR: 提出了一种针对小地球数据的时空预测方法，通过量化时空模式并利用表格基础模型实现跨场景的准确预测，在多个典型场景中优于传统深度学习和基础模型。


<details>
  <summary>Details</summary>
Motivation: 小地球数据具有有限的短期监测变异性，呈现稀疏但有意义的测量值，通常表现出时空相关性。传统深度学习模型需要针对不同场景进行特定任务训练，而基础模型虽然不需要特定任务训练，但往往对预训练分布的全局均值存在预测偏差。

Method: 提出了一种简单而稳健的方法，核心思想是表征和量化小地球数据的时空模式，然后利用表格基础模型进行跨不同场景的准确预测。

Result: 在三个典型场景中的比较结果表明，该预测方法在大多数情况下比图深度学习模型（T-GCN）和表格基础模型（TabPFN）实现了更优的准确性，并表现出更强的鲁棒性。

Conclusion: 该方法为小地球数据的时空预测提供了一种有效的解决方案，能够克服传统模型需要特定任务训练和基础模型存在预测偏差的问题，在多个实际场景中展现出优越性能。
...
### [297] [AB-PINNs: Adaptive-Basis Physics-Informed Neural Networks for Residual-Driven Domain Decomposition](https://arxiv.org/abs/2510.08924)
*Jonah Botvinick-Greenhouse,Wael H. Ali,Mouhacine Benosman,Saviz Mowlavi*

Main category: cs.LG

TL;DR: 本文提出了自适应基物理信息神经网络（AB-PINNs），这是一种新颖的域分解方法，通过动态调整子域来适应未知解的内在特征，并在训练过程中根据残差损失引入新的子域，特别适用于多尺度问题求解。


<details>
  <summary>Details</summary>
Motivation: 传统PINNs在求解复杂多尺度偏微分方程时面临挑战，静态域分解方法需要大量超参数调优且容易陷入局部最小值。本文旨在开发一种能够动态适应解特征的域分解方法，提高PINNs在多尺度问题中的表现。

Method: 提出AB-PINNs方法，包含两个关键机制：1）现有子域根据未知解的内在特征进行动态自适应调整；2）在训练过程中根据高残差损失区域动态引入新的子域，类似于经典网格细化技术。

Result: 全面的数值结果表明，AB-PINNs在求解各种复杂多尺度偏微分方程方面表现出色，能够有效捕获不同尺度的解特征，并显著优于静态域分解方法。

Conclusion: AB-PINNs提供了一种灵活有效的域分解方法，能够动态适应解的多尺度特征，减少超参数调优需求，防止陷入局部最小值，为复杂多尺度PDE求解提供了有前景的解决方案。
...
### [298] [Bi-level Meta-Policy Control for Dynamic Uncertainty Calibration in Evidential Deep Learning](https://arxiv.org/abs/2510.08938)
*Zhen Yang,Yansong Ma,Lei Chen*

Main category: cs.LG

TL;DR: 提出了一种名为元策略控制器（MPC）的动态元学习框架，用于改进证据深度学习的校准和泛化能力，通过自适应调整KL散度系数和狄利克雷先验强度来优化不确定性建模。


<details>
  <summary>Details</summary>
Motivation: 传统的证据深度学习方法使用静态超参数进行不确定性校准，在动态数据分布下适应性差，导致高风险决策任务中的校准和泛化性能不佳。

Method: 采用双层优化方法：内层通过动态配置的损失函数更新模型参数；外层通过策略网络基于多目标奖励优化KL散度系数和类别特定的狄利克雷先验强度，平衡预测准确性和不确定性质量。

Result: 大量实验结果表明，MPC显著提高了模型预测的可靠性和校准性，在各种任务中改善了不确定性校准、预测准确性以及基于置信度的样本剔除后的性能保持。

Conclusion: 提出的可学习狄利克雷先验能够灵活适应类别分布和训练动态，相比固定先验方法具有更好的适应性，显著提升了证据深度学习的不确定性建模能力。
...
### [299] [Variability Aware Recursive Neural Network (VARNN): A Residual-Memory Model for Capturing Temporal Deviation in Sequence Regression Modeling](https://arxiv.org/abs/2510.08944)
*Haroon Gharwi,Kai Shu*

Main category: cs.LG

TL;DR: VARNN是一种用于时间序列回归的新型神经网络架构，通过显式学习近期预测残差的误差记忆来重新校准后续预测，在非平稳、异方差的时间序列数据中表现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据存在非平稳行为、机制转换和时间变化的噪声（异方差），这些会降低标准回归模型的鲁棒性。需要开发能够处理这些挑战的新方法。

Method: VARNN架构在前馈预测器基础上增加了学习的误差记忆状态，该状态从短期上下文步骤的残差中更新，作为变异性和漂移的信号，然后用于调节当前时间步的最终预测。

Result: 在多个领域数据集（电器能耗、医疗保健、环境监测）上的实验结果表明，VARNN相比静态、动态和循环基线模型实现了更优的性能，获得了更低的测试MSE，且计算开销最小。

Conclusion: VARNN模型在漂移和波动环境下提供了鲁棒的预测，显示出其作为时间序列学习框架的潜力。
...
### [300] [When LLM Agents Meet Graph Optimization: An Automated Data Quality Improvement Approach](https://arxiv.org/abs/2510.08952)
*Zhihan Zhang,Xunkai Li,Yilong Zuo,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: LAGA是一个统一的多智能体框架，通过四个协作智能体（检测、规划、行动、评估）实现自动化闭环，解决文本属性图的质量问题，在各种场景下显著提升图质量和任务性能。


<details>
  <summary>Details</summary>
Motivation: 文本属性图的图神经网络性能对输入质量高度敏感，现有方法主要关注改进模型架构而忽视系统化优化图数据本身，导致实际效果有限。

Method: 提出LAGA多智能体框架，包含检测、规划、行动、评估四个协作智能体。行动智能体采用双编码器和三目标设计，捕获跨模态互补信息并进行整体图质量增强。
...
### [301] [Analytical Survey of Learning with Low-Resource Data: From Analysis to Investigation](https://arxiv.org/abs/2510.08962)
...
TL;DR: 本文综述了在低资源数据学习中的泛化误差和标签复杂性分析，探讨了多种优化策略和学习范式，旨在实现有限资源下的稳健泛化。
...
Method: 使用PAC框架中的不可知主动采样理论分析低资源数据学习的泛化误差和标签复杂性，并研究梯度信息优化、元迭代优化、几何感知优化和LLM驱动的优化等策略。

Result: 提供了针对低资源数据学习的多种优化策略，并概述了可以从低资源数据中受益的多种学习范式，包括领域迁移、强化反馈和层次结构建模。

Conclusion: 总结了关键发现并强调了它们对低资源数据学习的意义，为实现有限资源下的稳健泛化提供了理论支持和实践指导。
...
### [302] [HiBBO: HiPPO-based Space Consistency for High-dimensional Bayesian Optimisation](https://arxiv.org/abs/2510.08965)
*Junyu Xuan,Wenlong Chen,Yingzhen Li*

Main category: cs.LG

TL;DR: HiBBO是一个新颖的贝叶斯优化框架，通过引入HiPPO方法来减少变分自编码器潜在空间与原始空间之间的函数分布不匹配，在高维基准任务中表现出优于现有VAE-BO方法的收敛速度和解质量。


<details>
  <summary>Details</summary>
Motivation: 传统的贝叶斯优化在高维空间中效果下降，而基于变分自编码器的方法虽然通过学习低维潜在表示来解决这个问题，但基于重构的目标函数会导致潜在空间与原始空间之间的函数分布不匹配，从而产生次优的优化性能。

Method: 提出HiBBO框架，在VAE的潜在空间构建中引入空间一致性，使用HiPPO（一种长期序列建模方法）来减少潜在空间与原始空间之间的函数分布不匹配。

Result: 在高维基准任务上的实验表明，HiBBO在收敛速度和解质量方面优于现有的VAE-BO方法。

Conclusion: 这项工作弥合了高维序列表示学习与高效贝叶斯优化之间的差距，为神经架构搜索、材料科学等领域的更广泛应用奠定了基础。
...
### [303] [Learning Regularizers: Learning Optimizers that can Regularize](https://arxiv.org/abs/2510.08968)
*Suraj Kumar Sahoo,Narayanan C Krishnan*

Main category: cs.LG

TL;DR: 本文研究表明学习优化器(LOs)能够学习并内化传统正则化技术(如SAM、GAM、GSAM)的效果，而无需在目标函数中显式应用这些正则化器。


<details>
  <summary>Details</summary>
Motivation: 探索学习优化器是否能够学习正则化效果，挑战传统优化方法中必须显式应用正则化器的必要性。

Method: 通过在标准基准数据集(MNIST、FMNIST、CIFAR)和神经网络架构(MLP、MLP-Relu、CNN)上进行广泛实验，比较有正则化和无正则化训练的学习优化器。

Result: 正则化学习优化器在测试准确率和泛化能力方面始终优于未正则化版本，并且能够将正则化效果保留并迁移到新的优化任务中。

Conclusion: 学习优化器能够内在地学习正则化特性，挑战了传统优化方法中需要显式正则化的常规做法。
...
### [304] [Diagnosing and Mitigating System Bias in Self-Rewarding RL](https://arxiv.org/abs/2510.08977)
*Chuyi Tan,Peiwen Yuan,Xinglin Wang,Yiwei Li,Shaoxiong Feng,Yueqi Zhang,Jiayi Shi,Ji Zhang,Boyuan Pan,Yao Hu,Kan Li*

Main category: cs.LG

TL;DR: 论文提出RLER方法解决RLIR中的系统偏差问题，通过集成模型、奖励插值和轨迹选择，在无标注样本上实现稳定扩展，性能比RLIR提升13.6%，仅比RLVR低3.6%。


<details>
  <summary>Details</summary>
Motivation: RLVR受限于标注样本数量，而RLIR在无标注设置下虽然能持续扩展，但性能和稳定性落后于RLVR。研究发现这是由于模型倾向于高估高置信度轨迹，导致有偏和不稳定的奖励估计。

Method: 提出RLER方法：1）使用集成模型聚合多样性；2）进行奖励插值；3）优化轨迹选择。通过三个偏差指标（噪声偏差、自偏差、符号偏差）来表征系统偏差。

Result: 实验显示RLER比RLIR提升13.6%，仅比RLVR低3.6%，在无标注样本上实现了稳定的扩展性能。
...
### [305] [FedL2T: Personalized Federated Learning with Two-Teacher Distillation for Seizure Prediction](https://arxiv.org/abs/2510.08984)
...
<details>
  <summary>Details</summary>
Motivation: 癫痫预测需要大量EEG数据，但标注成本高且涉及隐私。联邦学习虽然能保护隐私，但在现实场景中由于患者间差异大，现有方法在异构客户端设置下性能不佳。

Method: 提出FedL2T框架，采用双教师知识蒸馏策略：每个客户端同时从全局聚合模型和动态分配的同伴模型学习。使用自适应多级蒸馏策略对齐预测输出和中间特征表示，并引入近端正则化项增强训练稳定性。
...
### [306] [Constraints-of-Thought: A Framework for Constrained Reasoning in Language-Model-Guided Search](https://arxiv.org/abs/2510.08992)
*Kamel Alrashedy,Vriksha Srihari,Zulfiqar Zaidi,Ridam Srivastava,Pradyumna Tambwekar,Matthew Gombolay*

Main category: cs.LG

TL;DR: 本文提出了Constraints-of-Thought (Const-o-T)框架，通过将推理步骤表示为(意图,约束)对，结合蒙特卡洛树搜索(MCTS)来改善大语言模型在多步规划中的约束对齐问题，在多个领域实现了更高的准确性和结构对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在多步规划中难以确保计划符合用户意图和符号约束，现有推理方法如CoT、ToT等虽然扩展了搜索空间，但经常产生不可行的动作或幻觉步骤。

Method: 提出Const-o-T框架，将每个推理步骤表示为(意图,约束)对，为MCTS提供结构化先验，压缩搜索空间并强制执行有效性，通过约束剪枝不可行分支，引导探索朝向语义有效的动作。

Result: 在Risk游戏、CAD代码生成和算术推理三个领域验证，该方法优于基线方法，实现了更高的准确性和更强的结构对齐。

Conclusion: Const-o-T为约束引导推理提供了可泛化的基础，使LLM能够进行更高效、约束对齐和领域适应的规划。
...
### [307] [PlatformX: An End-to-End Transferable Platform for Energy-Efficient Neural Architecture Search](https://arxiv.org/abs/2510.08993)
...
Method: PlatformX包含四个关键组件：能量驱动的搜索空间、可跨设备转移的核级能量预测器、平衡能量和准确性的帕累托多目标搜索算法，以及使用外部监测器自动进行设备上功率测量的高分辨率运行时能量分析系统。

Result: 在多个移动平台上评估显示，PlatformX显著减少了搜索开销，同时保持了准确性和能量保真度。它识别出的模型准确率高达0.94，或每次推理仅消耗0.16 mJ能量，在准确性和效率上都优于MobileNet-V2。
...
### [308] [SQS: Bayesian DNN Compression through Sparse Quantized Sub-distributions](https://arxiv.org/abs/2510.08999)
*Ziyi Wang,Nan Jiang,Guang Lin,Qifan Song*

Main category: cs.LG

TL;DR: 提出了一种通过贝叶斯变分学习同时进行剪枝和低比特量化的统一框架SQS，在保持性能的同时实现比现有方法更高的压缩率。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常单独使用权重剪枝或低比特量化，导致压缩率不理想。需要一种统一框架来同时进行剪枝和量化以获得更好的压缩效果。

Method: 使用spike-and-slab先验诱导稀疏性，并采用高斯混合模型(GMMs)建模量化权重以实现低比特精度。基于贝叶斯变分学习理论。

Result: 在ResNet、BERT-base、Llama3和Qwen2.5模型上的实验表明，该方法比现有方法获得更高的压缩率，同时保持可比的性能下降。

Conclusion: SQS框架在理论上提供了稀疏和量化深度神经网络的一致变分方法，在实践中实现了比现有方法更优的压缩效果。
...
### [309] [LLM Unlearning on Noisy Forget Sets: A Study of Incomplete, Rewritten, and Watermarked Data](https://arxiv.org/abs/2510.09007)
*Changsheng Wang,Yihua Zhang,Dennis Wei,Jinghan Jia,Pin-Yu Chen,Sijia Liu*

Main category: cs.LG

TL;DR: 该论文研究了在噪声遗忘数据集下的大语言模型遗忘问题，发现即使遗忘数据被扰动或质量较低，只要核心语义信号保留，遗忘方法仍然保持稳健。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法假设可以访问干净、定义明确的遗忘数据样本，但现实世界的遗忘数据往往是低质量、合成重写或带有水印的，这引发了遗忘可靠性的担忧。

Method: 系统性地在噪声遗忘数据集上对最先进的LLM遗忘方法（RMU和NPO）进行基准测试，并提出基于显著性的解释框架来分析遗忘稳健性。

Result: 研究发现遗忘对扰动具有惊人的稳健性，只要核心语义信号得以保留。关键语义组件在表面形式大幅变化的情况下仍然保持一致的遗忘驱动作用。

Conclusion: 遗忘算法主要受深层语义线索而非浅层词汇模式引导，这解释了在噪声遗忘数据下遗忘方法的稳健性。
...
### [310] [Value-State Gated Attention for Mitigating Extreme-Token Phenomena in Transformers](https://arxiv.org/abs/2510.09017)
*Rui Bu,Haofeng Zhong,Wenzheng Chen,Yangyan Li*

Main category: cs.LG

TL;DR: 提出Value-State Gated Attention (VGA)机制，通过基于值向量的可学习门控来打破注意力机制中的无效循环，解决极端token现象（如注意力汇聚和价值状态流失），提升模型性能、量化保真度和可解释性。


<details>
  <summary>Details</summary>
Motivation: Transformer架构的大模型容易受到极端token现象的影响，包括注意力汇聚和价值状态流失，这些问题导致模型性能下降、量化保真度降低和可解释性变差，源于模型学习到的一种低效的'无操作'行为。

Method: 提出Value-State Gated Attention (VGA)机制，引入基于值向量的可学习、数据依赖的门控来调节输出，通过理论分析证明基于值状态的门控比基于输入嵌入的方法更有效地解耦价值和注意力分数的更新。

Result: 实验表明VGA显著减轻了注意力汇聚的形成并稳定了价值状态规范，带来了性能提升、鲁棒的量化保真度和增强的模型可解释性。

Conclusion: VGA通过直接打破注意力机制中的无效循环，提供了一种简单、专用且稳定的架构机制，有效解决了Transformer模型中的极端token问题。
...
### [311] [Slim Scheduler: A Runtime-Aware RL and Scheduler System for Efficient CNN Inference](https://arxiv.org/abs/2510.09018)
*Ian Harshbarger,Calvin Chidambaram*

Main category: cs.LG

TL;DR: Slim Scheduler是一个混合调度框架，结合PPO强化学习策略和贪心调度器，用于协调可瘦身模型的分布式推理。它通过分层设计减少搜索空间复杂度，在保持准确性的同时显著降低延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络调度研究主要关注固定宽度的静态端到端模型，忽略了适应异构硬件和运行时条件波动的动态方法。

Method: 采用分层调度设计：本地贪心调度器处理请求批处理和实例扩展，PPO路由器学习全局路由策略（设备选择、宽度比例、批配置）。

Result: 相比纯随机任务分配基线，Slim Scheduler可实现：延迟降低96.45%，能耗降低97.31%（使用最瘦模型，准确率70.3%）；或在保持准确率的同时降低平均延迟和能耗组合。

Conclusion: 该混合调度框架能有效平衡效率和吞吐量，适应异构硬件环境，提供多种准确率-延迟权衡选项。
...
### [312] [MagicDock: Toward Docking-oriented De Novo Ligand Design via Gradient Inversion](https://arxiv.org/abs/2510.09020)
*Zekai Chen,Xunkai Li,Sirui Zhang,Henan Sun,Jia Li,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: MagicDock是一个用于从头配体设计的框架，通过梯度反演和可微分表面建模解决现有方法的局限性，在9个场景中相比最先进方法分别提升了27.1%和11.7%的性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有从头配体设计方法存在的三个主要问题：伪从头设计、有限对接建模和配体类型不灵活。

Method: 采用梯度反演框架，结合可微分表面建模，使用可学习的3D点云表示精确捕捉结合细节，并为不同配体类型提供定制化设计。

Result: 在9个不同场景的广泛实验中，MagicDock在蛋白质配体设计上比最先进方法平均提升27.1%，在分子配体设计上平均提升11.7%。

Conclusion: MagicDock通过创新的梯度反演和可微分表面建模方法，有效解决了从头配体设计的关键挑战，为生物医学应用提供了强大的工具。
...
### [313] [The Environmental Impacts of Machine Learning Training Keep Rising Evidencing Rebound Effect](https://arxiv.org/abs/2510.09022)
*Clément Morand,Anne-Laure Ligozat,Aurélie Névéol*

Main category: cs.LG

TL;DR: 本文分析了机器学习模型训练的环境影响，发现即使采用优化策略，显卡生产和模型训练的环境影响仍在持续增长，存在反弹效应，单纯提高效率无法确保可持续性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习性能提升，计算需求激增，硬件、算法和碳优化策略被提出以控制能耗和环境影响，但需要评估这些策略是否能实现可持续的ML模型训练。

Method: 通过估算过去十年中包括大语言模型在内的著名AI系统训练的环境影响，重点关注显卡的生命周期分析。

Result: 发现两个关键趋势：显卡生产影响持续增长；ML模型训练能耗和环境影响呈指数增长，即使采用位置转移等减排策略，优化策略也无法缓解训练带来的影响，存在反弹效应。

Conclusion: 必须考虑硬件的整个生命周期而非仅使用阶段，单纯提高效率无法确保ML的可持续性，还需要减少AI活动并质疑资源密集型训练的规模和频率。
...
### [314] [The Attacker Moves Second: Stronger Adaptive Attacks Bypass Defenses Against Llm Jailbreaks and Prompt Injections](https://arxiv.org/abs/2510.09023)
...
TL;DR: 本文批评当前语言模型防御评估方法的不足，提出应使用适应性攻击者进行更严格的评估，并通过系统优化方法成功绕过了12种最新防御措施。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型防御评估存在缺陷，主要使用静态攻击集或弱优化方法，无法真实反映防御的实际效果。

Method: 采用系统调优和扩展通用优化技术，包括梯度下降、强化学习、随机搜索和人工引导探索，来对抗各种防御机制。

Result: 成功绕过了12种最新防御措施，大多数防御的攻击成功率超过90%，而原始报告中的攻击成功率接近零。

Conclusion: 未来的防御工作必须考虑更强的攻击方法，如本文所述，才能做出可靠和令人信服的鲁棒性声明。
...
### [315] [Convergence of optimizers implies eigenvalues filtering at equilibrium](https://arxiv.org/abs/2510.09034)
*Jerome Bolte,Quoc-Tung Le,Edouard Pauwels*

Main category: cs.LG

TL;DR: 本文从反向视角出发，假设优化器收敛到任意点，分析不同优化器作为特征值过滤器的行为。研究发现梯度下降法天然避免最尖锐的最小值，而SAM算法更积极地偏好更宽的盆地。基于此提出了两种增强特征值过滤的新算法。


<details>
  <summary>Details</summary>
Motivation: 深度学习训练中多种优化器倾向于找到接近全局最优解，本文从假设收敛到任意点的反向视角出发，探索不同优化器作为特征值过滤器的内在机制。

Method: 利用广义Hadamard-Perron稳定流形定理，对一般半代数C²函数进行理论分析，无需额外非退化条件或全局Lipschitz边界假设。提出了两种增强特征值过滤的新算法。

Result: 理论分析和数值实验表明，梯度下降法避免最尖锐最小值，SAM算法更偏好宽盆地，新提出的算法能更有效地促进宽最小值。

Conclusion: 不同优化器本质上是特征值过滤器，其行为由超参数决定。这一视角为理解优化器行为提供了新框架，并启发了更有效的算法设计。
...
### [316] [Robust Driving Control for Autonomous Vehicles: An Intelligent General-sum Constrained Adversarial Reinforcement Learning Approach](https://arxiv.org/abs/2510.09041)
*Junchao Fan,Xiaolin Chang*

Main category: cs.LG

TL;DR: 提出IGCARL方法，通过战略目标对抗者和约束强化学习，解决深度强化学习在自动驾驶中对战略性对抗攻击的脆弱性问题，显著提升对抗攻击下的鲁棒性和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒方法存在三个关键问题：仅针对短视对抗攻击训练、难以引发真正安全关键事件、缺乏鲁棒约束导致训练不稳定和政策漂移，限制了DRL在自动驾驶中的实际部署。

Method: IGCARL包含战略目标对抗者和鲁棒驾驶智能体。对抗者利用DRL的时序决策能力执行战略性多步攻击，采用一般和博弈目标诱导安全关键事件；智能体通过与对抗者交互学习鲁棒驾驶策略，在约束条件下优化以确保稳定学习。

Result: 实验表明IGCARL相比最先进方法至少提高27.9%的成功率，显著增强了对对抗攻击的鲁棒性，提升了DRL自动驾驶的安全性和可靠性。

Conclusion: IGCARL通过战略性对抗训练和约束优化，有效解决了DRL自动驾驶中的对抗攻击脆弱性问题，为实际部署提供了更安全可靠的解决方案。
...
### [317] [Spatio-Temporal Graph Convolutional Networks for EV Charging Demand Forecasting Using Real-World Multi-Modal Data Integration](https://arxiv.org/abs/2510.09048)
*Jose Tupayachi,Mustafa C. Camur,Kevin Heaslip,Xueping Li*

Main category: cs.LG

TL;DR: 本研究提出了TW-GCN框架，结合图卷积网络和时序架构来预测美国田纳西州的电动汽车充电需求，发现3小时中期预测在响应性和稳定性之间达到最佳平衡，且1DCNN模型表现最优。
...
Method: 使用图卷积网络结合时序架构的TW-GCN框架，利用真实交通流量、天气条件和美国最大EV基础设施公司提供的专有数据，捕捉空间依赖性和时间动态。

Result: 实验显示3小时中期预测达到最佳平衡，1DCNN模型表现最稳定。区域分析显示东、中、西田纳西州预测精度存在差异，反映了站点密度、人口和本地需求变化对模型性能的影响。
...
### [318] [Improving Anomaly Detection in Industrial Time Series: The Role of Segmentation and Heterogeneous Ensemble](https://arxiv.org/abs/2510.09079)
*Emilio Mastriani,Alessandro Costa,Federico Incardona,Kevin Munari,Sebastiano Spinello*

Main category: cs.LG

TL;DR: 该论文研究了在工业生产环境中，通过将时间序列分割技术（特别是变化点检测）与异质集成方法相结合来增强异常检测性能。结果表明，在集成算法之前应用分割作为预处理步骤显著提高了AUC-ROC指标。


<details>
  <summary>Details</summary>
Motivation: 在工业生产的多元时间序列环境中，减少时间不确定性并提高异常检测的准确性。通过分割技术识别状态变化，帮助区分正常和异常条件。

Method: 使用变化点检测（特别是ChangeFinder算法）进行时间序列分割作为预处理步骤，然后应用异质集成算法（包括PCA和LSTM、随机森林和XGBoost）进行异常检测。

Result: 应用分割预处理后，AUC-ROC指标从0.8599（PCA和LSTM集成）显著提高到0.9760（随机森林和XGBoost集成）。分割技术减少了时间模糊性，促进了监督算法的学习过程。

Conclusion: 在工业异常检测中，将分割技术作为预处理步骤与异质集成方法相结合能够显著提高检测性能。未来工作将探索引入基于变化点研究的加权特征来进一步优化早期异常检测。
...
### [319] [FLToP CTC: Frame-Level Token Pruning via Relative Threshold for Efficient and Memory-Saving Decoding on Diverse Platforms](https://arxiv.org/abs/2510.09085)
*Atul Shree,Harshith Jupuru*

Main category: cs.LG

TL;DR: FLToP CTC是一种新的CTC解码算法，通过基于相对概率阈值的帧级token剪枝，显著降低计算和内存需求，同时保持WER性能几乎不变。在LibriSpeech上实现10.5倍运行速度提升和2.78倍内存减少。


<details>
  <summary>Details</summary>
Motivation: CTC-based ASR系统在资源受限环境中面临计算和内存瓶颈，传统CTC解码器需要高达90%的处理时间，效率低下。

Method: 提出帧级token剪枝(FLToP CTC)解码算法，使用相对概率阈值动态剪除每帧中低概率token。
...
### [320] [Neural Codecs as Biosignal Tokenizers](https://arxiv.org/abs/2510.09095)
...
<details>
  <summary>Details</summary>
Motivation: 神经生理记录（如EEG）产生高维、嘈杂的时间序列数据，通常需要大量预处理和手工特征提取。现有的大型预训练模型在生物信号解码方面存在挑战，需要一种更有效的表示学习方法。

Method: 提出了BioCodec框架，受神经编解码器启发，将生物信号的低级特征捕获为离散标记。在数千小时的EEG数据上预训练，支持多种下游任务。

Result: BioCodec在临床诊断、睡眠生理、语音解码和运动想象等多个任务中表现优异，特别是在低资源设置下。该方法也适用于其他生物信号（如EMG），性能与最先进模型相当。

Conclusion: BioCodec为生物信号标记化提供了一个通用的解决方案，在多个应用场景中具有竞争力，代码和模型已开源。
...
### [321] [AdaPM: a Partial Momentum Algorithm for LLM Training](https://arxiv.org/abs/2510.09103)
*Yimu Zhang,Yuanshi Liu,Cong Fang*

Main category: cs.LG

TL;DR: AdaPM是一种自适应训练策略，通过部分动量设计实现内存高效优化器，在保持性能的同时减少90%以上的动量内存使用，结合二阶统计量技术可进一步减少95%的优化器状态内存。
...
Method: 提出AdaPM方法，采用非均匀动量设计：对大多数块不需要完整动量，通过偏置校正技术增强部分动量以缓解性能损失。

Result: 经验验证表明，该方法在60M到1.5B语言模型预训练、监督微调和RLHF中，减少90%以上动量内存的同时保持效率和性能，结合二阶统计量技术可进一步减少95%优化器状态内存，为GPT-2 1.5B预训练节省超过30%GPU小时。

Conclusion: AdaPM通过自适应部分动量设计成功实现了内存高效的优化器，在保持模型性能的同时显著减少了内存使用和训练成本。
...
### [322] [MemLoss: Enhancing Adversarial Training with Recycling Adversarial Examples](https://arxiv.org/abs/2510.09105)
*Soroush Mahdi,Maryam Amirmazlaghani,Saeed Saravani,Zahra Dehghanian*

Main category: cs.LG

TL;DR: MemLoss是一种新的对抗训练方法，利用先前生成的对抗样本（记忆对抗样本）来提高模型的鲁棒性和准确性，同时保持对干净数据的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗训练方法在提高模型鲁棒性时往往会牺牲对干净数据的准确性，需要一种平衡的方法来同时提升自然准确性和对抗鲁棒性。

Method: 通过利用先前训练周期中生成的记忆对抗样本来增强对抗训练过程，这些样本在多个训练周期中被重复使用。

Result: 在CIFAR-10等多个数据集上的实验结果表明，MemLoss相比现有对抗训练方法获得了更好的准确性，同时保持了强大的对抗攻击鲁棒性。

Conclusion: MemLoss提供了一种有效的对抗训练方法，能够在保持干净数据性能的同时，平衡地提升模型的自然准确性和对抗鲁棒性。
...
### [323] [On the Fairness of Privacy Protection: Measuring and Mitigating the Disparity of Group Privacy Risks for Differentially Private Machine Learning](https://arxiv.org/abs/2510.09114)
*Zhi Yang,Changwu Huang,Ke Tang,Xin Yao*

Main category: cs.LG

TL;DR: 本文提出了一种新的成员推断游戏来高效审计数据记录的近似最坏情况隐私风险，并改进了DP-SGD算法，通过自适应组特定梯度裁剪策略减少组间隐私风险差异，提升差分隐私机器学习中的隐私保护公平性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注平均情况下的隐私风险评估，可能低估组间隐私风险差异，且最坏情况隐私风险评估方法耗时较长，限制了实际应用。

Method: 引入新型成员推断游戏来高效审计近似最坏情况隐私风险；改进标准DP-SGD算法，采用自适应组特定梯度裁剪策略。

Result: 实验结果表明，该方法能更严格地衡量组隐私风险，可靠评估组间隐私风险差异；改进的算法有效减少了组间隐私风险差异。

Conclusion: 提出的方法能够提升差分隐私机器学习中的隐私保护公平性，为组间隐私风险评估提供了更可靠的工具。
...
### [324] [Regret Bounds for Adversarial Contextual Bandits with General Function Approximation and Delayed Feedback](https://arxiv.org/abs/2510.09127)
*Orin Levy,Liad Erez,Alon Cohen,Yishay Mansour*

Main category: cs.LG

TL;DR: 本文研究了具有延迟反馈的上下文多臂老虎机问题，提出了两种算法：一种针对有限策略类的最优后悔边界算法，另一种针对一般函数逼近的算法，并给出了相应的后悔边界分析和下界证明。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的在线学习场景中，反馈往往存在延迟，这种延迟可能由对手选择。现有的上下文多臂老虎机算法大多假设即时反馈，无法有效处理延迟反馈的情况，因此需要开发能够处理延迟反馈的算法。

Method: 提出了两种算法：1）针对有限策略类的算法，直接访问策略类；2）针对一般函数逼近的算法，使用在线最小二乘回归预言机。特别地，分析了基于Hedge的Vovk聚合预测器作为最小二乘回归预言机的稳定性。

Result: 对于有限策略类，获得了最优的期望后悔边界O(√(KT log|Π|) + √(D log|Π|))；对于一般函数逼近，获得了O(√(KTℛ_T(O)) + √(d_max D β))的后悔边界，其中通过稳定性分析将β限制为log|F|。同时证明了Ω(√(KT log|F|) + √(D log|F|))的下界。

Conclusion: 本文成功解决了延迟反馈下的上下文多臂老虎机问题，提出了高效的算法并建立了严格的后悔边界。对于一般函数逼近的情况，虽然与下界存在√d_max的差距，但这是该问题上的首个非平凡结果，为后续研究奠定了基础。
...
### [325] [Score-Based Density Estimation from Pairwise Comparisons](https://arxiv.org/abs/2510.09146)
*Petrus Mikkola,Luigi Acerbi,Arto Klami*

Main category: cs.LG

TL;DR: 该论文提出了一种从成对比较中估计密度的方法，通过将未观测的目标密度与经过温度调节的胜者密度相关联，利用分数匹配学习胜者的分数，然后通过'去温度调节'来估计目标密度。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于专家知识获取和从人类反馈中学习的需求，旨在通过成对比较来估计复杂的多元信念密度。

Method: 方法包括：将目标密度与温度调节的胜者密度关联；通过分数匹配学习胜者分数；使用扩散模型在通过分数缩放退火朗之万动力学生成的温度调节样本上训练；在Bradley-Terry模型下估计温度调节场。

Result: 该方法能够仅使用数百到数千个成对比较，学习模拟专家的复杂多元信念密度。

Conclusion: 证明了信念密度和胜者密度的分数向量是共线的，通过位置相关的温度调节场连接，并给出了该场的解析公式和估计器。
...
### [326] [Logits Replay + MoClip: Stabilized, Low-Cost Post-Training with Minimal Forgetting](https://arxiv.org/abs/2510.09152)
*Suming Qiu,Jing Li,Zhicheng Zhou,Junjie Huang,Linyuan Qiu,Zhijie Sun*

Main category: cs.LG

TL;DR: 提出Logits Replay + MoClip两阶段框架，通过压缩logit空间的监督信号和优化器层面的稳定化，在提升领域性能的同时缓解通用能力遗忘，并降低40%以上的训练成本。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在领域适应训练中面临的权衡问题：提升专业领域性能往往以牺牲通用能力为代价。现有方法在计算、数据访问或适应性方面成本高昂。

Method: 两阶段框架：Stage 0记录动态Top-K token子集（包含黄金标签）；Stage 1重放这些紧凑子集计算精确重归一化损失。同时设计MoClip优化器，限制梯度动量旋转并基于arctan2重缩放更新。

Result: 在通信技术(CT)和NL2SQL任务上提升领域性能，同时在通用基准(MMLU、BBH、GPQA、MATH)上缓解遗忘，训练成本降低40%以上。

Conclusion: 该方法为LLMs的领域适应提供了可扩展、架构无关的路径，无需牺牲泛化能力。
...
### [327] [Agentic-KGR: Co-evolutionary Knowledge Graph Construction through Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2510.09156)
*Jing Li,Zhijie Sun,Zhicheng Zhou,Suming Qiu,Junjie Huang,Haijia Sun,Linyuan Qiu*

Main category: cs.LG

TL;DR: Agentic-KGR是一个通过多轮强化学习实现大语言模型与知识图谱协同进化的框架，解决了现有知识增强LLMs依赖静态知识库导致的覆盖不足和时效性问题。


<details>
  <summary>Details</summary>
Motivation: 当前知识增强的大语言模型依赖静态、预构建的知识库，存在覆盖差距和时间过时问题，限制了它们在动态信息环境中的有效性。

Method: 提出Agentic-KGR框架，包含三个关键创新：动态模式扩展机制、检索增强记忆系统、可学习的多尺度提示压缩方法，通过多轮强化学习实现LLMs与知识图谱的协同进化。

Result: 实验结果显示在知识抽取任务中显著优于监督基线和单轮强化学习方法，与GraphRAG集成后在问答任务中表现出优越性能，在准确性和知识覆盖方面都有显著提升。

Conclusion: Agentic-KGR框架通过协同进化机制有效解决了知识增强LLMs的覆盖不足和时效性问题，为动态知识环境下的知识管理提供了新的解决方案。
...
### [328] [Cross-Representation Benchmarking in Time-Series Electronic Health Records for Clinical Outcome Prediction](https://arxiv.org/abs/2510.09159)
*Tianyi Chen,Mingcheng Zhu,Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: 本文提出了首个系统性基准来比较EHR表示方法，包括多变量时间序列、事件流和用于LLM的文本事件流。研究发现事件流模型表现最佳，预训练模型在少样本设置中高效，特征选择策略需根据临床环境调整。


<details>
  <summary>Details</summary>
Motivation: 由于评估实践不一致，电子健康记录（EHR）中患者数据的最佳表示方法仍不明确，需要系统性的比较基准来指导临床预测任务。

Method: 建立标准化数据整理和评估流程，在MIMIC-IV数据集（ICU任务）和EHRSHOT数据集（纵向护理）上评估多种建模方法，包括Transformer、MLP、LSTM、Retain、CLMBR、基于计数的模型和8-20B LLMs，并分析基于数据缺失的特征剪枝影响。

Result: 事件流模型始终提供最强性能；预训练模型在少样本设置中样本效率高；ICU预测中剪枝稀疏特征有益，而纵向任务中保留这些特征至关重要。

Conclusion: 通过统一可复现的流程，结果为基于临床环境和数据制度选择EHR表示提供了实用指导，事件流模型是最佳选择，特征选择策略需适应具体临床环境。
...
### [329] [Efficient Resource-Constrained Training of Vision Transformers via Subspace Optimization](https://arxiv.org/abs/2510.09160)
*Le-Trung Nguyen,Enzo Tartaglione,Van-Tam Nguyen*

Main category: cs.LG

TL;DR: 本文提出WASI方法，通过将训练限制在固定子空间内，显著降低Transformer模型的内存占用和计算成本，同时保持与标准训练相当的精度。


<details>
  <summary>Details</summary>
Motivation: 随着AI在日常生活中的应用日益广泛，能耗和数据隐私成为紧迫问题。设备端学习直接在边缘设备上训练模型，既能降低能耗又能保护数据隐私，但现代神经网络的规模扩大成为设备端训练的主要障碍。

Method: 基于模型关键信息存在于固定子空间的理念，提出Weight-Activation Subspace Iteration (WASI)方法，通过将训练限制在该子空间内，缓解反向传播的内存瓶颈并提升Transformer模型的推理效率。

Result: WASI在保持与标准训练相当精度的同时，将内存使用降低高达62倍，计算成本(FLOPs)降低高达2倍。在Raspberry Pi 5上，WASI实现约1.5倍于标准训练的训练和推理速度。
...
### [330] [Robustness and Regularization in Hierarchical Re-Basin](https://arxiv.org/abs/2510.09174)
*Benedikt Franke,Florian Heinrich,Markus Lange,Arne Raul*

Main category: cs.LG

TL;DR: 本文重新审视了Git Re-Basin模型合并方法，提出了一种分层模型合并方案，显著优于标准MergeMany算法。研究发现Re-Basin能增强合并模型的对抗性和扰动鲁棒性，但实际性能下降比原作者报告的要大得多。


<details>
  <summary>Details</summary>
Motivation: 深入分析Git Re-Basin这一新的模型合并方法，探索更有效的模型合并策略，并验证其在鲁棒性方面的实际效果。

Method: 提出分层模型合并方案，与标准MergeMany算法进行对比，通过实验评估Re-Basin方法在模型合并中的表现。

Result: 分层模型合并方案显著优于标准算法，Re-Basin能增强合并模型的对抗性和扰动鲁棒性，且参与合并的模型越多效果越强，但实际性能下降比原作者报告的要严重。

Conclusion: Git Re-Basin方法在鲁棒性方面有积极效果，但需要重新评估其实际性能影响，分层合并方案提供了更好的替代方案。
...
### [331] [Beyond Pairwise Connections: Extracting High-Order Functional Brain Network Structures under Global Constraints](https://arxiv.org/abs/2510.09175)
*Ling Zhan,Junjie Huang,Xiaoyao Yu,Wenyu Chen,Tao Jia*

Main category: cs.LG

TL;DR: 提出GCM框架，通过全局约束从功能脑网络数据中学习高阶结构，在5个数据集上相比基线方法提升30.6%准确率并减少96.3%计算时间。


<details>
  <summary>Details</summary>
Motivation: 传统功能脑网络建模依赖局部成对交互，难以捕捉高阶依赖关系；现有超图建模方法计算负担重且启发式，无法端到端学习数据分布中的网络结构。

Method: 提出全局约束导向的多分辨率（GCM）框架，整合4种全局约束（信号同步、被试身份、期望边数、数据标签）来学习4个不同分辨率水平（样本/被试/组/项目）的功能脑网络结构。

Result: 在5个数据集和2种任务设置下，相比9个基线和10个先进方法，GCM实现了相对准确率提升30.6%，计算时间减少96.3%。实验验证了各组件贡献和模型可解释性。
...
### [332] [RepDL: Bit-level Reproducible Deep Learning Training and Inference](https://arxiv.org/abs/2510.09180)
*Peichen Xie,Xian Zhang,Shuo Chen*

Main category: cs.LG

TL;DR: RepDL是一个开源库，通过强制正确舍入和顺序不变性来解决深度学习中的非确定性和不可复现性问题，确保跨计算环境的比特级可复现训练和推理。


<details>
  <summary>Details</summary>
Motivation: 深度学习中存在非确定性和不可复现性的挑战，这源于随机数生成和浮点计算。虽然随机性可以通过确定性配置控制，但浮点不一致性问题仍未解决。

Method: 开发RepDL开源库，通过强制正确舍入和顺序不变性来确保浮点计算的确定性。

Result: RepDL能够确保深度学习训练和推理在不同计算环境中的比特级可复现性。

Conclusion: RepDL成功解决了深度学习中的浮点计算不一致性问题，提供了跨平台的可复现性解决方案。
...
### [333] [On the Implicit Adversariality of Catastrophic Forgetting in Deep Continual Learning](https://arxiv.org/abs/2510.09181)
*Ze Peng,Jian Zhang,Jintao Guo,Lei Qi,Yang Gao,Yinghuan Shi*

Main category: cs.LG

TL;DR: 本文揭示了持续学习中的灾难性遗忘本质上是新任务训练对旧任务知识的对抗性攻击，新任务梯度会自动与旧任务损失景观的尖锐方向对齐，从而快速增加旧任务损失。作者提出backGP方法来解决这一问题，相比梯度投影方法平均减少10.8%的遗忘并提高12.7%的准确率。


<details>
  <summary>Details</summary>
Motivation: 持续学习的目标是实现类似人类积累新技能的能力，但其核心挑战——灾难性遗忘——在深度网络中的根本原因尚未被完全理解。本文旨在揭示灾难性遗忘的深层机制。

Method: 通过理论分析发现，新任务训练实际上是针对旧任务知识的对抗性攻击，新任务梯度会自动与旧任务损失景观的尖锐方向对齐。作者提出backGP方法来解决由反向传播引起的对齐问题，该方法在梯度投影方法的基础上进一步优化。

Result: 实验表明，backGP方法相比梯度投影方法平均减少10.8%的遗忘并提高12.7%的准确率，有效缓解了灾难性遗忘问题。

Conclusion: 灾难性遗忘的根本原因在于新任务训练对旧任务知识的对抗性攻击，这种攻击源于训练的低秩偏差。backGP方法通过解决反向传播引起的对齐问题，显著改善了持续学习性能。
...
### [334] [Multimodal Prompt Optimization: Why Not Leverage Multiple Modalities for MLLMs](https://arxiv.org/abs/2510.09201)
*Yumin Choi,Dongki Kim,Jinheon Baek,Sung Ju Hwang*

Main category: cs.LG

TL;DR: 本文提出了多模态提示优化问题，并开发了MPO框架，通过联合优化文本和非文本提示，在图像、视频、分子等多种模态上超越了仅文本的优化方法。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型(MLLMs)在图像、视频等非文本模态上展现出强大能力，但现有的提示优化方法仍局限于文本领域，限制了MLLMs的完整潜力。

Method: 提出了多模态提示优化器(MPO)，这是一个统一框架，通过保持对齐的联合更新来优化多模态提示，并利用贝叶斯选择策略基于先前评估指导候选提示的选择。
...
### [335] [FM-IRL: Flow-Matching for Reward Modeling and Policy Regularization in Reinforcement Learning](https://arxiv.org/abs/2510.09222)
*Zhenglin Wan,Jingxuan Wu,Xingrui Yu,Chubin Zhang,Mingcong Lei,Bo An,Ivor Tsang*

Main category: cs.LG

TL;DR: 本文提出了一种结合流匹配（FM）教师模型和简单MLP学生策略的方法，通过在线强化学习解决FM策略在未见场景中泛化能力差的问题，同时避免了FM策略梯度不稳定和推理成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 流匹配（FM）在模仿学习中能有效克隆专家行为，但由于缺乏环境交互和探索，在未见场景中泛化能力差。直接在线优化FM策略又面临梯度不稳定和高推理成本的挑战。

Method: 使用简单的MLP结构作为学生策略进行环境探索和在线RL更新，同时利用教师FM模型构建奖励函数，并作为正则化器来稳定策略学习。

Result: 实验表明该方法显著提高了学习效率、泛化能力和鲁棒性，特别是在从次优专家数据学习时表现突出。

Conclusion: 通过将FM的表达能力与简单MLP策略的在线探索能力相结合，成功解决了FM策略在在线学习中的局限性，实现了高效稳定的策略学习。
...
### [336] [Prime Implicant Explanations for Reaction Feasibility Prediction](https://arxiv.org/abs/2510.09226)
...
Method: 引入了针对该领域的质蕴涵解释新表述（也称为最小充分原因），并提出了在小规模反应预测任务中计算此类解释的算法。

Result: 初步实验表明，质蕴涵解释方法能够保守地捕捉真实解释，虽然常包含冗余的键和原子，但始终能捕获预测反应可行性所必需的分子属性。
...
### [337] [Incentivizing Time-Aware Fairness in Data Sharing](https://arxiv.org/abs/2510.09240)
*Jiangwei Chen,Kieu Thao Nguyen Pham,Rachael Hwee Ling Sim,Arun Verma,Zhaoxuan Wu,Chuan-Sheng Foo,Bryan Kian Hsiang Low*

Main category: cs.LG

TL;DR: 提出了一种公平且时间感知的数据共享框架，为早期参与方提供更高奖励，以激励及时参与协作机器学习。


<details>
  <summary>Details</summary>
Motivation: 现有协作框架假设所有参与方同时加入，但现实中由于数据清理时间、法律障碍或认知延迟，参与方可能在不同时间加入。早期参与方承担更高风险并鼓励其他观望方贡献，因此应获得更高奖励。
...
### [338] [A PCA-based Data Prediction Method](https://arxiv.org/abs/2510.09246)
*Peteris Daugulis,Vija Vagale,Emiliano Mancini,Filippo Castiglione*

Main category: cs.LG

TL;DR: 提出了一种结合传统数学和机器学习的新方法，用于预测（填补）缺失数据，该方法基于现有数据和候选集之间移位线性子空间距离的概念。
...
Method: 该方法将现有数据集表示为由其主成分张成的子空间，基于移位线性子空间之间的距离概念，在欧几里得度量下给出解决方案。

Result: 描述了一种新颖的缺失数据预测方法，结合了传统数学和机器学习元素。

Conclusion: 提出的基于移位线性子空间距离的方法为缺失数据预测提供了一种新的解决方案。
...
### [339] [Mitigating Model Drift in Developing Economies Using Synthetic Data and Outliers](https://arxiv.org/abs/2510.09294)
...
TL;DR: 该研究探讨了在金融机器学习模型中应对模型漂移的问题，特别是在中亚和高加索地区的发展中经济体。研究提出使用合成异常值来提高模型对不可预见冲击的稳定性，并引入了一个两级评估框架来衡量性能退化和冲击严重程度。


<details>
  <summary>Details</summary>
Motivation: 金融机器学习模型容易受到模型漂移的影响，特别是在发展中经济体如中亚和高加索地区，这些地区频繁且不可预测的宏观经济冲击会破坏金融数据的稳定性。目前缺乏针对这些地区金融数据漂移缓解方法的研究。

Method: 研究调查了使用合成异常值的方法来提高模型稳定性，这是一种较少被探索的方法。引入了一个两级评估框架来测量性能退化的程度和冲击的严重性。在宏观经济表格数据集上进行了实验。

Result: 实验结果显示，与基线模型相比，添加少量合成异常值通常能提高稳定性，但最佳添加量因数据集和模型而异。

Conclusion: 合成异常值可以作为提高金融机器学习模型对不可预见冲击稳定性的有效方法，但需要根据具体数据集和模型调整使用量。
...
### [340] [Large Language Model Prompt Datasets: An In-depth Analysis and Insights](https://arxiv.org/abs/2510.09316)
*Yuanming Zhang,Yan Lin,Arijit Khan,Huaiyu Wan*

Main category: cs.LG

TL;DR: 本文首次系统性地收集和分析了来自GitHub、社交媒体等平台的多样化提示数据集，涵盖不同任务、语言、工程技术和模态。通过分析提示构造的共同点和差异，提出了一种基于句法嵌入的提示优化方法，通过引导LLM将提示重写为中心表示来提升模型输出的意义性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的广泛部署，来自各种平台的多样化提示数据集不断涌现，这些数据集促进了LLM的广泛应用和提示工程的改进。然而，目前缺乏对这些提示数据集的系统性整理和分析。

Method: 1) 从多个渠道收集广泛的提示数据集；2) 选择代表性数据集进行系统性分析，揭示不同类别提示构造的共性和差异；3) 提出基于词性和依存结构句法嵌入的提示优化方法，通过识别提示的中心表示并引导LLM向该中心重写提示。

Result: 成功构建了涵盖多种下游任务、语言、工程技术和模态的提示数据集集合。提出的提示优化方法能够有效提升模型输出的意义性。数据集和代码已公开可用。

Conclusion: 这项工作首次系统性地整理了多样化的提示数据集，揭示了提示构造的规律性特征，并提出了有效的提示优化方法，为提示工程和LLM应用提供了重要资源和方法支持。
...
### [341] [Residual-Informed Learning of Solutions to Algebraic Loops](https://arxiv.org/abs/2510.09317)
*Felix Brandt,Andreas Heuermann,Philip Hannebohm,Bernhard Bachmann*

Main category: cs.LG

TL;DR: 提出一种基于残差的机器学习方法，用神经网络替代Modelica模型中的代数环，无需监督数据集，通过残差训练解决多解问题，在IEEE 14-Bus系统中实现60%的仿真加速。


<details>
  <summary>Details</summary>
Motivation: 传统Modelica模型中的代数环会显著增加仿真时间，需要一种能够替代代数环同时保持准确性的高效方法。

Method: 使用前馈神经网络，直接在损失函数中使用代数环的残差进行训练，无需监督数据，并解决多解一致性问题。

Result: 在IEEE 14-Bus系统上，相比传统仿真方法实现了60%的仿真时间减少，同时通过误差控制机制保持了相同的精度水平。

Conclusion: 基于残差的机器学习方法能够有效替代代数环，显著提升仿真效率，同时确保解的准确性和一致性。
...
### [342] [Rate optimal learning of equilibria from data](https://arxiv.org/abs/2510.09325)
*Till Freihaut,Luca Viano,Emanuele Nevali,Volkan Cevher,Matthieu Geist,Giorgia Ramponi*

Main category: cs.LG

TL;DR: 该论文填补了多智能体模仿学习(MAIL)的理论空白，证明了非交互式MAIL的统计下界并提出了首个具有接近最优样本复杂度的交互式算法MAIL-WARM。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体模仿学习中的理论缺口，特别是在非交互式和交互式设置下的样本复杂度问题，以及行为克隆在某些环境中的失败情况。

Method: 在非交互式设置中证明了统计下界并识别了全策略偏差集中系数；在交互式设置中提出了结合无奖励强化学习与交互式MAIL的框架，并实例化为MAIL-WARM算法。

Result: MAIL-WARM算法将样本复杂度从O(ε^-8)改进到O(ε^-2)，匹配了下界结果；数值实验支持理论分析并展示了行为克隆在网格世界等环境中的失败。

Conclusion: 该工作为多智能体模仿学习提供了理论基础和高效算法，填补了理论空白并实现了接近最优的样本复杂度。
...
### [343] [Safety Game: Balancing Safe and Informative Conversations with Blackbox Agentic AI using LP Solvers](https://arxiv.org/abs/2510.09330)
*Tuan Nguyen,Long Tran-Thanh*

Main category: cs.LG

TL;DR: 提出了一种模型无关的黑箱安全对齐框架，通过博弈论方法在推理时平衡安全性和有用性，无需重新训练或访问模型内部。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法主要在训练阶段进行，成本高且不灵活，无法适应新要求；推理时对齐方法仍需要访问模型内部，不适合第三方利益相关者。

Method: 将安全性与有用性的权衡建模为两人零和博弈，使用线性规划求解器在推理时计算均衡策略。

Result: 证明了黑箱安全对齐的可行性，为资源受限的组织提供了可扩展的安全执行途径。

Conclusion: 该框架为各种利益相关者，特别是资源受限的小型组织，在快速发展的LLM生态系统中实施安全对齐提供了可行方案。
...
### [344] [Efficient Bayesian Inference from Noisy Pairwise Comparisons](https://arxiv.org/abs/2510.09333)
...
<details>
  <summary>Details</summary>
Motivation: 标准评估指标往往无法反映人类偏好，而人类评估虽然更可靠但成本高且存在噪声。现有Bradley-Terry方法要么忽略评分者变异性，要么缺乏收敛保证。

Method: 提出BBQ方法，使用贝叶斯Bradley-Terry变体显式建模评分者质量，通过期望最大化算法提供单调似然收敛保证，对不可靠参与者进行降权或移除。

Result: 实证结果显示BBQ实现更快收敛、良好校准的不确定性估计，以及与基线Bradley-Terry模型相比更稳健、可解释的排名，即使在有噪声或众包评分者情况下也表现良好。

Conclusion: BBQ框架能够实现更可靠和成本效益高的生成模型人类评估。
...
### [345] [Deep Learning to Identify the Spatio-Temporal Cascading Effects of Train Delays in a High-Density Network](https://arxiv.org/abs/2510.09350)
*Vu Duc Anh Nguyen,Ziyue Li*

Main category: cs.LG

TL;DR: 本文提出了一个名为XGeoAI的新框架，用于实时、可解释的多步列车延误预测。该框架采用两阶段自回归图注意力网络(GAT)模型，在荷兰铁路网络数据集上进行训练和评估，结果显示在延误事件分类精度方面优于基准模型。


<details>
  <summary>Details</summary>
Motivation: 铁路网络的运营效率受到列车延误连锁效应的持续影响。虽然已有研究使用图神经网络建模铁路网络结构，但现有方法缺乏能够提供网络范围多步自回归预测，同时具备实时可解释性的框架，这对于决策支持至关重要。

Method: 开发了一个两阶段自回归图注意力网络(GAT)模型，将系统表示为运营事件(到达和出发)的时空图，并加入了平台和车站拥堵等细粒度特征。使用顺序k步提前预测协议进行严格评估，模拟真实世界中预测误差可能累积的情况。

Result: 结果显示，虽然提出的GATv2模型在纯误差指标(MAE)上被更简单的持续性基准模型挑战，但在延误事件分类精度方面始终表现更高，这对于可靠的决策支持工具来说是一个关键优势。

Conclusion: 该研究成功开发了一个实时可解释的多步列车延误预测框架，在延误事件分类方面表现出色，为铁路实时交通管理提供了有价值的决策支持工具。
...
### [346] [The Potential of Second-Order Optimization for LLMs: A Study with Full Gauss-Newton](https://arxiv.org/abs/2510.09378)
*Natalie Abreu,Nikhil Vyas,Sham Kakade,Depen Morwani*

Main category: cs.LG

TL;DR: 本文研究了在LLM预训练中应用完整高斯牛顿预条件处理的效果，发现相比现有优化器能显著减少训练迭代次数，同时发现分层GN预条件处理几乎能达到完整GN方法的性能。


<details>
  <summary>Details</summary>
Motivation: 探索在LLM大规模预训练中，计算效率高的近似方法会损失多少性能，通过建立迭代复杂度的实际上限来评估高斯牛顿预条件处理的效果。

Method: 对最多1.5亿参数的transformer模型应用完整高斯牛顿预条件处理，并与SOAP、Muon等强基线进行比较，同时研究分层GN预条件处理的性能。

Result: 完整GN更新相比现有优化器带来显著收益，训练迭代次数减少5.4倍；分层GN预条件处理几乎匹配完整GN方法的性能。

Conclusion: GN近似对预条件处理非常有效，高阶损失项对收敛速度可能不关键；分层Hessian结构包含足够信息实现大部分潜在收益；当前近似方法与理想化分层oracle存在显著性能差距。
...
### [347] [Task-Level Insights from Eigenvalues across Sequence Models](https://arxiv.org/abs/2510.09379)
*Rahel Rickenbach,Jelena Trisovic,Alexandre Didier,Jerome Sieber,Melanie N. Zeilinger*

Main category: cs.LG

TL;DR: 本文通过动力学系统框架将softmax、norm和线性注意力表示为动力学系统，并与状态空间模型(SSMs)进行特征值谱比较分析，揭示了特征值对序列模型记忆和长程依赖建模的影响，为理解和改进序列模型提供了原则性方法。


<details>
  <summary>Details</summary>
Motivation: 虽然softmax注意力驱动了最先进的序列模型性能，但其二次复杂度限制了可扩展性，促使了线性替代方案如状态空间模型(SSMs)的发展。然而，这些替代方案在信息处理方面的根本差异仍然理解不足。

Method: 利用最近提出的动力学系统框架将softmax、norm和线性注意力表示为动力学系统，通过分析它们各自的特征值谱与SSMs进行结构化比较，并在多种序列模型和基准测试上进行广泛的实证分析。

Result: 特征值影响记忆和长程依赖建模的关键方面，揭示了与任务需求一致的频谱特征。架构修改对特征值谱和任务性能的影响存在对应关系。

Conclusion: 特征值分析作为解释、理解并最终改进序列模型能力的原理性指标的地位得到了进一步强化。
...
### [348] [CHUCKLE -- When Humans Teach AI To Learn Emotions The Easy Way](https://arxiv.org/abs/2510.09382)
*Ankush Pratap Singh,Houwei Cao,Yong Liu*

Main category: cs.LG

TL;DR: CHUCKLE是一个基于人类感知的课程学习框架，利用众包数据中的标注者一致性和对齐度来定义样本难度，在情感识别任务中提高了模型准确率和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别的课程学习方法通常依赖启发式、数据驱动或模型驱动的难度定义，忽视了人类感知难度这一在主观任务中至关重要的因素。

Method: 提出CHUCKLE框架，通过众包数据集中标注者的一致性和对齐度来定义样本难度，假设对人类具有挑战性的样本对机器学习模型同样困难。

Result: 实验结果显示，CHUCKLE相比非课程学习基线，LSTM模型的相对平均准确率提高了6.56%，Transformer模型提高了1.61%，同时减少了梯度更新次数。

Conclusion: CHUCKLE框架通过整合人类感知难度，不仅提升了情感识别模型的准确率，还提高了训练效率和模型鲁棒性。
...
### [349] [HINT: Helping Ineffective Rollouts Navigate Towards Effectiveness](https://arxiv.org/abs/2510.09388)
*Xinyi Wang,Jinyi Han,Zishang Jiang,Tingyun Li,Jiaqing Liang,Sihang Jiang,Zhaoqian Dai,Shuguang Ma,Fei Yu,Yanghua Xiao*

Main category: cs.LG

TL;DR: 本文提出了HINT框架，通过提供启发式提示而非直接答案来解决强化学习中训练亲和力低的问题，在数学推理任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在任务难度超过模型能力时会出现奖励稀疏和训练效率低的问题，而使用外部指导的方法往往导致策略更新误导，核心问题是训练亲和力低。

Method: 提出了HINT框架，引入Affinity指标来监控探索效率和训练稳定性，通过提供启发式提示引导模型自主发现解决方案，保持其自主推理能力。

Result: 在数学推理任务上的广泛实验表明，HINT一致优于现有方法，在不同规模模型上都实现了最先进的结果，同时表现出更稳定的学习和更高的数据效率。

Conclusion: HINT框架有效解决了强化学习中训练亲和力低的问题，通过自适应提示机制提高了模型的推理能力和训练效率。
...
### [350] [Design Principles for Sequence Models via Coefficient Dynamics](https://arxiv.org/abs/2510.09389)
*Jerome Sieber,Antonio Orvieto,Melanie N. Zeilinger,Carmen Amo Alonso*

Main category: cs.LG

TL;DR: 本文提出了一个统一框架，将深度序列模型的输出操作显式表示为自主线性动力系统的输出，揭示了不同架构间的共同数学主题，并建立了架构选择与模型特性之间的设计原则。


<details>
  <summary>Details</summary>
Motivation: 为了系统比较Transformer、状态空间模型、门控线性RNN等深度序列模型，并深入理解这些架构的共同数学基础，需要建立一个统一的分析框架。

Method: 开发了一个统一框架，将线性组合系数建模为由脉冲输入驱动的自主线性动力系统的输出，从而显式地表达序列模型的输出操作。

Result: 该框架揭示了不同架构间的共同数学主题，能够统一捕捉softmax注意力以及RNN、SSM等相关模型，并建立了架构选择与模型特性之间的设计原则。

Conclusion: 该框架不仅解释了近期设计的经验成功，还为系统设计新的序列模型架构提供了指导原则，识别了表达能力与高效实现之间的权衡、输入选择性的几何约束，以及数值稳定训练和信息保持的稳定性条件。
...
### [351] [Cross-Receiver Generalization for RF Fingerprint Identification via Feature Disentanglement and Adversarial Training](https://arxiv.org/abs/2510.09405)
*Yuhao Pan,Xiucheng Wang,Nan Cheng,Wenchao Xu*

Main category: cs.LG

TL;DR: 本文提出了一种对抗训练和风格迁移相结合的射频指纹识别框架，旨在解决接收器引起的特征偏移问题，提高跨接收器场景下的识别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 射频指纹识别在实际部署中面临接收器引起的特征偏移问题，导致模型过度拟合特定接收器模式，当更换接收器时性能显著下降。

Method: 通过对抗训练和风格迁移技术，显式解耦发射器和接收器特征，强制学习域不变表示，隔离真实的硬件特征与接收器伪影。

Result: 在多接收器数据集上的广泛实验表明，该方法在多种接收器设置下平均准确率比现有最优基线提高高达10%。

Conclusion: 所提出的框架能够有效应对跨接收器变异性，确保射频指纹识别系统在实际部署中的鲁棒性和可靠性。
...
### [352] [What Do Temporal Graph Learning Models Learn?](https://arxiv.org/abs/2510.09416)
*Abigail J. Hayes,Tobias Schumacher,Markus Strohmaier*

Main category: cs.LG

TL;DR: 本文系统评估了7个时序图学习模型在捕捉8种基本图属性方面的能力，揭示了模型在某些属性上表现良好但在其他属性上存在局限性的混合结果。


<details>
  <summary>Details</summary>
Motivation: 针对当前时序图学习基准结果可靠性受到质疑的问题，研究旨在探究时序图学习模型实际利用哪些图属性来形成预测，以提供更可靠的模型评估。

Method: 使用合成和真实世界数据集，系统评估7个模型在捕捉8种基本时序图属性（包括结构特征如密度、时序模式如最近性、边形成机制如同质性等）的能力。

Result: 研究发现混合结果：模型能够很好地捕捉某些属性，但在重现其他属性方面失败，暴露了重要局限性。

Conclusion: 研究结果为时序图学习模型的实际应用提供了实用见解，并推动了时序图学习研究中更多基于可解释性的评估方法。
...
### [353] [Weight Initialization and Variance Dynamics in Deep Neural Networks and Large Language Models](https://arxiv.org/abs/2510.09423)
*Yankun Han*

Main category: cs.LG

TL;DR: 该论文系统研究了权重初始化对神经网络训练的影响，包括ReLU多层感知机和GPT-2风格transformer，发现1e-2到1e-1的标准差范围具有最佳稳定性，Kaiming初始化优于Xavier，并观察到transformer中浅层权重方差快速扩张而深层变化缓慢的现象。


<details>
  <summary>Details</summary>
Motivation: 研究权重初始化如何影响训练初期的信号传播和梯度流动，为现代深度神经网络提供理论指导和实用初始化策略。

Method: 采用对数扫描初始标准差来识别消失/爆炸梯度区域，比较Kaiming和Xavier初始化效果，并在12层GPT-2风格模型中跟踪Q/K/V权重方差在预训练中的变化。

Result: 发现1e-2到1e-1标准差范围具有最宽稳定性带；Kaiming初始化在ReLU网络中收敛更快更稳定；transformer浅层权重方差快速扩张，深层变化缓慢并趋于稳定窄带。

Conclusion: 连接经典初始化原理与现代transformer行为，提供了简单实用的鲁棒训练初始化方案，强调了深度依赖的权重方差平衡现象。
...
TL;DR: STaTS是一个轻量级的无监督框架，用于结构感知的时间序列摘要，通过多分辨率变化点检测和分段摘要，实现高达30倍的序列压缩，同时保留核心时间动态。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列模型通常处理原始或固定窗口序列，将所有时间步视为同等重要，导致在处理长序列或噪声序列时效率低下、鲁棒性差且可扩展性有限。

Method: 使用基于BIC的统计差异准则检测多时间分辨率的变化点，然后使用均值或GMM等生成模型对每个片段进行摘要，实现模型无关的预处理。

Result: 在150多个数据集上的实验表明，STaTS能够达到完整模型85-90%的性能，同时显著降低计算成本，在噪声条件下提高鲁棒性，优于均匀和基于聚类的压缩基线。
...
### [355] [Bandits with Single-Peaked Preferences and Limited Resources](https://arxiv.org/abs/2510.09425)
*Gur Keinan,Rotem Torkan,Omer Ben-Porat*

Main category: cs.LG

TL;DR: 本文研究了在线随机匹配问题，提出了在单峰偏好结构下的高效算法，分别针对已知和未知偏好结构的情况设计了两种算法，获得了次线性遗憾界。


<details>
  <summary>Details</summary>
Motivation: 解决在线随机匹配问题中由于NP难度导致的计算不可行性，利用社会选择理论中的单峰偏好结构来克服这一障碍。

Method: 设计了基于PQ树的顺序逼近方法，针对未知单峰结构的情况开发了高效算法；对于已知单峰结构的情况，开发了类似UCB的高效算法。

Result: 未知结构时获得O(UKT^{2/3})的遗憾界，已知结构时获得O(U√(TK))的遗憾界。

Conclusion: 单峰偏好结构能够显著简化在线匹配问题的计算复杂度，使得在预算约束下实现高效的在线学习成为可能。
...
### [356] [On Uniformly Scaling Flows: A Density-Aligned Approach to Deep One-Class Classification](https://arxiv.org/abs/2510.09452)
*Faried Abu Zaid,Tim Katzke,Emmanuel Müller,Daniel Neider*

Main category: cs.LG

TL;DR: 本文证明了均匀缩放流（USFs）在理论上连接了深度单类分类和密度估计两种异常检测范式，将最大似然训练简化为Deep SVDD目标，并展示了USFs作为非USFs的替代品能带来性能提升和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 连接深度单类分类（如Deep SVDD）和密度估计（如标准化流）这两种广泛研究的异常检测范式，建立理论桥梁。

Method: 使用均匀缩放流（USFs）——具有常数雅可比行列式的标准化流，证明其最大似然训练等价于Deep SVDD目标，并防止表示崩溃。

Result: USFs在负对数似然和潜在范数之间建立了比Deep SVDD或非USFs更紧密的对齐，作为非USFs的替代品在多个基准测试和模型骨干上带来一致的性能提升和训练稳定性改善。

Conclusion: USFs统一了两种主要的异常检测范式，既推进了理论理解又提升了实际性能，建议在现代异常检测架构中使用USFs作为非USFs的即插即用替代品。
...
### [357] [Adaptive Attacks on Trusted Monitors Subvert AI Control Protocols](https://arxiv.org/abs/2510.09462)
*Mikhail Terekhov,Alexander Panfilov,Daniil Dzenhaliou,Caglar Gulcehre,Maksym Andriushchenko,Ameya Prabhu,Jonas Geiping*

Main category: cs.LG

TL;DR: 本文研究了针对AI控制协议的自适应攻击，发现当攻击者知道监控模型和协议细节时，可以通过嵌入公开已知或零样本提示注入的方式，持续规避各种监控器并完成恶意任务。


<details>
  <summary>Details</summary>
Motivation: 现有AI控制协议主要依赖LLM监控器作为防御机制，但这些监控器可能成为单点故障。当不受信任的模型了解协议和监控模型时，可能发起自适应攻击绕过防御。

Method: 研究者在两个主要AI控制基准上实例化了一种简单的自适应攻击向量，攻击者在模型输出中嵌入公开已知或零样本提示注入来规避监控。

Result: 前沿模型能够持续规避多样化的监控器并完成恶意任务。攻击对当前依赖监控器的协议具有普遍有效性，甚至Defer-to-Resample协议会产生反效果，其重采样机制会放大提示注入攻击。
...
### [358] [Interpretable Machine Learning for Predicting Startup Funding, Patenting, and Exits](https://arxiv.org/abs/2510.09465)
*Saeid Mashhadi,Amirhossein Saghezchi,Vesal Ghassemzadeh Kashani*

Main category: cs.LG

TL;DR: 本研究开发了一个可解释的机器学习框架来预测初创企业的结果，包括融资、专利申请和退出。使用Crunchbase和USPTO数据构建了2010-2023年的企业季度面板数据，评估了三个时间范围：12个月内获得下一轮融资、24个月内专利存量增长、36个月内通过IPO或收购退出。


<details>
  <summary>Details</summary>
Motivation: 开发一个可解释的机器学习框架来预测初创企业的关键结果，为创新金融提供透明且可复现的排名系统。

Method: 构建2010-2023年企业季度面板数据，使用开发窗口（2010-2019）进行预处理，应用逆流行权重和SMOTE-NC处理类别不平衡，比较逻辑回归和树集成方法（随机森林、XGBoost、LightGBM、CatBoost），使用PR-AUC和AUROC进行评估。

Result: 专利、融资和退出预测的AUROC值分别达到0.921、0.817和0.872，表明模型具有很高的预测准确性。

Conclusion: 该框架为创新金融提供了透明且可复现的排名系统，能够有效预测初创企业的关键发展结果。
...
### [359] [Geodesic Calculus on Latent Spaces](https://arxiv.org/abs/2510.09468)
*Florine Hartwig,Josua Sassen,Juliane Braunsmann,Martin Rumpf,Benedikt Wirth*

Main category: cs.LG

TL;DR: 该论文提出了一个框架，将自编码器的潜在流形描述为隐式子流形，并开发了离散黎曼几何计算工具来近似经典几何算子。该方法通过最小化去噪目标学习潜在流形的近似投影，支持在潜在流形上使用不同的黎曼几何，能够计算连接给定端点的测地线路径和通过黎曼指数映射发射测地线。


<details>
  <summary>Details</summary>
Motivation: 自编码器的潜在流形提供了数据的低维表示，可以从几何角度进行研究。然而，实际应用中隐式表示往往存在不准确性，需要开发鲁棒的工具来描述和分析这些潜在流形。

Method: 将潜在流形描述为隐式子流形，开发离散黎曼几何计算工具来近似经典几何算子。通过最小化去噪目标学习潜在流形的近似投影，该方法独立于底层自编码器，支持在潜在流形上使用不同的黎曼几何。

Result: 该框架能够在潜在流形上计算连接给定端点的测地线路径，并通过黎曼指数映射发射测地线。方法在各种自编码器上进行了评估，包括在合成数据和真实数据上训练的模型。

Conclusion: 提出的方法为分析自编码器的潜在流形提供了有效的几何工具，能够处理实际应用中常见的隐式表示不准确性问题，支持多种黎曼几何的应用，为潜在流形的几何分析提供了实用框架。
...
### [360] [CRPS-LAM: Regional ensemble weather forecasting from matching marginals](https://arxiv.org/abs/2510.09484)
*Erik Larsson,Joel Oskarsson,Tomas Landelius,Fredrik Lindsten*

Main category: cs.LG

TL;DR: CRPS-LAM是一种概率性区域天气预报模型，使用CRPS目标训练，通过单次前向传播生成集合成员，采样速度比扩散模型快39倍，在MEPS区域数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的模型在区域天气预报中表现良好但采样时计算成本高，需要更高效的集合预报方法。

Method: 使用CRPS目标训练模型，通过采样和注入单个潜在噪声向量，在单次前向传播中生成集合成员。

Result: 采样速度比扩散模型快39倍，在MEPS区域数据集上误差与扩散模型相当，同时保留了精细尺度预报细节。

Conclusion: CRPS-LAM是概率性区域天气预报的有效方法，在保持精度的同时显著提高了计算效率。
...
### [361] [Limitations of Normalization in Attention Mechanism](https://arxiv.org/abs/2508.17821)
*Timur Mudarisov,Mikhail Burtsev,Tatiana Petrova,Radu State*

Main category: cs.LG

TL;DR: 本文研究注意力机制中归一化的局限性，分析了softmax归一化在token选择中的几何分离问题，发现随着选择token数量增加，模型区分信息性token的能力下降，梯度敏感性在低温度设置下带来训练挑战。


<details>
  <summary>Details</summary>
Motivation: 研究注意力机制中归一化的理论局限性，特别是softmax缩放对token选择能力的影响，为改进注意力架构提供理论基础。

Method: 建立理论框架分析模型的token选择能力和几何分离，通过预训练GPT-2模型进行实验验证，分析注意力机制的关键行为。

Result: 随着选择token数量增加，模型区分信息性token的能力下降，趋向均匀选择模式；softmax归一化在低温度设置下存在梯度敏感性挑战。

Conclusion: 研究推进了对基于softmax的注意力机制的理解，表明需要开发更稳健的归一化和选择策略来改进未来注意力架构。
...
### [362] [Locally Optimal Private Sampling: Beyond the Global Minimax](https://arxiv.org/abs/2510.09485)
*Hrad Ghoukasian,Bonwoo Lee,Shahab Asoodeh*

Main category: cs.LG

TL;DR: 本文研究在局部差分隐私(LDP)约束下从分布中采样的最优方法，提出了局部极小极大风险框架，并证明了全局最优的LDP采样器在局部邻域内也是最优的。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注全局极小极大最优性，但实际应用中往往需要针对特定分布邻域进行优化。本文从局部视角出发，研究在固定分布P0邻域内的极小极大风险，这更贴近实际隐私数据采样场景。

Method: 1) 将先前纯LDP工作扩展到泛函LDP框架；2) 证明当分布类限制在P0邻域内时，全局最优的泛函LDP采样器也是局部最优采样器；3) 推导局部极小极大最优采样器的闭式表达式。

Result: 确定了局部极小极大风险的确切值，该值依赖于P0和隐私级别。提出的局部最优采样器在包含公共数据的隐私采样场景中，经验性地优于现有的全局极小极大采样器。

Conclusion: 局部框架为隐私采样提供了更实用的优化方法，特别是在有公共数据可用的场景下，局部最优采样器能显著提升性能。
...
### [363] [Near-Optimal Second-Order Guarantees for Model-Based Adversarial Imitation Learning](https://arxiv.org/abs/2510.09487)
*Shangzhe Li,Dongruo Zhou,Weitong Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种基于模型的对抗模仿学习算法（MB-AIL），在仅使用离线专家演示和在线无奖励交互的情况下，实现了无视野依赖的、二阶样本复杂度保证，并在理论和实验上证明了其最优性。


<details>
  <summary>Details</summary>
Motivation: 当前在线对抗模仿学习虽然实证效果显著，但对其在线交互的益处和随机性影响的理解仍不充分。本文旨在填补这一理论空白，研究如何在仅有离线专家演示和在线无奖励交互的情况下实现高效的模仿学习。

Method: 提出基于模型的对抗模仿学习算法（MB-AIL），使用通用函数逼近来处理专家数据和奖励自由交互，通过模型学习来减少样本复杂度。

Result: MB-AIL实现了无视野依赖的二阶样本复杂度保证，该界限与相关策略下回报的方差成比例，在系统接近确定性时会收紧。理论证明MB-AIL在在线交互方面达到了极小极大最优样本复杂度，实验验证其样本效率优于现有方法。

Conclusion: MB-AIL算法在理论和实验上都表现出优越性能，为在线对抗模仿学习提供了理论基础和实用解决方案，特别是在样本效率和随机性处理方面具有显著优势。
...
### [364] [Performance Analysis of Machine Learning Algorithms in Chronic Kidney Disease Prediction](https://arxiv.org/abs/2510.09493)
*Iftekhar Ahmed,Tanzil Ebad Chowdhury,Biggo Bushon Routh,Nafisa Tasmiya,Shadman Sakib,Adil Ahmed Chowdhury*

Main category: cs.LG

TL;DR: 本研究针对慢性肾病（CKD）诊断，使用八种机器学习算法构建预测模型，其中随机森林和逻辑回归表现最佳，准确率达到99%。


<details>
  <summary>Details</summary>
Motivation: 全球约10%人口受慢性肾病影响，需要有效的风险评估和监测方法。机器学习因其快速准确的检测能力，可在医疗诊断中发挥重要作用。

Method: 从UCL机器学习仓库获取CKD数据集，使用'均值-众数'和'随机抽样方法'填补缺失值，然后应用随机森林、SVM、朴素贝叶斯、逻辑回归、KNN、XGBoost、决策树和AdaBoost八种机器学习算法建立模型。

Result: 随机森林和逻辑回归模型准确率最高，达到99%；AdaBoost、XGBoost、朴素贝叶斯、决策树和SVM表现次之；KNN分类器表现最差，准确率仅为73%。

Conclusion: 机器学习模型特别是随机森林和逻辑回归，在慢性肾病诊断中表现出极高的准确性，可为临床实践提供有效的计算机辅助诊断工具。
...
### [365] [Geo-Aware Models for Stream Temperature Prediction across Different Spatial Regions and Scales](https://arxiv.org/abs/2510.09500)
*Shiyuan Luo,Runlong Yu,Shengyu Chen,Yingda Fan,Yiqun Xie,Yanhua Li,Xiaowei Jia*

Main category: cs.LG

TL;DR: Geo-STARS是一个地理感知的时空建模框架，用于预测不同流域和空间尺度的河流水温。它通过地理感知嵌入技术捕获跨区域和尺度的共享模式，在数据稀疏情况下仍能有效预测，在真实数据集上表现出优越的泛化性能。
...
Method: 提出Geo-STARS框架，引入地理感知嵌入技术，利用地理信息显式捕获跨空间区域和尺度的共享原则和模式，并将其集成到门控时空图神经网络中，使模型能够在稀疏或无观测数据情况下学习复杂时空模式。

Result: 使用美国东海岸多个流域37年的真实数据集进行评估，Geo-STARS在跨区域和跨尺度的泛化性能上优于最先进的基线方法。
...
### [366] [Automated Evolutionary Optimization for Resource-Efficient Neural Network Training](https://arxiv.org/abs/2510.09566)
*Ilia Revin,Leon Strelkov,Vadim A. Potemkin,Ivan Kireev,Andrey Savchenko*

Main category: cs.LG

TL;DR: PETRA是一个新的AutoML框架，通过进化优化模型架构和训练策略，结合剪枝、量化和损失正则化，显著减小模型大小（最高75%）、降低延迟（最高33%）并提高吞吐量（13%），且不影响目标指标。
...
Method: 开发了PETRA自动化机器学习框架，应用进化优化来优化模型架构和训练策略，包括剪枝、量化和损失正则化技术。

Result: 在金融事件序列、图像和时间序列基准测试中，PETRA显著减少了模型大小（最高75%）、延迟（最高33%），并提高了吞吐量（13%），且目标指标没有明显下降。

Conclusion: PETRA框架有效解决了神经网络模型的优化挑战，在保持性能的同时显著提升了模型的效率和可扩展性。
...
### [367] [MODE: Learning compositional representations of complex systems with Mixtures Of Dynamical Experts](https://arxiv.org/abs/2510.09594)
*Nathan Quiblier,Roy Friedman,Matthew Ricci*

Main category: cs.LG

TL;DR: 提出了MODE（混合动态专家）框架，通过神经门控机制将复杂动态分解为稀疏可解释组件，能够无监督发现行为模式并在模式转换间进行准确长期预测，特别适用于生命科学中噪声转换的动态系统。


<details>
  <summary>Details</summary>
Motivation: 生命科学中的动态系统通常包含重叠行为模式的复杂混合，细胞亚群可能从循环动态转向平衡动态或分支到不同发育命运，这些转换可能显得噪声且不规则，对传统基于流的建模技术构成挑战。

Method: MODE是一个图形建模框架，使用神经门控机制将复杂动态分解为稀疏可解释组件，允许代理跳转到不同的控制规律，特别适用于噪声转换场景。

Result: 在合成和真实计算生物学数据集上的评估显示，MODE在无监督分类任务中表现优异，在具有挑战性的预测任务中成功模拟细胞生物学中的关键循环和分支过程，在人类单细胞RNA测序数据中能够区分增殖与分化动态并预测细胞命运承诺。

Conclusion: MODE框架能够有效处理生命科学中复杂动态系统的行为模式发现和长期预测，特别适用于噪声转换场景，在计算生物学中具有重要应用价值。
...
### [368] [BaNEL: Exploration Posteriors for Generative Modeling Using Only Negative Rewards](https://arxiv.org/abs/2510.09596)
*Sangyun Lee,Brandon Amos,Giulia Fanti*

Main category: cs.LG

TL;DR: BaNEL是一种基于贝叶斯负证据学习的算法，专门处理生成模型在稀疏奖励环境下的训练问题，仅使用失败样本进行后训练，同时最小化奖励评估次数。


<details>
  <summary>Details</summary>
Motivation: 在重要问题的最困难实例中，基础生成模型往往只能获得接近零的奖励信号，且奖励函数的调用成本高昂。这为标准的基于奖励的后训练带来了根本不同的学习挑战。

Method: 将学习失败规律的问题转化为另一个生成建模问题，利用该模型评估新数据是否类似于之前见过的失败样本，并引导生成远离这些失败模式。

Result: BaNEL在多个稀疏奖励任务上，即使没有观察到任何成功样本，也能提升模型性能，在成功率上比现有新颖性奖励方法高出几个数量级，同时使用更少的奖励评估。

Conclusion: BaNEL提供了一种有效的方法来处理稀疏奖励环境下的生成模型训练问题，通过仅利用失败样本和最小化奖励评估，实现了显著的性能提升。
